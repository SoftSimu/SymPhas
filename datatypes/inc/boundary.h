...
#define NUM_BOUNDARY_CONSTANTS \
  5  //!< Maximum number of parameters for a boundary.

#define GAUSSIAN_COEF \
  1.0  //!< The default value used in the Gaussian condition.
#define GAUSSIAN_STD \
  1.0  //!< The default value of the standard deviation used in the Gaussian
       //!< condition.
#define GAUSSIAN_B \
  1.0  //!< The default value of the offset of the mean of the Gaussian
       //!< condition.
#define SIN_COEF_A \
  1.0  //!< The first default coefficient for the sine condition.
#define SIN_COEF_B \
  1.0              //!< The second default coefficient for the cosine condition.
...
#define LINEAR_B \
  0.0  //!< The default value of the offset of the linear condition.
#define CONSTANT_A \
  0.0  //!< The default value of the constant value of the constant condition.
#define UNIFORM_A \
  0  //!< The default starting value of the uniform distribution condition.
#define UNIFORM_B \
  1  //!< The default end value of the uniform distribution condition.
#define TIME_SCALE \
  1.0  //!< The default multiplier on the time in time-dependent conditions.
//! \endcond
...
enum class BoundaryType {
  ...
  PERIODIC0,    //!< Same as PERIODIC when used as a boundary parameter. In
                //!< generating boundaries, represents sides that are only
                //!< opposite.
  PERIODIC3XZ,  //!< Same as PERIODIC when used as a boundary parameter.
                //!< Periodic in x and z (but not y).
  PERIODIC3XY,  //!< Same as PERIODIC when used as a boundary parameter.
...
enum class BoundaryTag {
  GAUSSIAN,  //!< Uses a Gaussian distribution to generate random values at each
             //!< point.
  RANDOM,    //!< Uses a uniform distribution to generate random values at each
             //!< point.
  TRIG,      //!< Evaluates a trigonometric function of the position on the
             //!< boundary.
...
using boundary_tag_list = symphas::lib::fixed_array_container<BoundaryTag, 2>;
}  // namespace symphas
...
struct boundary_info {
  ...
  boundary_info(BoundaryTag tag0, const double* params, int argc)
      : tag{tag0, BoundaryTag::NONE},
        params{(argc > 0) ? new double[argc]{} : nullptr},
        argc{argc} {}
  boundary_info(BoundaryTag tag0, double param)
      : boundary_info(tag0, &param, 1) {}
  boundary_info(boundary_tag_list const& tag, const double* params, int argc)
      : tag{tag[0], tag[1]},
        params{(argc > 0) ? new double[argc]{} : nullptr},
        argc{argc} {
    if (argc > 0) {
  ...
  boundary_info(boundary_tag_list const& tag, double param)
      : boundary_info(tag, &param, 1) {}

  boundary_info()
      : tag{BoundaryTag::NONE, BoundaryTag::NONE}, params{nullptr}, argc{0} {}
  boundary_info(boundary_info const& other)
      : boundary_info(other.tag, other.params, other.argc) {}
...
struct b_element_type {
  ...
  b_element_type(BoundaryType type, const double* params, int argc)
      : b_element_type(type, {BoundaryTag::NONE, BoundaryTag::NONE}, params,
                       argc) {}
  ...
  b_element_type(BoundaryType type)
      : b_element_type(type, {BoundaryTag::NONE, BoundaryTag::NONE}, nullptr,
                       0) {}

  b_element_type(BoundaryType type,
                 symphas::internal::boundary_info const& info)
      : b_element_type(type, info.tag, info.params, info.argc) {}
  b_element_type(symphas::internal::boundary_info const& info)
      : b_element_type(BoundaryType::DEFAULT, info) {}
   ...
   */
  b_element_type()
      : b_element_type(BoundaryType::DEFAULT,
                       {BoundaryTag::NONE, BoundaryTag::NONE}, nullptr, 0) {}
...
struct b_data_type : std::map<Side, symphas::b_element_type> {
  ...
  b_data_type(size_t dim,
              symphas::b_element_type const& interval = BoundaryType::NONE)
      : parent_type() {
...
inline auto operator<<(BoundaryType type, symphas::boundary_tag_list tags) {
  return symphas::b_element_type(type, tags);
}

inline auto operator<<(BoundaryType type,
                       symphas::internal::boundary_info const& info) {
...
struct Boundary {
  ...
  virtual Boundary<T, D>* clone() const = 0;

  //! Get the parameters used to construct this boundary.
  /*!
   * Returns a new parameters object that can be used to construct a new
   * boundary object with identical properties.
   */
...
struct BoundaryApplied<T, 0, BoundaryType::DEFAULT> : Boundary<T, 0> {
   ...
   */
  virtual void update(T& val, axis_coord_t, axis_coord_t,
                      double time) const = 0;

  template <typename vector_type = T,
            typename T0 = typename vector_element_type<vector_type>::type>
  void update(multi_value<1, T0> val, axis_coord_t, axis_coord_t,
              double time) const {
  ...
  void update(carry_value<T> val, axis_coord_t, axis_coord_t,
              double time) const {
    if (val.is_valid()) {
...
struct BoundaryApplied<T, 1, BoundaryType::DEFAULT> : Boundary<T, 1> {
   ...
   * \param time The solution time at which the boundary is updated.
   */
  virtual void update(T& val, axis_coord_t x, axis_coord_t,
                      double time) const = 0;

  template <typename vector_type = T,
            typename T0 = typename vector_element_type<vector_type>::type>
  void update(multi_value<2, T0> val, axis_coord_t x, axis_coord_t,
              double time) const {
  ...
  void update(carry_value<T> val, axis_coord_t x, axis_coord_t,
              double time) const {
    if (val.is_valid()) {
...
struct BoundaryApplied<T, 2, BoundaryType::DEFAULT> : Boundary<T, 2> {
   ...
   * \param time The solution time at which the boundary is updated.
   */
  virtual void update(T& val, axis_coord_t x, axis_coord_t y,
                      double time) const = 0;

  template <typename vector_type = T,
            typename T0 = typename vector_element_type<vector_type>::type>
  void update(multi_value<3, T0> val, axis_coord_t x, axis_coord_t y,
              double time) const {
  ...
  void update(carry_value<T> val, axis_coord_t x, axis_coord_t y,
              double time) const {
    if (val.is_valid()) {
...
symphas::b_element_type get_parameters(
    BoundaryDefaultTagged<T, D, tags...> const* boundary) {
  ...
  symphas::b_element_type bdata(BoundaryType::DEFAULT, {tags...}, params,
                                NUM_BOUNDARY_CONSTANTS);
...
struct BoundarySupportClass {
   ...
   */
  Boundary<T, D>* clone() const { return new B<T, D>(*cast()); }

  void set_defaults() { (*static_cast<B<T, D>*>(this)).set_defaults(); }

  symphas::b_element_type get_parameters() const {
    return grid::internal::get_parameters(cast());
  }

  const B<T, D>* cast() const { return static_cast<B<T, D> const*>(this); }
};
...
template <typename T, size_t D>
using BoundaryTrigTime =
    BoundaryDefaultTagged<T, D, BoundaryTag::TRIG, BoundaryTag::TIME>;
template <typename T, size_t D>
using BoundaryLinearTime =
    BoundaryDefaultTagged<T, D, BoundaryTag::LINEAR, BoundaryTag::TIME>;
template <typename T, size_t D>
...
struct BoundaryDefaultTagged<T, D, BoundaryTag::TRIG>
  ...
  Boundary<T, D>* clone() const { return support_type::clone(); }
...
struct BoundaryDefaultTagged<T, 0, BoundaryTag::GAUSSIAN>
  ...
  void set_defaults() { _A = GAUSSIAN_COEF; }

  Boundary<T, 0>* clone() const { return support_type::clone(); }
...
struct BoundaryDefaultTagged<T, 1, BoundaryTag::GAUSSIAN>
  ...
  Boundary<T, 1>* clone() const { return support_type::clone(); }
...
struct BoundaryDefaultTagged<T, 2, BoundaryTag::GAUSSIAN>
  ...
  void update(T& val, axis_coord_t x, axis_coord_t y, double) const {
    ...
    val = _A *
          std::exp(-(std::pow(x - _B, 2.0) + std::pow(y - _C, 2.0)) /
                   std::pow(_D, 2.0) / 2.0) *
          symphas::lib::get_identity<T>();
  }
  ...
  Boundary<T, 2>* clone() const { return support_type::clone(); }
...
struct BoundaryDefaultTagged<T, 0, BoundaryTag::LINEAR>
  ...
  void set_defaults() { _A = LINEAR_COEF; }
...
struct BoundaryDefaultTagged<T, D, BoundaryTag::RANDOM>
  ...
  //! Create a new boundary with the given parameters.
  BoundaryDefaultTagged(double* constants, size_t num_constants)
      : parent_type(constants, num_constants),
        support_type(),
        mt_eng{std::random_device{}()},
        prob_dist(_A, _B),
   ...
   */
  void update(T& val, axis_coord_t, axis_coord_t, double) const { update(val); }
 ...
 protected:
  ...
  void update(vector_t<1>& val) const { val = {prob_dist(mt_eng)}; }

  void update(vector_t<2>& val) const {
    scalar_t a = th(mt_eng), r = prob_dist(mt_eng);
    val = {r * cos(a), r * sin(a)};
  }

  void update(vector_t<3>& val) const {
    scalar_t a = th(mt_eng), b = th(mt_eng), r = prob_dist(mt_eng);
...
template <typename T>
void BoundaryApplied<T, 2, BoundaryType::DEFAULT>::init(double* _v,
                                                        double* _h) {
  std::copy(_v, _v + 4, v);
  std::copy(_h, _h + 2, h);
}
...
extern std::map<const char*, BoundaryType, symphas::lib::any_case_comparator>
    boundary_key_map;
extern std::map<const char*, BoundaryTag, symphas::lib::any_case_comparator>
    boundary_tag_key_map;
  ...
  if (data.tag[1] == BoundaryTag::TIME) {
      ...
      case BoundaryType::DEFAULT: {
        switch (data.tag[0]) {
          case BoundaryTag::TRIG: {
            return new grid::BoundaryTrigTime<T, D>(data.params, data.argc);
          }
          case BoundaryTag::LINEAR: {
            return new grid::BoundaryLinearTime<T, D>(data.params, data.argc);
          }
          default:
            fprintf(SYMPHAS_WARN,
                    "initializing boundary encountered the wrong tag\n");
        }
        break;
      }
      default:
    ...
    switch (data.type) {
      case BoundaryType::PERIODIC: {
        return new grid::BoundaryApplied<T, D, BoundaryType::PERIODIC>{};
      }
      case BoundaryType::DEFAULT: {
        switch (data.tag[0]) {
          case BoundaryTag::GAUSSIAN: {
            return new grid::BoundaryGaussian<T, D>(data.params, data.argc);
          }
          case BoundaryTag::TRIG: {
            return new grid::BoundaryTrig<T, D>(data.params, data.argc);
          }
          case BoundaryTag::CONSTANT: {
            return new grid::BoundaryConstant<T, D>(data.params, data.argc);
          }
          case BoundaryTag::LINEAR: {
            return new grid::BoundaryLinear<T, D>(data.params, data.argc);
          }
          case BoundaryTag::RANDOM: {
            return new grid::BoundaryRandom<T, D>(data.params, data.argc);
          }
          default:
            fprintf(SYMPHAS_WARN,
                    "initializing boundary encountered the wrong tag\n");
...
struct boundary_setup {
 ...
 public:
   ...
   */
  template <typename T>
  void operator()(grid::Boundary<T, D>* boundaries[],
...
template <>
template <typename T>
void boundary_setup<0>::operator()(
    grid::Boundary<T, 0>* boundaries[], symphas::b_data_type const& bdata,
    symphas::interval_data_type const& intervals) {
  // there are only 2 boundaries for a 1d system, interval_index = 0 for left
  // and interval_index = 1 for right the boundaries correspond to only one
  // point

  for (auto side : {Side::LEFT, Side::RIGHT}) {
    if (bdata.find(side) != bdata.end()) {
      boundaries[symphas::side_to_index(side)] =
          symphas::internal::new_boundary<T, 0>(bdata.at(side));
...
template <>
template <typename T>
void boundary_setup<1>::operator()(
    grid::Boundary<T, 1>* boundaries[], symphas::b_data_type const& bdata,
    symphas::interval_data_type const& intervals) {
  // each boundary condition is an edge, and is specified by an interval [a, b]
  // the intervals are assigned to each boundary relative to the global
  // coordinate system by rotating the system clockwise so that the boundary for
  // which the interval is constructed is at the top of the system, then the
  // interval is selected to begin at the left

  for (auto side : {Side::LEFT, Side::RIGHT, Side::TOP, Side::BOTTOM}) {
    if (bdata.find(side) != bdata.end()) {
      boundaries[symphas::side_to_index(side)] =
          symphas::internal::new_boundary<T, 1>(bdata.at(side));
...
template <>
template <typename T>
void boundary_setup<2>::operator()(
    grid::Boundary<T, 2>* boundaries[], symphas::b_data_type const& bdata,
    symphas::interval_data_type const& intervals) {
  // each boundary condition is a surface, and is specified by an interval [x0,
  // x1], [y0, y1] note that the order the axes are mentioned in this
  // specification also indicates the local x and y the first mentioned axis is
  // the local x axis, the second menteiond axis is the local y axis the
  // interval order is determined by considering the orientation of the face
  // after `rotating' the cube to get to it, for the back face, the rotation is
  ...
  for (auto side : {Side::LEFT, Side::RIGHT, Side::TOP, Side::BOTTOM,
    ...
    if (bdata.find(side) != bdata.end()) {
      boundaries[symphas::side_to_index(side)] =
          symphas::internal::new_boundary<T, 2>(bdata.at(side));

 ...
 * The arrangement of elements constituting the boundary is equal to the
 * exclusion of the interior grid, which is a grid of the elements with
 * dimensions equal to that of the containing grid less twice the
 * #BOUNDARY_DEPTH, from the containing grid
 */
...
inline void set_interval(double (&interval)[2], double& h, double left,
                         double right, double width = 0) {
...
inline void set_count(double (&interval)[2], double& h, double left,
                      double right, len_type count) {
  double width = std::abs(right - left) / (count - 1);
  set_interval(interval, h, left, right, width);
}

inline void set_count_from_r(double (&interval)[2], double& h, len_type count,
                             double width, double ratio) {
  double length0 = ((double)count / range_count(interval, h)) * (width / h) *
                   range_length(interval, h);
...
inline void set_count_from_r(double (&interval)[2], double& h, len_type count,
                             double ratio) {
  double length0 =
      (double(count) / range_count(interval, h)) * range_length(interval, h);
  double pos = interval[0] + ratio * range_length(interval, h);
 ...
 */
__host__ __device__ inline constexpr iter_type side_to_index(Side side) {
 ...
 */
__host__ __device__ inline constexpr iter_type axis_to_index(Axis axis) {
 ...
 */
__host__ __device__ inline constexpr Side index_to_side(iter_type i) {
 ...
 */
__host__ __device__ inline constexpr Axis index_to_axis(iter_type i) {
...
struct interval_element_type {
 ...
 public:
  interval_element_type(const double (&data)[2], double h = 1.)
      : interval_element_type(data, data, h) {}
  interval_element_type(const double (&domain)[2], const double (&data)[2],
                        double h)
  ...
  interval_element_type(len_type length)
      : interval_element_type(0, length - 1) {}
  ...
  len_type get_count() const { return get_domain_count(); }
   ...
   */
  axis_coord_t left() const { return data[0]; }
   ...
   * the interval is initialized with.
   */
  axis_coord_t right() const { return data[1]; }
   ...
   * the interval is initialized with.
   */
  axis_coord_t domain_left() const { return domain[0]; }
   ...
   * the interval is initialized with.
   */
  axis_coord_t domain_right() const { return domain[1]; }
  ...
  //! Return the length of the interval on the real axis.
  double length() const { return interval_length(); }
  ...
  void set_interval_count(double left, len_type count) {
    symphas::internal::set_interval(data, left,
                                    (left + count * width()) / domain_length());
  }
   ...
   * \param right The right endpoint of the interval.
   */
  void set_domain_count(len_type count) { set_domain_count_from_r(count); }
  ...
  //! Resize the interval with the given number of elements, with constant
  //! width.
  /*!
   * See set_interval(double, double, double).
   *
   ...
   * \param width The spatial distance between points in the interval.
   */
  void set_count(len_type count) { set_count_from_r(count, 0.5); }
  ...
  void set_count(len_type count, double width) {
    set_count_from_r(count, width, 0.5);
  }
...
struct interval_data_type : std::map<Axis, interval_element_type> {
  ...
  interval_data_type(size_t dim, interval_element_type const& interval = {})
      : parent_type() {
...
struct box_list : symphas::lib::array_container<T> {
  ...
  __host__ __device__ box_list(T dim0, T dim1, T dim2)
      : box_list(dim0, dim1, dim2,
                 (dim1 > 0 && dim2 > 0)   ? 3
                 : (dim1 > 0 || dim2 > 0) ? 2
                                          : 1) {}
  __host__ __device__ box_list(T dim0, T dim1)
      : box_list(dim0, dim1, 0, (dim1 == 0) ? 1 : 2) {}
  __host__ __device__ box_list(T dim0) : box_list(dim0, 0, 0, 1) {}
  __host__ __device__ box_list() : box_list(0, 0, 0, 0) {}

  template <typename TT = T,
            std::enable_if_t<std::is_same<TT, len_type>::value, int> = 0>
  __host__ __device__ box_list(symphas::interval_data_type const& intervals)
      : box_list((intervals.find(Axis::X) != intervals.end())
                     ? intervals.at(Axis::X).get_count()
                     : 0,
                 (intervals.find(Axis::Y) != intervals.end())
                     ? intervals.at(Axis::Y).get_count()
                     : 0,
                 (intervals.find(Axis::Z) != intervals.end())
                     ? intervals.at(Axis::Z).get_count()
                     : 0) {}

  template <typename TT = T,
            std::enable_if_t<std::is_same<TT, double>::value, int> = 0>
  __host__ __device__ box_list(symphas::interval_data_type const& intervals)
      : box_list((intervals.find(Axis::X) != intervals.end())
                     ? intervals.at(Axis::X).width()
                     : 0,
                 (intervals.find(Axis::Y) != intervals.end())
                     ? intervals.at(Axis::Y).width()
                     : 0,
                 (intervals.find(Axis::Z) != intervals.end())
                     ? intervals.at(Axis::Z).width()
                     : 0) {}

  __host__ __device__ box_list(const T* data, size_t n)
      : parent_type(data, n) {}
  ...
  std::tuple<T, T> _2() const { return {operator[](0), operator[](1)}; }

  std::tuple<T> _1() const { return {operator[](0)}; }

 protected:
  __host__ __device__ box_list(T dim0, T dim1, T dim2, size_t n)
      : parent_type(nullptr, n) {
    if (n > 0) data[0] = dim0;
    if (n > 1) data[1] = dim1;
    if (n > 2) data[2] = dim2;
  }
...
struct symphas::grid_info {
 ...
 public:
  symphas::interval_data_type
      intervals;  //!< Extent of the grid in the spatial axes.
  std::map<Axis, len_type> strides;  //!< Stride of the array.

  grid_info() : intervals{}, strides{} {}

  template <size_t... Is>
  grid_info(const double (&intervals)[sizeof...(Is)][2], double width,
            std::index_sequence<Is...>)
      : intervals{{symphas::index_to_axis(Is),
                   interval_element_type(intervals[Is][0],
                                         intervals[Is][1] - width, width)}...},
        strides{} {
    update_strides();
  }

  template <size_t D>
  grid_info(const double (&intervals)[D][2], double width = 1.)
      : grid_info(intervals, width, std::make_index_sequence<D>{}) {}

  template <size_t... Is>
  grid_info(const len_type (&intervals)[sizeof...(Is)][2],
            std::index_sequence<Is...>)
      : intervals{{symphas::index_to_axis(Is),
                   interval_element_type(intervals[Is][0],
                                         intervals[Is][1] - 1)}...},
  ...
  template <size_t D>
  grid_info(const len_type (&intervals)[D][2])
      : grid_info(intervals, std::make_index_sequence<D>{}) {}

  template <size_t... Is>
  grid_info(const double (&domain)[sizeof...(Is)][2],
            const double (&intervals)[sizeof...(Is)][2], double width,
            std::index_sequence<Is...>)
      : intervals{{symphas::index_to_axis(Is),
                   interval_element_type(domain[Is], intervals[Is], width)}...},
        strides{} {
    update_strides();
  }

  template <size_t D>
  grid_info(const double (&domain)[D][2], const double (&intervals)[D][2],
            double width = 1.)
      : grid_info(domain, intervals, width, std::make_index_sequence<D>{}) {}

  template <size_t... Is>
  grid_info(const len_type (&dims)[sizeof...(Is)],
            const len_type (&intervals)[sizeof...(Is)][2],
            std::index_sequence<Is...>)
      : intervals{{symphas::index_to_axis(Is),
                   interval_element_type(
                       arr_d2_t{0, dims[Is] - 1},
                       arr_d2_t{intervals[Is][0], intervals[Is][1] - 1},
                       1.)}...},
        strides{} {
    update_strides();
  }
  ...
  template <size_t D>
  grid_info(const len_type (&dims)[D], const len_type (&intervals)[D][2])
      : grid_info(dims, intervals, std::make_index_sequence<D>{}) {}

  template <size_t... Is>
  grid_info(const len_type (&intervals)[sizeof...(Is)][2], const double* widths,
            std::index_sequence<Is...>)
      : intervals{{symphas::index_to_axis(Is),
                   interval_element_type(
                       arr_d2_t{intervals[Is][0] * widths[Is],
                                intervals[Is][0] * widths[Is]},
                       widths[Is])}...},
        strides{} {
    update_strides();
  }
  ...
  template <size_t D>
  grid_info(const len_type (&intervals)[D][2], const double* widths)
      : grid_info(intervals, widths, std::make_index_sequence<D>{}) {}
   ...
   */
  grid_info(interval_data_type const& intervals)
      : intervals{intervals}, strides{} {
    update_strides();
  }

  grid_info(interval_data_type const& intervals,
            std::map<Axis, len_type> const& strides)
   ...
   */
  grid_info(const len_type* dims, size_t dimension)
      : grid_info(make_intervals(dims, dimension)) {}
  ...
  grid_info(grid_info&& other) noexcept : grid_info() { ::swap(*this, other); }
   ...
   */
  auto& at(Axis ax) { return intervals.at(ax); }
   ...
   * \param ax The axis to get the interval from.
   */
  const auto& at(Axis ax) const { return intervals.at(ax); }
   ...
   * \param ax The axis to get the interval from.
   */
  const auto& operator[](Axis ax) const { return at(ax); }
   ...
   * \param ax The axis to get the interval from.
   */
  auto& operator[](Axis ax) { return intervals[ax]; }
   ...
   * number of axes represented by the interval data.
   */
  len_type dimension() const { return static_cast<len_type>(intervals.size()); }
  ...
  void set_strides(std::map<Axis, len_type> const& other) { strides = other; }
  ...
  void update_strides() { set_strides(intervals); }
  ...
  grid::pos_list left() const {
    ...
    for (iter_type i = 1; i < dimension(); ++i) {
      ...
      pos[i] = iter_type((interval.left() - interval.domain_left()) /
                         interval.width());
    }
    return pos;
  }

  operator symphas::interval_data_type() const { return intervals; }

  decltype(auto) begin() const { return intervals.begin(); }

  decltype(auto) end() const { return intervals.end(); }

  decltype(auto) begin() { return intervals.begin(); }

  decltype(auto) end() { return intervals.end(); }
};
...
namespace symphas {
inline bool is_valid(grid_info const& ginfo) { return (ginfo.dimension() > 0); }
}  // namespace symphas

...
struct make_forward_plan<D, complex_t, complex_t> {
  auto operator()(complex_t* from, complex_t* to, const len_type* dims) {
    return symphas::dft::new_fftw_plan<D, complex_t, complex_t>{}(
        from, to, dims, false, false);
  }
};
...
struct FourierGrid : Grid<T, D> {
  ...
  FourierGrid(const len_type* dimensions)
      : parent_type{dimensions},
        p_in_out{symphas::dft::new_fftw_plan<D, S, T>{}(values, values,
                                                        dimensions)} {}
   ...
   * Fourier transform.
   */
  S* values_src_cast() { return reinterpret_cast<S*>(values); }

  const S* values_src_cast() const { return reinterpret_cast<S*>(values); }
  ...
  ~FourierGrid() { symphas::dft::fftw_destroy_plan(p_in_out); }
...
struct FourierGrid<S, any_vector_t<T, D>, D> : Grid<any_vector_t<T, D>, D> {
   ...
   */
  FourierGrid(const len_type* dimensions)
      : parent_type{dimensions}, p_in_out{0} {
    for (iter_type i = 0; i < D; ++i) {
      p_in_out[i] =
          symphas::dft::new_fftw_plan<D, S, T>{}(values[i], values[i], dims);
    }
  }
  FourierGrid(FourierGrid<S, T, D> const& other)
      : parent_type{other}, p_in_out{0} {
    for (iter_type i = 0; i < D; ++i) {
      p_in_out[i] =
          symphas::dft::new_fftw_plan<D, S, T>{}(values[i], values[i], dims);
    }
   ...
   * Fourier transform.
   */
  S* values_src_cast() { return reinterpret_cast<S*(&)[D]>(values); }
...
struct MapGrid : Grid<T, D> {
  ...
  MapGrid(S src, len_type const* dimensions)
      : parent_type{dimensions}, src{src} {}
  MapGrid(S src, std::initializer_list<len_type> dimensions)
      : parent_type{dimensions}, src{src} {}
  MapGrid(parent_type const& src_grid)
      : parent_type{src_grid.dims}, src{src_grid.values} {}
...
template <typename S, typename T, size_t D,
          template <size_t, typename, typename> typename make_ft_helper =
              symphas::internal::make_forward_plan>
struct MapGridFourier : FourierGridTarget<T, D> {
  ...
  friend void swap(MapGridFourier<S, T, D, make_ft_helper>& first,
                   MapGridFourier<S, T, D, make_ft_helper>& second) {
    using std::swap;
  ...
  ~MapGridFourier() { symphas::dft::fftw_destroy_plan(p_in_out); }
...
struct MapGridFourier<any_vector_t<S, D>, any_vector_t<T, D>, D, make_ft_helper>
  ...
  friend void swap(MapGridFourier<S, T, D>& first,
                   MapGridFourier<S, T, D>& second) {
    using std::swap;

    // swap(*static_cast<source_parent_type*>(&first),
    // *static_cast<source_parent_type*>(&second));
    swap(*static_cast<parent_type*>(&first),
         *static_cast<parent_type*>(&second));
    swap(first.src, second.src);
    swap(first.p_in_out, second.p_in_out);
 ...
 */
template <typename S, typename T, size_t D>
struct MapGridInverseFourier
    : MapGridFourier<S, T, D, symphas::internal::make_backward_plan> {
  using parent_type =
      MapGridFourier<S, T, D, symphas::internal::make_backward_plan>;
  using parent_type::dims;
...
struct MapGridInverseFourier<any_vector_t<S, D>, any_vector_t<T, D>, D>
    : MapGridFourier<any_vector_t<S, D>, any_vector_t<T, D>, D,
                     symphas::internal::make_backward_plan> {
  using parent_type = MapGridFourier<any_vector_t<S, D>, any_vector_t<T, D>, D,
                                     symphas::internal::make_backward_plan>;
  using parent_type::dims;
  using parent_type::p_in_out;
  using parent_type::parent_type;
...
template <typename S, typename T, size_t D,
          template <size_t, typename, typename> typename make_ft_helper =
              MapGridFourierArg>
struct MapGridFourier : FourierGridTarget<T, D> {
  using FourierGridTarget<T, D>::FourierGridTarget;
};

template <typename S, typename T, size_t D>
struct MapGridInverseFourier : MapGridFourier<S, T, D> {
  using MapGridFourier<S, T, D>::MapGridFourier;
};
...
namespace symphas::internal {
template <size_t D>
inline void transform_fourier_map(fftw_plan p_in_out, complex_t* in,
                                  complex_t* out, const len_type* dims) {
  symphas::dft::fftw_execute(p_in_out);
}

template <size_t D>
inline void transform_fourier_map(fftw_plan p_in_out, scalar_t* in,
                                  complex_t* out, const len_type* dims) {
  symphas::dft::fftw_execute(p_in_out);
...
template <size_t D>
inline void inverse_transform_fourier_map(fftw_plan p_in_out, complex_t* in,
                                          complex_t* out,
                                          const len_type* dims) {
  symphas::dft::fftw_execute(p_in_out);
...
template <size_t D>
inline void inverse_transform_fourier_map(fftw_plan p_in_out, complex_t* in,
                                          scalar_t* out, const len_type* dims) {
...
template <size_t D>
inline void transform_fourier_grid(fftw_plan p_in_out, complex_t* in,
                                   complex_t* out, const len_type* dims) {
  symphas::dft::fftw_execute(p_in_out);
}

template <size_t D>
inline void transform_fourier_grid(fftw_plan p_in_out, scalar_t* in,
                                   complex_t* out, const len_type* dims) {
...
void MapGridFourier<S, T, D, make_ft_helper>::update() {
  symphas::internal::transform_fourier_map<D>(p_in_out, src.values, values,
                                              dims);
}

template <typename S, typename T, size_t D,
          template <size_t, typename, typename> typename make_ft_helper>
void MapGridFourier<any_vector_t<S, D>, any_vector_t<T, D>, D,
                    make_ft_helper>::update() {
  for (iter_type i = 0; i < D; ++i) {
    symphas::internal::transform_fourier_map<D>(p_in_out[i], src.values[i],
                                                values[i], dims);
  }
}
...
void MapGridInverseFourier<S, T, D>::update() {
  symphas::internal::inverse_transform_fourier_map<D>(p_in_out, src.values,
                                                      values, dims);
}

template <typename S, typename T, size_t D>
void MapGridInverseFourier<any_vector_t<S, D>, any_vector_t<T, D>,
                           D>::update() {
  for (iter_type i = 0; i < D; ++i) {
    symphas::internal::inverse_transform_fourier_map<D>(
        p_in_out[i], src.values[i], values[i], dims);
  }
}
...
void FourierGrid<S, T, D>::update() {
  symphas::internal::transform_fourier_grid<D>(p_in_out, values_src_cast(),
                                               values, dims);
}
...
void FourierGrid<S, any_vector_t<T, D>, D>::update() {
  for (iter_type i = 0; i < D; ++i) {
    symphas::internal::transform_fourier_grid<D>(
        p_in_out[i], values_src_cast()[i], values[i], dims);
  }
}

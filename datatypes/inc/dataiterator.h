...
namespace symphas::lib
{
	...
	struct region_iterator_container_impl
	{
		...
		template<size_t... Is>
		region_iterator_container_impl(const iter_type(&dims)[D], region_iterator_data_type data, iter_type pos, std::index_sequence<Is...>) : pos{ pos }, dims{ dims[Is]... }, data{ data } {}
		region_iterator_container_impl(const iter_type(&dims)[D], region_iterator_data_type data, iter_type pos) : region_iterator_container_impl(dims, data, pos, std::make_index_sequence<D>{}) {}
		template<typename T>
		region_iterator_container_impl(T&& regions, iter_type pos = 0) : region_iterator_container_impl(std::forward<T>(regions).dims, std::forward<T>(regions).regions.begin(), pos) {}

		operator grid::region_interval<D>() const
		{
			return grid::region_interval(dims, static_cast<interval_entry_t const&>(data[pos]));
		}

		arr_entry_t& operator[](iter_type i)
		{
			return (data[pos])[i];
		}

		const arr_entry_t& operator[](iter_type i) const
		{
			return (data[pos])[i];
		}

		operator const interval_entry_t& () const
		{
			return (data[pos]);
		}

		operator interval_entry_t& ()
		{
			return (data[pos]);
		}
	...
	struct basic_forward_iterator_container<grid::region_interval_multiple<D>> :
		region_iterator_container_impl<D, typename std::vector<typename grid::region_interval_multiple<D>::subdomain_t>::iterator>
	{
		using parent_type = region_iterator_container_impl<D, typename std::vector<typename grid::region_interval_multiple<D>::subdomain_t>::iterator>;
		using parent_type::parent_type;
	};

	template<size_t D>
	struct basic_forward_iterator_container<const grid::region_interval_multiple<D>> :
		region_iterator_container_impl<D, typename std::vector<typename grid::region_interval_multiple<D>::subdomain_t>::const_iterator>
	{
		using parent_type = region_iterator_container_impl<D, typename std::vector<typename grid::region_interval_multiple<D>::subdomain_t>::const_iterator>;
		using parent_type::parent_type;
	};
...
namespace grid
{
	...
	iter_type stride_element(const len_type(&dims)[D], std::index_sequence<I0, Is...>)
	{
	...
	struct region_interval
	{
		iter_type intervals[D][2];	//!< Intervals of the region within the global dimensions.
		iter_type dims[D];			//!< Global dimensions.

		template<size_t... Is>
		region_interval(const len_type(&dims)[D], len_type boundary_size, std::index_sequence<Is...>) :
		...
		region_interval& operator+=(region_interval<0> const& other)
		{
			return *this;
		}
		...
		{
			return grid::index_from_position(n, 
				dims, 
				(iter_type[D]) { -intervals[Is][0]... }, 
				(iter_type[D]) { stride_element<Is>((iter_type[D]) { (intervals[Is][1] - intervals[Is][0])... })... });
		}
		...
		{
			return grid::index_from_position(n, 
				(iter_type[D]) { (intervals[Is][1] - intervals[Is][0])... }, 
				(iter_type[D]) { intervals[Is][0]... }, 
				(iter_type[D]) { stride_element<Is>(dims)... });
		}
	...
	struct region_interval_multiple : region_interval<D>
	{
		...
		struct subdomain_t
		{
			...
			template<size_t... Is>
			subdomain_t(const iter_type(&intervals)[D][2], std::index_sequence<Is...>) : intervals{ { intervals[Is][0], intervals[Is][1] }... } {}
			subdomain_t(const iter_type(&intervals)[D][2]) : subdomain_t(intervals, std::make_index_sequence<D>{}) {}
		...
		region_interval_multiple(const iter_type(&intervals)[D][2]) : parent_type(intervals) {}
		region_interval_multiple(region_interval<D> const& region) : parent_type(region.dims), regions{ break_region<interval_type>(intervals, region) } {}

		region_interval_multiple(const len_type(&dims)[D], len_type boundary_size) : parent_type(dims), regions{}
		{
			for (iter_type i = 0; i < D; ++i)
			{
		...
		region_interval_multiple& operator+=(std::vector<interval_type> const& add_regions)
		{
					...
					for (auto it = work_list.begin(); it < work_list.end();)
					{
						...
						for (const auto& diff_region : diff_regions)
						{
							...
							if (grid::is_overlapping(diff_region, *it))
							{
								...
								auto diff = it - work_list.begin();
								work_list.erase(work_list.begin() + diff, work_list.begin() + diff + 1);
								for (auto check_region : { first, second, third })
								{
									if (grid::length<D>(check_region) > 0)
									{
										...
										else if (!grid::is_fully_overlapping(diff_region, check_region))
										{
		...
		region_interval_multiple& operator+=(region_interval_multiple<D> const& add_regions)
		{
		...
		region_interval_multiple& operator-=(region_interval_multiple<D> const& diff_regions)
		{
	...
	struct region_extent
	{
		...
		region_extent(region_interval<D> const& interval) : region_extent(interval, std::make_index_sequence<D>{}) {}
		...
		region_extent(const iter_type(&intervals)[D][2]) : region_extent(intervals, std::make_index_sequence<D>{}) {}

		template<size_t... Is>
		region_extent(const len_type(&origin)[D], const len_type(&dims)[D], len_type boundary_size, std::index_sequence<Is...>) :
			dims{ dims[Is]...},
			stride{ stride_element<Is>(dims)... },
			shift{ ((origin[Is] + boundary_size) * stride[Is])... },
			delta{ compute_delta(dims, stride) } {}

		region_extent(const len_type(&origin)[D], const len_type(&dims)[D], len_type boundary_size = 0) : 
			region_extent(origin, dims, boundary_size, std::make_index_sequence<D>{}) {}

		template<size_t... Is>
		region_extent(const len_type(&dims)[D], len_type boundary_size, std::index_sequence<Is...>) :
			dims{ dims[Is]...},
			stride{ stride_element<Is>(dims)... },
		...
		region_extent(grid::select_region<D> const& region) : region_extent(region.origin, region.dims, region.boundary_size) {}

		region_extent(symphas::grid_info const& info) : region_extent(region_interval<D>(info)) {}
	...
	//	iter_type global_dims[D];		//!< Dimensions of the global domain.
	//	iter_type origin[D];			//!< Origin of the region, to check for bounds wrapping.
	...
	//	template<size_t... Is>
	//	region_group(region_interval<D> const& interval, std::index_sequence<Is...>) :
	//		parent_type(interval, std::index_sequence<Is...>{}),
	//		global_dims{ interval.dims[Is]... },
	//		origin{ interval[Is][0]... } {}

	//	region_group(region_interval<D> const& interval) : region_group(interval, std::make_index_sequence<D>{}) {}
	...
	struct region_index_list
	{
		...
		region_index_list(region_interval<D> const& intervals) : 
			iters{ (grid::length<D>(intervals) > 0) ? new iter_type[grid::length<D>(intervals)] : nullptr }, len{ grid::length<D>(intervals) }
		{
			...
			{
				iter_type pos[D];
				iters[n] = grid::index_from_position(n, region.dims, origin, region.stride);
			}
		}

		region_index_list(const len_type* dims, len_type boundary_size) :
			iters{ (grid::length_interior<D>(dims, boundary_size) > 0) ? new iter_type[grid::length_interior<D>(dims, boundary_size)] : nullptr }, 
		...
		region_index_list(region_index_list<D> const& other) : iters{ new iter_type[other.len] }, len{ other.len }
		{
	...
	template<size_t D>
	using region_const_iterator = symphas::lib::basic_forward_iterator<const grid::region_interval_multiple<D>>;
	template<size_t D>
	using region_iterator = symphas::lib::basic_forward_iterator<grid::region_interval_multiple<D>>;
	...
	{
		return region_const_iterator<D>(*this, regions.size());
	}
	...
	template<size_t D>
	region_interval_multiple<D> operator-(region_interval<D> const& first, region_interval<D> const& second)
	{
	...
	template<size_t D>
	region_interval_multiple<D> operator-(region_interval<D> const& first, region_interval_multiple<D> const& second)
	{
		region_interval_multiple<D> region(first);
		return region -= second;
	}
	...
	template<size_t D>
	region_interval_multiple<D> operator-(region_interval_multiple<D> const& first, region_interval<D> const& second)
	{
		region_interval_multiple<D> region(first);
		return region -= second;
	}
	...
	template<size_t D>
	region_interval_multiple<D> operator-(region_interval_multiple<D> const& first, region_interval_multiple<D> const& second)
	{
		region_interval_multiple<D> region(first);
		return region -= second;
	}
	...
	template<size_t D>
	region_interval<D> operator-(region_interval<D> const& first, region_empty const& second)
	{
		return first;
	}

	template<size_t D>
	region_interval_multiple<D> operator-(region_interval_multiple<D> const& first, region_empty const& second)
	{
		return first;
	}

	template<size_t D>
	region_empty operator-(region_empty const& first, region_interval<D> const& second)
	{
		return first;
	}

	template<size_t D>
	region_empty operator-(region_empty const& first, region_interval_multiple<D> const& second)
	{
		return first;
	}

	inline auto operator+(region_interval<0> const& first, region_interval<0> const& second)
	{
		return first;
	}
	...
	template<size_t D>
	auto operator+(region_interval<D> const& first, region_interval<D> const& second)
	{
		auto result(first);
		result += second;
		return result;
	...
	template<size_t D>
	auto operator+(region_interval_multiple<D> const& first, region_interval_multiple<D> const& second)
	{
		auto result(first);
		result += second;
		return result;
	...
	template<size_t D>
	auto operator+(region_interval<D> const& first, region_interval<0> const& second)
	{
		return first;
	}

	template<size_t D>
	auto operator+(region_interval<0> const& first, region_interval<D> const& second)
	{
		return second + first;
	}
	...
	inline auto operator+(region_interval<0> const& first, region_size const& second)
	{
		return second;
	}

	inline auto operator+(region_size const& first, region_interval<0> const& second)
	{
		return second + first;
	}
	...
	template<size_t D>
	auto operator+(region_size const& first, region_interval_multiple<D> const& second)
	{
		return second + first;
	}

	template<size_t D>
	auto operator+(region_interval_multiple<D> const& first, region_empty const& second)
	{
		return first;
	}

	template<size_t D>
	auto operator+(region_empty const& first, region_interval_multiple<D> const& second)
	{
		return second + first;
	}
	...
	template<size_t D>
	auto operator+(region_interval_multiple<D> const& first, region_interval<D> const& second)
	{
		auto result(first);
		result += second;
		return result;
	...
	template<size_t D>
	auto operator+(region_interval<D> const& first, region_interval_multiple<D> const& second)
	{
		return second + first;
	}
	...
	template<size_t D>
	auto operator+(region_interval<0> const& first, region_interval_multiple<D> const& second)
	{
		return second;
	}

	template<size_t D>
	auto operator+(region_interval_multiple<D> const& first, region_interval<0> const& second)
	{
		return second + first;
	}
	...
	//template<size_t D>
	//iter_type add_grid_position(iter_type offset, iter_type(&pos)[D], const iter_type (&dims)[D], const iter_type (&stride)[D])
	//{
	//	for (iter_type i = 0; i < D; ++i)
	//	{
	...
	//	iter_type dims[D];			//!< Dimensions of the local region within global region.
	//	iter_type stride[D];		//!< Stride of the global region.
	//	iter_type pos[D];			//!< Position inside the region represented by n.
	//	iter_type n;


	//	template<size_t... Is>
	//	region_position(region_interval<D> const& interval, std::ptrdiff_t pos, std::index_sequence<Is...>) :
	//		dims{ (interval[Is][1] - interval[Is][0])... },
	//		stride{ stride_element<Is>(interval.dims)... },
	//		pos{ position((iter_type)pos, (interval[Is][1] - interval[Is][0]), stride[Is])...},
	//		n{ grid::index_from_position((iter_type)pos, dims, (iter_type[D]) { interval[Is][0]... }, stride) }
	//	{}

	//	region_position(region_interval<D> const& interval, std::ptrdiff_t pos) : region_position(interval, pos, std::make_index_sequence<D>{}) {}
...
namespace symphas::internal
{
	...
	struct iterator_difference_type : iterator_difference_type_impl<iterator_difference_type<G>>
	{
		...
		iterator_difference_type(difference_type pos) : iterator_difference_type(nullptr, pos) {}
		iterator_difference_type(G* ptr, difference_type pos) : parent_type(pos), ptr{ ptr } {}
	...
	struct iterator_selection_difference_type : iterator_difference_type_impl<iterator_selection_difference_type<G>>
	{
		...
		iterator_selection_difference_type(difference_type pos) : iterator_selection_difference_type(nullptr, nullptr, pos) {}
		iterator_selection_difference_type(G* ptr, const iter_type* iters, difference_type pos) : parent_type(pos), ptr{ ptr }, iters{ iters } {}
	...
	struct iterator_region_difference_type : iterator_difference_type_impl<iterator_region_difference_type<G, D>>
	{
		using parent_type = iterator_difference_type_impl<iterator_region_difference_type<G, D>>;
		using difference_type = typename parent_type::difference_type;
		using parent_type::pos;

		iterator_region_difference_type(difference_type pos) : iterator_region_difference_type(nullptr, grid::region_extent<D>(), pos) {}
		iterator_region_difference_type(G* ptr, grid::region_interval<D> intervals, difference_type pos) : parent_type(pos), ptr{ ptr }, region{ intervals } {}
		iterator_region_difference_type(G* ptr, grid::region_extent<D> region, difference_type pos) : parent_type(pos), ptr{ ptr }, region{ region } {}
		iterator_region_difference_type() : iterator_region_difference_type(nullptr, {}, 0) {}
	...
	template<typename G, size_t D>
	struct iterator_group_difference_type : iterator_difference_type_impl<iterator_group_difference_type<G, D>>
	{
		using parent_type = iterator_difference_type_impl<iterator_group_difference_type<G, D>>;
		using difference_type = typename parent_type::difference_type;
		using parent_type::pos;

		iterator_group_difference_type(difference_type pos) : iterator_group_difference_type(nullptr, {}, pos) {}
		iterator_group_difference_type(G* ptr, grid::region_interval<D> intervals, difference_type pos) : parent_type(pos), ptr{ ptr }, region{ intervals } {}
		iterator_group_difference_type() : iterator_group_difference_type(nullptr, {}, 0) {}
...
namespace symphas
{
	...
	template<iterator_key type> struct iterator_policy {};
	 ...
	 */
	template<typename G>
	struct data_iterator : iterator_type_impl<data_iterator<G>,
	...
	>
	{
		 ...
		 */
		template<typename specialized_difference = symphas::internal::iterator_difference_type<G>>
		data_iterator(symphas::internal::iterator_difference_type_impl<specialized_difference> const& ptr)
		...
		{
			return symphas::internal::data_value_type<G>{}(ptr.ptr, ptr.pos);
		};

		//! Dereference past the iterator.
		inline decltype(auto) operator[](difference_type given_pos)
		{
			return symphas::internal::data_value_type<G>{}(ptr.ptr, iter_type(ptr.pos + given_pos));
		}
	 ...
	 */
	template<typename G>
	struct data_iterator_selection : iterator_type_impl<data_iterator_selection<G>,
		std::random_access_iterator_tag,
		typename symphas::internal::data_value_type<G>::type,
		typename symphas::internal::data_value_type<G>::ref,
		std::ptrdiff_t,
	...
	>
	{
		 ...
		 * which is the first index in the iterator.
		 */
		template<typename specialized_difference = symphas::internal::iterator_selection_difference_type<G>>
		data_iterator_selection(symphas::internal::iterator_difference_type_impl<specialized_difference> const& ptr)
			: ptr{ *static_cast<specialized_difference const*>(&ptr) } {}

		data_iterator_selection(difference_type pos = {})
			: ptr{ pos } {}
		 ...
		 * which is the first index in the iterator.
		 */
		explicit data_iterator_selection(G& data, const iter_type* iters, difference_type pos = 0)
			: ptr{ &data, iters, pos } {}
		 ...
		 */
		template<size_t D>
		explicit data_iterator_selection(G& data, grid::region_index_list<D> const& list, difference_type pos = 0)
		...
		inline decltype(auto) operator[](difference_type given_pos)
		{
			return symphas::internal::data_value_type<G>{}(ptr.ptr, ptr.iters[ptr.pos + given_pos]);
		}
	 ...
	 */
	template<typename G, size_t D>
	struct data_iterator_region : iterator_type_impl<data_iterator_region<G, D>,
		std::random_access_iterator_tag,
		typename symphas::internal::data_value_type<G>::type,
		typename symphas::internal::data_value_type<G>::ref,
		std::ptrdiff_t,
	...
	>
	{
		 ...
		 * which is the first index in the iterator.
		 */
		template<typename specialized_difference = symphas::internal::iterator_region_difference_type<G, D>>
		data_iterator_region(symphas::internal::iterator_difference_type_impl<specialized_difference> const& ptr)
			: ptr{ *static_cast<specialized_difference const*>(&ptr) } {}

		data_iterator_region(difference_type pos = {})
			: ptr{ pos } {}
		 ...
		 * which is the first index in the iterator.
		 */
		explicit data_iterator_region(G& data, grid::region_interval<D> const& interval, difference_type pos = 0)
		...
		explicit data_iterator_region(G* data, grid::region_extent<D> const& region, difference_type pos = 0)
			: ptr{ data, region, pos } {}
		explicit data_iterator_region(G& data, grid::region_extent<D> const& region, difference_type pos = 0)
			: ptr{ data, region, pos } {}
	 ...
	 */
	template<typename G, size_t D>
	struct data_iterator_group : iterator_type_impl<data_iterator_group<G, D>,
		std::random_access_iterator_tag,
		typename symphas::internal::data_value_type<G>::type,
		typename symphas::internal::data_value_type<G>::ref,
		std::ptrdiff_t,
	...
	>
	{
		 ...
		 */
		template<typename specialized_difference = symphas::internal::iterator_region_difference_type<G, D>>
		data_iterator_group(symphas::internal::iterator_difference_type_impl<specialized_difference> const& ptr)
			: ptr{ *static_cast<specialized_difference const*>(&ptr) } {}

		data_iterator_group(difference_type pos = {})
			: ptr{ pos } {}
		 ...
		 * which is the first index in the iterator.
		 */
		explicit data_iterator_group(G& data, grid::region_interval<D> const& interval, difference_type pos = 0)
			: ptr{ &data, interval, pos } {}
...
namespace symphas
{
	...
	struct iterator_type_impl
	{
		...
		friend specialized_iterator operator+(difference_type const& offset, specialized_iterator const& rhs)
		{
			auto result(rhs);

...
namespace grid {
/*!
 * Checks if two regions overlap by comparing their boundaries.
 *
 * Determines if there is any overlap between two regions by comparing the left
 * and right boundaries of each region. A region is defined by its left and
 * right boundaries along a single dimension.
 *
 * \param left0 The left boundary of the first region.
 * \param left1 The left boundary of the second region.
 * \param right0 The right boundary of the first region.
 * \param right1 The right boundary of the second region.
 * \return True if the regions overlap, false otherwise.
 */
__host__ __device__ inline bool is_overlapping_compare(iter_type left0,
                                                       iter_type left1,
                                                       iter_type right0,
                                                       iter_type right1) {
 ...
 * right boundaries along a single dimension.
 *
 * \param region0 The left and right boundaries of the first region.
 * \param region1 The left and right boundaries of the second region.
 * \return True if the regions overlap, false otherwise.
 */
template <size_t D, size_t... Is>
__host__ __device__ bool is_overlapping(const iter_type (&region0)[D][2],
                                        const iter_type (&region1)[D][2],
                                        std::index_sequence<Is...>) {
  return (is_overlapping_compare(region0[Is][0], region1[Is][0], region0[Is][1],
                                 region1[Is][1]) &&
          ...);
}
...
/*!
 * Checks if two regions are fully overlapping.
 *
 * Determines if one region is fully contained within another region by
 * comparing the left and right boundaries of each region. A region is defined
 * by its left and right boundaries along a single dimension.
 *
 * \param region0 The left and right boundaries of the first region.
 * \param region1 The left and right boundaries of the second region.
 * \return True if the first region is fully contained within the second region,
 * false otherwise.
 */
template <size_t D, size_t... Is>
__host__ __device__ bool is_fully_overlapping(const iter_type (&region0)[D][2],
                                              const iter_type (&region1)[D][2],
                                              std::index_sequence<Is...>) {
  return (
      (region0[Is][0] <= region1[Is][0] && region0[Is][1] >= region1[Is][1]) &&
      ...);
}
 ...
 */
template <size_t D>
__host__ __device__ bool is_fully_overlapping(
    const iter_type (&region0)[D][2], const iter_type (&region1)[D][2]) {
  return is_fully_overlapping(region0, region1, std::make_index_sequence<D>{});
}

/*!
 * Checks if two regions are in contact.
 *
 * Determines if two regions are in contact by comparing the left and right
 * boundaries of each region. A region is defined by its left and right
 * boundaries along a single dimension.
 *
 * \param region0 The left and right boundaries of the first region.
 * \param region1 The left and right boundaries of the second region.
 * \return True if the regions are in contact, false otherwise.
 */
__host__ __device__ inline bool is_contact_overlapping_compare(
    iter_type left0, iter_type left1, iter_type right0, iter_type right1) {
  return (right0 >= left1 && right1 >= left0);
  // return (((region0[Is][0] < region1[Is][0]) ? region0[Is][0] :
  // region1[Is][0] <= (region0[Is][1] < region1[Is][1]) ? region0[Is][1] :
  // region1[Is][1]) && ...);
}
 ...
 */
template <size_t D, size_t... Is>
__host__ __device__ bool is_contact_overlapping(
    const iter_type (&region0)[D][2], const iter_type (&region1)[D][2],
    std::index_sequence<Is...>) {
  return (is_contact_overlapping_compare(region0[Is][0], region1[Is][0],
                                         region0[Is][1], region1[Is][1]) &&
          ...);
}
...
__host__ __device__ bool is_contact_overlapping(
    const iter_type (&region0)[D][2], const iter_type (&region1)[D][2]) {
  return is_contact_overlapping(region0, region1,
 ...
 *
 * Determines the union of two regions by comparing the left and right
 * boundaries of each region. A region is defined by its left and right
 * boundaries along a single dimension.
 *
 * \param result The left and right boundaries of the resulting region.
 * \param region0 The left and right boundaries of the first region.
 * \param region1 The left and right boundaries of the second region.
 */
template <size_t D>
__host__ __device__ void get_region_union(iter_type (&result)[D][2],
                                          const iter_type (&region0)[D][2],
                                          const iter_type (&region1)[D][2]) {
  for (iter_type i = 0; i < D; ++i) {
    result[i][0] =
        (region0[i][0] < region1[i][0]) ? region0[i][0] : region1[i][0];
    result[i][1] =
        (region0[i][1] > region1[i][1]) ? region0[i][1] : region1[i][1];
  }
}
 ...
 *
 * Determines the intersection of two regions by comparing the left and right
 * boundaries of each region. A region is defined by its left and right
 * boundaries along a single dimension.
 *
 * \param result The left and right boundaries of the resulting region.
 ...
 */
template <size_t D>
__host__ __device__ void get_region_intersection(
    iter_type (&result)[D][2], const iter_type (&region0)[D][2],
    const iter_type (&region1)[D][2]) {
  for (iter_type i = 0; i < D; ++i) {
    result[i][0] =
        (region0[i][0] > region1[i][0]) ? region0[i][0] : region1[i][0];
...
__host__ __device__ inline bool is_overlapping(
    const iter_type (&region0)[1][2], const iter_type (&region1)[1][2]) {
  return is_overlapping<1>(region0, region1);
}
__host__ __device__ inline bool is_overlapping(
    const iter_type (&region0)[2][2], const iter_type (&region1)[2][2]) {
  return is_overlapping<2>(region0, region1);
}
__host__ __device__ inline bool is_overlapping(
    const iter_type (&region0)[3][2], const iter_type (&region1)[3][2]) {
  return is_overlapping<3>(region0, region1);
}

__host__ __device__ inline bool is_fully_overlapping(
    const iter_type (&region0)[1][2], const iter_type (&region1)[1][2]) {
...
__host__ __device__ inline bool is_contact_overlapping(
    const iter_type (&region0)[1][2], const iter_type (&region1)[1][2]) {
...
__host__ __device__ inline void get_region_union(
    iter_type (&result)[1][2], const iter_type (&region0)[1][2],
    const iter_type (&region1)[1][2]) {
  get_region_union<1>(result, region0, region1);
}
__host__ __device__ inline void get_region_union(
    iter_type (&result)[2][2], const iter_type (&region0)[2][2],
    const iter_type (&region1)[2][2]) {
  get_region_union<2>(result, region0, region1);
}
__host__ __device__ inline void get_region_union(
    iter_type (&result)[3][2], const iter_type (&region0)[3][2],
    const iter_type (&region1)[3][2]) {
  get_region_union<3>(result, region0, region1);
}

__host__ __device__ inline void get_region_intersection(
    iter_type (&result)[1][2], const iter_type (&region0)[1][2],
    const iter_type (&region1)[1][2]) {
 ...
 */
template <size_t D>
__host__ __device__ len_type length_interior(
    len_type const* dimensions, len_type boundary_size = BOUNDARY_DEPTH);

template <>
__host__ __device__ inline len_type length_interior<1>(
    len_type const* dimensions, len_type boundary_size) {
  if (dimensions != nullptr) {
    len_type len = dimensions[0] - boundary_size - boundary_size;
    return (len > 0) ? len : 0;
  }
  return 0;
}
...
template <size_t D>
__host__ __device__ len_type length_interior(len_type const* dimensions,
                                             len_type boundary_size) {
  if (dimensions != nullptr) {
    len_type len = dimensions[D - 1] - boundary_size - boundary_size;
    if (len > 0) return len * length_interior<D - 1>(dimensions, boundary_size);
    return 0;
  }
  return 0;
}
 ...
 */
template <size_t D>
__host__ __device__ len_type length(symphas::interval_data_type intervals) {
 ...
 */
template <size_t D>
__host__ __device__ len_type
length_interior(symphas::interval_data_type intervals) {
  len_type dimensions[D];
  for (iter_type i = 0; i < D; ++i) {
    dimensions[i] =
        intervals.at(symphas::index_to_axis(i)).get_interval_count();
  }
...
template <typename T>
struct carry_value {
  __host__ __device__ carry_value() : value{&fallback}, fallback{0} {}
  __host__ __device__ carry_value(T fallback)
      : value{&this->fallback}, fallback{fallback} {}
  __host__ __device__ carry_value(T* value, T fallback)
      : value{value}, fallback{fallback} {}
  __host__ __device__ carry_value(T* value, T fallback, bool valid)
      : value{this->value = (valid) ? value : &this->fallback},
        fallback{fallback} {}
  carry_value(carry_value<T> const& other)
      : carry_value{other.value, other.fallback, other.is_valid()} {}
  carry_value(carry_value<T>&& other) : carry_value() { swap(*this, other); }

  __host__ __device__ friend void swap(carry_value<T>& first,
                                       carry_value<T>& second) {
    using std::swap;
    T* tmp = second.value;
    if (first.is_valid()) {
  ...
  __host__ __device__ carry_value& operator=(T const& other) {
    *value = (is_valid()) ? other : *value;
    return *this;
  }

  __host__ __device__ carry_value& operator=(carry_value<T> const& other) {
...
namespace grid {
/*!
 * Determines if two positions in a grid of dimension D are the same.
 *
 * This function compares two positions, each represented as an array of
 * coordinates, to check if they are identical across all dimensions. It is a
 * template function that can be applied to grids of any dimensionality.
 *
 * \tparam D The dimensionality of the grid.
 * \tparam Is A parameter pack representing a sequence of indices, used for
 * unpacking the dimensions. \param pos0 The first position to compare,
 * represented as an array of coordinates. \param pos1 The second position to
 * compare, represented as an array of coordinates. \return True if the
 * positions are the same across all dimensions, false otherwise.
 */
template <size_t D, size_t... Is>
__host__ __device__ inline bool is_same(const len_type (&pos0)[D],
                                        const len_type (&pos1)[D],
                                        std::index_sequence<Is...>) {
 ...
 *
 * Check if two positions are the same by comparing the values of each
...
__host__ __device__ inline iter_type position(iter_type n, len_type dim,
                                              const iter_type stride) {
  return (n / stride) % dim;
}

/*!
 * Calculates the stride along a specified axis in a 1-dimensional grid.
 *
 * This function computes the stride, which is the step size to move one element
 * along the specified axis in memory. For a 1-dimensional grid, the stride is
 * always 1, indicating consecutive elements along the axis. This function is
 * templated on the axis to allow for future expansion to grids of higher
 * dimensionality where the stride may vary depending on the axis.
 *
 * \tparam ax The axis along which to calculate the stride. Defaults to Axis::X.
 * \param[out] stride The calculated stride for the specified axis. For a 1D
 * grid, this is always set to 1. \param[in] dims The dimensions of the grid.
 * For a 1D grid, this is a single-element array containing the size of the
 * grid.
 */
template <Axis ax = Axis::X>
__device__ __host__ void get_stride(len_type (&stride)[1],
                                    len_type const* dims) {
  stride[0] = 1;
}

/*!
 * Calculates the stride along a specified axis in a 2-dimensional grid.
 *
 * This function computes the stride, which is the step size to move one element
 * along the specified axis in memory. For a 2-dimensional grid, the stride is
 * the size of the other dimension, indicating that elements are stored
 * consecutively along the specified axis. This function is templated on the
 * axis to allow for future expansion to grids of higher dimensionality where
 * the stride may vary depending on the axis.
 *
 * \tparam ax The axis along which to calculate the stride. Defaults to Axis::X.
 * \param[out] stride The calculated stride for the specified axis. For a 2D
 * grid, this is set to the size of the other dimension. \param[in] dims The
 * dimensions of the grid. For a 2D grid, this is a two-element array containing
 * the size of each dimension.
 */
template <Axis ax = Axis::X>
__device__ __host__ void get_stride(len_type (&stride)[2],
                                    len_type const* dims) {
...
/*!
 * Calculates the stride along a specified axis in a 3-dimensional grid.
 *
 * This function computes the stride, which is the step size to move one element
 * along the specified axis in memory. For a 3-dimensional grid, the stride is
 * the size of the other two dimensions, indicating that elements are stored
 * consecutively along the specified axis. This function is templated on the
 * axis to allow for future expansion to grids of higher dimensionality where
 * the stride may vary depending on the axis.
 *
 * \tparam ax The axis along which to calculate the stride. Defaults to Axis::X.
 * \param[out] stride The calculated stride for the specified axis. For a 3D
 * grid, this is set to the size of the other two dimensions. \param[in] dims
 * The dimensions of the grid. For a 3D grid, this is a three-element array
 * containing the size of each dimension.
 */
template <Axis ax = Axis::X>
__device__ __host__ void get_stride(len_type (&stride)[3],
                                    len_type const* dims) {
  if constexpr (ax == Axis::X) {
    stride[0] = 1;
    stride[1] = dims[0];
...
template <size_t D>
__host__ __device__ inline void get_grid_position(iter_type (&pos)[D],
...
__host__ __device__ inline void get_grid_position(iter_type (&pos)[1],
                                                  const len_type (&dims)[1],
                                                  iter_type n) {
  pos[0] = n;
}

__host__ __device__ inline void get_grid_position(iter_type (&pos)[2],
                                                  const len_type (&dims)[2],
                                                  iter_type n) {
  pos[1] = n / dims[0];
  pos[0] = n - dims[0] * pos[1];
}

__host__ __device__ inline void get_grid_position(iter_type (&pos)[3],
                                                  const len_type (&dims)[3],
                                                  iter_type n) {
  pos[0] = n % dims[0];
...
__host__ __device__ inline void get_grid_position_offset(
    iter_type (&pos)[1], const len_type (&dims)[1], const len_type (&offset)[1],
    iter_type n) {
  pos[0] = n + offset[0];
}

__host__ __device__ inline void get_grid_position_offset(
    iter_type (&pos)[3], const len_type (&dims)[3], const len_type (&offset)[3],
    iter_type n) {
...
template <size_t D>
__host__ __device__ void get_grid_position_offset(iter_type (&pos)[D],
                                                  const len_type (&dims)[D],
                                                  const len_type (&offset)[D],
                                                  iter_type n) {
...
__host__ __device__ inline void get_grid_position(pos_list& pos,
                                                  dim_list const& dims,
                                                  dim_list const& stride,
                                                  iter_type n) {
...
__host__ __device__ inline void get_grid_position_offset(pos_list& pos,
                                                         dim_list const& dims,
                                                         dim_list const& stride,
                                                         pos_list const& offset,
                                                         iter_type n) {
  get_grid_position(pos, dims, stride, n);
  for (iter_type i = 0; i < pos.n; ++i) {
    pos[i] += offset[i];
...
/*
 * helper functions to create an array for iterating over the interior values of
 * a grid
 *
 * works by creating function with variables of static duration; calling the get
 * function will create the array of interior index values the first time, then
 * each subsequent call is made to immediately return the inner indices that was
 * last used (based on the product of the dimensions)
 *
 * hence, each grid creation will immediately have access to the same list of
 * inner values, which increase runtime and more importantly, decreases memory
 * cost
 */
template <size_t I>
void fill_inner_arr(iter_type*(&inner_i), len_type const* dimensions,
                    iter_type& index);

template <>
inline void fill_inner_arr<0>(iter_type*(&inner_i), len_type const* dimensions,
                              iter_type& index) {
  for (iter_type i = 0; i < dimensions[0] - BOUNDARY_DEPTH - BOUNDARY_DEPTH;
       ++i) {
    *(inner_i++) = index++;
  }
}
...
template <>
inline void fill_inner_arr<1>(iter_type*(&inner_i), len_type const* dimensions,
                              iter_type& index) {
  iter_type inc = BOUNDARY_DEPTH + BOUNDARY_DEPTH;
  for (iter_type i = 0; i < dimensions[1] - BOUNDARY_DEPTH - BOUNDARY_DEPTH;
       ++i, index += inc) {
...
void fill_inner_arr(iter_type*(&inner_i), len_type const* dimensions,
  ...
  for (iter_type i = 0; i < dimensions[I] - BOUNDARY_DEPTH - BOUNDARY_DEPTH;
       ++i, index += inc) {
...
template <size_t D>
iter_type get_init_index(len_type const* dimensions) {
  return grid::length<D - 1>(dimensions) * BOUNDARY_DEPTH +
         get_init_index<D - 1>(dimensions);
}
...
template <size_t D>
void insert_dimensions_arr(dim_arr_type<D>*(&dimensions_arr), len_type index,
                           len_type const* dimensions) {
  for (iter_type i = 0; i < D; ++i) {
...
template <size_t D>
void insert_inner_arr(iter_type**(&inner_arr), len_type index,
                      len_type const* dimensions) {
...
void extend_dimensions_arr(dim_arr_type<D>*(&dimensions_arr), len_type len) {
  dim_arr_type<D>* dimensions_arr_2 =
      new dim_arr_type<D>[len + INNERS_ARR_INCREMENT];
...
iter_type* interior_indices_list(len_type const* dimensions) {
  ...
  static len_type capacity =
      INNERS_ARR_START;        // the total capacity of the arrays
...
struct select_grid_index<1> {
  ...
  template <typename T>
  T& operator()(T* values, iter_type n) const {
    return values[n];
  }

  template <typename T>
  const T& operator()(T const* values, iter_type n) const {
    return values[n];
  }

  template <typename T>
  multi_value<1, T> operator()(T* (&values)[1], iter_type n) const {
  ...
  template <typename T>
  multi_value<1, T> operator()(const T* (&values)[1], iter_type n) const {
    multi_value<1, T> value;
    value[0] = values[0][n];
    return value;
  }
...
struct select_grid_index<2> {
  ...
  template <typename T>
  multi_value<2, T> operator()(T* (&values)[2], iter_type x,
                               iter_type y) const {
  ...
  template <typename T>
  multi_value<2, T> operator()(const T* (&values)[2], iter_type x,
                               iter_type y) const {
    multi_value<2, T> value;
    value[0] = operator()(values[0], x, y);
    value[1] = operator()(values[1], x, y);
...
struct select_grid_index<3> {
  select_grid_index(const len_type (&dims)[3])
      : stride{dims[0], dims[0] * dims[1]} {}
  len_type stride[2];
  ...
  template <typename T>
  const T& operator()(T const* values, iter_type x, iter_type y,
                      iter_type z) const {
    return values[z * stride[1] + y * stride[0] + x];
  }

  template <typename T>
  multi_value<3, T> operator()(T* (&values)[3], iter_type x, iter_type y,
                               iter_type z) const {
  ...
  template <typename T>
  multi_value<3, T> operator()(const T* (&values)[3], iter_type x, iter_type y,
                               iter_type z) const {
    multi_value<3, T> value;
    value[0] = operator()(values[0], x, y, z);
    value[1] = operator()(values[1], x, y, z);
...
__device__ __host__ inline double distance(iter_type pos0, iter_type pos1) {
  double delta = pos0 - pos1;
  return delta * delta;
}

template <size_t D, size_t... Is>
__device__ __host__ double distance(const iter_type (&pos0)[D],
                                    const iter_type (&pos1)[D],
                                    std::index_sequence<Is...>) {
...
__device__ __host__ inline bool is_same_point(iter_type pos0, iter_type pos1) {
  return pos0 == pos1;
}

template <size_t D, size_t... Is>
__device__ __host__ bool is_same_point(const iter_type (&pos0)[D],
                                       const iter_type (&pos1)[D],
                                       std::index_sequence<Is...>) {
...
__device__ __host__ inline bool is_in_region(iter_type pos,
                                             const iter_type dims) {
  return (pos >= 0 && pos < dims);
}

template <size_t D, size_t... Is>
__device__ __host__ bool is_in_region(const iter_type (&pos)[D],
...
__device__ __host__ inline bool is_in_region_boundary(iter_type pos,
                                                      iter_type dims,
                                                      len_type boundary_size) {
...
__device__ __host__ bool is_in_region(const iter_type (&pos)[D],
                                      ...
                                      const iter_type (&offset)[D],
                                      len_type boundary_size) {
  return is_in_region(pos, dims, boundary_size, offset,
                      std::make_index_sequence<D>{});
}

__device__ __host__ inline bool is_in_region(iter_type pos, iter_type left,
                                             iter_type right) {
...
template <size_t D, size_t... Is>
__device__ __host__ bool is_in_region(const iter_type (&intervals0)[D][2],
                                      const iter_type (&intervals1)[D][2],
                                      std::index_sequence<Is...>) {
  return is_in_region(symphas::arr_n_t<D>{intervals0[Is][0]...}.as_array(),
                      intervals1, std::make_index_sequence<D>{}) &&
         is_in_region(symphas::arr_n_t<D>{intervals0[Is][1]...}.as_array(),
                      intervals1, std::make_index_sequence<D>{});
}
...
__device__ __host__ inline bool not_in_region(const iter_type pos,
                                              const iter_type dims) {
  return (pos < 0 || pos >= dims);
}

template <size_t D, size_t... Is>
__device__ __host__ bool not_in_region(const iter_type (&pos)[D],
                                       const iter_type (&dims)[D],
                                       std::index_sequence<Is...>) {
...
__device__ __host__ inline bool not_in_region_boundary(const iter_type pos,
                                                       const iter_type dims,
                                                       len_type boundary_size) {
  return (pos < boundary_size || pos >= dims - boundary_size);
}
...
__device__ __host__ inline bool not_in_region(iter_type pos, iter_type left,
                                              iter_type right) {
  return (pos < left || pos >= right);
}
...
__device__ __host__ auto compute_periodic_stride(
    len_type (&stride)[D], const len_type (&dims)[D],
...
__device__ __host__ auto compute_periodic_strides(len_type (&strides)[N][D],
                                                  const len_type (&dims)[D],
                                                  std::index_sequence<Is...>) {
  (compute_periodic_stride(strides[Is], dims,
                           symphas::lib::nth_periodic_shift_t<Is, D>{}),
   ...);
}
...
// auto compute_periodic_offset0(len_type(&offset), const len_type(&dims)[D],
// std::integer_sequence<bool, Fs...>)
//{
...
//	compute_periodic_stride(stride, dims, std::integer_sequence<bool,
// Fs...>{}); 	get_stride(stride0, dims);

//	for (iter_type i = 0; i < D; ++i)
//	{
//		offset += dims[i] * stride[i]; //dims[i] * extend[i] *
// stride[i];
//	}
//}

// template<size_t D, size_t N, size_t... Is>
// auto compute_periodic_offsets(len_type(&offsets)[N][2], const
// len_type(&dims)[D], std::index_sequence<Is...>)
//{
//	(compute_periodic_offset0(offsets[Is][0], dims, get_periodic_shift<Is,
// D>()), ...); 	(compute_periodic_offset1(offsets[Is][1], dims,
// get_periodic_shift<Is, D>()), ...);
// }
...
struct select_region {
  select_region()
      : offset{}, stride{}, origin{}, dims{}, len{}, boundary_size{} {}

  select_region(const len_type (&origin)[D], const len_type (&dims)[D],
                len_type boundary_size = 0)
      : offset{},
        stride{},
        origin{},
  ...
  template <typename T>
  __device__ __host__ inline carry_value<T> operator()(
      iter_type (&pos)[D], T* values, const iter_type (&domain_dims)[D],
      const T& empty) const {
    for (iter_type i = 0; i < D; ++i) {
      pos[i] = (pos[i] >= origin[i] + boundary_size)
                   ? pos[i] - origin[i]
                   : pos[i] - origin[i] + domain_dims[i] - 2 * boundary_size;
    }
  ...
  template <typename T>
  __device__ __host__ inline carry_value<const T> operator()(
      iter_type (&pos)[D], const T* values, const iter_type (&domain_dims)[D],
      const T& empty) const {
    for (iter_type i = 0; i < D; ++i) {
      pos[i] = (pos[i] >= origin[i] + boundary_size)
                   ? pos[i] - origin[i]
  ...
  // template<typename T>
  // carry_value<const T> operator()(T const* values, const
  // iter_type(&origin)[D], const iter_type(&dims)[D], const T& empty, iter_type
  // (&pos)[D]) const
  //{
  //	for (iter_type i = 0; i < D; ++i)
  //	{
  ...
  //	for (iter_type i = 0; i < D; ++i)
  //	{
  //		pos[i] = (pos[i] >= 0) ? pos[i] : ((pos[i] - boundary_size +
  // domain_dims[i]) % domain_dims[i] + boundary_size);
  //	}
  //
  //	if (is_in_region(pos, dims, boundary_size))
  //	{
  ...
  template <typename T>
  __device__ __host__ inline multi_value<D, T> operator()(
  ...
  template <typename T>
  __device__ __host__ inline multi_value<D, T> operator()(
      iter_type (&pos)[D], T* (&values)[D], const iter_type (&domain_dims)[D],
      const T (&empty)[D]) const {
    multi_value<D, T> value;
    for (iter_type i = 0; i < D; ++i) {
      pos[i] = (pos[i] >= origin[i] + boundary_size)
  ...
  template <typename T>
  __device__ __host__ inline multi_value<D, T> operator()(
      iter_type (&pos)[D], T* const (&values)[D],
...
template <size_t D>
select_region(const len_type (&)[D], const len_type (&)[D],
              len_type boundary_size = 0) -> select_region<D>;
...
struct Block {
  ...
  explicit Block(const len_type* len, size_t dimensions = 1)
      : Block((len != nullptr) ? grid::length(len, dimensions) : 0) {}
  ...
  Block(Block<T>&& other) noexcept : Block() { swap(*this, other); }
...
struct multi_value {
  ...
  __device__ __host__ multi_value(T* (&value)[N], iter_type offset = 0)
      : multi_value() {
  ...
  __device__ __host__ multi_value(T* const (&value)[N], iter_type offset = 0)
      : multi_value() {
    for (iter_type i = 0; i < N; ++i) {
      this->value[i] = value[i] + offset;
    }
  ...
  __device__ __host__ multi_value(T* const value) : multi_value() {
    for (iter_type i = 0; i < N; ++i) {
      this->value[i] = &value[i];
    }
  }

  __device__ __host__ multi_value(T (&value)[N]) : multi_value() {
    for (iter_type i = 0; i < N; ++i) {
      this->value[i] = &value[i];
    }
  }

  __device__ __host__ multi_value(const T (&value)[N]) : multi_value() {
    for (iter_type i = 0; i < N; ++i) {
  ...
  __device__ __host__ T& operator[](iter_type i) { return *(value[i]); }

  __device__ __host__ bool is_valid() { return true; }

  //! Return a multi_value as a vector for compatibility.
  __device__ __host__ operator any_vector_t<T, N>() const;
   ...
   */
  __device__ __host__ multi_value& operator=(any_vector_t<T, N> const& vector);
  __device__ __host__ multi_value& operator=(any_vector_t<T, N>&& vector);
  __device__ __host__ multi_value& operator=(multi_value<N, T> const& other);
  __device__ __host__ multi_value& operator=(multi_value<N, T>&& other);
  __device__ __host__ multi_value& operator=(T const& other);

  __device__ __host__ multi_value(multi_value<N, T> const& other)
      : multi_value(other.value) {}
  __device__ __host__ multi_value(multi_value<N, T>&& other)
      : multi_value(other.value) {}
  ...
  __device__ __host__ multi_value() : value{} {}
};
...
struct MultiBlock {
  ...
  explicit MultiBlock(const len_type* len, size_t dimensions = 1)
      : MultiBlock((len != nullptr) ? grid::length(len, dimensions) : 0) {}
   ...
   * \param i The index of the value to return.
   */
  __device__ __host__ multi_value<N, T> operator[](iter_type i) const {
  ...
  T* operator()(Axis ax) const { return values[symphas::axis_to_index(ax)]; }
...
template <size_t N, typename T>
multi_value<N, T>& multi_value<N, T>::operator=(
...
struct Grid : Block<T> {
 ...
 public:
  ...
  Grid(grid::dim_list dimensions)
      : Block<T>{grid::length<D>(dimensions)}, dims{0} {
  ...
  const Grid<T, D>& as_grid() const { return *this; }
...
struct Grid<any_vector_t<T, D>, D> : MultiBlock<D, T> {
 ...
 public:
  ...
  const Grid<any_vector_t<T, D>, D>& as_grid() const { return *this; }

  Grid<any_vector_t<T, D>, D>& as_grid() { return *this; }
...
struct BoundaryGrid : Grid<T, D> {
  ...
  BoundaryGrid(grid::dim_list dimensions)
      : BoundaryGrid(static_cast<const len_type*>(dimensions)) {}
  ...
  const BoundaryGrid<T, D>& as_grid() const { return *this; }

  BoundaryGrid<T, D>& as_grid() { return *this; }
...
struct BoundaryGrid<T, 3> : Grid<T, 3> {
  ...
  const BoundaryGrid<T, 3>& as_grid() const { return *this; }
...
struct BoundaryGrid<T, 2> : Grid<T, 2> {
  ...
  const BoundaryGrid<T, 2>& as_grid() const { return *this; }
...
struct BoundaryGrid<T, 1> : Grid<T, 1> {
  ...
  const BoundaryGrid<T, 1>& as_grid() const { return *this; }
...
struct RegionalGrid : Grid<T, D> {
 ...
 public:
  RegionalGrid(grid::dim_list dimensions, T empty = REGIONAL_GRID_OUTER_VALUE,
               len_type boundary_size = BOUNDARY_DEPTH)
      : RegionalGrid(static_cast<const len_type*>(dimensions), empty,
                     boundary_size) {}
  RegionalGrid(const len_type* dimensions, T empty = REGIONAL_GRID_OUTER_VALUE,
               len_type boundary_size = BOUNDARY_DEPTH)
  ...
  RegionalGrid(RegionalGrid<T, D> const& other)
  ...
  const RegionalGrid<T, D>& as_grid() const { return *this; }
  ...
  void adjust(const iter_type (&new_origin)[D]) {
    grid::adjust_origin_to_from(parent_type::values, new_origin, region.origin,
                                region.dims, parent_type::dims, empty,
  ...
  void adjust(const iter_type (&new_origin)[D], const len_type (&new_dims)[D]) {
    ...
    grid::adjust_region_to_from(new_values, new_origin, new_dims,
                                parent_type::values, region.origin, region.dims,
                                parent_type::dims, empty, region.boundary_size);

    delete[] parent_type::values;
    parent_type::values = new_values;
...
struct RegionalGrid<any_vector_t<T, D>, D> : Grid<any_vector_t<T, D>, D> {
 ...
 public:
  template <size_t... Is>
  RegionalGrid(const len_type* dimensions, const T (&empty)[D],
  ...
  RegionalGrid(grid::dim_list dimensions, const T (&empty)[D],
               len_type boundary_size = BOUNDARY_DEPTH)
      : RegionalGrid(static_cast<const len_type*>(dimensions), empty,
                     boundary_size) {}

  template <size_t... Is>
  RegionalGrid(const len_type* dimensions, T empty, len_type boundary_size,
               std::index_sequence<Is...>)
      : RegionalGrid(dimensions,
                     symphas::array_values_type<T, D>{
                         symphas::lib::repeat_value<Is>(empty)...},
                     boundary_size, std::index_sequence<Is...>{}) {}
  RegionalGrid(const len_type* dimensions, T empty = REGIONAL_GRID_OUTER_VALUE,
               len_type boundary_size = BOUNDARY_DEPTH)
      : RegionalGrid(dimensions, empty, boundary_size,
                     std::make_index_sequence<D>{}) {}
  RegionalGrid(grid::dim_list dimensions, T empty = REGIONAL_GRID_OUTER_VALUE,
               len_type boundary_size = BOUNDARY_DEPTH)
      : RegionalGrid(static_cast<const len_type*>(dimensions), empty,
  ...
  template <size_t... Is>
  RegionalGrid(Grid<any_vector_t<T, D>, D>&& other,
  ...
  RegionalGrid(Grid<any_vector_t<T, D>, D> const& other)
      : RegionalGrid(other, std::make_index_sequence<D>{}) {}
  RegionalGrid(Grid<any_vector_t<T, D>, D>&& other) noexcept
      : RegionalGrid(other, std::make_index_sequence<D>{}) {}

  RegionalGrid(RegionalGrid<any_vector_t<T, D>, D> const& other)
      : RegionalGrid(nullptr, other.empty, other.region.boundary_size) {
    using std::swap;
  ...
  RegionalGrid<any_vector_t<T, D>, D>& as_grid() { return *this; }
  ...
  void adjust(const iter_type (&new_origin)[D]) {
    for (iter_type i = 0; i < D; ++i) {
      grid::adjust_origin_to_from(MultiBlock<D, T>::values[i], new_origin,
                                  region.origin, region.dims, parent_type::dims,
                                  empty[i], region.boundary_size);
    }
    region.update(new_origin);
  }
  ...
  void adjust(const iter_type (&new_origin)[D], const len_type (&new_dims)[D]) {
    for (iter_type i = 0; i < D; ++i) {
      ...
      grid::adjust_region_to_from(new_values, new_origin, new_dims,
                                  MultiBlock<D, T>::values[i], region.origin,
                                  region.dims, parent_type::dims, empty[i],
                                  region.boundary_size);
...
struct RegionalGridMPI : RegionalGrid<T, D> {
  ...
  RegionalGridMPI(symphas::multi_thr_info_type const& info,
                  const len_type* dimensions)
      : parent_type(mpi_dims(info, dimensions)) {
    update_dimensions(dimensions);
  }

  RegionalGridMPI(symphas::multi_thr_info_type const& info,
                  const len_type* dimensions, T empty)
      : parent_type(mpi_dims(info, dimensions), empty) {
    update_dimensions(dimensions);
  }

  RegionalGridMPI(symphas::multi_thr_info_type const& info,
                  const len_type* dimensions, T empty, len_type boundary_size)
      : parent_type(mpi_dims(info, dimensions), empty, boundary_size) {
    update_dimensions(dimensions);
  }

  RegionalGridMPI(symphas::multi_thr_info_type const& info,
                  grid::dim_list dimensions)
      : parent_type(mpi_dims(info, dimensions)) {
    update_dimensions(dimensions);
  }

  RegionalGridMPI(symphas::multi_thr_info_type const& info,
                  grid::dim_list dimensions, T empty)
      : parent_type(mpi_dims(info, dimensions), empty) {
    update_dimensions(dimensions);
  }

  RegionalGridMPI(symphas::multi_thr_info_type const& info,
                  grid::dim_list dimensions, T empty, len_type boundary_size)
      : parent_type(mpi_dims(info, dimensions), empty, boundary_size) {
    update_dimensions(dimensions);
  }

  RegionalGridMPI(RegionalGridMPI<T, D> const& other)
      : RegionalGridMPI(nullptr, other.empty, other.region.boundary_size) {
    using std::swap;
  ...
  __device__ __host__ auto operator[](iter_type n) const
      -> std::invoke_result_t<decltype(&parent_type::operator[]), parent_type,
                              iter_type> {
    if (parent_type::values != nullptr)
      return parent_type::operator[](n);
    else
      return parent_type::empty;
  }

  const RegionalGridMPI<T, D>& as_grid() const { return *this; }

  RegionalGridMPI<T, D>& as_grid() { return *this; }
 ...
 protected:
  void update_dimensions(const len_type* dimensions) {
    if (dimensions != nullptr) {
      ...
      parent_type::region = {parent_type::dims,
                             parent_type::region.boundary_size};
    }
  }

  auto mpi_dims(symphas::multi_thr_info_type const& info,
                const len_type* dimensions) {
    return (info.is_in_node()) ? dimensions : nullptr;
  }
};

template <typename T, size_t D>
struct RegionalGridMPI<any_vector_t<T, D>, D>
    : RegionalGrid<any_vector_t<T, D>, D> {
  ...
  RegionalGridMPI(symphas::multi_thr_info_type const& info,
                  const len_type* dimensions, const T (&empty)[D])
      : parent_type(mpi_dims(info, dimensions), empty) {
    update_dimensions(dimensions);
  }
  ...
  RegionalGridMPI(symphas::multi_thr_info_type const& info,
                  grid::dim_list dimensions, const T (&empty)[D])
      : parent_type(mpi_dims(info, dimensions), empty) {
    update_dimensions(dimensions);
  }
  ...
  RegionalGridMPI(RegionalGridMPI<any_vector_t<T, D>, D> const& other)
      : RegionalGridMPI(nullptr, other.empty, other.region.boundary_size) {
    using std::swap;
  ...
  __device__ __host__ auto operator[](iter_type n) const
      -> std::invoke_result_t<decltype(&parent_type::operator[]), parent_type,
                              iter_type> {
    if (parent_type::values[0] != nullptr)
      return parent_type::operator[](n);
    else
      return parent_type::empty;
  }
  ...
  RegionalGridMPI<any_vector_t<T, D>, D>& as_grid() { return *this; }
...
struct make<RegionalGridMPI> {
  ...
  template <typename T, size_t D>
  static RegionalGridMPI<T, D> apply(
      symphas::multi_thr_info_type const& thr_info,
...
struct value_type_of {
 protected:
  ...
  static constexpr auto call_wrap(G g) { return cast(g); }

  using wrapped_type = typename std::invoke_result_t<
      decltype(&grid::value_type_of<G>::call_wrap), G>;
...
struct dimension_of {
 protected:
  ...
  using wrapped_type =
      typename std::invoke_result_t<decltype(&grid::dimension_of<G>::call_wrap),
                                    G>;

...
operator()(const grid::Boundary<T, 1>*, GridCUDA<T, 2>& grid) {
  ...
  grid::update_bottom_left_boundary_2d(grid.values, grid.dims[0], grid.dims[1]);
}
...
void symphas::internal::update_boundary<BoundaryType::PERIODIC, Side::RIGHT,
  ...
  grid::update_top_right_boundary_2d(grid.values, grid.dims[0], grid.dims[1]);
  grid::update_bottom_right_boundary_2d(grid.values, grid.dims[0],
                                        grid.dims[1]);
}

...
struct Field {
   ...
   */
  Field(X data_x, Y data_y, len_type len = 1)
      : data{data_x, data_y}, len{len} {}
  Field(len_type len = 1) : data{X{}, Y{}}, len{len} {}

  //! Get the reference to the independent data.
  const X& data_x() const { return data.first; }

  //! Get the reference to the dependent data.
  const Y& data_y() const { return data.second; }

  //! Get the reference to the independent data.
  X& data_x() { return data.first; }

  //! Get the reference to the dependent data.
  Y& data_y() { return data.second; }

  //! Get the length of the dependent data array.
  len_type length() const { return len; }
 ...
 *
 * Specialization of symphas::Field to take into account pointer memory
 * management. The symphas::Field will move the given independent data into the
 * shared pointer, then the field can be manipulated at will with no special
 * consideration for memory management.
 *
...
struct Field<X, Y*> {
  ...
  Field(Field<X, Y*> const& other)
      : Field(other.data.first, other.data.second, other.len) {}
  Field(Field<X, Y*>&& other)
      : Field(other.data.first, other.data.second, other.len) {}
  ...
  std::shared_ptr<Y[]>& data_y() { return data.second; }

  //! Get the reference to the dependent data array.
  std::shared_ptr<Y[]> const& data_y() const { return data.second; }

  //! Get the reference to the dependent data array.
  Y* field_y() { return data.second.get(); }

  //! Get the reference to the dependent data array.
  const Y* field_y() const { return data.second.get(); }

  //! Get the length of the dependent data array.
  len_type length() const { return len; }

  //! Access an element in the dependent data array.
  Y& operator[](iter_type i) { return data_y()[i]; }

  //! Access an element in the dependent data array.
  const Y& operator[](iter_type i) const { return data_y()[i]; }
 ...
 *
 * Specialization of symphas::Field to take into account pointer memory
 * management. The symphas::Field will move the given arrays into the shared
 * pointers, then the field can be manipulated at will with no special
 * consideration for memory management.
 *
 * This means that if a raw pointer is given, it must
 * not be deleted.
...
struct Field<X*, Y*> {
  ...
  Field(Grid<Y, D> const& grid)
      : Field(symphas::lib::new_system_axis_list<D>(grid.dims), new Y[grid.len],
              grid.len) {
  ...
  std::shared_ptr<X[]>& data_x() { return data.first; }

  //! Get the reference to the dependent data array.
  std::shared_ptr<Y[]>& data_y() { return data.second; }

  //! Get the reference to the dependent data array.
  std::shared_ptr<X[]> const& data_x() const { return data.first; }

  //! Get the reference to the independent data array.
  std::shared_ptr<Y[]> const& data_y() const { return data.second; }

  //! Get the reference to the dependent data array.
  X* field_x() { return data.first.get(); }

  //! Get the reference to the dependent data array.
  const X* field_x() const { return data.first.get(); }
...
auto& sort_data(FieldAxis<1, Y*>& data, const len_type* dims) {
  symphas::lib::sort_data(data.x, data.y, dims[0], symphas::lib::regular_sort);
  return data;
}

template <typename Y>
auto& sort_data(FieldAxis<2, Y*>& data, const len_type* dims) {
  symphas::lib::sort_data(data.x, data.y, dims[0], dims[1],
...
auto& sort_data(FieldAxis<3, Y*>& data, const len_type* dims) {
  symphas::lib::sort_data(data.x, data.y, dims[0], dims[1], dims[1],
                          symphas::lib::regular_sort);
  return data;
}
}  // namespace internal
...
/*!
 * Obtains the data type of the x-axis of the given Field type. This is always
 * the direct type, which in the case of a Field of an array in `X` (e.g.
 * `Field<X*, _>`) would be a pointer type. This is likewise for FieldAxis,
 * which inherits from `Field<axis_nd_t<D>, _>`.
 */
template <typename F>
...
/*!
 * Obtains the data type of the y-axis of the given Field type. This is always
 * the direct type, which in the case of a Field of an array in `Y` (e.g.
 * `Field<_, Y*>`) would be a pointer type.
 */
template <typename F>
 ...
 */
template <typename Y, typename = decltype(symphas::dft::dft(
                          std::declval<Y*>(), std::declval<complex_t*>(),
                          std::declval<len_type>(), std::declval<bool>()))>
auto fourier_transform(const axis_1d_type* data_x, const Y* data_y,
                       const len_type len, bool backward = false) {
...
template <typename Y, typename = decltype(symphas::dft::dft(
                          std::declval<Y*>(), std::declval<complex_t*>(),
                          std::declval<len_type>(), std::declval<len_type>(),
                          std::declval<bool>()))>
auto fourier_transform(const axis_2d_type* data_x, const Y* data_y,
                       const len_type len, bool backward = false) {
...
template <typename Y, typename = decltype(symphas::dft::dft(
                          ...
                          std::declval<len_type>(), std::declval<len_type>(),
                          std::declval<len_type>(), std::declval<bool>()))>
auto fourier_transform(const axis_3d_type* data_x, const Y* data_y,
                       const len_type len, bool backward = false) {
...
template <typename Y, typename = decltype(symphas::dft::dft(
                          std::declval<Y*>(), std::declval<complex_t*>(),
                          std::declval<len_type>(), std::declval<bool>()))>
auto fourier_transform(const iter_type* data_x, const Y* data_y,
                       const len_type len, bool backward = false) {
  complex_t* dft_y = new complex_t[len];
 ...
 */
template <typename X, typename Y,
          typename std::enable_if<!std::is_pointer<X>::value, int>::type = 0,
          typename = decltype(symphas::dft::dft(
              std::declval<Y*>(), std::declval<complex_t*>(),
              std::declval<len_type>(), std::declval<bool>()))>
auto fourier_transform(const X data_x, const Y* data_y, const len_type len,
                       bool backward = false) {
  complex_t* dft_y = new complex_t[len];
  symphas::dft::dft(data_y, dft_y, len, backward);
 ...
 * This is primarily an overload of symphas::lib::fourier_transform(const X,
 * const Y*, const len_type).
 *
 * \param data The data which is transformed.
 */
template <typename X, typename Y>
auto fourier_transform(Field<X, Y*> const& data, bool backward = false) {
  return fourier_transform(data.data_x(), data.field_y(), data.length(),
                           backward);
}
...
template <typename X, typename Y>
auto fourier_transform(Field<X*, Y*> const& data, bool backward = false) {
  return fourier_transform(data.field_x(), data.field_y(), data.length(),
                           backward);
}
}  // namespace symphas::lib
 ...
 *
 * \param rhs The symphas::Field which is added pointwise to the symphas::Field
 * on the left hand side.
 */
template <typename X, typename Y1, typename Y2>
symphas::Field<X, Y1*>& operator+=(symphas::Field<X, Y1*>& lhs,
                                   symphas::Field<X, Y2*>& rhs) {
 ...
 */
template <typename X, typename Y1, typename Y2>
auto operator+(symphas::Field<X*, Y1*> const& lhs,
               symphas::Field<X*, Y2*> const& rhs) {
  len_type len = std::min(lhs.length(), rhs.length());
 ...
 *
 * \param rhs The symphas::Field which subtracts point-wise from the to the
 * symphas::Field on the left hand side.
 */
template <typename X, typename Y1, typename Y2>
symphas::Field<X, Y1*>& operator-=(symphas::Field<X, Y1*>& lhs,
                                   symphas::Field<X, Y2*> const& rhs) {
...
/*!
 * Operator applied to the a symphas::Field, returning a new symphas::Field with
 * all values with the unary negative operator applied to them.
 */
template <typename X, typename Y>
...
symphas::FieldAxis<D, Y1*>& operator+=(symphas::FieldAxis<D, Y1*>& lhs,
                                       symphas::FieldAxis<D, Y2*> const& rhs) {
  len_type len = std::min(lhs.length(), rhs.length());
  for (int i = 0; i < len; ++i) {
    lhs[i] += rhs[i];
  }
 ...
 * addition operator.
 * \param rhs The symphas::FieldAxis<D, Y> instance on the right hand side of
 * the addition operator.
 */
template <size_t D, typename Y1, typename Y2>
auto operator+(symphas::FieldAxis<D, Y1*> const& lhs,
               symphas::FieldAxis<D, Y2*> const& rhs) {
  len_type len = std::min(lhs.length(), rhs.length());
...
template <size_t D, typename Y1, typename Y2>
symphas::FieldAxis<D, Y1*>& operator-=(symphas::FieldAxis<D, Y1*>& lhs,
                                       const symphas::FieldAxis<D, Y2*>& rhs) {
  len_type len = std::min(lhs.length(), rhs.length());
  for (int i = 0; i < len; ++i) {
    lhs[i] -= rhs[i];
  }
...
auto new_radial_avg_field(const T* data_y, len_type L, double dx) {
  ...
  iter_type L_left = L / 2, L_right = L / 2 - cL;
...
template <typename T>
auto new_radial_avg_field(const T* data_y, len_type L, len_type M, double dx,
                          double dy) {
  using Y = std::remove_const_t<T>;

  iter_type cL = (L % 2 == 0) ? 1 : 0;
  iter_type cM = (M % 2 == 0) ? 1 : 0;
  iter_type L_left = L / 2, L_right = L / 2 - cL, M_left = M / 2,
            M_right = M / 2 - cM;
  ...
  iter_type bl = L * (M_left - 1) + (L_left - 1),
  ...
  for (iter_type j = 0; j < M_right; ++j) {
    ...
    n = m + 1;
    for (iter_type i = 0; i < L_right; ++i, ++n) {
      Y value = data_y[bl--] + data_y[tl--] + data_y[br++] + data_y[tr++];
  ...
  for (iter_type i = 0; i < len - duplicates; ++i) {
    cs[i] = cs[i];
    iter_type j = i + 1;
    while (std::abs(data_x[i] - data_x[j]) < 1E-3 && j < len - duplicates) {
      avgs[i] += avgs[j];
      cs[i] += cs[j];
      ++j;
    }
...
template <typename T>
auto new_radial_avg_field(const T* data_y, len_type L, len_type M, len_type N,
  ...
  iter_type L_left = L / 2, L_right = L / 2 - cL, M_left = M / 2,
            M_right = M / 2 - cM, N_left = N / 2, N_right = N / 2 - cN;
  ...
  iter_type fbl = L * M * (N_left - 1) + L * (M_left - 1) + (L_left - 1),
  ...
  for (iter_type k = 0; k < N_right; ++k) {
    ...
    for (iter_type j = 0; j < M_right; ++j) {
      ...
      avgs[dmap[m]] += (data_y[fbl + 1] + data_y[ftl + 1] + data_y[bbl + 1] +
                        data_y[btl + 1]);
      cs[dmap[m]] += 4;

      n = m + 1;
      for (iter_type i = 0; i < L_right; ++i, ++n) {
        Y value = data_y[fbl--] + data_y[ftl--] + data_y[fbr++] +
                  data_y[ftr++] + data_y[bbl--] + data_y[btl--] +
      ...
      if (cL > 0) {
        avgs[dmap[n]] +=
            (data_y[fbl--] + data_y[ftl--] + data_y[bbl--] + data_y[btl--]);
        cs[dmap[m]] += 4;
        n += 1;
      }
      m = n;
    ...
    if (cM > 0) {
      ...
      n = m + 1;
      for (iter_type i = 0; i < L_right; ++i, ++n) {
        Y value = data_y[fbl--] + data_y[bbl--] + data_y[fbr++] + data_y[bbr++];
  ...
  if (cN > 0) {
    ...
    for (iter_type j = 0; j < M_right; ++j) {
      ...
      n = m + 1;
      for (iter_type i = 0; i < L_right; ++i, ++n) {
        Y value = data_y[fbl--] + data_y[ftl--] + data_y[fbr++] + data_y[ftr++];
...
template <typename T>
auto linear_interpolate(axis_coord_t ax, T ay, axis_coord_t bx, T by,
                        axis_coord_t point) {
 ...
 */
template <typename T>
std::vector<std::pair<double, T>> trim_sides(
    std::vector<std::pair<axis_1d_type, T>> const& data, double ratio) {
  size_t len = data.size(), chop = static_cast<size_t>((ratio / 2.0) * len);
  auto first = data.begin() + chop, last = data.end() - chop;
 ...
 */
template <typename T>
std::vector<std::pair<double, T>> trim_right(
    std::vector<std::pair<axis_1d_type, T>> const& data, double ratio) {
  size_t len = data.size(), chop = static_cast<size_t>(ratio * len);
  auto first = data.begin(), last = data.end() - chop;
 ...
 */
template <typename T>
std::vector<std::pair<double, T>> trim_left(
    std::vector<std::pair<axis_1d_type, T>> const& data, double ratio) {
  size_t len = data.size(), chop = static_cast<size_t>(ratio * len);
  auto first = data.begin() + chop, last = data.end();
 ...
 */
template <typename T>
std::vector<std::pair<double, T>> smooth_data(
    std::vector<std::pair<axis_1d_type, T>> const& data, double ratio = 0.5) {
  ...
  auto split = [&](iter_type i) {
    iter_type count = len / data_len, rem = len - (data_len * count);
  ...
  for (iter_type i = 0; i < data_len; ++i) {
    ...
    data_avg.emplace_back(
        (data[split(i)].first + data[split(i + 1) - 1].first) / 2.0, value);
  }
...
template <typename T>
std::vector<std::pair<double, T>> smooth_data(
    std::vector<std::pair<axis_1d_type, T>> const& data, len_type points) {
 ...
 */
template <typename T>
std::vector<std::pair<double, T>> balance_axis(
    std::vector<std::pair<axis_1d_type, T>> const& data, axis_coord_t dx) {
  ...
  while (cursor_x < final_x) {
    ...
    if (cursor_x < next_x) {
      ...
      axis_coord_t next_y =
          (2.0 / dx) * (sum - last_y * dx + 0.5 * dx * last_y);
      data_bal.emplace_back(next_x, next_y);
    }
    // Otherwise, interpolate the value linearly.
    else {
      axis_coord_t next_y = symphas::internal::linear_interpolate(
          last_x, last_y, cursor_x, cursor_y, next_x);
      data_bal.emplace_back(next_x, next_y);
    }
  }
 ...
 */
template <typename T>
std::vector<std::pair<double, T>> balance_axis_ls(
 ...
 */
template <typename T>
std::vector<std::pair<double, T>> balance_axis_as(
    std::vector<std::pair<axis_1d_type, T>> const& data) {
...
template <>
inline auto new_system_axis_list<1>(
...
template <>
inline auto new_system_axis_list<2>(
    symphas::interval_data_type const& intervals) {
  auto& ix = intervals.at(Axis::X);
  ...
  double len_x = ix.interval_length();
  double len_y = iy.interval_length();
...
template <>
inline auto new_system_axis_list<3>(
    symphas::interval_data_type const& intervals) {
  auto& ix = intervals.at(Axis::X);
  auto& iy = intervals.at(Axis::Y);
 ...
 */
template <typename T>
inline len_type get_sorted_dimensions(
    std::vector<std::pair<axis_1d_type, T>> sorted_data) {
  return static_cast<iter_type>(sorted_data.size());
}

template <typename T>
inline grid::dim_list get_sorted_dimensions(
    std::vector<std::pair<axis_2d_type, T>> sorted_data) {
  len_type L = 1, M = 0;
  double y0 = sorted_data.front().first[1];
  for (auto it = sorted_data.begin() + 1;
       it < sorted_data.end() && y0 == it->first[1]; ++it) {
    ++L;
  }
...
inline grid::dim_list get_sorted_dimensions(const axis_3d_type* sorted_data,
                                            len_type len) {
  len_type L = 0, M = 0, N = 0;
  double z0 = (*sorted_data)[2], y0 = (*sorted_data)[1];
...
inline grid::dim_list get_sorted_dimensions(const axis_2d_type* sorted_data,
                                            len_type len) {
  len_type L = 0, M = 0;
...
auto radial_avg(FieldAxis<1, Y> const& data) {
  ...
  double const dx = ((*(data.x + 1)) - (*data.x));
...
auto radial_avg(FieldAxis<2, Y> const& data) {
  ...
  double const dx = ((*(data.x + 1))[0] - (*data.x)[0]),
...
auto radial_avg(FieldAxis<3, Y> const& data) {
  auto [L, M, N] =
      symphas::lib::get_sorted_dimensions(data.x, data.length())._3();
...
template <typename T>
grid::dim_list get_sorted_dimensions(
    std::vector<std::pair<axis_3d_type, T>> sorted_data) {
  len_type L = 0, M = 0, N = 0;
  double z0 = sorted_data.front().first[2], y0 = sorted_data.front().first[1];
...
grid::dim_list get_dimensions(
  ...
  auto&& dims =
      get_dimensions<D>(axis_values, static_cast<len_type>(data.size()));
  delete[] axis_values;
  return dims;
}
 ...
 */
template <typename T>
void fill_sorted_ranges(std::vector<std::pair<axis_3d_type, T>> const& data,
...
template <typename T>
void fill_sorted_ranges(std::vector<std::pair<axis_2d_type, T>> const& data,
...
template <typename T>
void fill_sorted_ranges(std::vector<std::pair<axis_1d_type, T>> const& data,
...
template <size_t D, typename T>
void fill_sorted_ranges(std::vector<std::pair<axis_nd_t<D / 2>, T>> const& data,
                        axis_coord_t (&ranges)[D], T (&extrema)[2]) {
  if (data.size() > 0) {
    fill_sorted_ranges(data, ranges);
    extrema[0] =
        std::max_element(data.begin(), data.end(), [&](auto a, auto b) {
...
inline void fill_sorted_ranges(const axis_3d_type* data_x, len_type len,
                               axis_coord_t (&ranges)[6]) {
  if (len > 0) {
...
inline void fill_sorted_ranges(const axis_2d_type* data_x, len_type len,
                               axis_coord_t (&ranges)[4]) {
  if (len > 0) {
...
inline void fill_sorted_ranges(const axis_1d_type* data_x, len_type len,
                               axis_coord_t (&ranges)[2]) {
  if (len > 0) {
    ranges[0] = *(data_x);
...
void fill_sorted_ranges(const T* data, len_type len, T (&ranges)[2]) {
  if (len > 0) {
    auto max = std::max_element(data, data + len,
                                [&](auto a, auto b) { return a < b; });
    auto min = std::max_element(data, data + len,
                                [&](auto a, auto b) { return a > b; });
    ranges[0] = min;
    ranges[1] = max;
  }
...
void fill_ranges(
    ...
    T (&ranges)[4]) {
  if (data.size() > 0) {
    auto max_it_y = std::max_element(
        data.begin(), data.end(),
        [&](auto a, auto b) { return std::get<2>(a) < std::get<2>(b); });
    auto min_it_y = std::max_element(
        data.begin(), data.end(),
        [&](auto a, auto b) { return std::get<2>(a) > std::get<2>(b); });
    auto max_it_x = std::max_element(
        data.begin(), data.end(),
        [&](auto a, auto b) { return std::get<1>(a) < std::get<1>(b); });
    auto min_it_x = std::max_element(
        data.begin(), data.end(),
        [&](auto a, auto b) { return std::get<1>(a) > std::get<1>(b); });
...
inline void fill_ranges(axis_3d_type const* data_x, len_type len,
                        axis_coord_t (&ranges)[6]) {
...
inline void fill_ranges(axis_2d_type const* data_x, len_type len,
                        axis_coord_t (&ranges)[4]) {
  // initialize infinite of opposite sign on ranges
  // so that the checks with data_x can work.
...
inline void fill_ranges(axis_1d_type const* data_x, len_type len,
                        axis_coord_t (&ranges)[2]) {
  // initialize infinite of opposite sign on ranges
  // so that the checks with data_x can work.

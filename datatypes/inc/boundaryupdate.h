...
struct update_boundary {
   ...
   */
  template <typename T>
  void operator()(const grid::Boundary<T, D>* boundary,
                  RegionalGrid<T, D + 1>& grid, double time);
  ...
  template <typename T>
  void operator()(const grid::Boundary<T, D>* boundary,
                  GridCUDA<T, D + 1>& grid, double time);
  ...
  template <typename T>
  void operator()(const grid::Boundary<T, D>* boundary,
                  RegionalGridCUDA<T, D + 1>& grid, double time);
...
operator()(const grid::Boundary<T, 2>*, Grid<T, 3>& grid) {
  iter_type offset;
  offset = grid.dims[0] * grid.dims[1] *
           (grid.dims[2] - BOUNDARY_DEPTH - BOUNDARY_DEPTH);
...
operator()(const grid::Boundary<T, 1>*, Grid<T, 2>& grid) {
  iter_type offset = grid.dims[0] - BOUNDARY_DEPTH - BOUNDARY_DEPTH;

  ITER_GRID2_LEFT({ grid[INDEX] = grid[INDEX + offset]; }, grid.dims[0],
                  grid.dims[1]);

  offset = (grid.dims[1] - BOUNDARY_DEPTH - BOUNDARY_DEPTH) * grid.dims[0] +
           (grid.dims[0] - BOUNDARY_DEPTH - BOUNDARY_DEPTH);
  ITER_GRID2_LEFT_TOP({ grid[INDEX] = grid[INDEX + offset]; }, grid.dims[0]);

  offset = -(grid.dims[1] - BOUNDARY_DEPTH - BOUNDARY_DEPTH) * grid.dims[0] +
           (grid.dims[0] - BOUNDARY_DEPTH - BOUNDARY_DEPTH);
  ITER_GRID2_BOTTOM_LEFT({ grid[INDEX] = grid[INDEX + offset]; }, grid.dims[0],
                         grid.dims[1]);
}
...
void symphas::internal::update_boundary<BoundaryType::PERIODIC, Side::RIGHT,
                                        1>::operator()(const grid::Boundary<T,
                                                       ...
                                                       Grid<T, 2>& grid) {
  iter_type offset = grid.dims[0] - BOUNDARY_DEPTH - BOUNDARY_DEPTH;
  ITER_GRID2_RIGHT({ grid[INDEX] = grid[INDEX - offset]; }, grid.dims[0],
                   grid.dims[1]);

  offset = -(grid.dims[1] - BOUNDARY_DEPTH - BOUNDARY_DEPTH) * grid.dims[0] -
           (grid.dims[0] - BOUNDARY_DEPTH - BOUNDARY_DEPTH);
  ITER_GRID2_RIGHT_BOTTOM({ grid[INDEX] = grid[INDEX + offset]; }, grid.dims[0],
                          grid.dims[1]);

  offset = (grid.dims[1] - BOUNDARY_DEPTH - BOUNDARY_DEPTH) * grid.dims[0] -
           (grid.dims[0] - BOUNDARY_DEPTH - BOUNDARY_DEPTH);
  ITER_GRID2_TOP_RIGHT({ grid[INDEX] = grid[INDEX + offset]; }, grid.dims[0]);
}
...
operator()(const grid::Boundary<T, 1>*, Grid<T, 2>& grid) {
  iter_type offset =
      grid.dims[0] * (grid.dims[1] - BOUNDARY_DEPTH - BOUNDARY_DEPTH);
  ITER_GRID2_TOP({ grid[INDEX] = grid[INDEX + offset]; }, grid.dims[0]);
}
...
void symphas::internal::update_boundary<BoundaryType::PERIODIC, Side::BOTTOM,
  ...
  iter_type offset =
      grid.dims[0] * (grid.dims[1] - BOUNDARY_DEPTH - BOUNDARY_DEPTH);
  ITER_GRID2_BOTTOM({ grid[INDEX] = grid[INDEX - offset]; }, grid.dims[0],
                    grid.dims[1]);
}
...
void symphas::internal::update_boundary<BoundaryType::PERIODIC0, Side::LEFT,
                                        1>::operator()(const grid::Boundary<T,
                                                       ...
                                                       Grid<T, 2>& grid) {
  iter_type offset = grid.dims[0] - BOUNDARY_DEPTH - BOUNDARY_DEPTH;
  ITER_GRID2_LEFT_ALL({ grid[INDEX] = grid[INDEX + offset]; }, grid.dims[0],
                      grid.dims[1]);
}
...
void symphas::internal::update_boundary<BoundaryType::PERIODIC0, Side::RIGHT,
                                        1>::operator()(const grid::Boundary<T,
                                                       ...
                                                       Grid<T, 2>& grid) {
  iter_type offset = grid.dims[0] - BOUNDARY_DEPTH - BOUNDARY_DEPTH;
  ITER_GRID2_RIGHT_ALL({ grid[INDEX] = grid[INDEX - offset]; }, grid.dims[0],
                       grid.dims[1]);
}
...
operator()(const grid::Boundary<T, 1>*, Grid<T, 2>& grid) {
  iter_type offset =
      grid.dims[0] * (grid.dims[1] - BOUNDARY_DEPTH - BOUNDARY_DEPTH);
  ITER_GRID2_TOP_ALL({ grid[INDEX] = grid[INDEX + offset]; }, grid.dims[0]);
}
...
void symphas::internal::update_boundary<BoundaryType::PERIODIC0, Side::BOTTOM,
  ...
  iter_type offset =
      grid.dims[0] * (grid.dims[1] - BOUNDARY_DEPTH - BOUNDARY_DEPTH);
  ITER_GRID2_BOTTOM_ALL({ grid[INDEX] = grid[INDEX - offset]; }, grid.dims[0],
                        grid.dims[1]);
}
...
operator()(const grid::Boundary<T, 0>*, Grid<T, 1>& grid) {
  iter_type offset = grid.dims[0] - BOUNDARY_DEPTH - BOUNDARY_DEPTH;
  ITER_GRID1_LEFT({ grid[INDEX] = grid[INDEX + offset]; });
}
...
void symphas::internal::update_boundary<BoundaryType::PERIODIC, Side::RIGHT,
                                        0>::operator()(const grid::Boundary<T,
                                                       ...
                                                       Grid<T, 1>& grid) {
  iter_type offset = grid.dims[0] - BOUNDARY_DEPTH - BOUNDARY_DEPTH;
  ITER_GRID1_RIGHT({ grid[INDEX] = grid[INDEX - offset]; }, grid.dims[0]);
}
...
void symphas::internal::update_boundary<
    ...
    2>::operator()(const grid::Boundary<T, 2>*, RegionalGrid<T, 3>& grid) {
...
template <typename T>
void symphas::internal::update_boundary<BoundaryType::PERIODIC, Side::BACK, 2>::
operator()(const grid::Boundary<T, 2>*, RegionalGrid<T, 3>& grid) {
  regional_update_boundary(
...
template <typename T>
void symphas::internal::update_boundary<BoundaryType::PERIODIC, Side::LEFT, 1>::
operator()(const grid::Boundary<T, 1>*, RegionalGrid<T, 2>& grid) {
  regional_update_boundary(symphas::lib::side_list<Side::LEFT, Side::LEFT>{},
                           grid);
...
void symphas::internal::update_boundary<
    BoundaryType::PERIODIC, Side::RIGHT,
    1>::operator()(const grid::Boundary<T, 1>*, RegionalGrid<T, 2>& grid) {
  regional_update_boundary(symphas::lib::side_list<Side::RIGHT, Side::RIGHT>{},
                           grid);
...
template <typename T>
void symphas::internal::update_boundary<BoundaryType::PERIODIC, Side::LEFT, 0>::
operator()(const grid::Boundary<T, 0>*, RegionalGrid<T, 1>& grid) {
  regional_update_boundary(symphas::lib::side_list<Side::LEFT>{}, grid);
}
...
void symphas::internal::update_boundary<
    BoundaryType::PERIODIC, Side::RIGHT,
    0>::operator()(const grid::Boundary<T, 0>*, RegionalGrid<T, 1>& grid) {
  regional_update_boundary(symphas::lib::side_list<Side::RIGHT>{}, grid);
}
...
operator()(const grid::Boundary<T, 2>* b, Grid<T, 3>& grid, double time) {
  auto* bd =
  ...
  ITER_GRID3_LEFT_TOP_FRONT({ bd->update(grid[INDEX], x[0], y[0], time); }, L,
                            M);

  ITER_GRID3_FRONT_TOP_RIGHT({ bd->update(grid[INDEX], x[1], y[0], time); }, L,
                             M);

  ITER_GRID3_LEFT_BOTTOM_FRONT({ bd->update(grid[INDEX], x[0], y[1], time); },
                               L, M);

  ITER_GRID3_FRONT_BOTTOM_RIGHT({ bd->update(grid[INDEX], x[1], y[1], time); },
                                L, M);
}
...
operator()(const grid::Boundary<T, 2>* b, Grid<T, 3>& grid, double time) {
  ...
  ITER_GRID3_BACK_TOP(
      { bd->update(grid[INDEX], x[0] + iter_i * h[0], y[0], time); }, L, M, N);

  ITER_GRID3_BACK_BOTTOM(
      { bd->update(grid[INDEX], x[0] + iter_i * h[0], y[1], time); }, L, M, N);

  ITER_GRID3_BACK_LEFT(
      { bd->update(grid[INDEX], x[0], y[0] + iter_j * h[1], time); }, L, M, N);

  ITER_GRID3_BACK_RIGHT(
      { bd->update(grid[INDEX], x[1], y[0] + iter_j * h[1], time); }, L, M, N);

  // corners

  ITER_GRID3_BACK_TOP_LEFT({ bd->update(grid[INDEX], x[0], y[0], time); }, L, M,
                           N);

  ITER_GRID3_RIGHT_TOP_BACK({ bd->update(grid[INDEX], x[1], y[0], time); }, L,
                            M, N);

  ITER_GRID3_BACK_BOTTOM_LEFT({ bd->update(grid[INDEX], x[0], y[1], time); }, L,
                              M, N);

  ITER_GRID3_RIGHT_BOTTOM_BACK({ bd->update(grid[INDEX], x[1], y[1], time); },
                               L, M, N);
}
...
operator()(const grid::Boundary<T, 2>* b, Grid<T, 3>& grid, double time) {
  ...
  ITER_GRID3_BACK_TOP_LEFT({ bd->update(grid[INDEX], x[1], y[0], time); }, L, M,
                           N);

  ITER_GRID3_LEFT_BOTTOM_FRONT({ bd->update(grid[INDEX], x[0], y[1], time); },
                               L, M);

  ITER_GRID3_BACK_BOTTOM_LEFT({ bd->update(grid[INDEX], x[1], y[1], time); }, L,
                              M, N);
}
...
operator()(const grid::Boundary<T, 2>* b, Grid<T, 3>& grid, double time) {
  ...
  ITER_GRID3_FRONT_TOP_RIGHT({ bd->update(grid[INDEX], x[0], y[0], time); }, L,
                             M);

  ITER_GRID3_RIGHT_TOP_BACK({ bd->update(grid[INDEX], x[1], y[0], time); }, L,
                            M, N);

  ITER_GRID3_FRONT_BOTTOM_RIGHT({ bd->update(grid[INDEX], x[0], y[1], time); },
                                L, M);
...
operator()(const grid::Boundary<T, 2>* b, Grid<T, 3>& grid, double time) {
  ...
  ITER_GRID3_BACK_TOP_LEFT({ bd->update(grid[INDEX], x[0], y[1], time); }, L, M,
                           N);

  ITER_GRID3_RIGHT_TOP_BACK({ bd->update(grid[INDEX], x[1], y[1], time); }, L,
                            M, N);
}
...
void symphas::internal::
    update_boundary<BoundaryType::DEFAULT, Side::BOTTOM, 2>::operator()(
        const grid::Boundary<T, 2>* b, Grid<T, 3>& grid, double time) {
  auto* bd =
      static_cast<grid::BoundaryApplied<T, 2, BoundaryType::DEFAULT> const*>(b);
  ...
  ITER_GRID3_LEFT_BOTTOM_FRONT({ bd->update(grid[INDEX], x[0], y[0], time); },
                               L, M);

  ITER_GRID3_FRONT_BOTTOM_RIGHT({ bd->update(grid[INDEX], x[1], y[0], time); },
                                L, M);
...
operator()(const grid::Boundary<T, 1>* b, Grid<T, 2>& grid, double time) {
  auto* bd =
  ...
  ITER_GRID2_LEFT({ bd->update(grid[INDEX], v0 + iter_i * h, 0, time); }, L, M);
...
operator()(const grid::Boundary<T, 1>* b, Grid<T, 2>& grid, double time) {
  ...
  ITER_GRID2_RIGHT({ bd->update(grid[INDEX], v0 + iter_i * h, 0, time); }, L,
                   M);
...
void symphas::internal::
    update_boundary<BoundaryType::DEFAULT, Side::BOTTOM, 1>::operator()(
        const grid::Boundary<T, 1>* b, Grid<T, 2>& grid, double time) {
  auto* bd =
      static_cast<grid::BoundaryApplied<T, 1, BoundaryType::DEFAULT> const*>(b);
  ...
  ITER_GRID2_BOTTOM({ bd->update(grid[INDEX], v0 + iter_i * h, 0, time); }, L,
                    M);
...
operator()(const grid::Boundary<T, 0>* b, Grid<T, 1>& grid, double time) {
  auto* bd =
...
template <typename T>
void symphas::internal::update_boundary<BoundaryType::DEFAULT, Side::FRONT, 2>::
operator()(const grid::Boundary<T, 2>* b, RegionalGrid<T, 3>& grid,
           double time) {
  regional_update_boundary(symphas::lib::side_list<Side::FRONT, Side::FRONT>{},
                           b, grid, time);
}
...
void symphas::internal::
    update_boundary<BoundaryType::DEFAULT, Side::BOTTOM, 2>::operator()(
        const grid::Boundary<T, 2>* b, RegionalGrid<T, 3>& grid, double time) {
  regional_update_boundary(
...
template <typename T>
void symphas::internal::update_boundary<BoundaryType::DEFAULT, Side::LEFT, 1>::
operator()(const grid::Boundary<T, 1>* b, RegionalGrid<T, 2>& grid,
           double time) {
...
void symphas::internal::
    update_boundary<BoundaryType::DEFAULT, Side::BOTTOM, 1>::operator()(
        const grid::Boundary<T, 1>* b, RegionalGrid<T, 2>& grid, double time) {
  regional_update_boundary(
...
template <typename T>
void symphas::internal::update_boundary<BoundaryType::DEFAULT, Side::LEFT, 0>::
operator()(const grid::Boundary<T, 0>* b, RegionalGrid<T, 1>& grid,
           double time) {
  regional_update_boundary(symphas::lib::side_list<Side::LEFT>{}, b, grid,
                           time);
}

...
// Gives the origin and size of the boundary in the global domain. "Boundary"
// refers to the ghost cells themselves, which will be updated using real data
// from the interior. The origin member represents the position in the region
// that the boundary starts, and the dimension is the size of the region.
...
struct boundary_interval<1, Side::LEFT> {
  boundary_interval(const len_type (&dims)[1], len_type boundary_size)
      : origin{0}, dims{boundary_size}, stride{1} {}
...
struct boundary_interval<1, Side::RIGHT> {
  boundary_interval(const len_type (&dims)[1], len_type boundary_size)
      : origin{dims[0] - 1}, dims{-boundary_size}, stride{-1} {}
...
struct boundary_interval<2, Side::LEFT> {
  boundary_interval(const len_type (&dims)[2], len_type boundary_size)
      : origin{0, 0}, dims{boundary_size, dims[1]}, stride{dims[1], 1} {}
...
struct boundary_interval<2, Side::BOTTOM> {
  boundary_interval(const len_type (&dims)[2], len_type boundary_size)
      : origin{0, 0}, dims{dims[0], boundary_size}, stride{1, dims[0]} {}
...
struct boundary_interval<3, Side::LEFT, Side::LEFT, Side::LEFT> {
  boundary_interval(const len_type (&dims)[3], len_type boundary_size)
      : origin{0, boundary_size, boundary_size},
        dims{boundary_size, dims[1] - boundary_size * 2,
             dims[2] - boundary_size * 2},
        stride{(dims[1] - boundary_size * 2) * (dims[2] - boundary_size * 2),
               (dims[1] - boundary_size * 2), 1} {}
...
struct boundary_interval<3, Side::LEFT, Side::LEFT, Side::FRONT> {
  boundary_interval(const len_type (&dims)[3], len_type boundary_size)
      : origin{0, boundary_size, 0},
        ...
        stride{(dims[1] - boundary_size * 2) * boundary_size,
               (dims[1] - boundary_size * 2), 1} {}
...
struct boundary_interval<3, Side::LEFT, Side::LEFT, Side::TOP> {
  boundary_interval(const len_type (&dims)[3], len_type boundary_size)
      : origin{0, dims[1] - 1, boundary_size},
        ...
        stride{(dims[2] - boundary_size * 2) * boundary_size,
               -(dims[2] - boundary_size * 2), 1} {}
...
struct boundary_interval<3, Side::BOTTOM, Side::BOTTOM, Side::BOTTOM> {
  boundary_interval(const len_type (&dims)[3], len_type boundary_size)
      : origin{boundary_size, 0, boundary_size},
        dims{dims[0] - boundary_size * 2, boundary_size,
             dims[2] - boundary_size * 2},
        stride{1, (dims[0] - boundary_size * 2) * (dims[2] - boundary_size * 2),
               (dims[0] - boundary_size * 2)} {}
...
struct boundary_interval<3, Side::BOTTOM, Side::BOTTOM, Side::FRONT> {
  boundary_interval(const len_type (&dims)[3], len_type boundary_size)
      : origin{boundary_size, 0, 0},
        ...
        stride{1, (dims[0] - boundary_size * 2),
               (dims[0] - boundary_size * 2) * boundary_size} {}
  ...
  boundary_interval(const len_type (&dims)[3], len_type boundary_size)
      : origin{boundary_size, dims[1] - 1, boundary_size},
        dims{dims[0] - boundary_size * 2, -boundary_size,
             dims[2] - boundary_size * 2},
        stride{1,
               -(dims[0] - boundary_size * 2) * (dims[2] - boundary_size * 2),
               (dims[0] - boundary_size * 2)} {}
...
struct boundary_interval<3, Side::FRONT, Side::FRONT, Side::FRONT> {
  boundary_interval(const len_type (&dims)[3], len_type boundary_size)
      : origin{boundary_size, boundary_size, 0},
        dims{dims[0] - boundary_size * 2, dims[1] - boundary_size * 2,
             boundary_size},
        stride{1, dims[0] - boundary_size * 2,
               (dims[0] - boundary_size * 2) * (dims[1] - boundary_size * 2)} {}
...
template <>
struct periodic_offset<2, BoundaryType::PERIODIC, Side::LEFT, Side::LEFT>
    : offset_data<2> {
...
template <>
struct periodic_offset<2, BoundaryType::PERIODIC, Side::LEFT, Side::TOP>
    : offset_data<2> {
  periodic_offset(const len_type (&dims)[2], len_type boundary_size)
      : offset_data<2>{dims[0] - boundary_size * 2,
                       -dims[0] + boundary_size * 2} {}
};
...
template <>
struct periodic_offset<2, BoundaryType::PERIODIC, Side::BOTTOM, Side::BOTTOM>
    : offset_data<2> {
  periodic_offset(const len_type (&dims)[2], len_type boundary_size)
...
template <>
struct periodic_offset<3, BoundaryType::PERIODIC, Side::LEFT, Side::LEFT,
                       Side::FRONT> : offset_data<3> {
  periodic_offset(const len_type (&dims)[3], len_type boundary_size)
      : offset_data<3>{dims[0] - boundary_size * 2, 0,
                       dims[2] - boundary_size * 2} {}
};

template <>
struct periodic_offset<3, BoundaryType::PERIODIC, Side::LEFT, Side::LEFT,
                       Side::BACK> : offset_data<3> {
  periodic_offset(const len_type (&dims)[3], len_type boundary_size)
      : offset_data<3>{dims[0] - boundary_size * 2, 0,
                       -dims[2] + boundary_size * 2} {}
};
...
template <>
struct periodic_offset<3, BoundaryType::PERIODIC, Side::LEFT, Side::LEFT,
                       Side::TOP> : offset_data<3> {
  periodic_offset(const len_type (&dims)[3], len_type boundary_size)
      : offset_data<3>{dims[0] - boundary_size * 2,
                       -dims[1] + boundary_size * 2, 0} {}
};

template <>
struct periodic_offset<3, BoundaryType::PERIODIC, Side::LEFT, Side::LEFT,
                       Side::BOTTOM> : offset_data<3> {
  periodic_offset(const len_type (&dims)[3], len_type boundary_size)
      : offset_data<3>{dims[0] - boundary_size * 2, dims[1] - boundary_size * 2,
                       0} {}
};
...
struct periodic_offset<3, BoundaryType::PERIODIC, Side::RIGHT, Side::RIGHT,
                       Side::RIGHT>
    : periodic_offset<3, BoundaryType::PERIODIC, Side::LEFT, Side::LEFT,
                      Side::LEFT> {
  using parent_type = periodic_offset<3, BoundaryType::PERIODIC, Side::LEFT,
                                      Side::LEFT, Side::LEFT>;
...
template <Side side0>
struct periodic_offset<3, BoundaryType::PERIODIC, Side::RIGHT, Side::RIGHT,
                       side0> : periodic_offset<3, BoundaryType::PERIODIC,
                                                Side::LEFT, Side::LEFT, side0> {
  using parent_type =
...
struct periodic_offset<3, BoundaryType::PERIODIC, Side::TOP, Side::TOP,
                       Side::FRONT> : offset_data<3> {
  periodic_offset(const len_type (&dims)[3], len_type boundary_size)
      : offset_data<3>{0, -dims[1] + boundary_size * 2,
                       dims[2] - boundary_size * 2} {}
};
...
struct periodic_offset<3, BoundaryType::PERIODIC, Side::BACK, Side::BACK,
                       Side::BACK>
    : periodic_offset<3, BoundaryType::PERIODIC, Side::FRONT, Side::FRONT,
                      Side::FRONT> {
  using parent_type = periodic_offset<3, BoundaryType::PERIODIC, Side::FRONT,
...
struct periodic_offset<3, BoundaryType::PERIODIC, Side::RIGHT>
    : periodic_offset<3, BoundaryType::PERIODIC, Side::RIGHT, Side::RIGHT,
                      Side::RIGHT> {
  using parent_type = periodic_offset<3, BoundaryType::PERIODIC, Side::RIGHT,
...
struct periodic_offset<3, BoundaryType::PERIODIC, Side::BACK>
    : periodic_offset<3, BoundaryType::PERIODIC, Side::BACK, Side::BACK,
                      Side::BACK> {
  using parent_type = periodic_offset<3, BoundaryType::PERIODIC, Side::BACK,
                                      Side::BACK, Side::BACK>;
  periodic_offset(const len_type (&dims)[3], len_type boundary_size)
      : parent_type(dims, boundary_size) {}
};
...
struct periodic_offset<3, BoundaryType::PERIODIC, Side::TOP>
    : periodic_offset<3, BoundaryType::PERIODIC, Side::TOP, Side::TOP,
                      Side::TOP> {
  using parent_type = periodic_offset<3, BoundaryType::PERIODIC, Side::TOP,
                                      Side::TOP, Side::TOP>;
  periodic_offset(const len_type (&dims)[3], len_type boundary_size)
      : parent_type(dims, boundary_size) {}
};
...
struct periodic_offset<3, BoundaryType::PERIODIC, Side::BOTTOM>
    : periodic_offset<3, BoundaryType::PERIODIC, Side::BOTTOM, Side::BOTTOM,
                      Side::BOTTOM> {
  using parent_type = periodic_offset<3, BoundaryType::PERIODIC, Side::BOTTOM,
...
__host__ __device__ void get_overlap(iter_type (&overlaps)[D][2],
  ...
  for (iter_type i = 0; i < D; ++i) {
    overlaps[i][0] =
        (adjusted_origin[i] > origin[i]) ? adjusted_origin[i] : origin[i];
    overlaps[i][1] = (adjusted_origin[i] + region.dims[i] < origin[i] + dims[i])
                         ? adjusted_origin[i] + region.dims[i]
                         : origin[i] + dims[i];
  }
}
...
template <Side... sides, size_t D>
__host__ __device__ void get_overlap_adjusted(
    iter_type (&overlaps)[D][2], grid::select_region<D> const& region,
    const iter_type (&origin)[D], const len_type (&dims)[D]) {
  iter_type adjusted_origin[D];
  ...
  return get_overlap<sides...>(overlaps, region, adjusted_origin,
                               adjusted_dims);
}

template <typename T, size_t D, Side... sides>
__host__ __device__ void get_overlap(
    iter_type (&overlaps)[D][2], RegionalGrid<T, D> const& grid,
...
__host__ __device__ bool is_overlapping(const len_type (&overlaps)[D][2],
...
template <size_t D>
__host__ __device__ void get_dims_and_origin(iter_type (&origin)[D],
                                             len_type (&dims)[D],
                                             const len_type (&overlaps)[D][2]) {
  for (iter_type i = 0; i < D; ++i) {
...
template <Side... sides, size_t D>
__host__ __device__ iter_type
get_regional_index(iter_type (&pos)[D], grid::select_region<D> const& region,
                   len_type const (&dims)[D]) {
  for (iter_type i = 0; i < D; ++i) {
    if (((symphas::axis_to_index(symphas::axis_of_side(sides)) == i) || ...)) {
      pos[i] = (pos[i] >= region.origin[i])
                   ? pos[i] - region.origin[i]
                   : pos[i] - region.origin[i] + dims[i];
    }
  }
  return grid::index_from_position(pos, region.stride);
}

template <Side... sides, typename T, size_t D>
__host__ __device__ iter_type get_regional_index(iter_type (&pos)[D],
                                                 RegionalGrid<T, D>& grid) {
  return get_regional_index(pos, grid.region, grid.dims);
}

template <Side... sides, size_t D>
__host__ __device__ bool side_non_wrapping(grid::select_region<D> const& region,
                                           const len_type (&dims)[D]) {
  bool flag = true;
  for (iter_type i :
       {symphas::axis_to_index(symphas::axis_of_side(sides))...}) {
    if (region.origin[i] + region.dims[i] > dims[i]) {
      flag = false;
    }
  }
  return flag;
...
template <Side... sides, typename T, size_t D>
__host__ __device__ bool side_non_wrapping(RegionalGrid<T, D>& grid) {
...
template <typename T, size_t D, Side... sides>
void regional_update_boundary(symphas::lib::side_list<sides...>,
                              RegionalGrid<any_vector_t<T, D>, D>& grid) {
  // to update the LEFT face.
  // first see if the grid region overlaps the left boundary.
  boundary_interval<D, sides...> interval(grid.dims, grid.region.boundary_size);
  ...
  if (is_overlapping(overlaps) && side_non_wrapping<sides...>(grid)) {
    ...
    if (is_overlapping(overlaps)) {
      ...
      for (iter_type n = 0; n < grid::length<D>(interval.dims); ++n) {
        ...
        iter_type index = get_regional_index<sides...>(pos, grid);
        for (iter_type i = 0; i < D; ++i) {
          grid.values[i][index - m] = grid.values[i][index];
        }
      }
    }
  }
...
inline iter_type get_boundary_offset(symphas::lib::side_list<Side::LEFT>,
                                     const iter_type (&pos)[2],
                                     const iter_type (&origin)[2]) {
  return std::abs(pos[1] - origin[1]);
}

inline iter_type get_boundary_offset(symphas::lib::side_list<Side::RIGHT>,
                                     const iter_type (&pos)[2],
                                     const iter_type (&origin)[2]) {
  return std::abs(pos[1] - origin[1]);
}

inline iter_type get_boundary_offset(symphas::lib::side_list<Side::TOP>,
                                     const iter_type (&pos)[2],
                                     const iter_type (&origin)[2]) {
  return std::abs(pos[0] - origin[0]);
}

inline iter_type get_boundary_offset(symphas::lib::side_list<Side::BOTTOM>,
                                     const iter_type (&pos)[2],
                                     const iter_type (&origin)[2]) {
  return std::abs(pos[0] - origin[0]);
}

inline std::pair<iter_type, iter_type> get_boundary_offset(
    symphas::lib::side_list<Side::LEFT>, const iter_type (&pos)[3],
    const iter_type (&origin)[3]) {
  return {std::abs(pos[2] - origin[2]), std::abs(pos[1] - origin[1])};
}

inline std::pair<iter_type, iter_type> get_boundary_offset(
    symphas::lib::side_list<Side::RIGHT>, const iter_type (&pos)[3],
    const iter_type (&origin)[3]) {
  return {std::abs(pos[2] - origin[2]), std::abs(pos[1] - origin[1])};
}

inline std::pair<iter_type, iter_type> get_boundary_offset(
    symphas::lib::side_list<Side::TOP>, const iter_type (&pos)[3],
    const iter_type (&origin)[3]) {
  return {std::abs(pos[0] - origin[0]), std::abs(pos[2] - origin[2])};
}

inline std::pair<iter_type, iter_type> get_boundary_offset(
    symphas::lib::side_list<Side::BOTTOM>, const iter_type (&pos)[3],
    const iter_type (&origin)[3]) {
  return {std::abs(pos[0] - origin[0]), std::abs(pos[2] - origin[2])};
}

inline std::pair<iter_type, iter_type> get_boundary_offset(
    symphas::lib::side_list<Side::FRONT>, const iter_type (&pos)[3],
    const iter_type (&origin)[3]) {
  return {std::abs(pos[0] - origin[0]), std::abs(pos[1] - origin[1])};
}

inline std::pair<iter_type, iter_type> get_boundary_offset(
    symphas::lib::side_list<Side::BACK>, const iter_type (&pos)[3],
    const iter_type (&origin)[3]) {
  return {std::abs(pos[0] - origin[0]), std::abs(pos[1] - origin[1])};
}

template <Side side, size_t D>
decltype(auto) get_boundary_offset(const iter_type (&pos)[D],
                                   const iter_type (&origin)[D]) {
  return get_boundary_offset(symphas::lib::side_list<side>{}, pos, origin);
}

template <Side side0, Side... sides, typename T>
void set_boundary_position(
    double (&pos0)[2],
    const grid::BoundaryApplied<T, 0, BoundaryType::DEFAULT>* bd,
    const iter_type (&pos)[1], const iter_type (&origin)[1], double v) {
  pos0[0] = v;
  pos0[1] = 0;
}
...
void set_boundary_position(
    double (&pos0)[2],
    const grid::BoundaryApplied<T, 1, BoundaryType::DEFAULT>* bd,
    const iter_type (&pos)[2], const iter_type (&origin)[2],
    const double (&v)[2]) {
...
void set_boundary_position(
    double (&pos0)[2],
    const grid::BoundaryApplied<T, 2, BoundaryType::DEFAULT>* bd,
    const iter_type (&pos)[3], const iter_type (&origin)[3],
    const double (&v)[4]) {
...
void apply_boundary_function(
    ...
    carry_value<T> const& value, const double (&pos)[2], double time) {
...
void apply_boundary_function(
    ...
    multi_value<Dm1 + 1, T> const& value, const double (&pos)[2], double time) {
...
void regional_update_boundary(symphas::lib::side_list<sides...>,
                              ...
                              RegionalGrid<T, D>& grid, double time) {
  // to update the LEFT face.
  // first see if the grid region overlaps the left boundary.
  boundary_interval<D, sides...> interval(grid.dims, grid.region.boundary_size);

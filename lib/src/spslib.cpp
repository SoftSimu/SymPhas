...
len_type symphas::lib::radial_length(const len_type (&dim)[3]) {
  ...
  len_type &l = _dim[2], &m = _dim[1], &n = _dim[0];
  return l * m * n - (m * (m - 1) * n / 2) - (l * (n - 1) * n / 2) +
         ((n - 2) * (n - 1) * n / 2) / 3;
}

// ********************************************************************************

std::tuple<double*, size_t*> symphas::lib::make_radial_arrays(
    const len_type (&dims)[1], double dx) {
...
std::tuple<double*, size_t*> symphas::lib::make_radial_arrays(
    const len_type (&dims)[2], double dx, double dy) {
   ...
   */
  len_type len = std::max(dims[0], dims[1]), wid = std::min(dims[0], dims[1]);
...
std::tuple<double*, size_t*> symphas::lib::make_radial_arrays(
    const len_type (&dims)[3], double dx, double dy, double dz) {
  len_type rlen = radial_length(dims);
  double* rmap = new double[rlen];
  size_t* cmap = new size_t[rlen];
  ...
  std::sort(_dim, _dim + 3);
  size_t len = _dim[2], wid = _dim[1], dep = _dim[0];
...
iter_type* symphas::lib::make_radial_index_map(const len_type (&dims)[2]) {
  ...
  size_t len = std::max(dims[0], dims[1]), wid = std::min(dims[0], dims[1]);
...
iter_type* symphas::lib::make_radial_index_map(const len_type (&dims)[3]) {
  ...
  size_t len = std::max(dims[0], std::max(dims[1], dims[2])),
  ...
  for (auto const& [_, i, j, k] : vecs) {
    for (auto ind : {i + j * dims[0] + k * dims[0] * dims[1],
...
void symphas::lib::to_file_name(char const* in, char* out, size_t count) {
  ...
  while (*inc) {
    if (*inc == ' ') {
      *outc++ = '-';
    } else if ((*inc >= 'A' && *inc <= 'Z') || (*inc >= '0' && *inc <= '9')) {
...
iter_type symphas::lib::pos_after_token(const char* content,
                                        const char* token) {
...
iter_type symphas::lib::pos_after_token(
    const char* content, std::initializer_list<const char*> tokens) {
...
void symphas::lib::write_ts_str(char* buffer) {
  ...
  sprintf(buffer, TIMESTAMP_FMT, now->tm_year + 1900, now->tm_mon + 1,
...
void symphas::lib::make_directory(const char* dir, int err_no) {
  make_directory(std::filesystem::path(dir), err_no);
}
...
//    if (!CreateDirectory(build_tree, NULL) &&
...
//      fprintf(SYMPHAS_ERR, "%d error creating directory '%s' with error %d\n",
//              err_no + 3, dir, errno);
//    }
//  }
// #endif
...
// #ifdef FILESYSTEM_HEADER_AVAILABLE
std::filesystem::path symphas::lib::get_parent_directory(
...
inline bool symphas::lib::origin_sort(axis_coord_t a, axis_coord_t b,
                                      len_type axis_len) {
  if (a < 0) {
    a += axis_len;
  }
...
inline bool symphas::lib::regular_sort(axis_coord_t a, axis_coord_t b,
                                       len_type) {
  return a < b;
}

...
((static_cast<size_t>(log2(N_DIM)) - log2(N_DIM) == 0) ? (static_cast<size_t>(log2(N_DIM)) << 8) : 0) + \
((static_cast<size_t>(log2(M_DIM)) - log2(M_DIM) == 0) ? (static_cast<size_t>(log2(M_DIM)) << 4) : 0) + \
((static_cast<size_t>(log2(L_DIM)) - log2(L_DIM) == 0) ? (static_cast<size_t>(log2(L_DIM))) : 0)
...
namespace symphas::dft
{
	...
	void fft(double* data_real, double* data_imag, std::complex<double>* out, size_t LL, size_t MM);
	void fft(double* data_real, double* data_imag, std::complex<double>* out, size_t LL, size_t MM, size_t NN);
...
namespace symphas::internal
{
	...
	inline void fft_alg(double* const (&r), double* const (&c), size_t N)
	{
		...
		{
			size_t const
				le = 1ull << (M - static_cast<size_t>(l)),
				le2 = le >> 1;

			std::complex<double>
				u{ 1.0, 0.0 },
				s{ cos(symphas::PI / le2), -sin(PI / le2) };
	 ...
	 * the second matrix.
	 */
	inline void tr_matrix(double* A, double* B, size_t N, size_t M, const int block_size)
	{
	 ...
	 * threads
	 */
	inline void fft_thr_run(iter_type i, double* const (&r), double* const (&c), size_t B)
	{
	 ...
	 */
	inline void fft_thr_loop(double* const (&r), double* const (&c), size_t A, size_t B, iter_type i)
	{
		for (iter_type n = ThreadPool::thr_start_i(A, i); n < ThreadPool::thr_start_i(A, i + 1); ++n)
		{
			fft_thr_run(n, r, c, B);
		}
	}
	 ...
	 * it performs the row-wise fft for each slice of the 3d system to recover the full fourier transform
	 */
	inline void fft_thr_job_3(double* ((&r)[2]), double* ((&c)[2]),
		ThreadPool& thr, std::complex<double>* out, size_t LL, size_t MM, size_t NN, iter_type i)
	{
		for (iter_type a = 0; a < NN; ++a) { fft_thr_loop(r[0] + MM * LL * a, c[0] + MM * LL * a, MM, LL, a); }
		thr.sync(i, 1);
		for (iter_type a = 0; a < NN; ++a) { fft_thr_loop(r[1] + MM * LL * a, c[1] + MM * LL * a, LL, MM, a); }
		thr.sync(i, 2);
		for (iter_type a = 0; a < LL; ++a) { fft_thr_loop(r[0] + MM * NN * a, c[0] + MM * NN * a, NN, MM, a); }
		thr.sync(i, 3);

		size_t
			n = ThreadPool::thr_start_i(LL * MM, i),
			e = ThreadPool::thr_start_i(LL * MM, i + 1),
			offset_z = ((LL * MM * NN) >> 1ull),
			offset_y = ((LL * MM) >> 1ull),
			offset_x = LL >> 1ull;

		auto
			* it = out + n + offset_z + offset_y + offset_x,
			* last = out + LL * MM * NN;

		// the indexing is changed here in order to align the values with the correct axes
		// in the native list, the dft of the center is put at the corners

		iter_type 
			nn = static_cast<iter_type>(n),
			ii = static_cast<iter_type>(n) % static_cast<iter_type>(LL),
			jj = (static_cast<iter_type>(n) / static_cast<iter_type>(LL)) % static_cast<iter_type>(MM);
	 ...
	 * it takes the row-wise slice, transposes it and then takes the row-wise (originally columns) slice
	 */
	inline void fft_thr_job_2(double*((&r)[2]), double*((&c)[2]), 
		ThreadPool& thr_p, std::complex<double>* out, size_t LL, size_t MM, iter_type i)
	{
	...
	template<size_t LEN>
	inline void copy_in(const scalar_t* data, scalar_t real_out[LEN], scalar_t imag_out[LEN])
	{
		size_t i = 0;
	...
	template<size_t LEN>
	inline void copy_in(const complex_t* data, scalar_t real_out[LEN], scalar_t imag_out[LEN])
	{
		size_t i = 0;
...
inline void symphas::dft::fft(double* data_real, double* data_imag, std::complex<double>* out, size_t LL, size_t MM)
{
	...
	thr_p.start_thr(symphas::internal::fft_thr_job_2, std::ref(pr), std::ref(pc), std::ref(thr_p), out, LL, MM);
...
inline void symphas::dft::fft(double* data_real, double* data_imag, std::complex<double>* out, size_t LL, size_t MM, size_t NN)
{
	...
	thr_p.start_thr(symphas::internal::fft_thr_job_3, std::ref(pr), std::ref(pc), std::ref(thr_p), out, LL, MM, NN);
		...
		thr_p.main_thr_sync([&]()
			{
				if (i == 0)
				{
					...
					{
						symphas::internal::tr_matrix(data_real + MM * LL * n, dftr_tr + MM * LL * n, LL, MM, 16);
						symphas::internal::tr_matrix(data_imag + MM * LL * n, dftc_tr + MM * LL * n, LL, MM, 16);
					}
				}
				else if (i == 1)
				{
					for (iter_type x = 0; x < NN; ++x)
					{
						for (iter_type y = 0; y < MM; ++y)
						{
							...
							{
								(data_real + MM * NN * x)[NN * y + z] = dftr_tr[x + LL * MM * z + LL * y];
								(data_imag + MM * NN * x)[NN * y + z] = dftc_tr[x + LL * MM * z + LL * y];
							}
						}
					}
				}

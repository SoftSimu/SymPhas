...
void testnextsave() {
  iter_type saveme = symphas::conf::config().save_settings.save.get_start();
  auto save = symphas::conf::config().save_settings.save;

  // this test assumes that the base values give the correct next value
  for (iter_type i = symphas::conf::config().save_settings.save.get_start();
       i <= symphas::conf::config().save_settings.save.get_stop();
  ...
  if (saveme - 1 != symphas::conf::config().save_settings.save.num_saves()) {

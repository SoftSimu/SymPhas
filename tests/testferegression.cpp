...
void testferegression() {
  ...
  // FE6: grad of sum-of-products containing a derivative. The buggy
  // combine_mixed_derivatives O1==1 path emits duplicate terms like
  // `d2v/dx2 + d2/dx2(v)` and factor-2 cross-derivative coefficients;
  // assert neither appears.
  ...
  // Detect the duplicate nested-derivative wrapper form:
  expect_no_substring(s6, "d2/dx2(v", "FE6 no nested-deriv wrapper",
                      fail_count);
  expect_no_substring(s6, "d2/dy2(v", "FE6 no nested-deriv wrapper (y)",
                      fail_count);

  // FE7: gradient over a sum where one summand contains a derivative.
  // Skipped here -- this scaffold's include order trips an unrelated
  // spslibseq template-instantiation issue for this form. FE4-FE6 cover
  ...
  // builds `coeff * OpAdd<OpDerivative, OpDerivative>` which distributes
  // through `distribute_adds` and resolves via
  // `make_derivative<Dd>::get(literal, expr, solver)`. The two forms
  // below (literal vs raw double) must both resolve without ambiguity.

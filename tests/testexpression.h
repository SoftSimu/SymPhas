...
inline void testexpression() {
  ...
  // printf("\n- test testexpressionnoisetype -----------------------------\n");
  // testexpressionnoisetype();
  // printf("\n- test test12varexpression ---------------------------------\n");
  // test12varexpression();
  // printf("\n- test testccliketypes -------------------------------------\n");
  // testccliketypes();
  // printf("\n- test testcoefftype ---------------------------------------\n");
  // testcoefftype();
...
/* test the speed of generating a random number (evaluating a complicated
 * expression) and taking the derivative (central space approximation)
 */
 ...
 *
 * compared with the MyEquation object for checking how long different methods
 * take
 */
...
struct MyWrapper {
  ...
  auto eval(int i) { return (t.*r)(a, i); }
...
struct Number {
  ...
  const complex_t& operator[](iter_type n) const { return values[n]; }

  complex_t& operator[](iter_type n) { return values[n]; }

  ~Number() { delete[] values; }
};

template <size_t Z>
struct NumberSpecialized : Number {
  NumberSpecialized() : Number() { fill_data(*this); }

  void update_data() { fill_data(*this); }
};
...
template <typename A, typename B>
using etai = eta_identity<typename expr::original_data_type<A>::type,
                          typename expr::original_data_type<B>::type>;
...
std::enable_if<(std::is_same<typename expr::base_data_type<A>::type, Eta>::value
&& std::is_same<typename expr::base_data_type<B>::type, Eta>::value), int>::type
= 0> auto eta_identity_prune(OpTerms<T, A, B> const& e)
{

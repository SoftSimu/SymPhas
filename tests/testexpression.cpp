...
void testexpressioneval()
{
	...
	printf("simple test of checking evaluation of expression at a single point\n");
	...
	printf("more indepth test using the laplacian in 1d at the first (inner) grid point\n");
	...
	auto test6 = ((op0 + expr::make_literal(2)) * (op0 + op0)).eval(grid0.dims[0] + 2);
	printf("%lf %lf %lf\n", test4, test5, test6);

	auto nl1 = op0 * op0 + expr::make_literal(2) * op0; // = 8
	auto nl2 = (expr::make_operator_derivative<2>(solver) * op0); // = -6
	auto nl3 = nl2 * op0; // = -12
	printf("%lf %lf %lf\n", nl1.eval(offset + 1), nl2.eval(offset + 1), nl3.eval(offset + 1));
}
...
void testexpressionspeed()
{
	...
	printf("testing the difference in evaluation between different methods; test case "
		"involves a random number (simulating a long expression that takes longer to "
	...
	MyWrapper weq(eq, &MyEquation<double>::eval, arr);
	{
		...
		{
			arr.values[n + i % (test_len - n)] += rand() / RAND_MAX + neighbour_average(arr.values + n + i % (test_len - n), n) + constant;
		}
	}
	{
	...
	}
	{
		...
		for (int i = 0; i < test_run; ++i)
		{
			arr.values[n + i % (test_len - n)] += eq.eval(arr, n + i % (test_len - n));
		}
	}
}
...
void testexpressionmodel()
{
	...
	double a = symphas::conf::config().get_coeff(0);
	double b = symphas::conf::config().get_coeff(1);
	...
	auto pfc0 = -(expr::make_literal(a) + expr::make_literal(a) * psi + expr::make_literal(b) * psi * psi) * psi;
	auto pfc1 = (expr::make_literal(b) * psi + expr::make_literal(2.) * expr::make_literal(b) * expr::make_laplacian(psi, solver) + expr::make_bilaplacian(psi, solver));
...
void testexpressionmodelspeed()
{
	...
	Grid<double, 1> g({ iter_cap - BOUNDARY_DEPTH * 2, iter_cap - BOUNDARY_DEPTH * 2 });
	Grid<double, 1> r({ iter_cap - BOUNDARY_DEPTH * 2, iter_cap - BOUNDARY_DEPTH * 2 });
	...
	printf("compares how long it takes to directly compute a simple expression on an array %d "
		"cells long, with how long it takes to compute the same thing inside an expression object.\n", iter_cap);
	{
	...
	printf("tests how long it takes to evaluate model A using different methods.\n");

	iter_type steps = symphas::conf::config().save.get_stop();
...

	{
		...
		{
			iter_type i = BOUNDARY_DEPTH + (grid0.dims[0] + BOUNDARY_DEPTH) * BOUNDARY_DEPTH;
			for (iter_type m = 0, mm = grid0.dims[1]; m < mm; ++m)
			{
				for (iter_type l = 0, ll = grid0.dims[0]; l < ll; ++l)
				{
					grid1.values[i] = stencil.laplacian(grid0.values + i) + (a - 4. * b * grid0.values[i] * grid0.values[i]) * grid0.values[i];
					++i;
				}
...

	{
		...
		auto r0 = (OpLiteral<double>(a) - OpLiteral(4.) * OpLiteral(b) * op * op) * op;
		auto e0 = l0 + r0;
	...
	{
		/* this is extremely (EXTREMELY) slow when using the following in place of the laplacian:
		 * OpFunction laplacian2(op, [&](auto a, auto n) { return solver.laplacian(a, n); });
		 * note the lack of reference on the 'a' parameter
		 */
		OpFunction laplacian2(op, [&](auto& a, auto n) { return solver.laplacian(a, n); });
		auto r0 = (dop = laplacian2 + (expr::make_literal(a) - expr::make_literal(4.) * expr::make_literal(b) * op * op) * op);
	...
	SolverFT<Stencil2d2h<5, 13, 6>> solver2(op.dims);
	{
		...
		{
			solver2.evaluate(expr::make_term(dop) = expr::make_laplacian(expr::make_term(op), solver2) + (expr::make_literal(a) - expr::make_literal(4.) * expr::make_literal(b) * expr::make_term(op) * expr::make_term(op)) * expr::make_term(op));
		}
	}
#endif
}
...
void testexpressionoperator()
{
	...
	auto deriv_exp1 = expr::make_operator_derivative<2>(solver) * (expr::make_literal(2) + expr::make_operator_derivative<2>(solver));
	auto deriv_exp2 = (expr::make_literal(2) + expr::make_operator_derivative<2>(solver)) * expr::make_operator_derivative<2>(solver);

	// distribution of derivative operator onto a variable
	auto dv1 = expr::make_operator_derivative<2>(solver) * (expr::make_literal(2) * op);
	...
	auto opcc4 = expr::make_operator_derivative<2>(solver) + expr::make_operator_derivative<4>(solver);
	auto opcc5 = expr::make_operator_derivative<2>(solver) + opcc4;
	auto opcc6 = expr::make_operator_derivative<4>(solver) + expr::make_literal(4.0);
...
void testexpressionoperatorspeed()
{
	...
	printf("test the evaluation speed of operators evaluated on expressions, typical of "
		"the PFC case with operator combinations except simplified");

	auto opce1 = expr::make_operator_derivative<2>(solver) * op + (expr::make_literal(a) - expr::make_literal(4.) * expr::make_literal(b) * op * op) * op;
	{
		Time t0("direct evaluation, no combination");
		expr::result(opce1, grid1);
	}

	auto opco1 = (expr::make_operator_derivative<2>(solver) + expr::make_literal(a)) - OpLiteral(4.) * OpLiteral(b) * op * op;
	auto opco2 = opco1 * op;
	{
...
void testexpressionstate()
{
	...
	auto applyme = (expr::make_literal(1) + expr::make_operator_derivative<2>(solver));
	auto applymeop = (applyme * applyme) * op;

	// verifies that expressions which have a state are identified as such
	auto state_pair = (dop = expr::make_laplacian(op + op, solver));
	constexpr bool state1 = expr::has_state<decltype(expr::make_laplacian(op + op, solver) * op * op)>::value;
	constexpr bool state2 = expr::has_state<decltype(state_pair)>::value;
	constexpr bool state3 = expr::has_state<decltype(std::make_tuple(dop = applymeop, dop = op + op, state_pair))>::value;
...
void testexpressionconvolution()
{
	...
	auto cc1 = expr::make_convolution::get(GaussianSmoothing<2>{ grid0.dims, 1, 1 }, op);
	auto cc2 = expr::make_literal(1.0) * expr::make_convolution::get(GaussianSmoothing<2>{ grid0.dims, 1, 1 }, cc1);
	auto cc3 = expr::make_convolution::get(GaussianSmoothing<2>{ grid0.dims, 1, 1 }, op) - op;
}
...
void testexpressionvariable()
{
	...
	auto ovar_test_add2 = expr::make_literal(2.) * ovar_2 * ovar_2 + ovar_2 * ovar_2;
	using GG = Grid<double, 2>;// decltype(expr::get<1>(ovar_2).data().get());
	constexpr auto is_nlc1 = expr::satisfies_combination<Variable<2, GG>, symphas::lib::types_list<Variable<2, GG>>, symphas::lib::types_list<Variable<2, GG>, Variable<2, GG>>>;
	constexpr auto is_nlc2 = expr::satisfies_combination<K<2>, symphas::lib::types_list<Variable<2, GG>>, symphas::lib::types_list<K<2>, Variable<2, GG>>>;
...
void testexpressionsort()
{
	...
	constexpr auto sortderiv1 = expr::derivative_index<0, decltype(ovar_2)>::value;
	constexpr auto sortderiv2 = expr::derivative_index<0, decltype(l0)>::value;
	constexpr auto sortderiv3 = expr::derivative_index<0, decltype(applyme * op)>::value;
	constexpr auto sortderiv4 = expr::derivative_index<0, decltype(y2 * op)>::value;
	constexpr auto sortderiv5 = expr::derivative_index<0, decltype(y22 * op)>::value;
	constexpr auto sortderiv6 = expr::derivative_index<0, decltype(y * op)>::value;

	// this test establishes that the smallest over the given bound is returned
	// when the given bound is over 5 in this case, then the largest dervative will be returned
	constexpr auto sortderiv7 = expr::derivative_index<0, decltype(y22 * op + y * op)>::value;
	constexpr auto sortderiv8 = expr::derivative_index<200, decltype(y22 * op + y * op)>::value;
	constexpr auto sortderiv9 = expr::derivative_index<2, decltype(y22 * op + op)>::value;
	constexpr auto sortderivA = expr::derivative_index<2, decltype(y22 * op + y * op + op)>::value;
...
void testexpressiondivision()
{
	...
	auto divr2 = (ovar_4 / ovar_2) * ovar_2; // simple cancellation by multiplication
	...
	auto divr6 = (ovar_2 * ovar_4 * ovar_5 * ovar_2)  / (ovar_5 * ovar_4 * ovar_2 * ovar_2);


	auto ee1 = (ovar_2 * ovar_2 * ovar_2) / (ovar_2 + ovar_2);	// this one should divide
...
// distinct from the evaluate function for expressions; tests the expression object
// which acts as its own simple evaluate block
...
void testderivativefactor()
{
	...
	auto [d_l, e_l] = expr::split::separate_operator(expr::make_laplacian(op, solver));
	...
	// automatically factor by the greatest derivative order (the smallest common derivative)
	...
	// checking the order
	constexpr auto sortderiv1 = expr::derivative_index<0, decltype(e_simple1)>::value;
	//constexpr auto sortderiv2 = expr::derivative_index<0, decltype(ed2)>::value;
...
complex_t*** read_data()
{
	...
	for (int n = 0; n < 2; ++n)
	{
		...
		char* open_name = new char[std::strlen(dir_name) + std::strlen(f_name[n]) + 1];
...
complex_t* get_u_data()
{
	return get_data<0>();
}

complex_t* get_v_data()
{
	return get_data<1>();
}

double* get_theta_data()
{
	return get_data<2>();
}
...
//	"\\bar u_{k}(t)", "\\bar v_{-k}(t')", "\\bar u_{-k}(t)", "\\bar v_{k}(t')", 
//	"v_k", "u_{-k}", "\\bar u_{-k}", "\\bar v_{k}",
//	"\\eta^\\dagger_{k}", "\\eta_{-k}", "\\eta^\\dagger_{-k}", "\\eta_{k}" };

//constexpr char* ORDER_PARAMETER_NAMES[] = {
//	"utable[[p, tp + dt]]", "vtable[[mp, tp]]", "utable[[mp, tp + dt]]", "vtable[[p, tp]]",
//	"Conjugate[utable[[p, tp]]]", "Conjugate[vtable[[mp, tp + dt]]]", "Conjugate[utable[[mp, tp]]]", "Conjugate[vtable[[p, tp + dt]]]",
...
void test12varexpression()
{
	...
	//auto fourth_moment = (sin(op_thetak) * dmk * dk) * (sin(op_thetak) * ddk * ddmk);
	//auto e_right = cos(op_thetak) + sin(op_thetak) * ddk * ddmk;
	//auto e_left = cos(op_thetak) + sin(op_thetak) * dmk * dk;
	...
	for (iter_type t = 0; t < t_len; ++t)
	{
		...
		printf("<E|E> = %.4lf + %.4lfi ~ %.24lf\n", real(product), imag(product), std::abs(product) * std::abs(product));
...
void testccliketypes()
{
	...
	using cct = symphas::lib::cc_like_types<G, G>;
...
void testcoefftype()
{
	...
	constexpr bool is_neg2 = symphas::internal::has_nmi_coeff<Gpos>;
	constexpr bool is_neg1 = symphas::internal::has_nmi_coeff<Gneg>;
	constexpr bool is_pos1 = symphas::internal::has_pmi_coeff<Gpos>;
	constexpr bool is_pos2 = symphas::internal::has_val_coeff<Gneg>;
}

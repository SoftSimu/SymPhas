...
struct solver_supported_type {
  static const bool value =
      symphas::internal::solver_supported_type_match<typename T::id_type,
                                                     Ty>::value;
};
...
struct solver_system_type {
  ...
  using type = typename symphas::internal::solver_system_type_match<
      typename T::id_type,
      internal::solver_system_type_index<T>>::template type<Ty, D>;
};
...
#define ASSOCIATE_SELECTABLE_SOLVER_SYSTEM_TYPE(SOLVER_NAME, SYSTEM_TYPE)    \
  namespace symphas::internal {                                              \
  constexpr size_t SOLVER_INDEX_NAME(SYSTEM_TYPE) = decltype(solver_counter( \
      solver_count_index<solver_id_type_##SOLVER_NAME,                       \
                         SOLVER_MAX_VARIATIONS>{}))::value;                  \
  template <>                                                                \
  struct solver_system_type_match<solver_id_type_##SOLVER_NAME,              \
                                  SOLVER_INDEX_NAME(SYSTEM_TYPE) - 1> {      \
    template <typename Ty, size_t D>                                         \
    using type = SYSTEM_TYPE<Ty, D>;                                         \
  };                                                                         \
  constexpr solver_count_index<solver_id_type_##SOLVER_NAME,                 \
                               SOLVER_INDEX_NAME(SYSTEM_TYPE)>               \
      solver_counter(solver_count_index<solver_id_type_##SOLVER_NAME,        \
                                        SOLVER_INDEX_NAME(SYSTEM_TYPE)>);    \
  }
...
#define ASSOCIATE_SOLVER_SYSTEM_TYPE(SOLVER_NAME, SYSTEM_TYPE)             \
  namespace symphas::internal {                                            \
  template <>                                                              \
  struct solver_system_type_match<solver_id_type_##SOLVER_NAME, 0> {       \
    template <typename Ty, size_t D>                                       \
    using type = SYSTEM_TYPE<Ty, D>;                                       \
  };                                                                       \
  constexpr solver_count_index<solver_id_type_##SOLVER_NAME, 1>            \
      solver_counter(solver_count_index<solver_id_type_##SOLVER_NAME, 1>); \
  }
...
#define SYMPHAS_SOLVER_SUPPORTED_TYPE(SOLVER_NAME, TYPE) \
  ...
  struct symphas::internal::solver_supported_type_match< \
      solver_id_type_##SOLVER_NAME, TYPE> {              \
...
#define SYMPHAS_SOLVER_ALL_SUPPORTED(SOLVER_NAME)        \
  template <>                                            \
  struct symphas::internal::solver_supported_type_match< \
      solver_id_type_##SOLVER_NAME, void> {              \
    static const bool value = true;                      \
  };
...
#define NEW_SOLVER_WITH_STENCIL(NAME)                                      \
  ...
  struct symphas::internal::solver_id<NAME<stencil_t, N>> {                \
    using type = solver_id_type_##NAME;                                    \
  };                                                                       \
  template <typename stencil_t, size_t N>                                  \
  constexpr size_t                                                         \
      symphas::internal::solver_system_type_index<NAME<stencil_t, N>> = N; \
  template <typename stencil_t, size_t N>                                  \
...
#define NEW_SOLVER(NAME, ...)                                                \
  ...
  struct NAME;                                                               \
  template <size_t N>                                                        \
  struct symphas::internal::solver_id<NAME<N>> {                             \
    using type = solver_id_type_##NAME;                                      \
  };                                                                         \
  template <size_t N>                                                        \
  constexpr size_t symphas::internal::solver_system_type_index<NAME<N>> = N; \
  template <size_t N>                                                        \
...
struct NoStencil {
  ...
  __host__ __device__ auto applied_generalized_derivative(G&& e,
                                                          iter_type n) const {
  ...
  template <typename G>
  __host__ __device__ auto applied_laplacian(G&& e, iter_type n) const {
    return expr::BaseData<G>::get(std::forward<G>(e), n);
  }

  template <typename G>
  __host__ __device__ auto applied_bilaplacian(G&& e, iter_type n) const {
    return expr::BaseData<G>::get(std::forward<G>(e), n);
  }

  template <Axis ax, typename G>
  __host__ __device__ auto applied_gradlaplacian(G&& e, iter_type n) const {
    return expr::BaseData<G>::get(std::forward<G>(e), n);
  }

  template <Axis ax, typename G>
  __host__ __device__ auto applied_gradient(G&& e, iter_type n) const {
    return expr::BaseData<G>::get(std::forward<G>(e), n);
  }
};
...
struct Solver {
  ...
  template <Axis ax, size_t O, typename G>
  __host__ __device__ decltype(auto) generalized_derivative(G&& e,
                                                            iter_type n) const {
    return cast().template applied_generalized_derivative<ax, O>(
        std::forward<G>(e), n);
  }

  template <Axis ax, size_t O, typename G>
  __host__ __device__ decltype(auto) generalized_directional_derivative(
      G&& e, iter_type n) const {
    return cast().template applied_generalized_directional_derivative<ax, O>(
        std::forward<G>(e), n);
  }

  template <size_t... Os, typename G>
  __host__ __device__ decltype(auto) generalized_mixed_derivative(
      G&& e, iter_type n) const {
    return cast().template applied_generalized_mixed_derivative<Os...>(
        std::forward<G>(e), n);
  }

  template <typename G>
  __host__ __device__ decltype(auto) laplacian(G&& e, iter_type n) const {
    return cast().applied_laplacian(std::forward<G>(e), n);
  }

  template <typename G>
  __host__ __device__ decltype(auto) bilaplacian(G&& e, iter_type n) const {
    return cast().applied_bilaplacian(std::forward<G>(e), n);
  }

  template <Axis ax, typename G>
  __host__ __device__ decltype(auto) gradlaplacian(G&& e, iter_type n) const {
    return cast().template applied_gradlaplacian<ax>(std::forward<G>(e), n);
  }

  template <Axis ax, typename G>
  __host__ __device__ decltype(auto) gradient(G&& e, iter_type n) const {
  ...
  template <typename R>
  void equation(R&& r, len_type len) const {
    cast().equation(std::forward<R>(r), len);
  }
  ...
  inline void equation(
      symphas::internal::solver_unsupported_equation const& r) const {
    cast().equation(r);
  }
  ...
  template <size_t En, typename... S, typename... Es,
            typename std::enable_if_t<
                symphas::internal::solver_supported_systems<Sp, S...>::value,
                int> = 0>
  decltype(auto) form_expr_all(std::tuple<S...> const& systems,
                               Es&&... es) const {
   ...
   */
  template <
      size_t En, typename S, typename... Es,
      typename std::enable_if_t<
          symphas::internal::solver_supported_systems<Sp, S>::value, int> = 0>
  decltype(auto) form_expr_all(std::pair<S*, len_type> const& systems,
                               Es&&... es) const {
    return cast().template form_expr<En>(systems, std::forward<Es>(es)...);
  }
   ...
   */
  static decltype(auto) make_solver() { return Sp::make_solver({0}); }

  //! Cast the solver to the specialized type.
  /*!
   * Cast the solver to the specialized type.
   */
  __host__ __device__ Sp const& cast() const {
  ...
  template <size_t En, typename SS, typename... Es>
  decltype(auto) form_expr(SS&& s, Es&&... es) const {
    return std::make_tuple(cast().template form_expr_one<En>(
        std::forward<SS>(s), std::forward<Es>(es))...);
  }
  ...
  void equations_apply(std::tuple<Rs...>& rs,
  ...
  template <typename... Rs, size_t... Is>
  void evaluate_apply(std::tuple<Rs...>&& rs,
                      std::index_sequence<Is...>) const {
    ((..., cast().evaluate_one(std::get<Is>(rs))));
  }
...
struct redirect_apply_derivative {
  ...
  __host__ __device__ auto operator()(Sp const& sp, G&& e, iter_type n) {
...
__host__ __device__ auto apply_mixed_derivative(Sp const& sp, G&& e,
                                                iter_type n) {
  return sp.template generalized_mixed_derivative<Os...>(std::forward<G>(e), n);
}

template <Axis ax, size_t O, typename Sp, typename G>
__host__ __device__ auto apply_directional_derivative(Sp const& sp, G&& e,
                                                      iter_type n) {
  return sp.template generalized_directional_derivative<ax, O>(
      std::forward<G>(e), n);
}
...
// std::enable_if_t<(O == 1), int> = 0> auto apply_derivative(Sp const& sp, G&&
// e, iter_type n)
//{
//	return sp.template gradient<ax>(std::forward<G>(e), n);
//}
//
// template<Axis ax, size_t O, typename Sp, typename G, typename
// std::enable_if_t<(O == 2), int> = 0> auto apply_derivative(Sp const& sp, G&&
// e, iter_type n)
//{
//	return sp.laplacian(std::forward<G>(e), n);
//}
//
// template<Axis ax, size_t O, typename Sp, typename G, typename
// std::enable_if_t<(O == 3), int> = 0> auto apply_derivative(Sp const& sp, G&&
// e, iter_type n)
//{
//	return sp.template gradlaplacian<ax>(std::forward<G>(e), n);
//}
//
// template<Axis ax, size_t O, typename Sp, typename G, typename
// std::enable_if_t<(O == 4), int> = 0> auto apply_derivative(Sp const& sp, G&&
// e, iter_type n)
//{
//	return sp.bilaplacian(std::forward<G>(e), n);
//}
...
struct Solver<Sp, N>::derivative {
  template <typename G>
  __host__ __device__ auto operator()(Sp const& sp, G&& e, iter_type n) const {
    return symphas::internal::redirect_apply_derivative<ax, O>{}(
        sp, std::forward<G>(e), n);
  }
...
struct Solver<Sp, N>::directional_derivative {
  template <typename G>
  __host__ __device__ auto operator()(Sp const& sp, G&& e, iter_type n) const {
    return symphas::internal::apply_directional_derivative<ax, O>(
        sp, std::forward<G>(e), n);
  }
  static const size_t order = O;
  static const Axis axis = ax;
...
struct max_order_axis<symphas::lib::axis_list<ax1, ax2>,
...
struct max_order_axis<symphas::lib::axis_list<ax1, ax2, ax3>,
                      std::index_sequence<O1, O2, O3>> {
  static const Axis value = (O1 >= O2 && O1 >= O3)   ? ax1
                            : (O2 >= O1 && O2 >= O3) ? ax2
                                                     : ax3;
};
}
...
struct Solver<Sp, N>::mixed_derivative {
  template <typename G>
  __host__ __device__ auto operator()(Sp const& sp, G&& e, iter_type n) const {
    return symphas::internal::apply_mixed_derivative<Os...>(
        sp, std::forward<G>(e), n);
  }
  static const size_t order = symphas::math::sum<Os...>();
  static const Axis axis = symphas::internal::max_order_axis<
      decltype(symphas::lib::make_axis_list<sizeof...(Os)>()),
      std::index_sequence<Os...>>::value;
  static const bool is_directional = true;
...
struct symphas::internal::solver_supported_system {
 protected:
  ...
  struct supported_wrap {
    static const bool value = solver_supported_type<T, void>::value ||
                              solver_supported_type<T, Ty>::value;
  };
...
#ifdef USING_CUDA
  template <typename Ty>
  static constexpr auto is_supported(BlockCUDA<Ty>) {
    return supported_wrap<Ty>{};
  }

  template <size_t N, typename Ty>
  static constexpr auto is_supported(MultiBlockCUDA<N, Ty>) {
    return supported_wrap<any_vector_t<Ty, N>>{};
  }
#endif

  static constexpr auto get_value(S s) { return is_supported(s); }

  using wrap_type = typename std::invoke_result_t<
      decltype(&solver_supported_system<T, S>::get_value), S>;
...
template <>
struct symphas::solver_system_type<void> {
  template <typename Ty, size_t D>
  using type = typename symphas::internal::solver_system_type_match<
...
//
//	/*
//	 * the parameter is a tuple with the first parameter being a Variable
// object with ref
//	 * base type, and the ref is a reference to the system; like this, the
// variable index is
...
////! Placeholder solver.
///*!
// * A placeholder solver which will avoid errors when no concrete solver is
// implemented

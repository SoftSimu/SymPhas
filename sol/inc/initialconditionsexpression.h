...
#define INITIAL_CONDITION_EQUATION(NAME, DIMENSIONS, EQUATION)                \
  ...
  INIT_EXPR_WRAPPER_FUNC(                                                     \
      INIT_EXPR_INDEX_NAME(NAME), DIMENSIONS,                                 \
      if (std::strcmp(name, #NAME) == 0) {                                    \
        return symphas::internal::run_init_expr<D, InitExpression_##NAME<D>>( \
            ax, values, interval, vdata, coeff, num_coeff);                   \
      })
  ...
  struct InitExpression_##NAME : InitExpression<D> {                           \
    ...
    auto get_equation() {                                                      \
      ...
      auto [x, y, z] = expr::make_coords<D>(parent_type::data.dims,            \
                                            parent_type::data.info.intervals); \
      UNUSED(x);                                                               \
      UNUSED(y);                                                               \
      UNUSED(z);                                                               \
...
#define NEXT_INIT_EXPR_INDEX(PREFIX_NAME)                            \
  ...
  constexpr init_expr_count_index<INIT_EXPR_INDEX_NAME(PREFIX_NAME)> \
      init_expr_counter(                                             \
          init_expr_count_index<INIT_EXPR_INDEX_NAME(PREFIX_NAME)>); \
  }
...
#define INIT_EXPR_WRAPPER_FUNC(N, DIMENSIONS, ...)                            \
  ...
  struct symphas::internal::init_expr_call_wrapper<D, N> {                    \
    template <typename T>                                                     \
    static int call(const char* name, Axis ax, T* values,                     \
                    grid::region_interval<D> const& interval,                 \
                    symphas::interval_data_type const& vdata,                 \
                    double const* coeff, size_t num_coeff) {                  \
      if constexpr (symphas::lib::is_value_in_seq<                            \
                        size_t, D,                                            \
                        std::index_sequence<SINGLE_ARG DIMENSIONS>>::value) { \
        __VA_ARGS__                                                           \
      }                                                                       \
      return init_expr_call_wrapper<D, N - 1>::call(                          \
...
struct InitExpression {
  struct AxisData {
    ...
    template <size_t... Is>
    AxisData(const len_type (&dims)[D], symphas::grid_info const& info,
             std::index_sequence<Is...>)
        : dims{dims[Is]...}, info{info} {}
    AxisData(const len_type (&dims)[D], symphas::grid_info const& info)
        : AxisData(dims, info, std::make_index_sequence<D>{}) {}
  ...
  InitExpression(grid::region_interval<D> const& interval,
                 symphas::interval_data_type const& vdata, double const* coeff,
                 size_t num_coeff)
      : coeff{(num_coeff > 0) ? new double[num_coeff] : nullptr},
        num_coeff{num_coeff},
        data{interval.dims, vdata} {
    std::copy(coeff, coeff + num_coeff, this->coeff);
  }

  InitExpression(InitExpression const& other)
      : InitExpression(other.coeff, other.num_coeff) {}
  InitExpression(InitExpression&& other) : InitExpression() {
    swap(*this, other);
  }
  InitExpression& operator=(InitExpression other) {
    swap(*this, other);
    return *this;
  }
...
struct InitExpressionBuild<D0, D1, Ds...> : InitExpression<D0>,
                                            InitExpressionBuild<D1, Ds...> {
  using parent_type = InitExpression<D0>;
  using parent_type::parent_type;
};
...
bool run_init_expr(Axis ax, T* values, grid::region_interval<D> const& interval,
                   symphas::interval_data_type const& vdata,
                   double const* coeff, size_t num_coeff) {
  EI init_expr{interval, vdata, coeff, num_coeff};
  auto expr = init_expr.get_equation();

  auto begin = symphas::internal::ic_iterator<T, D, decltype(expr)>(
      ax, values, expr, interval);
  auto end = symphas::internal::ic_iterator<T, D, decltype(expr)>(
      ax, values, expr, interval, grid::length<D>(interval));

...
void update_for_regional(PhaseFieldSystem<G, T, D>& system,
...
void update_for_regional(PhaseFieldSystem<RegionalGrid, T, D>& system,
                         symphas::grid_info& info) {
  grid::resize_adjust_region(system, info);

  for (auto& [axis, interval] : info) {
    double offset = system.region.boundary_size * interval.width();
    system.info[axis].set_interval(interval.left() + offset,
                                   interval.right() - offset);
...
template <typename T, size_t D>
void update_for_regional(PhaseFieldSystem<RegionalGridMPI, T, D>& system,
                         symphas::grid_info& info) {
  grid::resize_adjust_region(system, info);
...
struct DataStepper {
  ...
  using SolverSystemApplied =
      typename symphas::solver_system_type<Sp>::template type<T, D>;

  template <typename T>
  struct solver_system_impl : SolverSystemApplied<T> {
    solver_system_impl(SolverSystemApplied<T> const& s)
        : SolverSystemApplied<T>(s) {}
  };

  template <typename T>
  T _get_solver_system_type(SolverSystemApplied<T>) {
    return {};
  }
  template <typename S>
  auto get_solver_system_type(S s) {
    return _get_solver_system_type(s);
  }

  template <typename S>
  struct solver_implicit_type {
    using type = std::invoke_result_t<
        decltype(&DataStepper<D, Sp>::get_solver_system_type<S>),
        DataStepper<D, Sp>, S>;
  };
  ...
  DataStepper(len_type len, SaveParams const& save = {})
      : data{(len > 0) ? new symphas::init_entry_type[len]{} : nullptr},
        len{len},
        save{save},
        dt{} {}
  ...
  void step(S& system) {
    ...
    symphas::io::read_info rinfo{next_index, system.id,
                                 data[system.id].file.get_name(),
                                 data[system.id].in == Inside::CHECKPOINT};
    rinfo.set_offset(false);

    symphas::grid_info ginfo(system.info);
    iter_type read_index =
        symphas::io::read_grid<solver_implicit_t<S>>(rinfo, &ginfo);
  ...
  void set_save_object(SaveParams const& save) { this->save = save; }

  iter_type index() const { return data->file.get_index(); }

  static DataStepper<D, Sp> make_solver(
      symphas::problem_parameters_type const& pp) {
    len_type len = (len_type)pp.length();
    DataStepper<D, Sp> ds(len);
    for (iter_type n = 0; n < ds.len; ++n) {
      if (pp.get_initial_data()[n].at(Axis::NONE).in == Inside::CHECKPOINT ||
        ...
        fprintf(SYMPHAS_ERR,
                "a virtual model can only be initialized "
...
struct ModelVirtual : Model<D, DataStepper<D, Sp>, S...> {
  ...
  auto modify_tdata(symphas::init_data_type& load, const char* dir,
                    iter_type start_index, bool checkpoint) const {
    load[Axis::NONE].in = (checkpoint) ? Inside::CHECKPOINT : Inside::FILE;
    load[Axis::NONE].file = {dir, start_index};
  }

  symphas::problem_parameters_type build_parameters(const char* dir,
                                                    iter_type start_index,
  ...
  symphas::problem_parameters_type build_parameters(
      const char* dir, symphas::problem_parameters_type const& parameters,
      bool checkpoint = true) {
    auto pp(parameters);

    for (auto p0 : pp) {
      auto& [tdata, vdata, bdata] = p0;
   ...
   */
  ModelVirtual(const char* solution_dir, SaveParams save,
               bool checkpoint = true)
      : parent_type(
            nullptr, 0,
            build_parameters(solution_dir, save.get_start(), checkpoint)) {
   ...
   * solution, rather than a plain file.
   */
  ModelVirtual(const char* solution_dir, iter_type step_index,
               iter_type start_index, iter_type stop_index,
               bool checkpoint = true)
      : ModelVirtual(solution_dir,
                     SaveParams{step_index, start_index, stop_index},
                     checkpoint) {}
   ...
   */
  ModelVirtual(const char* solution_dir, iter_type step_index,
               iter_type stop_index, bool checkpoint = true)
      : ModelVirtual(solution_dir, step_index, stop_index, params::start_index,
                     checkpoint) {}

  ModelVirtual(const char* solution_dir,
               symphas::problem_parameters_type const& parameters,
               bool checkpoint = true)
      : parent_type(nullptr, 0,
                    build_parameters(solution_dir, parameters, checkpoint)) {
  ...
  void update(double time) { parent_type::update_systems(time); }
...
struct ModelVirtual<0> {
  ...
  ModelVirtual(const char* dir, iter_type start, iter_type end)
      : start{start},
        ...
        dir{(dir && std::strlen(dir) > 0) ? new char[std::strlen(dir) + 1]{}
                                          : nullptr} {
    if (this->dir) {
  ...
  ModelVirtual(const char* dir, iter_type start)
      : ModelVirtual(dir, start, start) {}
  ModelVirtual(const char* dir) : ModelVirtual(dir, 0, 0) {}
  ModelVirtual() : ModelVirtual(nullptr, 0, 0) {}

  ModelVirtual(ModelVirtual const& other)
      : ModelVirtual(other.dir, other.start, other.end) {}
  ModelVirtual(ModelVirtual&& other) : ModelVirtual() { swap(*this, other); }
...
using ArrayModelVirtual =
    ModelVirtual<D, Sp, symphas::internal::field_array_t<void>, S...>;

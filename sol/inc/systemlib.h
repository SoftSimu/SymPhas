...
struct WriteParallel {
  ...
  WriteParallel(WriteParallel const& other)
      : snapshot{other.snapshot}, done{true}, thr{nullptr} {}
  WriteParallel(WriteParallel&& other)
      : snapshot{other.snapshot}, done{true}, thr{nullptr} {
    swap(*this, other);
  }
  ...
  friend void swap(WriteParallel& first, WriteParallel& second) {
    using std::swap;
    swap(first.snapshot, second.snapshot);
    swap(first.done, second.done);
    swap(first.thr, second.thr);
  }

  void write(symphas::io::write_info w, symphas::grid_info g) const {
    finish_last_write();
    done = false;
    thr = new std::thread{&WriteParallel<T>::write_delegate, snapshot.values, w,
                          g};
  }
  ...
  ~WriteParallel() { finish_last_write(); }
 ...
 protected:
  ...
  static void write_delegate(const T* values, symphas::io::write_info w,
                             symphas::grid_info g) {
...
struct SystemInfo {
  ...
  auto const get_id() const { return id; }
...
struct SystemData<Grid<T, D>> : Grid<T, D>, SystemInfo {
   ...
   * underlying grid.
   */
  operator const Grid<T, D>&() const { return as_grid(); }
   ...
   * underlying grid.
   */
  operator Grid<T, D>&() { return as_grid(); }
   ...
   */
  void persist(T* out) const { grid::copy(*this, out); }
   ...
   */
  void fill(const T* in) const { grid::fill(in, *this); }

 protected:
  SystemData() : Grid<T, D>{}, SystemInfo{} {}
};
...
struct SystemData<BoundaryGrid<T, D>> : BoundaryGrid<T, D>, SystemInfo {
 ...
 protected:
  SystemData() : BoundaryGrid<T, D>{}, SystemInfo{} {}
};
...
struct SystemData<RegionalGrid<T, D>> : RegionalGrid<T, D>, SystemInfo {
   ...
   * that is, all non-boundary elements. Return the count of these elements.
   */
  auto length() const { return grid::length_interior<D>(region.dims); }
 ...
 protected:
  SystemData() : RegionalGrid<T, D>{}, SystemInfo{} {}
};
...
struct SystemData<RegionalGridMPI<T, D>> : RegionalGridMPI<T, D>, SystemInfo {
   ...
   * grid instance and information defining that grid.
   */
  SystemData(symphas::interval_data_type const& vdata,
             symphas::multi_thr_info_type const& thr_info)
      : RegionalGridMPI<T, D>{grid::construct<::RegionalGridMPI, T, D>(thr_info,
                                                                       vdata)},
 ...
 */
template <typename T, size_t D>
struct PersistentSystemData<RegionalGrid<T, D>>
    : SystemData<RegionalGrid<T, D>> {
 ...
 */
template <typename T, size_t D>
struct PersistentSystemData<RegionalGridMPI<T, D>>
    : SystemData<RegionalGridMPI<T, D>> {
   ...
   * grid instance and information defining that grid.
   */
  PersistentSystemData(symphas::interval_data_type const& vdata,
                       symphas::multi_thr_info_type const& thr_info)
...
//! Default argument overload of symphas::io::save_grid(T*,
...
void symphas::io::save_grid(PersistentSystemData<G> const& sys) {
  ...
  symphas::io::save_grid(sys.values, symphas::io::write_info{0, sys.get_id()},
...
void symphas::io::save_grid(PersistentSystemData<G> const& sys,
  ...
  symphas::io::save_grid(
      sys.values,
      symphas::io::write_info{file, 0, sys.get_id(), DataFileType::NAMED_DATA},
      sys.dims, sys.info.dimension());
}
...
struct time_step_list {
  ...
  time_step_list(double dt = 1.0, double time = 0)
      : t_dts{new double[2]{time, dt}}, dts_len{1} {}
  time_step_list(double* dts, double* t_dts, size_t dts_len)
      : t_dts{(dts_len > 0) ? new double[dts_len * 2]{} : nullptr},
        dts_len{dts_len} {
  ...
  time_step_list(time_step_list const& other)
      : t_dts{(other.dts_len > 0) ? new double[other.dts_len * 2]{} : nullptr},
        dts_len{other.dts_len} {
  ...
  //! Clear all the time steps and replace it with one time step set at the
  //! default value.
  ...
  inline size_t get_num_time_steps() const { return dts_len; }

  double get_time_step(double time = 0) const;

  auto begin() const {
    return symphas::lib::zip(symphas::stride_type(t_dts, 2, dts_len),
                             symphas::stride_type(t_dts + 1, 2, dts_len))
        .begin();
  }
  ...
  ~time_step_list() { delete[] t_dts; }
};

struct problem_parameters_element;

struct problem_parameters_type;
void update_thr_info(multi_thr_info_type* thr_info,
                     problem_parameters_type const& problem_parameters,
                     int i = 0);
...
struct problem_parameters_type {
 protected:
  //! Data for the initial conditions of each system.
  symphas::init_data_type* tdata;
  //! Data for the intervals of each system.
  symphas::interval_data_type* vdata;
  //! Boundary data for each system.
 ...
 public:
  ...
  problem_parameters_type(const size_t len)
      : tdata{(len > 0) ? new symphas::init_data_type[len]{} : nullptr},
        ...
        num_fields_len{1},
        dt_list{},
        modifiers{new size_t[1]{}},
        time{TIME_INIT},
        index{params::start_index} {}

  problem_parameters_type(len_type* num_fields, size_t num_fields_len)
      : problem_parameters_type(
            (num_fields != nullptr && num_fields_len > 0)
                ? std::reduce(num_fields, num_fields + num_fields_len)
                : 0) {
    if (num_fields_len > 0) {
  ...
  problem_parameters_type(problem_parameters_type&& other) noexcept
      : problem_parameters_type() {
    swap(*this, other);
  }
  ...
  inline void set_initial_data(symphas::init_data_type const& tdata_set,
                               iter_type i) {
    if (i < len) {
      tdata[i] = tdata_set;
      ...
      throw std::out_of_range(
          "given initial data element larger than list length\n");
    }
  }
  void set_initial_data(symphas::init_entry_type const& tentry_set,
                        iter_type i) {
   ...
   * \param n The length of the list.
   */
  void set_interval_data(const symphas::interval_data_type* vdata_set,
                         size_t n);
  inline void set_interval_data(const symphas::interval_data_type* vdata_set) {
    set_interval_data(vdata_set, len);
  }
  inline void set_interval_data(symphas::interval_data_type const& vdata_set,
                                iter_type i) {
    if (i < len) {
      vdata[i] = vdata_set;
    } else {
      throw std::out_of_range(
          "given interval data element larger than list length\n");
    }
  }
  ...
  inline void set_boundary_data(symphas::b_data_type const& bdata_set,
                                iter_type i) {
    if (i < len) {
      bdata[i] = bdata_set;
    } else {
      throw std::out_of_range(
          "given boundary data element larger than list length\n");
    }
  }

  //! Set a list of number of fields, when the number of fields is
  //! runtime-configuration.
  /*
   * Set the number of fields of each field type that will be initialized in a
   * model When a model has multiple order parameters types that can have their
   * total count set from the configuration, the count will be taken from the
   * parameter set here. The order of the lengths corresponds to the order of
   * the configurable fields. If the length of the given list is smaller than
   * the number of systems represented by this object, the remaining elements
   * will be initialized to the first of this list.
   *
   * \param num_fields_set The list of field lengths to set. The i-th element
   * in the given list applies to the i-th phase-field type for configurable
   * field counts. \param n The length of the list.
   */
  ...
  inline void set_num_fields(len_type num_fields_set, iter_type i) {
    if (i < num_fields_len) {
      ...
      throw std::out_of_range(
          "given field length element larger than list length\n");
    }
  }
  ...
  inline const symphas::b_data_type* get_boundary_data() const { return bdata; }

  //! Get the list of initial data.
  inline symphas::init_data_type* get_initial_data() { return tdata; }

  //! Get the list of interval data.
  inline symphas::interval_data_type* get_interval_data() { return vdata; }

  //! Get the list of boundary data.
  inline symphas::b_data_type* get_boundary_data() { return bdata; }

  inline const len_type* get_num_fields() const { return num_fields; }

  inline size_t get_num_fields_len() const { return num_fields_len; }

  inline len_type* get_num_fields() { return num_fields; }
  ...
  using modifier_list_type =
      symphas::lib::array_container<symphas::ModelModifiers>;
   ...
   */
  inline size_t get_dimension(iter_type i = 0) const { return vdata[i].size(); }
  ...
  friend void swap(symphas::problem_parameters_type& first,
                   symphas::problem_parameters_type& second);
  ...
  void extend(size_t new_len) {
    if (new_len > len) {
      auto pp0(problem_parameters_type::init_default(get_dimension(),
                                                     new_len - len));
      if (len == 0) {
        swap(pp0, *this);
      } else {
        ...
        pp.set_boundary_data(get_boundary_data(), n);
        pp.set_initial_data(get_initial_data(), n);
        pp.set_interval_data(get_interval_data(), n);
        pp.set_time_steps(dt_list);
        pp.time = time;
        pp.index = index;

        pp.set_boundary_data(pp0.get_boundary_data()[0], n);
        pp.set_initial_data(pp0.get_initial_data()[0], n);
        pp.set_interval_data(pp0.get_interval_data()[0], n);
  ...
  auto begin() {
    return symphas::lib::zip(symphas::stride_type(tdata, length()),
                             symphas::stride_type(vdata, length()),
                             symphas::stride_type(bdata, length()))
        .begin();
  }
...
void swap(symphas::problem_parameters_type& first,
          symphas::problem_parameters_type& second);
...
struct boundary_side_type {
  Side side;
  b_element_type bdata;
};

//! Generate an interval
template <char... n>
constexpr auto operator""_h() {
  return interval_element_type(1, symphas::lib::char_to_number<n...>{}());
}
...
inline interval_data_type operator*(interval_element_type const& a,
                                    interval_element_type const& b) {
...
inline interval_data_type operator*(interval_data_type const& a,
                                    interval_element_type const& b) {
 ...
 */
inline interval_element_type operator/(interval_element_type const& vdata0,
                                       interval_discrete_element const& width) {
 ...
 * interval length. The left point will be kept the same.
 */
inline interval_data_type operator/(interval_data_type const& vdata0,
                                    interval_discrete_element const& width) {
 ...
 */
template <size_t N>
inline interval_data_type operator/(
    interval_data_type const& vdata0,
    interval_discretization_type<N> const& width) {
  interval_data_type vdata(vdata0);
  for (auto& [axis, interval] : vdata) {
...
inline problem_parameters_element operator<<(problem_parameters_element a,
                                             interval_data_type const& vdata) {
  a.parameters->set_interval_data(vdata, a.index);
  return a;
}

// problem_parameters_element& operator<<(problem_parameters_element a,
// interval_element_type& vdata0)
// {
...
inline problem_parameters_element operator<<(problem_parameters_element a,
                                             b_data_type const& bdata) {
  a.parameters->set_boundary_data(bdata, a.index);
  return a;
}

inline problem_parameters_element operator<<(problem_parameters_element a,
                                             init_data_type const& tdata) {
...
inline problem_parameters_element operator<<(
    problem_parameters_element a, interval_boundary_data_type const& wdata) {
  return (a << wdata.vdata << wdata.bdata);
}

// Generate system grid parameter data

inline interval_boundary_element_type operator||(
    b_element_type const& bdata, interval_element_type const& vdata) {
...
inline interval_boundary_element_type operator||(
    b_element_type const& bdata, interval_boundary_element_type const& wdata0) {
  auto wdata(wdata0);
  wdata.bdata[0] = bdata;
  return wdata;
}
...
inline interval_boundary_element_type operator||(
    BoundaryType boundary, interval_element_type const& vdata) {
  return (b_element_type(boundary) || vdata);
}

inline interval_boundary_element_type operator||(
    BoundaryType boundary, interval_boundary_element_type const& wdata) {
  return (b_element_type(boundary) || wdata);
}

inline interval_boundary_element_type operator||(
    BoundaryTag tag, interval_element_type const& vdata) {
  return (b_element_type(BoundaryType::DEFAULT, {tag}) || vdata);
}

inline interval_boundary_element_type operator||(
    BoundaryTag tag, interval_boundary_element_type const& wdata) {
  return (b_element_type(BoundaryType::DEFAULT, {tag}) || wdata);
}

inline interval_boundary_element_type operator||(
    interval_element_type const& vdata, b_element_type const& bdata) {
  interval_boundary_element_type wdata;
  wdata.bdata[1] = bdata;
  if (bdata.type == BoundaryType::PERIODIC) {
    wdata.bdata[0] = bdata;
...
inline interval_boundary_element_type operator||(
    interval_boundary_element_type const& wdata0, b_element_type const& bdata) {
  auto wdata(wdata0);
  wdata.bdata[1] = bdata;
  return wdata;
}
...
inline interval_boundary_element_type operator||(
    interval_element_type const& vdata, BoundaryType boundary) {
  return (vdata || b_element_type(boundary));
}

inline interval_boundary_element_type operator||(
    interval_boundary_element_type const& wdata, BoundaryType boundary) {
  return (wdata || b_element_type(boundary));
}

inline interval_boundary_element_type operator||(
    interval_element_type const& vdata, BoundaryTag tag) {
  return (vdata || b_element_type(BoundaryType::DEFAULT, {tag}));
}

inline interval_boundary_element_type operator||(
    interval_boundary_element_type const& wdata, BoundaryTag tag) {
  return (wdata || b_element_type(BoundaryType::DEFAULT, {tag}));
}

inline interval_boundary_data_type operator*(
    interval_boundary_element_type const& wdata0,
    interval_boundary_element_type const& wdata1) {
  interval_data_type vdata;
...
inline interval_boundary_data_type operator*(
    interval_boundary_data_type const& wwdata0,
    interval_boundary_element_type const& wdata) {
  auto wwdata(wwdata0);
...
inline boundary_side_type operator/(Side side, b_element_type const& type) {
  return {side, type};
}

inline boundary_side_type operator/(Side side, BoundaryTag tag) {
  b_element_type type(BoundaryType::DEFAULT, {tag});
  return side / type;
}

inline boundary_side_type operator/(Side side, BoundaryType b) {
  b_element_type type(b);
  return side / type;
}

inline boundary_side_type operator<<(
    boundary_side_type type, symphas::internal::boundary_info const& info) {
  symphas::internal::boundary_info info0(info);
  info0.tag = type.bdata.tag;
  return {type.side, type.bdata.type << info0};
}

inline boundary_side_type operator<<(boundary_side_type type, double value) {
  symphas::internal::boundary_info binfo(type.bdata.tag, &value, 1);
  return {type.side, binfo};
}

inline auto operator<<(BoundaryTag type,
                       std::initializer_list<double> const& values) {
  return symphas::internal::boundary_info(type, values.begin(),
                                          int(values.size()));
}

inline interval_boundary_data_type operator<<(
    interval_data_type const& interval, boundary_side_type const& boundary) {
  interval_boundary_data_type vdata{interval, {}};
  vdata.bdata[boundary.side] = boundary.bdata;
  return vdata;
}

inline interval_boundary_data_type operator<<(
    boundary_side_type const& boundary, interval_data_type const& interval) {
  interval_boundary_data_type vdata{interval, {}};
  vdata.bdata[boundary.side] = boundary.bdata;
  return vdata;
}
...
inline interval_boundary_data_type operator<<(
    interval_boundary_data_type const& vdata,
    boundary_side_type const& boundary) {
  interval_boundary_data_type wdata(vdata);
  wdata.bdata[boundary.side] = boundary.bdata;
  return vdata;
}

//
// inline boundary_side_type operator<<(boundary_side_type const& type,
//                                     double value) {
//  return {side, type};
//}
//
...
inline init_entry_type operator<<(init_entry_type inside,
                                  spec_list const& params) {
...
template <typename T>
inline init_entry_type operator<<=(Inside inside,
                                   std::initializer_list<T> const& data) {
...
inline problem_parameters_type operator<<(
    interval_boundary_data_type const& wwdata, init_data_type const& tdata) {
...
inline problem_parameters_type operator<<(
    interval_boundary_data_type const& wwdata, Inside const& inside_type) {
  return wwdata << init_data_type(inside_type);
}

inline problem_parameters_type operator<<(
    Inside const& inside_type, interval_boundary_data_type const& wwdata) {
  return wwdata << init_data_type(inside_type);
}

inline problem_parameters_type operator<<(
    interval_boundary_data_type const& wwdata, init_entry_type const& tdata) {
  return wwdata << init_data_type(tdata);
}

inline problem_parameters_type operator<<(
    init_entry_type const& tdata, interval_boundary_data_type const& wwdata) {
  return wwdata << init_data_type(tdata);
}
...
inline problem_parameters_element problem_parameters_type::operator[](
    int index) {
...
inline problem_parameters_type& operator+=(
    problem_parameters_type& left, problem_parameters_type const& right) {
  iter_type index = (iter_type)left.length();
  left.extend(left.length() + right.length());
  auto* b_data = left.get_boundary_data();
  auto* t_data = left.get_initial_data();
  auto* v_data = left.get_interval_data();

  for (iter_type i = 0; i < right.length(); ++i) {
    b_data[index + i] = right.get_boundary_data()[i];
    t_data[index + i] = right.get_initial_data()[i];
    v_data[index + i] = right.get_interval_data()[i];
  }
...
#define INITIAL_CONDITION_INVALID_MSG \
  "the given initial condition algorithm is not valid\n"

namespace symphas::internal {

template <typename T, size_t D>
void populate_tdata(symphas::init_data_type const& tdata, Grid<T, D>& data,
                    [[maybe_unused]] symphas::grid_info* info,
                    grid::region_interval<D> const& region,
                    [[maybe_unused]] size_t id) {
  auto ginfo =
      InitialConditions<T, D>{tdata, info->intervals, data.dims, id}.initialize(
          data.values, region);
...
void populate_tdata(symphas::init_data_type const& tdata,
                    Grid<any_vector_t<T, D>, D>& data,
                    [[maybe_unused]] symphas::grid_info* info,
                    grid::region_interval<D> const& region,
                    [[maybe_unused]] size_t id) {
  any_vector_t<T, D>* values = new any_vector_t<T, D>[data.len];
  auto ginfo = InitialConditions<any_vector_t<T, D>, D>{tdata, info->intervals,
                                                        data.dims, id}
                   .initialize(values, region);
  if (!symphas::is_valid(ginfo)) {
    fprintf(SYMPHAS_ERR, INITIAL_CONDITION_INVALID_MSG);
  }
...
template <typename T, size_t D>
void update_info_for_regional(RegionalGrid<T, D> const& data,
                              symphas::grid_info const& info,
                              symphas::grid_info* to_update) {
  for (const auto& [axis, interval] : info) {
    auto boundary = interval.width() * data.region.boundary_size;
    (*to_update)[axis].set_interval(interval.left() + boundary,
                                    interval.right() - boundary);
  }
}
...
void populate_tdata(symphas::init_data_type const& tdata,
  ...
  if (!symphas::is_valid(ginfo)) {
    ...
    for (auto [ax0, interval0] : ginfo) {
      ...
      if (interval0.left() != interval1.left() ||
          interval0.right() != interval1.right()) {
        update_region = true;
      }
    }
    if (update_region) {
...
template <typename T, size_t D>
void populate_tdata(symphas::init_data_type const& tdata,
                    RegionalGrid<any_vector_t<T, D>, D>& data,
                    [[maybe_unused]] symphas::grid_info* info,
                    grid::region_interval<D> const& region,
                    [[maybe_unused]] size_t id) {
  any_vector_t<T, D>* values = new any_vector_t<T, D>[data.len];
...
inline void update_thr_info(multi_thr_info_type* thr_info,
                            problem_parameters_type const& problem_parameters,
                            int i) {
  int flag = 0;

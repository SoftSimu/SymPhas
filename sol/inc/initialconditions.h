 ...
 * Initial conditions for a system are quite flexible; they can be defined from
 * an algorithm, a file or even a symbolic expression.
 *
 * \section Overview
 *
 ...
 * |------------|--------------------------------------|-------------|
 * |`GAUSSIAN`| 		Uses a Gaussian distribution to randomly assign
 * values.|  *None* |
 * |`UNIFORM`| 			Uses a uniform distribution to randomly assign
 * values.| *None* |
 * |`CAPPED`| 			Values are assigned to be either the minimum or
 * maximum parameter.| *None* |
 * |`CONSTANT`|			All values are assigned to the same parameter.|
 * *None* |
 * |`CIRCLE`| 			Value are assigned to be the shape of a circle.|
 * `RANDOM`, `A`, `A+RANDOM` |
 * |`HEXAGONAL`| 		Values are assigned into circles arranged in a
 * hexagonal pattern.| `RANDOM` |
 * |`CUBIC`| 			Values are assigned into circles arranged in a
 * cubic pattern.| `RANDOM` |
 * |`SQUARE`|			Values are assigned to be the shape of a
 * square.| `RANDOM`, `A`, `A+RANDOM` |
 * |`SQUARESEEDS`| 		Values are put into randomly arranged squares.|
 * `RANDOM`, `A`, `A+RANDOM`, `B`, `B+RANDOM`  |
 * |`CIRCLESEEDS`|		Values are put into randomly arranged circles.|
 * `RANDOM`, `A`, `A+RANDOM`, `B`, `B+RANDOM` |
 * |`VORONOI`|		A Voronoi crystal is generated.| `FIXEDSEED` `RANDOM`,
 * `FIXEDSEED+RANDOM`, `+VARA`, `+VARB`, `+VARC`,`+VARA+VARB`, `+VARA+VARC` |
 * |`BUBBLE`|		An arrangement of bubbles is generated.| `FIXEDSEED`
 * `RANDOM`, `FIXEDSEED+RANDOM`, `+VARA`, `+VARB`, `+VARC`,`+VARA+VARB`,
 * `+VARA+VARC` |
 * |`SPIRALHEX`|		A hexagonal arrangement where each field
 * corresponds to a single point, spirally ordered.| `VARA` |
 ...
 * | `RANDOM`|		The generation is modified to include some kind of
 * randomness.| | `VARA`|		The A variation is chosen (different
 * from the default variation). Usually implies periodic boundaries are taken
 * into account. | | `VARB`|		The B variation is chosen.| | `INVERT`|
 * The interior and outer values are switched in the generation. <br> (In some
 * algorithms, the generated value is multiplied by -1.)| | `NONE`|
 * Represents no tag.|
 *
 * If an initial condition is generated with <span
 * style="color:violet">`Inside`</span>`::NONE` and <span
 * style="color:violet">`InsideTag`</span>`::NONE`, then no values are
 * populated. If an initial condition is combined with an invalid modifier, the
 * program reports an error and no values will be populated. The default
 * constructed  <span style="color:teal">`init_entry_type`</span> is initialized
 * with <span style="color:violet">`Inside`</span>`::NONE` and <span
 * style="color:violet">`InsideTag`</span>`::NONE`, so it is valid but will not
 * generate any initial conditions.
 *
 *
 ...
 * When we are using the default IOType::GNU option, we can load from text
 * files. The checkpoint file uses the following format for text files:
 *
 * ```
 ...
 * index `index` then follows where each row is on a new line. For every
 * additional grid block that is also included in the file, only the index is
 * needed. The index of the next data block will not be separated from the
 * prevoius block by any empty new lines.
 *
 * Now, create the initial conditions parameter and the system can be
 * initialized.
 *
 * ```
 * symphas::init_entry_type tdata{ Inside::FILE, { index,
 * "path/to/file/filename" } }; System<scalar_t, 2> sys(tdata, vdata);
 * ```
 *
...
struct InitialConditionsData {
   ...
   */
  InitialConditionsData(symphas::init_entry_type const& init,
                        symphas::interval_data_type const& vdata,
                        len_type const* dims, size_t id = 0)
...
struct make_new_ic_delegate {
  InitialConditionsData<D>* operator()(symphas::init_entry_type const& init,
                                       symphas::interval_data_type const& vdata,
                                       len_type const* origin,
                                       size_t id) const {
    if (init.in == in) {
      return check_tags_start(init, vdata, origin, id,
                              typename tags_for_init_value<in>::type{});
    } else {
      return nullptr;
    }
  }
 ...
 protected:
  InitialConditionsData<D>* check_tags(symphas::init_entry_type const& init,
                                       symphas::interval_data_type const& vdata,
                                       len_type const* origin,
                                       size_t id) const {
    return new InitialConditionsAlg<D, in>(
        init, vdata, grid::interior_dimensions(vdata), id);
  }

  template <typename... other_tag_types, InsideTag... tags>
  InitialConditionsData<D>* check_tags(symphas::init_entry_type const& init,
                                       symphas::interval_data_type const& vdata,
                                       len_type const* origin, size_t id,
                                       init_tag_values_list<tags...>,
                                       other_tag_types... other_tags) const {
    size_t check_tag = build_intag(tags...);
    if (init.intag == check_tag) {
      return new InitialConditionsAlg<D, in, tags...>(
          init, vdata, grid::interior_dimensions(vdata), id);
    } else {
      return check_tags(init, vdata, origin, id, other_tags...);
    }
  }
  ...
  InitialConditionsData<D>* check_tags_start(
      ...
      symphas::interval_data_type const& vdata, len_type const* origin,
      size_t id, symphas::lib::types_list<tag_tuple_types...>) const {
    return check_tags(init, vdata, origin, id, tag_tuple_types{}...);
  }
};
}  // namespace symphas::internal
 ...
 * The square will have \ref params::init_inside_val "interior" values assigned
 *for the points:
 * \f{gather*}
 ...
 */
template <size_t D>
struct InitialConditionsAlg<D, Inside::SQUARE> : InitialConditionsData<D> {
...
struct InitialConditionsAlg<D, Inside::SQUARE, InsideTag::RANDOM>
  ...
  InitialConditionsAlg(symphas::init_entry_type const& init,
                       symphas::interval_data_type const& vdata,
                       len_type const* dims, size_t id)
      : parent_type(init, vdata, dims, id) {
    offsets = symphas::internal::RandomDeltas<D>(1, dims, IC_SQUARE_RND_FACTOR);
  }

  scalar_t operator[](iter_type) const override;

  symphas::internal::RandomDeltas<D>
      offsets;  //!< Manages a list of random offsets.
};
...
//! Generate a square randomly offset from the center.
/*!
 * See InitialConditionsAlg<D, Inside::SQUARE, InsideTag::VARA>. The offset is
 * chosen by applying #IC_SQUARE_RND_FACTOR to RandomOffsets.
 */
template <size_t D>
struct InitialConditionsAlg<D, Inside::SQUARE, InsideTag::RANDOM,
                            InsideTag::VARA>
    : InitialConditionsAlg<D, Inside::SQUARE, InsideTag::VARA,
                           InsideTag::RANDOM> {
  using parent_type = InitialConditionsAlg<D, Inside::SQUARE, InsideTag::VARA,
                                           InsideTag::RANDOM>;
  using parent_type::parent_type;
};
 ...
 *
 * - In 2D, two parameters are given: \f$r_x, r_y\f$.
 * The circle will have \ref params::init_inside_val "interior" values assigned
 *for the points:
 * \f{gather*}
 ...
 */
template <size_t D>
struct InitialConditionsAlg<D, Inside::CIRCLE> : InitialConditionsData<D> {
  using parent_type = InitialConditionsData<D>;
  using parent_type::parent_type;
  scalar_t operator[](iter_type) const override;
};
...
struct InitialConditionsAlg<D, Inside::CIRCLE, InsideTag::RANDOM,
                            InsideTag::VARA>
    : InitialConditionsAlg<D, Inside::CIRCLE, InsideTag::VARA,
                           InsideTag::RANDOM> {
  using parent_type = InitialConditionsAlg<D, Inside::CIRCLE, InsideTag::VARA,
                                           InsideTag::RANDOM>;
  using parent_type::parent_type;
};
 ...
 * \f[
 *		\left[x_0 + (n-1)\frac{\hat{x}}{c_x}, x_0 +
 *n\frac{\hat{x}}{c_x}\right) \bigcup
 *		\left[y_0 + (m-1)\frac{\hat{y}}{c_y}, y_0 +
 *m\frac{\hat{y}}{c_y}\right).
 * \f]
...
template <size_t D>
struct InitialConditionsAlg<D, Inside::CUBIC> : InitialConditionsData<D> {
  using parent_type = InitialConditionsData<D>;
  using parent_type::parent_type;
  scalar_t operator[](iter_type) const override;
};
...
struct InitialConditionsAlg<D, Inside::CUBIC, InsideTag::RANDOM>
  ...
  InitialConditionsAlg(symphas::init_entry_type const& init,
    ...
    offsets = symphas::internal::RandomDeltas<D>(prod, local_dims,
                                                 IC_CUBIC_RND_FACTOR);
  }
 ...
 * the InitialConditionsAlg<D, Inside::CIRCLE> function, with a different
 *radius.
 */
template <size_t D>
...
struct symphas::internal::tags_for_init_value<Inside::CUBIC> {
  using type =
      symphas::lib::types_list<init_tag_values_list<InsideTag::VARA>,
                               init_tag_values_list<InsideTag::RANDOM>>;
};
 ...
 */
template <size_t D>
struct InitialConditionsAlg<D, Inside::HEXAGONAL> : InitialConditionsData<D> {
  using parent_type = InitialConditionsData<D>;
  using parent_type::parent_type;
  scalar_t operator[](iter_type) const override;
};
...
struct InitialConditionsAlg<D, Inside::HEXAGONAL, InsideTag::RANDOM>
  ...
  InitialConditionsAlg(symphas::init_entry_type const& init,
    ...
    offsets =
        symphas::internal::RandomDeltas<D>(prod, local_dims, IC_HEX_RND_FACTOR);
  }
...
struct symphas::internal::tags_for_init_value<Inside::HEXAGONAL> {
  using type =
      symphas::lib::types_list<init_tag_values_list<InsideTag::RANDOM>>;
};
 ...
 */
template <size_t D>
struct InitialConditionsAlg<D, Inside::SEEDSSQUARE> : InitialConditionsData<D> {
  using parent_type = InitialConditionsData<D>;
  using parent_type::parent_type;
  ...
  symphas::internal::RandomDeltas<D>
      offsets;  //!< Manages a list of random offsets.
  symphas::internal::RandomOffsets<len_type, D>
      lengths;  //!< Manages a list of random sizes.
};

//! Randomly places squares which are randomly sized throughout the system.
/*!
 * This algorithm modifies InitialConditionsAlg<D, Inside::SEEDSSQUARE> by
 * adding randomness to the value generated inside each seed.
 *
...
struct InitialConditionsAlg<D, Inside::SEEDSSQUARE, InsideTag::RANDOM>
  ...
  InitialConditionsAlg(symphas::init_entry_type const& init,
    ...
    size_t n = static_cast<size_t>(init.data.gp[0]);
    scalar_t in_value =
        (symphas::internal::tag_bit_compare(init.intag, InsideTag::INVERT)
             ? init.data.gp[3]
             : init.data.gp[2]);
    scalar_t value_rng[] = {in_value * (1 - IC_SEED_RND_FACTOR)};

    values = symphas::internal::RandomOffsets<scalar_t, 1>(n, value_rng,
                                                           IC_SEED_RND_FACTOR);
  }

  scalar_t operator[](iter_type) const override;

  symphas::internal::RandomOffsets<scalar_t, 1>
      values;  //!< Manages a list of random values.
};
 ...
 *
 * Instead of using a constant \f$\mathcal{S}\f$ (From InitialConditionsAlg<D,
 * Inside::SEEDSSQUARE>), the algorithm chooses a random scaling factors for
 * each side derived from the corresponding system axis length. Without loss of
...
//! Randomly places squares which are randomly sized throughout the system.
/*!
 * This algorithm combines InitialConditionsAlg<D, Inside::SEEDSSQUARE,
 * InsideTag::VARA> and InitialConditionsAlg<D, Inside::SEEDSSQUARE,
 * InsideTag::RANDOM>.
 */
template <size_t D>
struct InitialConditionsAlg<D, Inside::SEEDSSQUARE, InsideTag::VARA,
...
struct InitialConditionsAlg<D, Inside::SEEDSSQUARE, InsideTag::RANDOM,
                            InsideTag::VARA>
    : InitialConditionsAlg<D, Inside::SEEDSSQUARE, InsideTag::VARA,
                           InsideTag::RANDOM> {
  using parent_type = InitialConditionsAlg<D, Inside::SEEDSSQUARE,
                                           InsideTag::VARA, InsideTag::RANDOM>;
  using parent_type::parent_type;
};
...
/*!
 * This algorithm combines InitialConditionsAlg<D, Inside::SEEDSSQUARE,
 * InsideTag::VARB> and InitialConditionsAlg<D, Inside::SEEDSSQUARE,
 * InsideTag::RANDOM>.
 */
template <size_t D>
struct InitialConditionsAlg<D, Inside::SEEDSSQUARE, InsideTag::VARB,
...
struct InitialConditionsAlg<D, Inside::SEEDSSQUARE, InsideTag::RANDOM,
                            InsideTag::VARB>
    : InitialConditionsAlg<D, Inside::SEEDSSQUARE, InsideTag::VARB,
                           InsideTag::RANDOM> {
  using parent_type = InitialConditionsAlg<D, Inside::SEEDSSQUARE,
                                           InsideTag::VARB, InsideTag::RANDOM>;
  using parent_type::parent_type;
};
...
struct InitialConditionsAlg<D, Inside::SEEDSSQUARE, InsideTag::VARC>
  ...
  InitialConditionsAlg(symphas::init_entry_type const& init,
      ...
      : parent_type(init, vdata, dims, id),
        ...
        seed_value_dis{(init.data.gp[2] < init.data.gp[3]) ? init.data.gp[2]
                                                           : init.data.gp[3],
                       (init.data.gp[3] > init.data.gp[2]) ? init.data.gp[3]
                                                           : init.data.gp[2]} {}
...
/*!
 * This algorithm combines InitialConditionsAlg<D, Inside::SEEDSSQUARE,
 * InsideTag::VARC> and InitialConditionsAlg<D, Inside::SEEDSSQUARE,
 * InsideTag::RANDOM>.
 */
template <size_t D>
struct InitialConditionsAlg<D, Inside::SEEDSSQUARE, InsideTag::VARC,
  ...
  InitialConditionsAlg(symphas::init_entry_type const& tdata,
                       symphas::interval_data_type const& vdata,
                       len_type const* dims, size_t id)
      : parent_type(tdata, vdata, dims, id),
        gen{std::random_device{}()},
        seed_value_dis{(tdata.data.gp[2] < tdata.data.gp[3]) ? tdata.data.gp[2]
                                                             : tdata.data.gp[3],
                       (tdata.data.gp[3] > tdata.data.gp[2])
                           ? tdata.data.gp[3]
                           : tdata.data.gp[2]} {}
...
struct InitialConditionsAlg<D, Inside::SEEDSSQUARE, InsideTag::RANDOM,
    ...
    : InitialConditionsAlg<D, Inside::SEEDSSQUARE, InsideTag::VARC,
                           InsideTag::RANDOM> {
  using parent_type = InitialConditionsAlg<D, Inside::SEEDSSQUARE,
                                           InsideTag::VARC, InsideTag::RANDOM>;
  using parent_type::parent_type;
};
...
/*!
 * This algorithm modifies InitialConditionsAlg<D, Inside::SEEDSSQUARE,
 * InsideTag::VARA> by populating the inside of the seed with random values from
 * the uniform distribution.
 *
 * Instead of using a constant \f$\mathcal{S}\f$ (From InitialConditionsAlg<D,
 * Inside::SEEDSSQUARE>), the algorithm chooses a random scaling factors for
 * each radius derived from the corresponding system axis length. Without loss
 * of generality, let \f$\hat{\mathcal{S}}_x\f$ be the scaling factor for the
 * \f$x\f$-axis; it is taken from the uniform distribution:
 * \f[ \mathcal{X} \sim \mathcal{D}(\mathcal{S} - \delta_x,
 * \mathcal{S} + \delta_x)\f].
 * Here, \f$\delta_x = \eta\hat{x}\f$, with value of \f$\eta\f$ given by
...
template <size_t D>
struct InitialConditionsAlg<D, Inside::SEEDSSQUARE, InsideTag::VARA,
                            InsideTag::RANDOM, InsideTag::VARC>
    : InitialConditionsAlg<D, Inside::SEEDSSQUARE, InsideTag::VARC,
                           InsideTag::VARA, InsideTag::RANDOM> {
  using parent_type =
      InitialConditionsAlg<D, Inside::SEEDSSQUARE, InsideTag::VARC,
...
template <size_t D>
struct InitialConditionsAlg<D, Inside::SEEDSSQUARE, InsideTag::VARC,
                            InsideTag::RANDOM, InsideTag::VARA>
    : InitialConditionsAlg<D, Inside::SEEDSSQUARE, InsideTag::VARC,
                           InsideTag::VARA, InsideTag::RANDOM> {
  using parent_type =
      InitialConditionsAlg<D, Inside::SEEDSSQUARE, InsideTag::VARC,
...
/*!
 * This algorithm modifies InitialConditionsAlg<D, Inside::SEEDSSQUARE,
 * InsideTag::VARB> by populating the inside of the seed with random values from
 * the uniform distribution.
 *
...
template <size_t D>
struct InitialConditionsAlg<D, Inside::SEEDSSQUARE, InsideTag::VARC,
                            InsideTag::RANDOM, InsideTag::VARB>
    : InitialConditionsAlg<D, Inside::SEEDSSQUARE, InsideTag::VARC,
                           InsideTag::VARB, InsideTag::RANDOM> {
  using parent_type =
      InitialConditionsAlg<D, Inside::SEEDSSQUARE, InsideTag::VARC,
...
//! Randomly places circles which are randomly sized throughout the system.
/*!
 * This algorithm modifies InitialConditionsAlg<D, Inside::SEEDSCIRCLE> by
 * adding randomness to the value generated inside each seed.
 *
 * The value of each seed is chosen from the uniform distribution:
 * \f[\mathcal{X} \sim \mathcal{D}(C - 2C\delta, C) \f]
 ...
 *
 * Instead of using a constant \f$\mathcal{S}\f$ (From InitialConditionsAlg<D,
 * Inside::SEEDSCIRCLE>), the algorithm chooses a random scaling factors for
 * each radius derived from the corresponding system axis length. Without loss
 * of generality, let \f$\hat{\mathcal{S}}_x\f$ be the scaling factor for the
 * \f$x\f$-axis; it is taken from the uniform distribution:
 * \f[ \mathcal{X} \sim \mathcal{D}(\mathcal{S} - \delta_x,
...
//! Randomly places circles which are randomly sized throughout the system.
/*!
 * This algorithm combines InitialConditionsAlg<D, Inside::SEEDSCIRCLE,
 * InsideTag::VARA> and InitialConditionsAlg<D, Inside::SEEDSCIRCLE,
 * InsideTag::RANDOM>.
 */
template <size_t D>
struct InitialConditionsAlg<D, Inside::SEEDSCIRCLE, InsideTag::VARA,
...
struct InitialConditionsAlg<D, Inside::SEEDSCIRCLE, InsideTag::RANDOM,
                            InsideTag::VARA>
    : InitialConditionsAlg<D, Inside::SEEDSCIRCLE, InsideTag::VARA,
                           InsideTag::RANDOM> {
  using parent_type = InitialConditionsAlg<D, Inside::SEEDSCIRCLE,
                                           InsideTag::VARA, InsideTag::RANDOM>;
  using parent_type::parent_type;
...
/*!
 * This algorithm combines InitialConditionsAlg<D, Inside::SEEDSCIRCLE,
 * InsideTag::VARB> and InitialConditionsAlg<D, Inside::SEEDSCIRCLE,
 * InsideTag::RANDOM>.
 */
template <size_t D>
struct InitialConditionsAlg<D, Inside::SEEDSCIRCLE, InsideTag::VARB,
...
struct InitialConditionsAlg<D, Inside::SEEDSCIRCLE, InsideTag::RANDOM,
                            InsideTag::VARB>
    : InitialConditionsAlg<D, Inside::SEEDSCIRCLE, InsideTag::VARB,
                           InsideTag::RANDOM> {
  using parent_type = InitialConditionsAlg<D, Inside::SEEDSCIRCLE,
                                           InsideTag::VARB, InsideTag::RANDOM>;
  using parent_type::parent_type;
...
/*!
 * This algorithm combines InitialConditionsAlg<D, Inside::SEEDSCIRCLE,
 * InsideTag::VARC> and InitialConditionsAlg<D, Inside::SEEDSCIRCLE,
 * InsideTag::RANDOM>.
 */
template <size_t D>
struct InitialConditionsAlg<D, Inside::SEEDSCIRCLE, InsideTag::VARC,
 ...
 */
template <size_t D>
struct InitialConditionsAlg<D, Inside::VORONOI> : InitialConditionsData<D> {
  using parent_type = InitialConditionsData<D>;
  ...
  InitialConditionsAlg(symphas::init_entry_type const& init,
    ...
    values = symphas::internal::RandomOffsets<scalar_t, 1>(
        N, value_rng, (init.data.gp[2] - init.data.gp[1]) / 2);
  }
...
/*!
 * Fill the bubble with N points, where N is the first parameter of the initial
 * conditions. The second and third parameters are the interior and exterior
 * values of the bubbles. The fourth parameter is the radius of the bubbles,
 * computed based on the area of placing the all the bubbles. Typically, setting
 * this value to 1 will mean that the desired number of bubbles cannot be
 * placed. The fifth parameter is the packing ratio, which represents how
 * tightly together the bubbles should be placed. When this value is zero, there
 * is no overlap between bubbles. For packing ratios close to zero, the final
 * number of bubbles may not be reached, but the algorithm will attempt to place
 * as many bubbles as possible.
 */
template <size_t D>
struct InitialConditionsAlg<D, Inside::BUBBLE> : InitialConditionsData<D> {
  using parent_type = InitialConditionsData<D>;
  ...
  InitialConditionsAlg(symphas::init_entry_type const& init,
    ...
    // For high packing ratios, the overlap will be positive because bubble can
    // be generated on top of each other. For small packing ratios, the overlap
    // will be ngative to give each bubble more space.
    scalar_t overlap = symphas::internal::compute_bubble_overlap(
        init.data.gp[3], R, init.data.gp[4]);

    // The change in overlap varies from .2R to 1R as overlap itself goes from
    // .2R to -2R, after which it is always R
    double overlap_eps = symphas::internal::compute_bubble_overlap_range(
        init.data.gp[3], R, init.data.gp[4]);

    // when overlap is negative, then the episolon needs to be added in order
    // for the offsets to be computed correctly.
    symphas::internal::RandomOffsets<scalar_t, 1> overlaps(N, overlap,
                                                           overlap_eps);
    symphas::internal::RandomOffsets<iter_type, D> start(1, dims);

    offsets = symphas::internal::get_bubble_positions<D>(
        N, R, overlap + overlap_eps, overlaps, dims, start.get_offset(0));
    offsets.sort();
...
//! Fills the system with circles in a hexagonal pattern, one per field and
//! spirally ordered.
/*!
 * Each field using this initial condition will have a circle generated in it
 * corresponding to a hexagonal arrangement, but the arrangement will be ordered
 * so that the fields correspond to points in the hexagonal pattern going in a
 * spiral from the center. The first parameter is the number of hex points to
 * generate, and the number of tiles will always be rounded up to accommodate
 * odd values so that each field can be populated. The next two values ar the
 * outer and interior values of the bubble.
 */
template <size_t D>
struct InitialConditionsAlg<D, Inside::SPIRALHEX> : InitialConditionsData<D> {
  using parent_type = InitialConditionsData<D>;
  ...
  InitialConditionsAlg(symphas::init_entry_type const& init,
    ...
    // double value_rng[] = { (init.data.gp[1] + init.data.gp[2]) / 2 };
    // values = symphas::internal::RandomOffsets<scalar_t, 1>(N, value_rng,
    // (init.data.gp[2] - init.data.gp[1]) / 2);
  }
  ...
  symphas::internal::RandomDeltas<D>
      offsets;  //!< Manages a list of random offsets.
  double tile_dims[D];
  // symphas::internal::RandomOffsets<scalar_t, 1> values;		//!<
  // Manages a list of random values.
};

template <size_t D>
struct InitialConditionsAlg<D, Inside::SIN> : InitialConditionsData<D> {
  using parent_type = InitialConditionsData<D>;
  ...
  scalar_t operator[](iter_type n) const override {
    return symphas::internal::get_function_value(
        n, &symphas::math::sin<scalar_t>, dims, vdata, init.data.gp);
  }
};
...
struct InitialConditionsAlg<D, Inside::SIN, InsideTag::VARA>
  ...
  scalar_t operator[](iter_type n) const override {
    return symphas::internal::get_function_value_A(
        n, &symphas::math::sin<scalar_t>, dims, vdata, init.data.gp);
  }
};
...
struct symphas::internal::tags_for_init_value<Inside::SIN> {
  using type = symphas::lib::types_list<init_tag_values_list<InsideTag::VARA>>;
};

template <size_t D>
struct InitialConditionsAlg<D, Inside::COS> : InitialConditionsData<D> {
  using parent_type = InitialConditionsData<D>;
  using parent_type::dims;
  using parent_type::init;
  using parent_type::parent_type;
  ...
  scalar_t operator[](iter_type n) const override {
    return symphas::internal::get_function_value(
        n, &symphas::math::cos<scalar_t>, dims, vdata, init.data.gp);
  }
};
...
/*!
 * If generating multiple fields, but the voronoi crystal should have the exact
 * same parameters, then this variation will always return the same values.
 */
template <size_t D>
struct InitialConditionsAlg<D, Inside::VORONOI, InsideTag::FIXEDSEED>
  ...
  InitialConditionsAlg(symphas::init_entry_type const& init,
                       ...
                       len_type const* dims, size_t id)
      : parent_type(init, vdata, dims, id),
        voronoi{get_voronoi(init, vdata, dims, id)} {}

  scalar_t operator[](iter_type n) const override { return voronoi[n]; }

  auto get_voronoi(symphas::init_entry_type const& init,
                   symphas::interval_data_type const& vdata,
                   len_type const* dims, size_t id) {
    static InitialConditionsAlg<D, Inside::VORONOI> v(init, vdata, dims, id);
    return v;
  }
...
template <size_t D>
struct InitialConditionsAlg<D, Inside::VORONOI, InsideTag::FIXEDSEED,
                            InsideTag::VARA> : InitialConditionsData<D> {
  using parent_type = InitialConditionsData<D>;
  ...
  InitialConditionsAlg(symphas::init_entry_type const& init,
                       ...
                       len_type const* dims, size_t id)
      : parent_type(init, vdata, dims, id),
        voronoi{get_voronoi(init, vdata, dims)} {}
...
struct InitialConditionsAlg<D, Inside::VORONOI, InsideTag::VARA,
                            InsideTag::FIXEDSEED>
    : InitialConditionsAlg<D, Inside::VORONOI, InsideTag::FIXEDSEED,
                           InsideTag::VARA> {
  using parent_type =
      InitialConditionsAlg<D, Inside::VORONOI, InsideTag::FIXEDSEED,
                           InsideTag::VARA>;
...
//! Generates values for a Voronoi Diagram where the crystal always has the same
//! shape.
/*!
 * If generating multiple fields, but the voronoi crystal should have the exact
 * same parameters, then this variation will always return the same crystal, but
 * the values of the crystal will be different.
 */
template <size_t D>
struct InitialConditionsAlg<D, Inside::VORONOI, InsideTag::FIXEDSEED,
                            InsideTag::RANDOM> : InitialConditionsData<D> {
  using parent_type = InitialConditionsData<D>;
  ...
  InitialConditionsAlg(symphas::init_entry_type const& init,
    ...
    double value_rng[] = {(init.data.gp[1] + init.data.gp[2]) / 2};
    values = symphas::internal::RandomOffsets<scalar_t, 1>(
        N, value_rng, (init.data.gp[1] - init.data.gp[2]) / 2);
  }
...
template <size_t D>
struct InitialConditionsAlg<D, Inside::VORONOI, InsideTag::RANDOM,
                            InsideTag::FIXEDSEED>
    : InitialConditionsAlg<D, Inside::VORONOI, InsideTag::FIXEDSEED,
                           InsideTag::RANDOM> {
  using parent_type =
...
template <size_t D>
struct InitialConditionsAlg<D, Inside::VORONOI, InsideTag::RANDOM,
                            InsideTag::FIXEDSEED, InsideTag::VARA>
    : InitialConditionsAlg<D, Inside::VORONOI, InsideTag::FIXEDSEED,
                           InsideTag::RANDOM, InsideTag::VARA> {
  using parent_type =
      InitialConditionsAlg<D, Inside::VORONOI, InsideTag::FIXEDSEED,
...
struct InitialConditionsAlg<D, Inside::VORONOI, InsideTag::FIXEDSEED,
  ...
  auto get_values(symphas::init_entry_type const& init, size_t id) const {
    double value_rng[] = {(init.data.gp[1] + init.data.gp[2]) / 2};
    static symphas::internal::RandomOffsets<scalar_t, 1> values0(
        N, value_rng, (init.data.gp[1] - init.data.gp[2]) / 2);

    double value_rng0[] = {init.data.gp[1]};
    symphas::internal::RandomOffsets<scalar_t, 1> values1(N, value_rng0, 0);
    values1.set_offset(id, values0.get_offset(id));
    return values1;
  }
...
template <size_t D>
struct InitialConditionsAlg<D, Inside::VORONOI, InsideTag::VARB,
                            InsideTag::RANDOM, InsideTag::FIXEDSEED>
    : InitialConditionsAlg<D, Inside::VORONOI, InsideTag::FIXEDSEED,
                           InsideTag::RANDOM, InsideTag::VARB> {
  using parent_type =
      InitialConditionsAlg<D, Inside::VORONOI, InsideTag::FIXEDSEED,
...
template <size_t D>
struct InitialConditionsAlg<D, Inside::VORONOI, InsideTag::FIXEDSEED,
                            InsideTag::RANDOM, InsideTag::VARA, InsideTag::VARB>
    : InitialConditionsAlg<D, Inside::VORONOI, InsideTag::FIXEDSEED,
                           InsideTag::RANDOM, InsideTag::VARB> {
  using parent_type =
      InitialConditionsAlg<D, Inside::VORONOI, InsideTag::FIXEDSEED,
...
template <size_t D>
struct InitialConditionsAlg<D, Inside::VORONOI, InsideTag::FIXEDSEED,
                            InsideTag::RANDOM, InsideTag::VARB, InsideTag::VARA>
    : InitialConditionsAlg<D, Inside::VORONOI, InsideTag::FIXEDSEED,
                           InsideTag::RANDOM, InsideTag::VARB> {
  using parent_type =
      InitialConditionsAlg<D, Inside::VORONOI, InsideTag::FIXEDSEED,
...
template <size_t D>
struct InitialConditionsAlg<D, Inside::VORONOI, InsideTag::RANDOM,
                            InsideTag::FIXEDSEED, InsideTag::VARB,
                            InsideTag::VARA>
    : InitialConditionsAlg<D, Inside::VORONOI, InsideTag::FIXEDSEED,
                           InsideTag::RANDOM, InsideTag::VARB> {
  using parent_type =
...
struct InitialConditionsAlg<D, Inside::VORONOI, InsideTag::FIXEDSEED,
  ...
  auto get_values(symphas::init_entry_type const& init, size_t id) const {
    double value_rng0[] = {init.data.gp[1]};
    symphas::internal::RandomOffsets<scalar_t, 1> values1(N, value_rng0, 0);
    values1.set_offset(id, init.data.gp[2]);
...
template <size_t D>
struct InitialConditionsAlg<D, Inside::VORONOI, InsideTag::RANDOM,
                            InsideTag::FIXEDSEED, InsideTag::VARC>
    : InitialConditionsAlg<D, Inside::VORONOI, InsideTag::FIXEDSEED,
                           InsideTag::RANDOM, InsideTag::VARC> {
  using parent_type =
      InitialConditionsAlg<D, Inside::VORONOI, InsideTag::FIXEDSEED,
...
template <size_t D>
struct InitialConditionsAlg<D, Inside::VORONOI, InsideTag::FIXEDSEED,
                            InsideTag::RANDOM, InsideTag::VARA, InsideTag::VARC>
    : InitialConditionsAlg<D, Inside::VORONOI, InsideTag::FIXEDSEED,
                           InsideTag::RANDOM, InsideTag::VARC> {
  using parent_type =
      InitialConditionsAlg<D, Inside::VORONOI, InsideTag::FIXEDSEED,
...
//! Fills the system with equally sized semi-overlapping bubbles.
/*!
 * If generating multiple fields, but the bubble arrangement should have the
 * exact same parameters, then this variation will always return the same
 * values.
 */
template <size_t D>
struct InitialConditionsAlg<D, Inside::BUBBLE, InsideTag::FIXEDSEED>
  ...
  InitialConditionsAlg(symphas::init_entry_type const& init,
                       ...
                       len_type const* dims, size_t id)
      : parent_type(init, vdata, dims, id),
        bubble{get_bubble(init, vdata, dims, id)} {}

  scalar_t operator[](iter_type n) const override { return bubble[n]; }

  auto get_bubble(symphas::init_entry_type const& init,
                  symphas::interval_data_type const& vdata,
                  len_type const* dims, size_t id) {
    static InitialConditionsAlg<D, Inside::BUBBLE> v(init, vdata, dims, id);
    return v;
  }
...
struct InitialConditionsAlg<D, Inside::BUBBLE, InsideTag::FIXEDSEED,
  ...
  InitialConditionsAlg(symphas::init_entry_type const& init,
                       ...
                       len_type const* dims, size_t id)
      : parent_type(init, vdata, dims, id),
        bubble{get_bubble(init, vdata, dims)} {}
...
struct InitialConditionsAlg<D, Inside::BUBBLE, InsideTag::VARA,
                            InsideTag::FIXEDSEED>
    : InitialConditionsAlg<D, Inside::BUBBLE, InsideTag::FIXEDSEED,
                           InsideTag::VARA> {
  using parent_type =
      InitialConditionsAlg<D, Inside::BUBBLE, InsideTag::FIXEDSEED,
                           InsideTag::VARA>;
...
/*!
 * Fill the bubble with N points, where N is the first parameter of the initial
 * conditions. The thid parameter of the initial conditions is the packing
 * ratio. For high packing ratios, the value may not be reached, but the
 * algorithm will attempt to place as many bubbles as possible.
 */
template <size_t D>
struct InitialConditionsAlg<D, Inside::BUBBLE, InsideTag::FIXEDSEED,
                            InsideTag::RANDOM> : InitialConditionsData<D> {
  ...
  InitialConditionsAlg(symphas::init_entry_type const& init,
    ...
    // when overlap is negative, then the episolon needs to be added in order
    // for the offsets to be computed correctly.
    static symphas::internal::RandomOffsets<scalar_t, 1> overlaps(N, overlap,
                                                                  overlap_eps);
    static symphas::internal::RandomOffsets<iter_type, D> start(1, dims);
    static auto offsets0 = symphas::internal::get_bubble_positions<D>(
        N, R, overlap + overlap_eps, overlaps, dims, start.get_offset(0));
...
template <size_t D>
struct InitialConditionsAlg<D, Inside::BUBBLE, InsideTag::RANDOM,
                            InsideTag::FIXEDSEED>
    : InitialConditionsAlg<D, Inside::BUBBLE, InsideTag::FIXEDSEED,
                           InsideTag::RANDOM> {
  using parent_type =
 ...
 * If generating multiple fields, but the bubble arrangement should have the
 * exact same parameters, then this variation will always return the same
 * arrangement, but the values of the arrangement will be different.
 *
 * This variation will apply periodic boundaries when determining the closest
 * point.
 */
...
//! Generates values for a bubble arrangement where the arrangement always has
//! the same shape.
/*!
 * If generating multiple fields, but the bubble arrangement should have the
 * exact same parameters, then this variation will always return the same
...
struct InitialConditionsAlg<D, Inside::BUBBLE, InsideTag::FIXEDSEED,
  ...
  auto get_values(symphas::init_entry_type const& init, size_t id) const {
    double value_rng[] = {(init.data.gp[1] + init.data.gp[2]) / 2};
    static auto values0 = symphas::internal::RandomOffsets<scalar_t, 1>(
        N, value_rng, (init.data.gp[2] - init.data.gp[1]) / 2);
...
template <size_t D>
struct InitialConditionsAlg<D, Inside::BUBBLE, InsideTag::VARB,
                            InsideTag::RANDOM, InsideTag::FIXEDSEED>
    : InitialConditionsAlg<D, Inside::BUBBLE, InsideTag::FIXEDSEED,
                           InsideTag::RANDOM, InsideTag::VARB> {
  using parent_type =
...
struct InitialConditionsAlg<D, Inside::BUBBLE, InsideTag::FIXEDSEED,
  ...
  InitialConditionsAlg(symphas::init_entry_type const& init,
    ...
    static auto offsets0 = symphas::internal::get_bubble_positions<D>(
        N, R, overlap + overlap_eps, overlaps, dims, start);
  ...
  auto get_select(size_t id) { return int(id % N); }
...
template <size_t D>
struct InitialConditionsAlg<D, Inside::GAUSSIAN> : InitialConditionsData<D> {
  using parent_type = InitialConditionsData<D>;
  using parent_type::init;
  ...
  scalar_t operator[](iter_type) const override { return dis(gen); }
...
template <size_t D>
struct InitialConditionsAlg<D, Inside::UNIFORM> : InitialConditionsData<D> {
  using parent_type = InitialConditionsData<D>;
  using parent_type::init;
...
template <size_t D>
struct InitialConditionsAlg<D, Inside::CONSTANT> : InitialConditionsData<D> {
  using parent_type = InitialConditionsData<D>;
  using parent_type::init;
  using parent_type::parent_type;

  scalar_t operator[](iter_type) const override { return init.data.gp[0]; }
...
struct InitialConditionsAlg<D, Inside::CAPPED>
  ...
  scalar_t operator[](iter_type n) const override {
    return (parent_type::operator[](n) < mean) ? init.data.gp[0]
                                               : init.data.gp[1];
  }
...
struct InitialConditionsAlg<D, Inside::NONE, InsideTag::NONE>
  ...
  scalar_t operator[](iter_type n) const override { return 0; }
...
struct InitialConditions {
  InitialConditions(symphas::init_data_type const& tdata,
                    symphas::interval_data_type const& vdata,
                    len_type const* origin, size_t id)
      : data{get_data(tdata, vdata, origin, id)}, id{id} {}
  ...
  auto end(Axis ax, T* values, grid::region_interval<D> const& interval) const {
    return symphas::internal::ic_iterator<T, D>(
        ax, values, *(data.at(ax)), interval, grid::length<D>(interval));
  }

  symphas::grid_info initialize(
      T* values, grid::region_interval<D> const& interval) const {
    symphas::grid_info ginfo(nullptr, 0);
    bool initialized = true;
    if (data.find(Axis::NONE) != data.end()) {
      ginfo = initialize(Axis::NONE, values, interval);
      initialized = symphas::is_valid(ginfo);
    }

    for (auto const& [key, entry] : data) {
      if (key != Axis::NONE) {
        ginfo = initialize(key, values, interval);
  ...
  symphas::grid_info initialize(
      Axis ax, T* values, grid::region_interval<D> const& interval) const {
    // symphas::grid_info interval_ginfo(interval.intervals,
    // symphas::grid_info(data.at(ax)->vdata).get_widths());
    auto* ax_data = data.at(ax);
    symphas::grid_info interval_ginfo(ax_data->vdata);

    // If the initial conditions are not set, then nothing is done.
    if (data.at(ax)->init.in == Inside::NONE) {
      if (symphas::internal::tag_bit_compare(ax_data->init.intag,
                                             InsideTag::NONE)) {
        return interval_ginfo;
      } else if (symphas::internal::tag_bit_compare(ax_data->init.intag,
                                                    InsideTag::DEFAULT)) {
        if (!symphas::internal::tag_bit_compare(ax_data->init.intag,
                                                InsideTag::NONE)) {
          ax_data->init.f_init->initialize(values, interval);
        }
        return interval_ginfo;
      } else {
...
#ifdef USING_IO
    else if (ax_data->init.in == Inside::FILE ||
             ax_data->init.in == Inside::CHECKPOINT) {
      symphas::io::read_info rinfo{ax_data->init.file.get_index(), id,
                                   ax_data->init.file.get_name(),
                                   ax_data->init.in == Inside::CHECKPOINT};
      ...
      for (iter_type n = 0; n < D; ++n) {
        ...
        if (file_dims[n] != interval.dims[n]) {
          ...
          fprintf(
              SYMPHAS_WARN,
              "%c-dimension read from file header (%d) is inconsistent with "
      ...
      if (data.at(ax)->init.file.get_index() != read_index) {
        fprintf(SYMPHAS_ERR,
                "system initialization requires the loaded datafile to contain "
                "the given index '%d'\n",
      ...
      bool filled = match_init_expr<D>(
          data.at(ax)->init.expr_data.get_name(), ax, values, interval,
          data.at(ax)->vdata, data.at(ax)->init.expr_data.get_coeff(),
          data.at(ax)->init.expr_data.get_num_coeff());
 ...
 protected:
  ...
  auto get_data(symphas::init_data_type const& tdata,
                symphas::interval_data_type const& vdata,
                len_type const* origin, size_t id) {
    ...
    for (auto& [key, entry] : tdata) {
      ...
      if (entry.in == Inside::NONE) {
        init = next_ic(entry, vdata, origin, id,
                       symphas::internal::init_values_list<>{});
      } else {
        init = next_ic(
            entry, vdata, origin, id,
            symphas::internal::init_values_list<ALL_INSIDE_GEN_VALUES>{});
      }
      data0[key] = init;
    }
    return data0;
  ...
  InitialConditionsData<D>* next_ic(symphas::init_entry_type const& init,
                                    symphas::interval_data_type const& vdata,
                                    len_type const* origin, size_t id,
                                    symphas::internal::init_values_list<>) {
    return new InitialConditionsAlg<D, Inside::NONE, InsideTag::NONE>(
        init, vdata, grid::interior_dimensions(vdata), id);
  }
  ...
  InitialConditionsData<D>* next_ic(
      symphas::init_entry_type const& init,
      symphas::interval_data_type const& vdata, len_type const* origin,
      size_t id, symphas::internal::init_values_list<in0, ins...>) {
    InitialConditionsData<D>* ic = icd_type<in0>{}(init, vdata, origin, id);
    if (!ic) {
      return next_ic(init, vdata, origin, id,
                     symphas::internal::init_values_list<ins...>{});
    } else {
      return ic;
    }
  }

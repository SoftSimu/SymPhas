...
/*!
 * Sets the factor by which the minimum iterable area found for a field is
 * expanded to become the regional grid.
 */
DLLSOL extern double regional_resize_factor;

//! Sets the resize time for regional grids.
/*!
 * Sets the time increment at which the regional grid dimensions and position is
 * reevalauted.
 */
...
/*!
 * Defines that the original shape, meaning the precise dimensions of the grid,
 * which is determined from the initial state of the field, will remain fixed
 * throughout the simulation. The position will still be reevaluated at the time
 * interval provided by regional_resize_time.
 */
DLLSOL extern bool regional_resize_is_fixed;

//! This value is added to the minimum value to get the cutoff.
/*!
 * The regional grid area is determined by evaluating which elements exceed the
 * cutoff. The cutoff is determined by adding this value to the minimum value of
 * the field.
 */
...
struct PhaseFieldSystem<BoundaryGrid, T, D>
    : PersistentSystemData<BoundaryGrid<T, D>>, BoundaryGroup<T, D> {
...
template <typename T, size_t D>
void PhaseFieldSystem<BoundaryGrid, T, D>::update(iter_type index,
                                                  double time) {
...
struct regional_system_info_type {
  ...
  regional_system_info_type()
      : next_resize{0},
        resize_delta{REGIONAL_GRID_RESIZE_TIME},
        fixed_resize{REGIONAL_GRID_RESIZE_IS_FIXED},
        cutoff{} {}
  regional_system_info_type(iter_type next_resize, double resize_delta,
                            bool fixed_resize, T cutoff)
      : next_resize{next_resize},
 ...
 */
template <typename T, size_t D>
struct PhaseFieldSystem<RegionalGrid, T, D>
    : PersistentSystemData<RegionalGrid<T, D>>, BoundaryGroup<T, D> {
...
template <typename T, size_t D>
struct PhaseFieldSystem<RegionalGridMPI, T, D>
    : PersistentSystemData<RegionalGridMPI<T, D>>, BoundaryGroup<T, D> {
...
auto min_value(RegionalGrid<any_vector_t<T, D>, D> const& grid) {
  ...
  auto e = std::min_element(it, it + grid.region.len, [](auto a, auto b) {
    using std::abs;
...
template <typename T>
void set_value_for_resize(T& cutoff, T min, double eps = 0) {
  cutoff = min + eps;
}

template <typename T, size_t D>
void set_value_for_resize(any_vector_t<T, D>& cutoff,
                          any_vector_t<T, D> const& min, double eps = 0) {
...
void set_value_for_resize(any_vector_t<T, D>& value,
                          multi_value<D, T> const& min, double eps = 0) {
  for (iter_type i = 0; i < D; ++i) {
    value[i] = min[i] + eps;
  }
}
...
template <typename T, size_t D>
void PhaseFieldSystem<RegionalGrid, T, D>::update(iter_type index,
                                                  double time) {
...
template <typename T, size_t D>
void PhaseFieldSystem<RegionalGridMPI, T, D>::update(iter_type index,
                                                     double time) {
...
PhaseFieldSystem<BoundaryGrid, T, D>::PhaseFieldSystem(
  ...
  symphas::internal::populate_tdata(tdata, *static_cast<Grid<T, D>*>(this),
                                    &info, region, id);
}
...
PhaseFieldSystem<RegionalGrid, T, D>::PhaseFieldSystem(
    ...
    : parent_type{get_extended_intervals(vdata), id},
      BoundaryGroup<T, D>{info.intervals, bdata},
      regional_info{} {
  grid::region_interval<D> region(RegionalGrid<T, D>::region.dims,
                                  RegionalGrid<T, D>::region.boundary_size);
...
PhaseFieldSystem<RegionalGridMPI, T, D>::PhaseFieldSystem(
    ...
    symphas::interval_data_type const& vdata, symphas::b_data_type const& bdata,
    size_t id)
    : parent_type{get_extended_intervals(vdata), tdata.thr_info},
      BoundaryGroup<T, D>{info.intervals, bdata},
      regional_info{},
      thr_info{tdata.thr_info} {
  if (thr_info.is_in_node()) {
    grid::region_interval<D> region(
        RegionalGridMPI<T, D>::region.dims,
        RegionalGridMPI<T, D>::region.boundary_size);
    symphas::internal::populate_tdata(tdata, *this, &info, region, id);

	...
	PFCTraitMode<PFC<D, Sp>>
{
	...
	void set_parameters()
	{
		...
		{
			symphas::lib::expand_append_list(DEFAULT_COEFF_VALUE, coeff, total_num_coeff - num_coeff, num_coeff);

			for (size_t i = ((num_coeff + 5) - base_num_coeff) / 6; i < num_interactions; ++i)
			{
	...
	auto get_dynamic_N()
	{
		return trait_dynamic::template construct_dynamic<N, PFC<D, Sp>::template dynamic_val<N>()>();
	}
	...
	auto get_mode_NM()
	{
		return trait_mode::template construct_mode<N, M, PFC<D, Sp>::template mode_val<N>()>();
	}
...
protected:
	...
	// equivalent to the indices covered by the sum in the coupling of the dynamical equation
	template<size_t N>
	...
	{
		return ((OpVoid{} + ... + coupled_pfc_dynamic_NM<N, make_arr_N<N>()[Is]>()));
	}

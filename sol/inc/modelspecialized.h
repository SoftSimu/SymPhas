...
template <typename G, typename Sp, typename coeff_t1 = OpIdentity,
          typename coeff_t2 = OpIdentity>
auto landau_fe(Sp const& solver, OpTerm<OpIdentity, G> const& term,
               coeff_t1 const& c1 = coeff_t1{},
               coeff_t2 const& c2 = coeff_t2{}) {
  return -c1 * expr::make_fraction<1, 2>() * expr::pow<2>(term) +
         c2 * expr::make_fraction<1, 4>() * expr::pow<4>(term) +
         OpChain(OpOperatorChain(expr::make_fraction<1, 2>(), OpIdentity{}),
...
template <typename G, typename Sp, typename coeff_t1 = OpIdentity,
          typename coeff_t2 = OpIdentity>
auto doublewell_fe(Sp const& solver, OpTerm<OpIdentity, G> const& term,
                   coeff_t1 const& c1 = coeff_t1{},
                   coeff_t2 const& c2 = coeff_t2{}) {
  return c2 * expr::make_fraction<1, 4>() *
             expr::pow<2>(expr::pow<2>(term) - c1) +
         OpChain(OpOperatorChain(expr::make_fraction<1, 2>(), OpIdentity{}),
                 expr::pow<2>(expr::make_operator_derivative<1>(solver)(term)));
}
...
template <typename G, typename Sp, typename coeff_t1 = OpIdentity,
          typename coeff_t2 = OpIdentity>
auto cellular_fe(Sp const& solver, OpTerm<OpIdentity, G> const& term,
                 coeff_t1 const& c1 = coeff_t1{}) {
  return expr::make_integer<30>() / (c1 * c1) * term * term *
             expr::pow<2>(expr::symbols::one - term) +
...
struct MakeEquationProvisional : parent_model {
 ...
 protected:
  ...
  decltype(auto) forward_systems(std::tuple<Ts1...> const& a,
                                 std::tuple<Ts2...> const& b,
  ...
  decltype(auto) forward_systems(std::tuple<Ts1...> const& a,
                                 std::tuple<Ts2...> const& b) const {
    return forward_systems(a, b, std::make_index_sequence<sizeof...(Ts1)>{},
...
struct ModelApplied {
  ...
  struct OpTypes {
    ...
    template <template <typename> typename Eq,
              typename eq_type =
                  Eq<symphas::internal::MakeEquation<Model<D, Sp, S...>>>>
    struct Specialized;
  ...
  struct ArrayType {
    ...
    template <template <typename> typename Eq,
              typename eq_type =
                  Eq<symphas::internal::MakeEquation<ArrayModel<D, Sp, Ts...>>>>
    struct Specialized;
  };
};
...
template <typename... field_types, size_t Dm, typename Sp>
struct expand_types_to_model<
    symphas::lib::types_list<field_array_t<void>, field_types...>, Dm, Sp> {
  using type =
      typename ModelApplied<Dm, Sp>::template ArrayType<field_types...>;
};
...
template <typename... field_types, size_t Dm, typename Sp, typename T0,
          typename... Ts>
struct expand_types_to_model<symphas::lib::types_list<field_types...>, Dm, Sp,
                             T0, Ts...> {
  using type = typename expand_types_to_model<
      symphas::lib::types_list<field_types..., T0>, Dm, Sp, Ts...>::type;
};
...
struct expand_types_to_model<symphas::lib::types_list<field_types...>, Dm, Sp,
  ...
  using type =
      typename expand_types_to_model<symphas::lib::types_list<field_types...>,
                                     Dm, Sp, Ts..., Rest...>::type;
};
...
struct ModelApplied<D, Sp>::OpTypes<S...>::Specialized : eq_type {
  ...
  using this_type = typename ModelApplied<D, Sp>::template OpTypes<
      S...>::template Specialized<Eq, eq_type>;
  using eqs =
      typename std::invoke_result_t<decltype(&parent_type::make_equations),
                                    parent_type>;
  eqs equations;

  Specialized(double const* coeff, size_t num_coeff,
              symphas::problem_parameters_type const& parameters)
      : parent_type(coeff, num_coeff, parameters),
...
struct ModelApplied<D, Sp>::ArrayType<Ts...>::Specialized : eq_type {
  ...
  using this_type = typename ModelApplied<D, Sp>::template ArrayType<
      Ts...>::template Specialized<Eq, eq_type>;

  using eqs =
      std::invoke_result_t<decltype(&parent_type::make_equations), parent_type>;
  eqs equations;
  ...
  void equation() {
    M::synchronize_systems(equations);
    M::solver.equation(equations, M::len);
...
struct select_dFE_impl<symphas::lib::types_list<dFE_ts...>,
...
struct select_dFE_impl<
    ...
    symphas::lib::types_list<Variable<N, expr::symbols::diff_F_symbol>,
                             Rest...>> {
  using type = typename select_dFE_impl<
      symphas::lib::types_list<dFE_ts...,
...
struct select_dFE_impl<symphas::lib::types_list<dFE_ts...>,
  ...
  using type =
      typename select_dFE_impl<symphas::lib::types_list<dFE_ts...>,
                               symphas::lib::types_list<Rest...>>::type;
};

template <typename List>
using select_dFE_t =
    typename select_dFE_impl<symphas::lib::types_list<>, List>::type;
...
struct select_dFE_i_impl<
    expr::symbols::i_<N, P>, symphas::lib::types_list<dFE_ts...>,
    symphas::lib::types_list<
        expr::symbols::diff_F_i_symbol<expr::symbols::i_<N, P0>>, Rest...>> {
  using type = typename select_dFE_i_impl<
      expr::symbols::i_<N, P>,
      symphas::lib::types_list<
          dFE_ts..., expr::symbols::diff_F_i_symbol<expr::symbols::i_<N, P0>>>,
      symphas::lib::types_list<Rest...>>::type;
};
...
struct select_dFE_i_impl<expr::symbols::i_<N, P>,
                         ...
                         symphas::lib::types_list<T, Rest...>> {
  using type =
      typename select_dFE_i_impl<expr::symbols::i_<N, P>,
                                 symphas::lib::types_list<dFE_ts...>,
                                 symphas::lib::types_list<Rest...>>::type;
};

template <typename I, typename List>
using select_dFE_i_t =
    typename select_dFE_i_impl<I, symphas::lib::types_list<>, List>::type;
...
inline constexpr bool is_index_divisible(OpIdentity) { return true; }

inline constexpr bool is_index_divisible(OpNegIdentity) { return true; }
...
constexpr bool is_index_divisible(symphas::lib::types_list<index_t>,
                                  OpExpression<E> const& e) {
  auto [lhs, rhs] =
      separate_index(index_t{}, *static_cast<E const*>(&e) - expr::val<N>);
...
struct dynamics_index_compare<
    N, expr::symbols::index_eq_N<expr::symbols::i_<N0, P0>, M>> {
  static const bool flag = (N >= M);
};

template <size_t N, int N0, int P0, int M0, int M1>
struct dynamics_index_compare<
    N, expr::symbols::index_NtN<expr::symbols::i_<N0, P0>, M0, M1>> {
...
struct dynamics_rule_compare {
  static const bool value =
      true;  //<! `true` means L rule has greater priority than R.
};
...
template <size_t N, size_t N0, size_t N1, typename I0, typename I1, typename E0,
          typename E1>
struct dynamics_rule_compare<N, special_dynamics<N0, I0, E0>,
                             special_dynamics<N1, I1, E1>> {
  static const bool value = dynamics_index_compare<N, I0>::value;
};

// template<size_t N, size_t N0, typename I0, typename E0, typename E1>
// struct dynamics_rule_compare<N, special_dynamics<N0, I0, E0>,
// special_dynamics<N, I0, E1>>
//{
//	static const bool value = dynamics_index_compare<N, I0>::value;
// };
...
struct dynamics_rule_search_impl<N, P, R, std::index_sequence<>,
                                 symphas::lib::types_list<>> {
  static const size_t value = P;
};

template <size_t N, size_t P, typename R, size_t I0, size_t... Is, typename T,
          typename... Rest>
struct dynamics_rule_search_impl<N, P, R, std::integer_sequence<bool, false>,
                                 std::index_sequence<I0, Is...>,
                                 symphas::lib::types_list<T, Rest...>> {
  static const size_t value =
      dynamics_rule_search_impl<N, I0, T, std::index_sequence<Is...>,
                                symphas::lib::types_list<Rest...>>::value;
};

template <size_t N, size_t P, typename R, size_t I0, size_t... Is, typename T,
          typename... Rest>
struct dynamics_rule_search_impl<N, P, R, std::integer_sequence<bool, true>,
                                 std::index_sequence<I0, Is...>,
                                 symphas::lib::types_list<T, Rest...>> {
  static const size_t value =
      dynamics_rule_search_impl<N, P, R, std::index_sequence<Is...>,
                                symphas::lib::types_list<Rest...>>::value;
};

template <size_t N, size_t P, typename R, size_t I0, size_t... Is, typename T,
          typename... Rest>
struct dynamics_rule_search_impl<N, P, R, std::index_sequence<I0, Is...>,
                                 symphas::lib::types_list<T, Rest...>> {
 protected:
...
constexpr size_t dynamics_rule_N = dynamics_rule_search_impl<
    N, 0, void,
    std::make_index_sequence<symphas::lib::types_list_size<T>::value>,
    T>::value;

template <typename E0, typename F, typename... S>
auto substitute_for_dynamics(symphas::lib::types_list<>, E0 const& dyn0,
                             F const& fe, std::tuple<S...> const& ops) {
  return dyn0;
}
...
auto substitute_for_dynamics(
    ...
    E0 const& dyn0, F const& fe, std::tuple<S...> const& ops) {
  return expr::transform::swap_grid<
      Variable<N, expr::symbols::diff_F_symbol>,
      Variable<Ns, expr::symbols::diff_F_symbol>...>(
      dyn0, expr::make_functional_derivative(fe, std::get<0>(ops)),
      expr::make_functional_derivative(
          fe, std::get<symphas::lib::index_of_value<size_t, Ns, Ns...> + 1>(
                  ops))...);
}
...
auto substitute_for_dynamics(
    symphas::lib::types_list<
        expr::symbols::diff_F_i_symbol<expr::symbols::i_<N, Ps>>...>,
    E0 const& dyn0) {
  return expr::transform::swap_grid<
      expr::symbols::diff_F_i_symbol<expr::symbols::i_<N, Ps>>...>(
      dyn0,
      expr::make_term<NN + size_t(Ps)>(expr::symbols::diff_F_symbol{})...);
}

template <size_t N, int N0, int... Q0s, int... P0s, typename E0,
          typename model_t>
auto substitute_ops(
    symphas::lib::types_list<expr::symbols::i_<N0, Q0s>...>,
    symphas::lib::types_list<
        expr::symbols::v_id_type<expr::symbols::i_<N0, P0s>>...>,
    E0 const& eq, model_t const& model) {
  if constexpr (model_num_parameters<model_t>::value > 0) {
    auto e0 =
        expr::transform::swap_grid<OpCoeffSwap<expr::symbols::i_<N0, Q0s>>...>(
            eq, expr::val<size_t(Q0s) + 1>...);
    return expr::transform::swap_grid<
        expr::symbols::i_<N0, Q0s>...,
        expr::symbols::v_id_type<expr::symbols::i_<N0, P0s>>...>(
        e0, expr::val<size_t(Q0s) + 1>...,
        model.template op<N + size_t(P0s)>()...);
  } else {
    auto e0 =
        expr::transform::swap_grid<OpCoeffSwap<expr::symbols::i_<N0, Q0s>>...>(
            eq, model.index);
    return expr::transform::swap_grid<
        expr::symbols::i_<N0, Q0s>...,
        expr::symbols::v_id_type<expr::symbols::i_<N0, P0s>>...>(
        e0, model.index + expr::val<size_t(Q0s) + 1>...,
        model.template op<N + size_t(P0s)>()...);
  }
}

template <size_t N, typename E0, typename model_t>
auto substitute_ops(symphas::lib::types_list<>, symphas::lib::types_list<>,
                    E0 const& eq, model_t const& model) {
  return eq;
}
...
auto substitute_ops(model_t const& model, E0 const& with_fes) {
  ...
  using i_types =
      select_all_i_<I0, expr::op_types_t<OpAdd<E0, OpTerm<OpIdentity, I0>>>>;
...
struct apply_special_dynamics<
    special_dynamics<N, void, dynamics_key_t<dynamic>>>
  ...
  template <size_t N0>
  auto select() const {
    return apply_special_dynamics<
        special_dynamics<N0, void, dynamics_key_t<dynamic>>>(
        dynamics_key_t<dynamic>{});
  }

  template <typename U_D, typename model_t, typename F, typename Sp>
  auto operator()(U_D const& dop, model_t const& model,
                  OpExpression<F> const& fe, Sp const& solver) {
    ...
    // return
    // apply_dynamics<symphas::internal::DynamicType::NONCONSERVED>{}(dop,
    // evolution, solver);
...
struct apply_special_dynamics<special_dynamics<N, void, E>> {
  ...
  auto operator()(U_D const& dop, model_t const& model,
    ...
    auto evolution =
        substitute_for_dynamics(df_v_types{}, e, *static_cast<F const*>(&fe),
                                all_ops(model, df_v_types{}));
...
auto get_parameterized_values(M const& model0,
                              Model<D, Sp, Ts...> const& model) {
...
template <typename M, size_t D, typename Sp, typename... Ts>
auto get_parameterized_values(M const& model0,
                              ArrayModel<D, Sp, Ts...> const& model) {
  return new parameterized_type<void, void>[sizeof...(Ts)] {
    parameterized_type<Ts>(model0)()...
  };
}
...
struct param_matrix_factory {
  ...
  param_matrix_factory(param_matrix_factory const& other)
      : coeff{(other.num_fields * other.num_coeff > 0)
                  ? new double[other.num_fields * other.num_coeff]
                  : nullptr},
        num_fields{other.num_fields},
        num_coeff{other.num_coeff} {
    std::copy(other.coeff, other.coeff + other.num_fields * other.num_coeff,
              coeff);
  }
 ...
 protected:
  template <typename M, size_t D, typename Sp, typename... Ts>
  double* generate_coeff_matrix(M const& model0,
                                Model<D, Sp, Ts...> const& model,
...
struct TraitEquation : parent_trait {
  ...
  template <size_t I>
  auto op() const {
    return const_cast<TraitEquation<enclosing_type, parent_trait>*>(this)
        ->template _op<I>();
  }
  ...
  auto generate_equations(std::tuple<dynamics_ts...> const& dynamics,
                          OpExpression<E> const& e) const {
    expr::printe(*static_cast<E const*>(&e), "free energy");
    return generate_equations_apply(*static_cast<E const*>(&e), dynamics,
                                    seq_t{});
  }
 ...
 protected:
  ...
  auto _op() {
    return expr::make_term<I>(
        NamedData(parent_trait::template grid<I>(), names_t{}(I)));
  }
 ...
 * MakeEquation.
 */
template <template <typename> typename enclosing_type, size_t D, typename Sp,
          typename... Ts>
struct TraitEquation<enclosing_type,
  ...
  TraitEquation(T0s&&... args)
      : parent_trait(std::forward<T0s>(args)...),
        index_value{0},
        index{index_value, 0, len - 1} {}
  ...
  template <size_t N>
  auto param_matrix() const {
    static symphas::internal::param_matrix_factory coeff_matrix(
        *this, parent_trait::len, N);
    return coeff_matrix;
  }
  ...
  auto generate_equations(std::tuple<dynamics_t> const& dynamics,
                          OpExpression<E> const& e) const {
    using namespace symphas::internal;
  ...
  auto make_equations(std::pair<Dd, Ee> const& a) const {
    using this_type = TraitEquation<
        enclosing_type,
        symphas::internal::MakeEquation<ArrayModel<D, Sp, Ts...>>>;
    using scheme_type =
        std::invoke_result_t<decltype(&this_type::_make_equations<Dd, Ee>),
                             this_type, std::pair<Dd, Ee>, iter_type>;
 ...
 protected:
  ...
  auto _make_equations(std::pair<Dd, Ee> a, iter_type i) const {
    ...
    return solver
        .template form_expr_one<model_num_parameters<parent_trait>::value>(
            parent_trait::systems_tuple(), a);
  }

  template <size_t I = 0>
  auto _op() {
    return expr::make_term_dynamic(
        index, NamedData(parent_trait::systems(), names_t{}()));
  }
...
#define DEFINE_MODEL_FIELD_NAMES(MODEL_NAME, NAMES)                          \
  ...
  struct model_field_name<model_##MODEL_NAME::TraitEquationModel>            \
      : model_field_name_builder<model_##MODEL_NAME::TraitEquationModel> {}; \
  template <>                                                                \
  struct model_field_name_format<model_##MODEL_NAME::TraitEquationModel> {   \
    static const char* value[];                                              \
  };                                                                         \
  inline const char* model_field_name_format<                                \
      model_##MODEL_NAME::TraitEquationModel>::value[] = {SINGLE_ARG NAMES};
...
#define DEFINE_MODEL_FIELD_NAMES_FORMAT(MODEL_NAME, FORMAT)                    \
  ...
  inline const char*                                                           \
      model_field_name_format<model_##MODEL_NAME::TraitEquationModel>::value = \
          FORMAT;
...
struct series_index_selection<names_t, std::tuple<Ss...>, Xs...> {
  ...
  auto substitute_systems(OpExpression<E> const& e, std::index_sequence<Ls...>,
    ...
    auto list = std::make_tuple(Term(expr::as_variable<Ns>(NamedData(
        SymbolicData(std::get<Ns>(*systems).as_grid()), names_t{}(Ns))))...);

    return expr::sum(*static_cast<E const*>(&e))
        .template select<Ls...>(Xs{}...)(([&](auto) { return list; })(Ls)...);
  }
  ...
  auto operator()(OpExpression<E> const& e) {
    return substitute_systems(
        *static_cast<E const*>(&e),
        symphas::lib::seq_repeating_value_t<sizeof...(Xs), size_t,
                                            sizeof...(Ss)>{},
...
template <typename names_t, typename S, typename... Xs>
struct series_index_selection<names_t, symphas::internal::field_array_t<S>,
                              Xs...> {
  series_index_selection(names_t, std::pair<S*, len_type> const& systems,
                         Xs... xs)
      : systems{const_cast<S*>(systems.first)},
        len{systems.second},
        limits{xs...} {}

  template <typename E, size_t... Ls>
  auto substitute_systems(OpExpression<E> const& e,
...
template <typename names_t, typename... Ss, typename... Xs>
series_index_selection(names_t, std::tuple<Ss...>, Xs...)
    -> series_index_selection<names_t, std::tuple<Ss...>, Xs...>;

template <typename names_t, typename S, typename... Xs>
series_index_selection(names_t, std::pair<S*, len_type>, Xs...)
    -> series_index_selection<names_t, symphas::internal::field_array_t<S>,
                              Xs...>;
}

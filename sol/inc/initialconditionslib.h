...
enum class Inside {
  ...
  CAPPED,     //!< Values are assigned to be either the minimum or maximum
              //!< parameter.
  CONSTANT,   //!< All values are assigned to the same parameter.
  CIRCLE,     //!< Value are assigned to be the shape of a circle.
  HEXAGONAL,  //!< Values are assigned into circles arranged in a hexagonal
              //!< pattern.
  ...
  SPIRALHEX,  //!< Generate hexagonal pattern but one circle per field, spirally
              //!< ordered.
  ...
  EXPRESSION,  //!< An equation that is specified similar to the model
               //!< definitions.
  NONE         //!< Represents no initial condition.
};

#define ALL_INSIDE_GEN_VALUES                                              \
  Inside::CAPPED, Inside::CIRCLE, Inside::SQUARE, Inside::CONSTANT,        \
      Inside::CUBIC, Inside::HEXAGONAL, Inside::GAUSSIAN, Inside::UNIFORM, \
      Inside::SEEDSCIRCLE, Inside::SEEDSSQUARE, Inside::VORONOI,           \
      Inside::BUBBLE, Inside::SPIRALHEX, Inside::SIN, Inside::COS
...
enum class InsideTag {
  ...
  FIXEDSEED,  //!< Randomness will not vary throughout the field; not supported
              //!< by all algorithms.
...
struct value_fill<complex_t> {
  value_fill() : gen{std::random_device{}()}, th{-symphas::PI, symphas::PI} {}

  complex_t operator()(Axis ax, complex_t const& current,
...
struct value_fill<vector_t<1>> {
  vector_t<1> operator()(Axis ax, vector_t<1> const& current,
                         scalar_t value) const {
    return {value};
  }
};
...
struct value_fill<vector_t<3>> : value_fill<complex_t> {
  ...
  vector_t<3> operator()(Axis ax, vector_t<3> const& current,
                         scalar_t value) const {
    using namespace std;
    switch (ax) {
      case Axis::NONE: {
        scalar_t a = th(gen), b = th(gen), r = std::abs(value);
...
inline size_t tagtoul(InsideTag t) { return static_cast<size_t>(t); }

extern std::map<const char*, Inside, symphas::lib::any_case_comparator>
    init_key_map;
extern std::map<const char*, InsideTag, symphas::lib::any_case_comparator>
    init_tag_key_map;

template <typename T, size_t D>
using ic_iterator_difference_type =
    symphas::internal::iterator_region_difference_type<T, D>;
// struct ic_iterator_difference_type :
//{
//	using parent_type =
// symphas::internal::iterator_region_difference_type<T, D>; 	using
...
struct ic_iterator
    : symphas::iterator_type_impl<ic_iterator<T, D, E>,
...
//! Creates a tag index from the given ::InsideTag. \relatesalso init_entry_type
//! const& InsideTag Inside
/*!
...
struct init_data_parameters {
  double* gp;  //!< The generation parameters for the algorithm generating the
               //!< values.
  ...
  template <typename T,
            std::enable_if_t<
                (std::is_same_v<T, int> || std::is_same_v<T, double>), int> = 0>
  init_data_parameters(std::initializer_list<T> const& data)
      : init_data_parameters(data.size()) {
    auto it = data.begin();
  ...
  init_data_parameters(init_data_parameters const& other)
      : init_data_parameters(other.N) {
    std::copy(other.gp, other.gp + other.N, gp);
  }
  init_data_parameters(init_data_parameters&& other) noexcept
      : init_data_parameters(0) {
    swap(*this, other);
  }
  ...
  ~init_data_parameters() { delete[] gp; }
  ...
  double& operator[](int index) { return gp[index]; }

  double const& operator[](int index) const { return gp[index]; }
...
struct init_data_read {
  init_data_read(const char* name, iter_type index)
      : index{index},
        name{(name && std::strlen(name) > 0) ? new char[std::strlen(name) + 1]
                                             : nullptr} {
    if (this->name) {
  ...
  init_data_read(init_data_read const& other)
      : init_data_read(other.name, other.index) {}
  init_data_read(init_data_read&& other) : init_data_read() {
    swap(*this, other);
  }
  init_data_read& operator=(init_data_read other) {
    swap(*this, other);
    return *this;
  }
...
struct init_data_functor<void> {
  ...
  virtual void initialize(scalar_t* values,
                          grid::region_interval<1> const& interval) const = 0;

  //! Initializes a list of scalar values.
  virtual void initialize(scalar_t* values,
                          grid::region_interval<2> const& interval) const = 0;

  //! Initializes a list of scalar values.
  virtual void initialize(scalar_t* values,
                          grid::region_interval<3> const& interval) const = 0;

  //! Initializes a list of complex values.
  virtual void initialize(complex_t* values,
                          grid::region_interval<1> const& interval) const = 0;
  ...
  virtual void initialize(vector_t<1>* values,
                          grid::region_interval<1> const& interval) const = 0;

  //! Initializes a list of 2 dimensional vector values.
  virtual void initialize(vector_t<2>* values,
                          grid::region_interval<2> const& interval) const = 0;

  //! Initializes a list of 3 dimensional vector values.
  virtual void initialize(vector_t<3>* values,
                          grid::region_interval<3> const& interval) const = 0;

  template <typename ret_type, size_t D>
  void initialize(ret_type* values,
                  grid::region_interval<D> const& interval) const {}

  //! Initializes a list of scalar values.
  void initialize(scalar_t* values, const len_type* dims,
                  size_t dimension) const {
    if (dimension == 1) {
      initialize(values,
                 grid::region_interval<1>(symphas::arr_l_t<1>{dims[0]}));
    } else if (dimension == 2) {
      initialize(values, grid::region_interval<2>(
                             symphas::arr_l_t<2>{dims[0], dims[1]}));
    } else if (dimension == 3) {
      initialize(values, grid::region_interval<3>(
                             symphas::arr_l_t<3>{dims[0], dims[1], dims[2]}));
    } else {
      throw;
    }
  }
...
struct init_data_functor_impl : init_data_functor<void> {
  //! Initializes a list of scalar values.
  void initialize(scalar_t* values,
                  grid::region_interval<1> const& interval) const override {
    cast().initialize(values, interval);
  }

  //! Initializes a list of scalar values.
  void initialize(scalar_t* values,
                  grid::region_interval<2> const& interval) const override {
    cast().initialize(values, interval);
  }

  //! Initializes a list of scalar values.
  void initialize(scalar_t* values,
                  grid::region_interval<3> const& interval) const override {
    cast().initialize(values, interval);
  }

  //! Initializes a list of complex values.
  void initialize(complex_t* values,
                  grid::region_interval<1> const& interval) const override {
    cast().initialize(values, interval);
  }
  ...
  //! Initializes a list of 1 dimensional vector values.
  void initialize(vector_t<1>* values,
                  grid::region_interval<1> const& interval) const override {
    cast().initialize(values, interval);
  }

  //! Initializes a list of 2 dimensional vector values.
  void initialize(vector_t<2>* values,
                  grid::region_interval<2> const& interval) const override {
    cast().initialize(values, interval);
  }

  //! Initializes a list of 3 dimensional vector values.
  void initialize(vector_t<3>* values,
                  grid::region_interval<3> const& interval) const override {
    cast().initialize(values, interval);
  }
...
template <typename T>
struct init_data_functor<Block<T>>
    : init_data_functor_impl<init_data_functor<Block<T>>> {
  using parent_type = init_data_functor<void>;
  using parent_type::initialize;
  ...
  void initialize(T* values, grid::region_interval<D> const& interval) const {
    auto it = symphas::data_iterator_region(values, interval);
    std::copy(symphas::data_iterator(source),
              symphas::data_iterator(source, grid::length<D>(interval)), it);
  }
...
struct init_data_functor<MultiBlock<N, T>>
    : init_data_functor_impl<init_data_functor<MultiBlock<N, T>>> {
  using parent_type = init_data_functor<void>;
  using parent_type::initialize;
...
template <typename T, size_t D>
init_data_functor(Grid<any_vector_t<T, D>, D>)
    -> init_data_functor<MultiBlock<D, T>>;
template <typename T, size_t D>
init_data_functor(BoundaryGrid<any_vector_t<T, D>, D>)
    -> init_data_functor<MultiBlock<D, T>>;
template <typename T, size_t D>
init_data_functor(RegionalGrid<any_vector_t<T, D>, D>)
    -> init_data_functor<MultiBlock<D, T>>;
template <typename T>
...
template <typename T, size_t D>
init_data_functor(GridCUDA<T, D>) -> init_data_functor<BlockCUDA<T>>;
template <typename T, size_t D>
init_data_functor(BoundaryGridCUDA<T, D>) -> init_data_functor<BlockCUDA<T>>;
template <typename T, size_t D>
init_data_functor(RegionalGridCUDA<T, D>) -> init_data_functor<BlockCUDA<T>>;
template <typename T, size_t D>
init_data_functor(GridCUDA<any_vector_t<T, D>, D>)
    -> init_data_functor<MultiBlockCUDA<D, T>>;
template <typename T, size_t D>
init_data_functor(BoundaryGridCUDA<any_vector_t<T, D>, D>)
    -> init_data_functor<MultiBlockCUDA<D, T>>;
template <typename T, size_t D>
init_data_functor(RegionalGridCUDA<any_vector_t<T, D>, D>)
    -> init_data_functor<MultiBlockCUDA<D, T>>;
...
struct init_data_expr : init_data_read {
  ...
  init_data_expr(init_data_expr const& other)
      : init_data_expr(other.name, other.coeff, other.num_coeff) {}
  init_data_expr(init_data_expr&& other) : init_data_expr() {
    swap(*this, other);
  }
  init_data_expr& operator=(init_data_expr other) {
    swap(*this, other);
    return *this;
  }
  ...
  size_t get_num_coeff() const { return num_coeff; }

  friend void swap(init_data_expr& first, init_data_expr& second) {
    using std::swap;
    swap(static_cast<init_data_read&>(first),
         static_cast<init_data_read&>(second));
...
struct init_entry_data_type {
  init_entry_data_type(init_data_functor<void> const* f_init)
      : f_init{f_init->make_copy()}, data{}, file{}, expr_data{} {}
  init_entry_data_type(init_data_parameters data)
      : f_init{nullptr}, data{data}, file{}, expr_data{} {}
  init_entry_data_type(init_data_read file)
      : f_init{nullptr}, data{}, file{file}, expr_data{} {}
  init_entry_data_type(init_data_expr expr_data)
      : f_init{nullptr}, data{}, file{}, expr_data{expr_data} {}
  ...
  init_data_functor<void>*
      f_init;  //!< Separate functor to generate the initial conditions.
  init_data_parameters
      data;  //!< The parameters used by the initial condition algorithm.
  ...
  ~init_entry_data_type() { delete f_init; }
};
...
struct init_entry_type : init_entry_data_type {
  ...
  init_entry_type()
      : init_entry_data_type(),
        in{Inside::NONE},
        intag{symphas::build_intag(InsideTag::NONE)} {}
  ...
  init_entry_type(Inside in, init_data_read file)
      : init_entry_data_type(file),
        in{in},
        intag{symphas::build_intag(InsideTag::DEFAULT)} {}
   ...
   * \param file Information about the file.
   */
  init_entry_type(init_data_read file) : init_entry_type(Inside::FILE, file) {}
  ...
  init_entry_type(init_data_expr expr_data)
      : init_entry_data_type(expr_data),
        in{Inside::EXPRESSION},
        intag{symphas::build_intag(InsideTag::DEFAULT)} {}
  ...
  init_entry_type(init_data_functor<F> const& f)
      : init_entry_data_type(&f),
        in{Inside::NONE},
        intag{symphas::build_intag(InsideTag::DEFAULT)} {}

  template <typename F, typename = std::invoke_result_t<
                            F, iter_type, len_type const*, size_t>>
  init_entry_type(F&& f)
      : init_entry_type(init_data_functor{std::forward<F>(f)}) {}

  init_entry_type(init_entry_type&& other) noexcept : init_entry_type() {
    swap(*this, other);
  }

  init_entry_type(init_entry_type const& other)
      : init_entry_type(other.in, other.intag,
                        *static_cast<init_entry_type const*>(&other)) {}
  ...
  friend void swap(init_entry_type& first, init_entry_type& second) {
    using std::swap;
    swap(*static_cast<init_entry_data_type*>(&first),
         *static_cast<init_entry_data_type*>(&second));
 ...
 protected:
  init_entry_type(Inside in, size_t intag, init_entry_data_type other)
      : init_entry_data_type(other), in{in}, intag{intag} {}
};
...
struct init_data_type : std::map<Axis, init_entry_type> {
  ...
  init_data_type(init_entry_type const& tdata)
      : parent_type({{Axis::NONE, tdata}}) {}
  template <typename... Ts>
  init_data_type(Inside in, Ts&&... args)
      : init_data_type(init_entry_type(in, std::forward<Ts>(args)...)) {}
  init_data_type(Inside in, symphas::init_data_parameters const& args)
      : init_data_type(init_entry_type(in, args)) {}
...
inline void swap(symphas::init_entry_type& first,
                 symphas::init_entry_type& second) {
...
struct RandomOffsets {
  ...
  const T* get_offset(size_t i) const { return offsets[i].data(); }
...
struct RandomOffsets<T, 1> {
  ...
  template <typename S,
            typename = std::enable_if_t<
                (!std::is_pointer_v<S> && !std::is_array_v<S>), int>>
  RandomOffsets(size_t n, S const& mean, double s)
      : RandomOffsets(n, &mean, s) {}

  auto get_offset(size_t i) const { return offsets[i]; }

  void set_offset(size_t i, T const& value) { offsets[i] = value; }
...
template <>
template <typename S>
inline RandomOffsets<double, 1>::RandomOffsets(size_t n, S const* means,
                                               double s) {
  offsets.resize(n);
...
template <>
template <typename S>
inline RandomOffsets<double, 2>::RandomOffsets(size_t n, S const* means,
                                               double s) {
  offsets.resize(n);
...
template <>
template <typename S>
inline RandomOffsets<double, 3>::RandomOffsets(size_t n, S const* means,
                                               double s) {
  offsets.resize(n);
...
template <>
template <typename S>
inline RandomOffsets<len_type, 1>::RandomOffsets(size_t n, S const* means,
                                                 double s) {
  offsets.resize(n);
  ...
  std::random_device rd;
  std::mt19937 gen(rd());
  std::uniform_int_distribution<> dis(static_cast<iter_type>(x_min),
                                      static_cast<iter_type>(x_max));
...
template <>
template <typename S>
inline RandomOffsets<len_type, 2>::RandomOffsets(size_t n, S const* means,
                                                 double s) {
  offsets.resize(n);
  ...
  std::uniform_int_distribution<> disx(static_cast<iter_type>(x_min),
                                       static_cast<iter_type>(x_max));
  std::uniform_int_distribution<> disy(static_cast<iter_type>(y_min),
                                       static_cast<iter_type>(y_max));
...
template <>
template <typename S>
inline RandomOffsets<len_type, 3>::RandomOffsets(size_t n, S const* means,
                                                 double s) {
  offsets.resize(n);
  ...
  std::uniform_int_distribution<> disy(static_cast<iter_type>(y_min),
                                       static_cast<iter_type>(y_max));
  std::uniform_int_distribution<> disz(static_cast<iter_type>(z_min),
                                       static_cast<iter_type>(z_max));
...
struct RandomDeltas {
   ...
   */
  RandomDeltas(std::vector<std::array<double, D>> const& points)
      : points{points} {}

  //! Return the element at the chosen index..
  std::array<double, D> get_delta(size_t i) const { return points[i]; }

  //! Return the list of all elements.
  auto get_deltas() const { return points; }
...
struct RandomDeltas<1> {
  ...
  auto get_delta(size_t i) const { return points[i]; }

  auto get_deltas() const { return points; }

  auto set_delta(size_t i, double const& value) { points[i] = value; }
...
inline bool is_in_square_2(iter_type cx, double ux, double aa, iter_type cy,
                           double uy, double bb) {
  return std::abs(cx - ux) < aa && std::abs(cy - uy) < bb;
}

inline bool is_in_circle_2(iter_type cx, double ux, double aa, iter_type cy,
                           double uy, double bb) {
  return (cx - ux) * (cx - ux) / (aa * aa) + (cy - uy) * (cy - uy) / (bb * bb) <
         1.0;
}

inline bool is_in_square_3(iter_type cx, double ux, double aa, iter_type cy,
                           double uy, double bb, iter_type cz, double uz,
                           double cc) {
  return std::abs(cx - ux) < aa && std::abs(cy - uy) < bb &&
         std::abs(cz - uz) < cc;
}

inline bool is_in_circle_3(iter_type cx, double ux, double aa, iter_type cy,
                           double uy, double bb, iter_type cz, double uz,
                           double cc) {
  return (cx - ux) * (cx - ux) / (aa * aa) + (cy - uy) * (cy - uy) / (bb * bb) +
             (cz - uz) * (cz - uz) / (cc * cc) <
...
inline bool is_in_square_2_dd(double ux, double aa, double uy, double bb) {
  return is_in_square_2(0, ux, aa, 0, uy, bb);
}

inline bool is_in_circle_2_dd(double ux, double aa, double uy, double bb) {
  return is_in_circle_2(0, ux, aa, 0, uy, bb);
}

inline bool is_in_square_3_dd(double ux, double aa, double uy, double bb,
                              double uz, double cc) {
  return is_in_square_3(0, ux, aa, 0, uy, bb, 0, uz, cc);
}

inline bool is_in_circle_3_dd(double ux, double aa, double uy, double bb,
                              double uz, double cc) {
...
inline bool is_in_square_A_2(double dx, double aa, double dy, double bb,
                             const len_type (&dims)[2]) {
  dx = dx - dims[0] * std::round(dx / dims[0]);
  dy = dy - dims[1] * std::round(dy / dims[1]);
  return std::abs(dx) < aa && std::abs(dy) < bb;
}

inline bool is_in_circle_A_2(double dx, double aa, double dy, double bb,
                             const len_type (&dims)[2]) {
  dx = dx - dims[0] * std::round(dx / dims[0]);
  dy = dy - dims[1] * std::round(dy / dims[1]);
  return (dx) * (dx) / (aa * aa) + (dy) * (dy) / (bb * bb) < 1.0;
}

inline bool is_in_square_A_3(double dx, double aa, double dy, double bb,
                             double dz, double cc, const len_type (&dims)[3]) {
  dx = dx - dims[0] * std::round(dx / dims[0]);
  dy = dy - dims[1] * std::round(dy / dims[1]);
  dz = dz - dims[2] * std::round(dz / dims[2]);
  return std::abs(dx) < aa && std::abs(dy) < bb && std::abs(dz) < cc;
}

inline bool is_in_circle_A_3(double dx, double aa, double dy, double bb,
                             double dz, double cc, const len_type (&dims)[3]) {
  dx = dx - dims[0] * std::round(dx / dims[0]);
  dy = dy - dims[1] * std::round(dy / dims[1]);
  dz = dz - dims[2] * std::round(dz / dims[2]);
  return (dx) * (dx) / (aa * aa) + (dy) * (dy) / (bb * bb) +
             (dz) * (dz) / (cc * cc) <
         1.0;
}

template <typename Pred>
double seeds_1(iter_type n, len_type const* dims,
               symphas::init_entry_type const& init_data, Pred p,
               RandomDeltas<1> const* offsets, double seed_val,
               double field_val) {
  ...
  for (iter_type i = 0; i < num_points; ++i) {
    ...
    if (p(cx, ux, aa)) {
      return (tag_bit_compare(init_data.intag, InsideTag::INVERT)) ? field_val
                                                                   : seed_val;
    }
  }
  return (tag_bit_compare(init_data.intag, InsideTag::INVERT)) ? seed_val
                                                               : field_val;
}

template <typename Pred>
double seeds_rnd_1(iter_type n, len_type const* dims,
                   symphas::init_entry_type const& init_data, Pred p,
                   RandomDeltas<1> const* offsets,
                   RandomOffsets<double, 1> const* values, double seed_val,
                   double field_val, double rnd_offset) {
  size_t num_points = static_cast<size_t>(init_data.data.gp[0]);
  double aa = dims[0] * init_data.data.gp[1] * IC_SEED_SCALE_FACTOR;
  iter_type const cx = n;
...
template <typename Pred>
double seeds_A_1(iter_type n, len_type const* dims,
                 symphas::init_entry_type const& init_data, Pred p,
                 RandomDeltas<1> const* offsets,
                 RandomOffsets<len_type, 1> const* scales, double seed_val,
                 double field_val) {
  size_t num_points = static_cast<size_t>(init_data.data.gp[0]);
  iter_type const cx = n;
...
template <typename Pred>
double seeds_A_rnd_1(iter_type n, len_type const* dims,
                     symphas::init_entry_type const& init_data, Pred p,
                     RandomDeltas<1> const* offsets,
                     RandomOffsets<len_type, 1> const* scales,
                     RandomOffsets<double, 1> const* values, double seed_val,
                     double field_val, double rnd_offset) {
  size_t num_points = static_cast<size_t>(init_data.data.gp[0]);
  iter_type const cx = n;
...
template <typename Pred>
double seeds_B_1(iter_type n, len_type const* dims,
                 symphas::init_entry_type const& init_data, Pred p,
                 RandomDeltas<1> const* offsets,
                 RandomOffsets<len_type, 1> const* scales, double seed_val,
                 double field_val) {
...
template <typename Pred>
double seeds_B_rnd_1(iter_type n, len_type const* dims,
                     symphas::init_entry_type const& init_data, Pred p,
                     RandomDeltas<1> const* offsets,
                     RandomOffsets<len_type, 1> const* scales,
                     RandomOffsets<double, 1> const* values, double seed_val,
                     double field_val, double rnd_offset) {
  return seeds_A_rnd_1(n, dims, init_data, p, offsets, scales, values,
                       rnd_offset, seed_val, field_val);
}

// ********************************************************************************

template <typename Pred>
double seeds_2(iter_type n, len_type const* dims,
               symphas::init_entry_type const& init_data, Pred p,
               RandomDeltas<2> const* offsets, double seed_val,
               double field_val) {
  size_t num_points = static_cast<size_t>(init_data.data.gp[0]);
  ...
  // x y cursor position
  iter_type const cx = n % dims[0], cy = n / dims[0];
...
template <typename Pred>
double seeds_rnd_2(iter_type n, len_type const* dims,
                   symphas::init_entry_type const& init_data, Pred p,
                   RandomDeltas<2> const* offsets,
                   RandomOffsets<double, 1> const* values, double seed_val,
                   double field_val, double rnd_offset) {
  size_t num_points = static_cast<size_t>(init_data.data.gp[0]);
...
template <typename Pred>
double seeds_A_2(iter_type n, len_type const* dims,
                 symphas::init_entry_type const& init_data, Pred p,
                 RandomDeltas<2> const* offsets,
                 RandomOffsets<len_type, 2> const* scales, double seed_val,
                 double field_val) {
  size_t num_points = static_cast<size_t>(init_data.data.gp[0]);
  ...
  for (iter_type i = 0; i < num_points; ++i) {
    ...
    double aa = dims[0] * init_data.data.gp[1] * IC_SEED_SCALE_FACTOR +
                scales->get_offset(i)[0];
    double bb = dims[1] * init_data.data.gp[1] * IC_SEED_SCALE_FACTOR +
                scales->get_offset(i)[1];
...
template <typename Pred>
double seeds_A_rnd_2(iter_type n, len_type const* dims,
                     symphas::init_entry_type const& init_data, Pred p,
                     RandomDeltas<2> const* offsets,
                     RandomOffsets<len_type, 2> const* scales,
                     RandomOffsets<double, 1> const* values, double seed_val,
                     double field_val, double rnd_offset) {
  size_t num_points = static_cast<size_t>(init_data.data.gp[0]);
...
template <typename Pred>
double seeds_B_2(iter_type n, len_type const* dims,
                 symphas::init_entry_type const& init_data, Pred p,
                 RandomDeltas<2> const* offsets,
                 RandomOffsets<len_type, 2> const* scales, double seed_val,
                 double field_val) {
  ...
  return seeds_A_2(n, square_dims, init_data, p, offsets, scales, seed_val,
                   field_val);
}

template <typename Pred>
double seeds_B_rnd_2(iter_type n, len_type const* dims,
                     symphas::init_entry_type const& init_data, Pred p,
                     RandomDeltas<2> const* offsets,
                     RandomOffsets<len_type, 2> const* scales,
                     RandomOffsets<double, 1> const* values, double seed_val,
  ...
  auto d = *std::min_element(dims, dims + 2);
  len_type square_dims[] = {d, d};
  return seeds_A_rnd_2(n, square_dims, init_data, p, offsets, scales, values,
                       rnd_offset, seed_val, field_val);
}
...
template <typename Pred>
double seeds_3(iter_type n, len_type const* dims,
               symphas::init_entry_type const& init_data, Pred p,
               RandomDeltas<3> const* offsets, double seed_val,
               double field_val) {
  size_t num_points = static_cast<size_t>(init_data.data.gp[0]);
  ...
  // x y cursor position
  iter_type const cx = n % dims[0], cy = (n / dims[0]) % dims[1],
                  cz = n / (dims[0] * dims[1]);
...
template <typename Pred>
double seeds_rnd_3(iter_type n, len_type const* dims,
                   symphas::init_entry_type const& init_data, Pred p,
                   RandomDeltas<3> const* offsets,
                   RandomOffsets<double, 1> const* values, double seed_val,
                   double field_val, double rnd_offset) {
  size_t num_points = static_cast<size_t>(init_data.data.gp[0]);
...
template <typename Pred>
double seeds_A_3(iter_type n, len_type const* dims,
                 symphas::init_entry_type const& init_data, Pred p,
                 RandomDeltas<3> const* offsets,
                 RandomOffsets<len_type, 3> const* scales, double seed_val,
                 double field_val) {
  size_t num_points = static_cast<size_t>(init_data.data.gp[0]);
...
template <typename Pred>
double seeds_A_rnd_3(iter_type n, len_type const* dims,
                     symphas::init_entry_type const& init_data, Pred p,
                     RandomDeltas<3> const* offsets,
                     RandomOffsets<len_type, 3> const* scales,
                     RandomOffsets<double, 1> const* values, double seed_val,
                     double field_val, double rnd_offset) {
  size_t num_points = static_cast<size_t>(init_data.data.gp[0]);
...
template <typename Pred>
double seeds_B_3(iter_type n, len_type const* dims,
                 symphas::init_entry_type const& init_data, Pred p,
                 RandomDeltas<3> const* offsets,
                 RandomOffsets<len_type, 3> const* scales, double seed_val,
                 double field_val) {
  ...
  len_type square_dims[] = {d, d};
  return seeds_A_3(n, square_dims, init_data, p, offsets, scales, seed_val,
                   field_val);
}

template <typename Pred>
double seeds_B_rnd_3(iter_type n, len_type const* dims,
                     symphas::init_entry_type const& init_data, Pred p,
                     RandomDeltas<3> const* offsets,
                     RandomOffsets<len_type, 3> const* scales,
                     RandomOffsets<double, 1> const* values, double seed_val,
  ...
  auto d = *std::min_element(dims, dims + 3);
  len_type square_dims[] = {d, d};
  return seeds_A_rnd_3(n, square_dims, init_data, p, offsets, scales, values,
                       rnd_offset, seed_val, field_val);
}

template <typename F>
auto get_function_value(iter_type n, F f, const len_type (&dims)[1],
                        symphas::interval_data_type vdata, const double* params,
                        double zero_at_left = false) {
...
template <typename F>
auto get_function_value(iter_type n, F f, const len_type (&dims)[2],
                        symphas::interval_data_type vdata, const double* params,
                        double zero_at_left = false) {
  double x = (n % dims[0]) * vdata[Axis::X].width() + vdata[Axis::X].left();
...
template <typename F>
auto get_function_value(iter_type n, F f, const len_type (&dims)[3],
                        symphas::interval_data_type vdata, const double* params,
                        double zero_at_left = false) {
  double x = (n % dims[0]) * vdata[Axis::X].width() + vdata[Axis::X].left();
  double y = ((n / dims[0]) % dims[1]) * vdata[Axis::Y].width() +
             vdata[Axis::Y].left();
  double z = (n / (dims[1] * dims[2])) * vdata[Axis::Z].width() +
             vdata[Axis::Z].left();
...
template <typename F>
auto get_function_value_A(iter_type n, F f, const len_type (&dims)[1],
                          symphas::interval_data_type vdata,
                          const double* params, double zero_at_left = false) {
  double x = (n % dims[0]) * vdata[Axis::X].width() + vdata[Axis::X].left();
...
template <typename F>
auto get_function_value_A(iter_type n, F f, const len_type (&dims)[2],
                          symphas::interval_data_type vdata,
                          const double* params, double zero_at_left = false) {
  double x = (n % dims[0]) * vdata[Axis::X].width() + vdata[Axis::X].left();
  double y = (n / dims[0]) * vdata[Axis::Y].width() + vdata[Axis::Y].left();
...
template <typename F>
auto get_function_value_A(iter_type n, F f, const len_type (&dims)[3],
                          symphas::interval_data_type vdata,
                          const double* params, double zero_at_left = false) {
  double x = (n % dims[0]) * vdata[Axis::X].width() + vdata[Axis::X].left();
  double y = ((n / dims[0]) % dims[1]) * vdata[Axis::Y].width() +
 ...
 * If the coverage is 0.5, the overlap will be about -125% of the radius
 * (meaning bubbles will be generated so they are away from each other).
 *
...
template <size_t D>
auto get_bubble_positions(
    size_t N, double R, double max_overlap,
    symphas::internal::RandomOffsets<scalar_t, 1> const& overlaps,
    const len_type* dims, iter_type pos0);

template <size_t D>
auto get_bubble_positions(
    size_t N, double R, double max_overlap,
    symphas::internal::RandomOffsets<scalar_t, 1> const& overlaps,
    const len_type* dims, const iter_type* pos0);

template <>
inline auto get_bubble_positions<1>(
    size_t N, double R, double max_overlap,
    symphas::internal::RandomOffsets<scalar_t, 1> const& overlaps,
    const len_type* dims, iter_type pos0) {
...
template <>
inline auto get_bubble_positions<2>(
    size_t N, double R, double max_overlap,
    symphas::internal::RandomOffsets<scalar_t, 1> const& overlaps,
    const len_type* dims, const iter_type* pos0) {
  return get_bubble_positions_2(N, R, max_overlap, overlaps, dims, pos0[0],
                                pos0[1]);
}

template <>
inline auto get_bubble_positions<3>(
    size_t N, double R, double max_overlap,
    symphas::internal::RandomOffsets<scalar_t, 1> const& overlaps,
    const len_type* dims, const iter_type* pos0) {
  return get_bubble_positions_3(N, R, max_overlap, overlaps, dims, pos0[0],
                                pos0[1], pos0[2]);
}

template <size_t D>
void get_num_hex_tiles(len_type (&num_tiles)[D], size_t N,
                       len_type const* dims);
template <>
...
template <size_t D>
symphas::internal::RandomDeltas<D> get_hex_positions(size_t N,
                                                     len_type const* dims);
template <>
...
template <size_t D>
symphas::internal::RandomDeltas<D> to_spiral_order(
    symphas::internal::RandomDeltas<D> const& positions, size_t N,
    len_type const* dims);
template <>

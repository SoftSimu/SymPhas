...
struct Model {
  ...
  using SolverSystemApplied = typename symphas::solver_system_type<
      Sp>::template type<symphas::internal::non_parameterized_type<D, Ty>, D>;
 ...
 public:
  ...
  using type_of_S = symphas::lib::type_at_index<
      N, symphas::internal::non_parameterized_type<D, S>...>;
  ...
  Model(double const* coeff, size_t num_coeff,
      ...
      : _s{construct_systems(
            parameters.get_initial_data(), parameters.get_interval_data(),
            parameters.get_boundary_data(), parameters.length())},
  ...
  Model(Model<D, Sp, S...>&& other) noexcept : Model() { swap(*this, other); }
  ...
  //	template<typename Sp0, typename = std::enable_if_t<!std::is_same<Sp,
  // Sp0>::value, int>> 	Model(Model<D, Sp0, S...> const& other) :
  // _s{ construct_systems(other.systems_tuple()) }, solver{
  // Sp::make_solver(generate_parameters()) }, coeff{ (other.get_num_coeff() >
  // 0) ? new double[other.get_num_coeff()] : nullptr }, num_coeff{
  // other.get_num_coeff() }, index{ other.index() }, time{ other.get_time()}
  // #ifdef VTK_ON
  //		, viz_update{ nullptr }
  // #endif
  //	{
  //		std::copy(other.get_coeff(), other.get_coeff() +
  // other.get_num_coeff(), coeff); 		update_systems(time);
  // visualize();
  //	}
  //
  //	template<typename Sp0, typename = std::enable_if_t<!std::is_same<Sp,
  // Sp0>::value, int>> 	Model(Model<D, Sp0, S...>&& other) noexcept :
  // Model()
  //	{
  //		swap(*this, other);
  //	}
  //
  //	template<typename Sp0, typename = std::enable_if_t<!std::is_same<Sp,
  // Sp0>::value, int>> 	Model<D, Sp0, S...>& operator=(Model<D, Sp0,
  // S...> other)
  //	{
  //		swap(*this, other);
  //		return *this;
  ...
  static constexpr size_t num_fields() {
    return (
        (static_cast<size_t>(
             std::is_same<
                 S0, symphas::internal::non_parameterized_type<D, S>>::value) +
         ...));
  }
  ...
  static constexpr size_t num_fields() {
    return ((static_cast<size_t>(
                 std::is_same<S0, symphas::internal::non_parameterized_type<
                                      D, S>>::value) +
             ...)) +
           num_fields<S1, Ss...>();
  }
  ...
  static constexpr int index_of_type = symphas::lib::nth_index_of_type<
      Type, I, symphas::internal::non_parameterized_type<D, S>...>;
  ...
  template <typename Type, typename F, typename... Args,
            typename std::enable_if_t<(num_fields<Type>() > 0), int> = 0>
  void do_for_field_type(F f, Args&&... args) const {
    do_for_field_type<Type>(std::make_index_sequence<num_fields<Type>()>{}, f,
  ...
  template <size_t N, typename F, typename... Args,
            typename std::enable_if<(N <= sizeof...(S)), int>::type = 0>
  ...
  template <typename Type, size_t I,
            typename std::enable_if_t<(num_fields<Type>() > I), int> = 0>
  ...
  auto& system() {
    ...
    return const_cast<chosen_sys_type&>(
        static_cast<const Model<D, Sp, S...>&>(*this).system<I>());
  }
   ...
   */
  const auto& systems_tuple() const { return _s; }
 ...
 protected:
  std::tuple<SolverSystemApplied<S>...>
  ...
  auto get_viz_field(std::integer_sequence<int, I0, Is...>) {
    return get_viz_field<size_t(I0)>(
        std::index_sequence<size_t(I0), size_t(Is)...>{});
  }
  ...
  auto get_viz_field(std::index_sequence<Is...>) {
    ...
    using seq_filtered =
        symphas::lib::filter_seq_t<seq, std::integer_sequence<int, -1>>;
  ...
  template <size_t... Is>
  void construct_interval_data(symphas::interval_data_type (&intervals)[SN],
  ...
  template <size_t... Is>
  auto populate_parameters(symphas::problem_parameters_type& parameters,
                           std::index_sequence<Is...>) const {
  ...
  void fill_interval_data(PhaseFieldSystem<BoundaryGrid, T0, D> const& system,
                          symphas::problem_parameters_type& parameters) const {
  ...
  template <size_t I, typename T0>
  void fill_interval_data(PhaseFieldSystem<RegionalGrid, T0, D> const& system,
                          symphas::problem_parameters_type& parameters) const {
    auto intervals = system.get_info().intervals;
    for (iter_type i = 0; i < D; ++i) {
      Axis ax = symphas::index_to_axis(i);
  ...
  void fill_interval_data(PhaseFieldSystem<G, T0, D> const& system,
                          symphas::problem_parameters_type& parameters) const {
  ...
  void fill_initial_data(PhaseFieldSystem<G, T0, D> const& system,
                         symphas::problem_parameters_type& parameters) const {
    parameters.set_initial_data(symphas::init_data_functor(system.as_grid()),
                                I);
  }
  ...
  auto construct_systems(const symphas::init_data_type* tdata,
    ...
    return std::make_tuple(
        SolverSystemApplied<S>(tdata[Is], vdata[Is], bdata[Is], Is)...);
  }
  ...
  auto construct_systems(const symphas::init_data_type* tdata,
    ...
    if (data_len < SN) {
      ...
      return construct_systems(tdata_extend, vdata_extend, bdata_extend,
                               std::make_index_sequence<SN>{});
    } else {
      return construct_systems(tdata, vdata, bdata,
                               std::make_index_sequence<SN>{});
    }
  }

  template <
      size_t I, typename other_sys_type,
      typename std::enable_if_t<
          !std::is_same<other_sys_type,
                        std::tuple_element_t<
                            I, std::tuple<SolverSystemApplied<S>...>>>::value,
          int> = 0>
  SolverSystemApplied<symphas::lib::type_at_index<I, S...>> construct_system(
      other_sys_type const& system) const {
    using value_type = symphas::lib::type_at_index<
        I, symphas::internal::non_parameterized_type<D, S>...>;
    ...
    auto new_system = SolverSystemApplied<symphas::lib::type_at_index<I, S...>>(
        symphas::init_data_type{}, system.info.intervals, boundaries,
        system.id);
  ...
  std::tuple<SolverSystemApplied<S>...> construct_systems(
      std::tuple<Ss...> const& systems, std::index_sequence<Is...>) const {
    return std::make_tuple(construct_system<Is>(std::get<Is>(systems))...);
  }

  template <typename... Ss>
  auto construct_systems(std::tuple<Ss...> const& systems) const {
    return construct_systems(systems,
  ...
  template <size_t... Is>
  void save_systems(const char* dir, const char* name,
                    std::index_sequence<Is...>) const {
    char* names[SN];
...
struct model_dimension {
 protected:
  ...
  static constexpr auto pack_dim(M* m) { return _pack_dim(m); }

  using T =
      typename std::invoke_result_t<decltype(&model_dimension<M>::pack_dim),
                                    M*>;
...
struct model_num_parameters {
 protected:
  ...
  static constexpr auto pack_count(M m) { return _pack_count(m); }

  using T = typename std::invoke_result_t<
      decltype(&model_num_parameters<M>::pack_count), M>;
...
struct model_solver {
 protected:
  ...
  static auto get_type(M m) { return _get_type(m); }

 public:
  using type =
      typename std::invoke_result_t<decltype(&model_solver<M>::get_type),
                                    M>::type;
};
...
struct model_base {
 ...
 public:
  using type =
      typename std::invoke_result_t<decltype(&model_base<M>::get_type), M>;
};
...
struct model_parameter_types {
 protected:
  ...
  static auto get_type(M m) { return symphas::lib::types_list<>{}; }

 public:
  using type = typename std::invoke_result_t<
      decltype(&model_parameter_types<M>::get_type), M>;
};
...
struct model_system_type<Model<D, Sp, S...>, N> {
  ...
  using solver_system_t =
      typename symphas::solver_system_type<Sp>::template type<type_of_S, D>;
  using call_t =
      decltype(&model_system_type<Model<D, Sp, S...>,
                                  N>::template get_type<solver_system_t>);
...
struct model_system_type<ArrayModel<D, Sp, S...>, N> {
  ...
  using solver_system_t =
      typename symphas::solver_system_type<Sp>::template type<type_of_S, D>;
  using call_t =
      decltype(&model_system_type<ArrayModel<D, Sp, S...>,
                                  N>::template get_type<solver_system_t>);
...
struct is_model_array_type_impl {
 protected:
  ...
  static constexpr auto check(M m) { return _check(m); }

  using T = typename std::invoke_result_t<
      decltype(&is_model_array_type_impl<M>::check), M>;

...
void symphas::time_step_list::set_time_step(double dt, double time,
                                            bool insert) {
  if (dts_len > 0) {
    if (t_dts[0] == time) {
      t_dts[0] = time;
        ...
        if (i == dts_len) {
          ...
          if (t_dts[i * 2] == time) {
            ...
            if (insert) {
              ...
              std::copy(t_dts + i * 2, t_dts + dts_len,
                        t_dts_updated + (i + 1) * 2);
...
//! Provide the time step used in the solution of the problem.
void symphas::time_step_list::set_time_steps(const double* dts,
                                             const double* t_dts,
                                             size_t dts_len) {
  if (dts_len > 0) {
    delete[] this->t_dts;
...
void symphas::problem_parameters_type::set_modifiers(const char* const* str,
                                                     size_t n) {
...
void symphas::problem_parameters_type::set_modifier(const char* str,
                                                    iter_type i) {
  if (str) {
    size_t next = 0;
...
symphas::problem_parameters_type::problem_parameters_type(
    problem_parameters_type const& other)
    : problem_parameters_type(other.len) {
...
void symphas::problem_parameters_type::set_initial_data(
    const symphas::init_data_type* tdata_set, size_t n) {
  if (n <= len && n > 0) {
    for (iter_type i = 0; i < n; ++i) {
      tdata[i] = tdata_set[i];
      symphas::update_thr_info(&tdata[i].thr_info, *this, i);
    }
    for (iter_type i = iter_type(n); i < len; ++i) {
      tdata[i] = tdata_set[0];
      symphas::update_thr_info(&tdata[i].thr_info, *this, i);
    }
  } else {
    fprintf(SYMPHAS_ERR,
            "the number of given initial data is larger than the number of "
            "systems\n");
  }
}

void symphas::problem_parameters_type::set_interval_data(
    const symphas::interval_data_type* vdata_set, size_t n) {
  if (n <= len && n > 0) {
    ...
    fprintf(SYMPHAS_ERR,
            "the number of given interval data is larger than the number of "
            "systems\n");
  }
}

void symphas::problem_parameters_type::set_boundary_data(
    const symphas::b_data_type* bdata_set, size_t n) {
  if (n <= len && n > 0) {
    for (size_t i = 0; i < n; ++i) {
      bdata[i] = bdata_set[i];
    }
    ...
    fprintf(SYMPHAS_ERR,
            "the number of given boundary data is larger than the number of "
            "systems\n");
  }
}

void symphas::problem_parameters_type::set_num_fields(len_type* num_fields_set,
                                                      size_t n) {
  if (n <= num_fields_len && n > 0) {
    ...
    fprintf(SYMPHAS_ERR,
            "the number of given field lengths is larger than the number of "
            "lengths\n");
  }
}
...
//! Provide the time step used in the solution of the problem.
void symphas::problem_parameters_type::set_time_steps(const double* dts,
                                                      const double* t_dts,
                                                      size_t dts_len) {
...
void symphas::problem_parameters_type::equalize_discretization(
    const len_type* dims) {
...
void symphas::swap(symphas::problem_parameters_type& first,

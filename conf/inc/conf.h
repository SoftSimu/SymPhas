 ...
 */
void setup_global_config(const char* file,
                         param_map_type const& param_map = {});
...
std::vector<iter_type> get_save_indices(
    Conf const& config = symphas::conf::config());
 ...
 */
iter_type next_save(iter_type index,
                    Conf const& config = symphas::conf::config());
 ...
 */
bool is_last_save(iter_type index,
                  Conf const& config = symphas::conf::config());
 ...
 */
Conf restore_checkpoint(param_map_type const& param_map, const char* dir,
                        int index = -1);
 ...
 */
struct Conf : SystemConf
#ifdef WITH_PROC_LIB
    ,
              DataConf
#endif
{
  Conf(std::vector<std::pair<std::string, std::string>> const& options,
       const char* title, const char* dir = "");
  Conf(const char* file);
  Conf(symphas::problem_parameters_type const& parameters, const char* title,
       const char* dir = "");
  Conf(SymPhasSettings const& settings) : SystemConf(settings) {}
 ...
 protected:
  ...
  friend Conf symphas::conf::make_config(const char* file,
                                         param_map_type const& param_map);
  friend Conf symphas::conf::restore_checkpoint(param_map_type const& param_map,

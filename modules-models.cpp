...
param_map_type symphas::build_params() {
  ...
  for (auto& param : param_map) {
    param.second.assign_method->set_default(param.second.parameter);
  }
 ...
 */
void symphas::init(const char* config, const char* const* param_list,
                   int num_params) {
#ifdef USING_MPI
  MPI_Init(NULL, NULL);
#endif
...
#endif
  params::assign(param_map["title"],
                 symphas::conf::config().name_settings.title);
}
 ...
 * \param num_params The number of command line arguments in the list.
 */
void symphas::init(const char* title, const char* const* param_list,
                   size_t num_params) {

...
int main() {
    ...
    double param_values[]{1.0, -0.1};  // Asymmetric coupling
    ...
    auto field01 = model.get_field<0>();
    auto field02 = model.get_field<1>();

    auto [min01, max01] =
        std::minmax_element(field01.values, field01.values + field01.len);
    auto [min02, max02] =
        std::minmax_element(field02.values, field02.values + field02.len);

    std::cout << "\nSimulation created!" << std::endl;
    std::cout << "Field 1 range: [" << *min01 << ", " << *max01 << "]"
              << std::endl;
    std::cout << "Field 2 range: [" << *min02 << ", " << *max02 << "]"
              << std::endl;

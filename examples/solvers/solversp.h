...
decltype(auto) form_expr_one(
    std::tuple<Ss...> const& systems,
    std::pair<Variable<Z, symphas::ref<S>>, E> const& e) const {
  auto&& [sys, equation] = e;
  auto&& [linear, nonlinear] =
      expr::split::by_linear(expr::apply_operators(equation));
  ...
  auto&& nonlinear_scheme = solver_sp::construct_nonlinear<Z, D>(
      systems, B_expression, linear_in_nonZ + non_op + nonlinear, h,
      sys.get().dims);

  auto&& name = expr::get_fourier_name(
      expr::get_op_name(expr::get_variable<Z>(equation)));
  ...
  auto&& scheme =
      expr::make_add(A_term * expr::make_term<Z + sizeof...(Ss)>(
                                  NamedData(std::move(term_ft), name)),
                     solver_sp::sthc_apply_on_scalar<T_src>(nonlinear_scheme));
...
template <size_t En, typename S, typename E,
          typename T_src = typename grid::value_type_of<S>::type,
          size_t D = grid::dimension_of<S>::value>
decltype(auto) form_expr_one(std::pair<S*, len_type> const& systems,
                             std::pair<DynamicVariable<S>, E> const& e) const {
  // auto&& [sys, equation] = e;
  // auto&& [linear, nonlinear] =
  // expr::split::by_linear(expr::apply_operators(equation));
  ////nonlinear.print(stdout);

  // auto&& [linear_in_Z, linear_in_nonZ] =
  // expr::split::separate_var<Z>(linear); auto&& [l_op, non_op] =
  // solver_sp::get_l_op<Z>(linear_in_Z, h);

  // auto&& A_expression = solver_sp::form_A_op<D>(l_op, dt, sys.get().dims);
  // auto&& B_expression = solver_sp::form_B_op<D>(l_op, dt, sys.get().dims);

  // auto&& nonlinear_scheme = solver_sp::construct_nonlinear<Z, vim D>(systems,
  // B_expression, linear_in_nonZ + non_op + nonlinear, h, sys.get().dims);

  // auto&& name =
  // expr::get_fourier_name(expr::get_op_name(expr::get_variable<Z>(equation)));
  ...
  // auto&& A_term = solver_sp::get_A_term<T_src>(A_expression);
  // auto&& scheme = expr::make_add(A_term * expr::make_term<Z +
  // sizeof...(Ss)>(NamedData(std::move(term_ft), name)), nonlinear_scheme);
  // expr::printe(scheme, "spectral scheme");
...
static auto make_solver(symphas::problem_parameters_type const& parameters) {
  ...
  for (iter_type i = 0; i < dim; ++i) {
    h[i] =
        parameters.get_interval_data()[0].at(symphas::index_to_axis(i)).width();
  }
  ...
  if constexpr (!expr::is_nonlinear<E>::value) {
    ...
    if (!warned) {
      fprintf(SYMPHAS_WARN,
              "A provisional equation which is used contains linear terms. "
              "Linear terms may cause incorrect construction of the spectral "
              "operators!\n");
      warned = true;
    }
  }

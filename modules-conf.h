...
bool run_model(M& model, Conf const& config) {
  ...
  run_model(model, n, config.simulation_settings.get_time_step_list());
 ...
 */
template <typename M>
void find_solution(M* models, len_type num_models, Conf const& config,
                   bool plotting_output = true, bool checkpoint = true) {
  auto save_points = symphas::conf::get_save_indices(config);
  find_solution(models, num_models,
                config.simulation_settings.get_time_step_list(),
                config.simulation_settings.save, save_points, config.get_result_dir(),
 ...
 */
template <typename M>
void find_solution(M& model, Conf const& config, bool plotting_output = true,
                   bool checkpoint = true) {
  auto save_points = symphas::conf::get_save_indices(config);
  find_solution(model, config.simulation_settings.get_time_step_list(),
                config.simulation_settings.save, save_points, config.get_result_dir(),
                plotting_output, checkpoint);
}
 ...
 */
template <typename M>
void find_solution(M* models, len_type num_models, bool plotting_output = true,
                   bool checkpoint = true) {
  find_solution(models, num_models, symphas::conf::config(), plotting_output,
                checkpoint);
}
 ...
 */
template <typename M>
void find_solution(M& model, bool plotting_output = true,
                   bool checkpoint = true) {
...
/*!
 * Calls the existing function to write the plotting configuration for the given
 * model, passing parameters from the configuration. See: write_plot_config().
...
void write_plot_config(M const& model, const char* directory,
  ...
  write_plot_config(model, directory, names,
                    symphas::conf::config().simulation_settings.save);
}
...
void write_plot_config(M const& model, const char* const* names) {
  write_plot_config(model, symphas::conf::config().get_result_dir(), names,
                    symphas::conf::config().simulation_settings.save);
}
...
void write_plot_config(M const& model, const char* directory,
  ...
  len_type len = (len_type)symphas::model_num_fields(model);
  char** names = new char* [len] {};

  iter_type i = 0;
  for (auto name : symphas::conf::config().name_settings.get_names(len)) {
    names[i] = nullptr;
    std::swap(names[i], name.data);
    ++i;
  }

  symphas::io::write_plot_config(model, directory, names,
                                 symphas::conf::config().simulation_settings.save);
...
template <typename M>
void write_plot_config(M const& model, const char* directory) {
  write_plot_config(model, directory,
                    symphas::conf::config().simulation_settings.save);
}
...
void write_plot_config(M const& model, SaveParams const& save) {
  write_plot_config(
      model, symphas::conf::config().directory_settings.result_dir, save);
}
...
void write_plot_config(M const& model) {
  write_plot_config(model,
                    symphas::conf::config().directory_settings.result_dir);
}

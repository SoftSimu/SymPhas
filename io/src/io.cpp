...
struct params::param_assign<symphas::IOType> : param_assign_base {
  symphas::IOType default_value;

  void assign(void* param, const char* value) {
    *static_cast<symphas::IOType*>(param) = extract_writer(value);
  }

  void set_default(void* param) {
    default_value = *static_cast<symphas::IOType*>(param);
  }
  ...
  param_assign_base* clone() const {
    return new param_assign<symphas::IOType>(*this);
  }
  ...
  symphas::IOType extract_writer(const char* value) {
    ...
    if (std::strcmp(writer_name, "GNU") == 0) {
      ...
      fprintf(SYMPHAS_WARN,
              "the given writer/reader type '%s' "
              "is not recognized\n",
              writer_name);
    }
...
DLLIO bool* params::single_io_file[2]{&params::single_input_file,
                                      &params::single_output_file};
DLLIO symphas::IOType* params::io_type[2]{&params::reader, &params::writer};

bool add_io_params(param_map_type& param_map) {
  using namespace params;

  param_map["plots-only"] = {
      &plots_only, param_assign<bool>{}, 'p',
      "only outputs plot configuration files, without running the model"};
  param_map["writer-type"] = {
      &writer, param_assign<symphas::IOType>{}, 'w',
      "chooses the writer type from 'gnu', 'xdr', 'column', 'movie', or 'csv'; "
      "use '-x' to set both reader and writer"};
  param_map["writer"] = {&writer, param_assign<symphas::IOType>{}};
  param_map["reader-type"] = {
      &reader, param_assign<symphas::IOType>{}, 'r',
      "chooses the writer type from 'gnu', 'xdr', 'column', 'movie', or 'csv'; "
      "use '-x' to set both reader and writer"};
  param_map["reader"] = {&reader, param_assign<symphas::IOType>{}};
  param_map["use-timestamp"] = {
      &use_timestamp, param_assign<bool>{}, 't',
      "uses the timestamp when generating the output directory"};
  param_map["timestamp"] = {&use_timestamp, param_assign<bool>{}};
  param_map["single-output_file"] = {
      &single_output_file, param_assign<bool>{}, 'o',
      "all data is appended to a single output file; use '-a' to set both "
      "single input and output"};
  param_map["single-input_file"] = {
      &single_input_file, param_assign<bool>{}, 'i',
      "all data is read from a single input file; use '-a' to set both single "
      "input and output"};
  param_map["single-output"] = {&single_output_file, param_assign<bool>{}};
  param_map["single-input"] = {&single_input_file, param_assign<bool>{}};
  param_map["checkpoint"] = {
      &checkpoint, param_assign<char*>{}, 'C',
      "load a checkpoint from this directory with a previous run, additionally "
      "appending ',N' to indicate the iteration index with N"};
  param_map["checkpoint-count"] = {&checkpoint_count, param_assign<int>{}, 'c',
                                   "choose the number of checkpoints to save"};

  param_map["single-io-file"] = {params::single_io_file,
                                 param_assign_multiple<bool, 2>{}, 'a'};

  param_map["io-type"] = {params::io_type,
                          param_assign_multiple<symphas::IOType, 2>{}, 'x'};
...
void symphas::io::parse_save_str(const char* value, SaveParams* save) {
  ...
  if (next0 > value) {
    ...
    if (*next0 == ',') {
      ...
      size_t nc = std::count_if(next0, next0 + std::strlen(next0),
                                [](char c) { return (c == ','); });
...
void symphas::io::save_as_str(SaveParams const* save, char* output,
                              size_t buffer_size) {
  if (save->type == SaveType::LIST) {
    ...
    snprintf(output, buffer_size, "%lf %d %s", save->base, save->get_start(),
             get_save_str(save->type));
  }
}

std::vector<iter_type> symphas::io::get_save_indices(
    SaveParams const& main_save, std::vector<SaveParams> const& other_saves) {
  std::vector<iter_type> saves;

  /* the checkpoint interval, computed by dividing the number of indices to
   * solve by the number of checkpoints to save, then each one will be put in
   */
  ...
  while (index < main_save.get_stop()) {
    iter_type next_checkpoint =
        (ci > 0) ? (index / ci + 1) * ci : main_save.get_stop();
    iter_type min = std::min(next_checkpoint, main_save.next_save(index));
...
iter_type symphas::io::next_save_from_list(
    iter_type index, SaveParams const& main_save,
    std::vector<SaveParams> const& other_saves) {
  return next_save_from_list(
      index, symphas::io::get_save_indices(main_save, other_saves));
}

iter_type symphas::io::next_save_from_list(
    iter_type index, std::vector<iter_type> const& saves) {
...
bool symphas::io::is_last_save(iter_type index,
                               std::vector<iter_type> const& saves) {
  return (!saves.empty()) ? index >= saves.back() : true;
}

iter_type symphas::io::next_checkpoint(iter_type index,
...
bool symphas::io::at_checkpoint(iter_type index, SaveParams const& save) {
  return index == symphas::io::next_checkpoint(index, save) ||
         save.current_save(index);
}

bool symphas::io::is_checkpoint_set() { return params::checkpoint != nullptr; }

void SaveParams::set_params(SaveType type, double base, iter_type start,
                            iter_type stop) {
...
bool SaveParams::index_zero_save(iter_type index) const {
  return (index == params::start_index && init_flag) ||
         get_stop() == params::start_index;
}
...
bool SaveParams::current_save(iter_type index) const {
  return index_zero_save(index) ||
         (index == next_save(index - 1) && index > params::start_index);
}
    ...
    if (stop == start) {
      ...
      switch (type) {
        ...
        case SaveType::DEFAULT:
          ...
          next = std::min(std::max(static_cast<iter_type>(
                                       base * (std::floor(current / base) + 1)),
                                   start),
                          stop);
          break;
        ...
        case SaveType::MULTIPLICATIVE: {
          ...
          int N = (base > stop - start) ? (stop - start)
                                        : static_cast<iter_type>(base);
          ...
          if (!std::isfinite(alph0)) {
            double step = Dn / N;
            next = std::min(
                std::max(static_cast<iter_type>(step * ((current / step) + 1)),
                         start),
                stop);
          } else {
            double end = start;
        ...
        case SaveType::EXP:
          ...
          if (current < 1) {
            ...
            next =
                std::min(std::max(static_cast<iter_type>(next0), start), stop);
          }
          break;
...
iter_type SaveParams::num_saves() const {
  if (type == SaveType::LIST) {
    ...
    if (base == 0) {
      ...
      switch (type) {
        case SaveType::DEFAULT: {
          return static_cast<iter_type>((stop - start) / base +
                                        ((init_flag) ? 1 : 0));
        }
        case SaveType::MULTIPLICATIVE: {
        ...
        case SaveType::EXP: {
          return static_cast<iter_type>(
                     std::log(static_cast<double>(stop - start)) /
                     std::log(base)) +
                 ((init_flag) ? 1 : 0);
        }
        default:

...
struct plotting_helper_impl : plotting_helper {
  virtual size_t get_dim() const override { return cast_const().get_dim(); }
  ...
  virtual len_type get_index(
      std::initializer_list<len_type> const& list) const override {
  ...
  virtual double get_position(Axis ax,
                              const len_type (&coords)[1]) const override {
    return cast_const().get_position(ax, &coords[0]);
  }

  virtual double get_position(Axis ax,
                              const len_type (&coords)[2]) const override {
    return cast_const().get_position(ax, &coords[0]);
  }

  virtual double get_position(Axis ax,
                              const len_type (&coords)[3]) const override {
    return cast_const().get_position(ax, &coords[0]);
  }

  virtual double get_position(
      Axis ax, std::initializer_list<len_type> const& list) const override {
...
template <>
struct symphas::io::plotting_helper_specialized<3>
    : symphas::io::plotting_helper_impl<plotting_helper_specialized<3>> {
  ...
  plotting_helper_specialized(symphas::io::write_info const& winfo,
    ...
    offset[2] = len_type((ginfo.INTERVAL_Z0 - ginfo.DOMAIN_Z0) / h[0]);
    offset[1] = len_type((ginfo.INTERVAL_Y0 - ginfo.DOMAIN_Y0) / h[1]);
    offset[0] = len_type((ginfo.INTERVAL_X0 - ginfo.DOMAIN_X0) / h[2]);
  ...
  len_type get_index(const len_type* coords) const {
    ...
    for (iter_type i = 0; i < 3; ++i) {
      coords_corrected[i] =
          (coords[i] + offset[i] + dims[i]) % dims[i] + offset0[i];
    }
    return grid::index_from_position<3>(
        symphas::arr_l_t<3>{coords_corrected[0], coords_corrected[1],
                            coords_corrected[2]},
        stride);
  }
  ...
  size_t get_dim() const { return 3; }

  len_type get_len(iter_type n) const { return len[n]; }
...
template <>
struct symphas::io::plotting_helper_specialized<2>
    : symphas::io::plotting_helper_impl<plotting_helper_specialized<2>> {
  ...
  plotting_helper_specialized(symphas::io::write_info const& winfo,
    ...
    offset[1] = len_type((ginfo.INTERVAL_Y0 - ginfo.DOMAIN_Y0) / h[1]);
    offset[0] = len_type((ginfo.INTERVAL_X0 - ginfo.DOMAIN_X0) / h[0]);
  ...
  len_type get_index(const len_type* coords) const {
    ...
    return grid::index_from_position<2>(
        symphas::arr_l_t<2>{coords_corrected[0], coords_corrected[1]}, stride);
  }
...
template <>
struct symphas::io::plotting_helper_specialized<1>
    : symphas::io::plotting_helper_impl<plotting_helper_specialized<1>> {
  ...
  len_type get_index(const len_type* coords) const {
    return grid::index_from_position<1>(
        symphas::arr_l_t<1>{((coords[0] + offset[0] + dims[0]) % dims[0]) +
                            offset0[0]},
        stride);
  }
  ...
  bool in_bounds(const len_type* coords) const { return (coords[0] < dims[0]); }

  size_t get_dim() const { return 1; }
...
template <>
struct symphas::io::plotting_helper_specialized<0>
    : symphas::io::plotting_helper_impl<plotting_helper_specialized<0>> {
  ...
  len_type get_index(const len_type* coords) const { return 0; }

  double get_position(Axis ax, const len_type* coords) const { return 0; }

  double get_position(Axis ax, len_type coord) const { return 0; }

  bool in_bounds(const len_type* coords) const { return false; }

  size_t get_dim() const { return 0; }

  len_type get_len(iter_type n) const { return 0; }
...
symphas::io::plotting_helper* symphas::io::new_helper(
    symphas::io::write_info const& winfo,
    symphas::io::block_info const& binfo) {
  switch (binfo.dimension()) {
    case 3:
      return new plotting_helper_specialized<3>(winfo, binfo);
    case 2:
      return new plotting_helper_specialized<2>(winfo, binfo);
    case 1:
      return new plotting_helper_specialized<3>(winfo, binfo);
    default:
      return new plotting_helper_specialized<0>(winfo, binfo);
  }
}
...
symphas::io::plotting_helper* symphas::io::new_helper(
    symphas::io::write_info const& winfo, symphas::grid_info const& ginfo) {
  switch (ginfo.dimension()) {
    case 3:
      return new plotting_helper_specialized<3>(winfo, ginfo);
    case 2:
      return new plotting_helper_specialized<2>(winfo, ginfo);
    case 1:
      return new plotting_helper_specialized<3>(winfo, ginfo);
    default:
      return new plotting_helper_specialized<0>(winfo, ginfo);
  }
}
...
DLLIO double symphas::io::gp::alignment::display_width =
    MULTI_OUTPUT_WIDTH_DEFAULT;
...
DLLIO std::pair<const char*, double*>
    symphas::io::gp::alignment::key_par_pairs[] = {
        {STR(MULTI_OUTPUT_WIDTH_KEY),
         &symphas::io::gp::alignment::display_width},
...
void symphas::io::gp::print_gp_header(int index, size_t id,
                                      symphas::grid_info const& ginfo,
                                      symphas::io::write_info const& winfo,
                                      FILE* f) {
  ...
  for (iter_type k = 0; k < GP_HELPER_LENZ; k++) {
    ...
    for (iter_type j = 0; j < GP_HELPER_LENY; j++) {
      ...
      for (iter_type i = 0; i < GP_HELPER_LENX; i++) {
        ...
        fprintf(
            f, "% 10lE ",
            std::sqrt(grid[ii][0] * grid[ii][0] + grid[ii][1] * grid[ii][1]));
      }
      fprintf(f, "\n");
    }
    if (k < GP_HELPER_LENZ - 1) {
  ...
  for (iter_type k = 0; k < GP_HELPER_LENZ; k++) {
    ...
    for (iter_type j = 0; j < GP_HELPER_LENY; j++) {
      ...
      for (iter_type i = 0; i < GP_HELPER_LENX; i++) {
        ...
        fprintf(f,
                "%." AXIS_OUTPUT_ACCURACY_STR "f %." AXIS_OUTPUT_ACCURACY_STR
                "f %." AXIS_OUTPUT_ACCURACY_STR
                "f "
                "%" DATA_OUTPUT_ACCURACY_STR
                "f "
                "%" DATA_OUTPUT_ACCURACY_STR
                "f "
                "%" DATA_OUTPUT_ACCURACY_STR
...
/*
 * functions to checkpoint the data to a file, separate from recording the data
 * for plotting
 */
...
void symphas::io::gp::save_grid(const scalar_t* grid,
  ...
  len_type N =
      (ginfo.dimension() < 3) ? 1 : ginfo.at(Axis::Z).get_interval_count();
  len_type M =
      (ginfo.dimension() < 2) ? 1 : ginfo.at(Axis::Y).get_interval_count();
...
void symphas::io::gp::save_grid(const vector_t<1>* grid,
                                symphas::io::write_info winfo,
                                symphas::grid_info ginfo) {
  ...
  for (iter_type i = 0; i < ginfo.at(Axis::X).get_interval_count(); i++) {
    fprintf(
        f, "%" CHECKPOINT_OUTPUT_ACCURACY_STR CHECKPOINT_OUTPUT_FORMAT_STR "\n",
        grid[i].v[0]);
  }
  fclose(f);
}
...
void symphas::io::gp::save_grid(const scalar_ptr_t (&grid)[1],
                                symphas::io::write_info winfo,
                                symphas::grid_info ginfo) {
  FILE* f = symphas::io::open_data_file(
      winfo.dir, winfo.index, winfo.id, DataFileType::CHECKPOINT_DATA);

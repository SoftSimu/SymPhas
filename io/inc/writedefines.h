 ...
 *
 * in general, the first string format indicates the type of data contained by
 * the data file, and the second string format is the subdata of that category,
 * most commonly the system id
 */
#define OUTPUT_DATA_FILE_FMT "%s_%s." OUTPUT_DATA_EXTENSION
#define OUTPUT_DATA_FILE_INDEX_FMT \
  "%s_%s_%0" OUTPUT_INDEX_WIDTH_STR "d." OUTPUT_DATA_EXTENSION
 ...
 * when generated for the case of latex output, instead of the simple dataname,
 * additional information is provided in the title including the title of the
 * simulation and the id of the system which is being plotted moreover, the case
 * of latex output changes the output of
 */
...
/* the full file name including directory for the data files generated by the
 * solution in the case of latex output, the title of the simulation preceeds
...
#define OUTPUT_LOC_LATEX_INDEX_FMT \
  OUTPUT_DATA_DIR "/" OUTPUT_DATA_FILE_LATEX_INDEX_FMT

/* the name of the latex file that will be generated by gnuplot when the
 * plotfile is executed
 */
#define OUTPUT_LATEX_FILE_FMT "%s_%s" POSTFIX_ID_FMT "_%d.tex"

//! \cond
#define DECLARE_SAVE_GRID_FUNCTIONS                                           \
  void save_grid(const scalar_t* grid, symphas::io::write_info winfo,         \
                 symphas::grid_info ginfo);                                   \
  void save_grid(const complex_t* grid, symphas::io::write_info winfo,        \
                 symphas::grid_info ginfo);                                   \
  void save_grid(const double_arr2* grid, symphas::io::write_info winfo,      \
                 symphas::grid_info ginfo);                                   \
  void save_grid(const vector_t<3>* grid, symphas::io::write_info winfo,      \
                 symphas::grid_info ginfo);                                   \
  void save_grid(const vector_t<2>* grid, symphas::io::write_info winfo,      \
                 symphas::grid_info ginfo);                                   \
  void save_grid(const vector_t<1>* grid, symphas::io::write_info winfo,      \
                 symphas::grid_info ginfo);                                   \
  void save_grid(const scalar_ptr_t(&grid)[3], symphas::io::write_info winfo, \
                 symphas::grid_info ginfo);                                   \
  void save_grid(const scalar_ptr_t(&grid)[2], symphas::io::write_info winfo, \
                 symphas::grid_info ginfo);                                   \
  void save_grid(const scalar_ptr_t(&grid)[1], symphas::io::write_info winfo, \
                 symphas::grid_info ginfo);

#define DECLARE_SAVE_GRID_PLOTTING_FUNCTIONS                                   \
  void save_grid_plotting(const scalar_t* grid, symphas::io::write_info winfo, \
                          symphas::grid_info ginfo);                           \
  void save_grid_plotting(const complex_t* grid,                               \
                          symphas::io::write_info winfo,                       \
                          symphas::grid_info ginfo);                           \
  void save_grid_plotting(const double_arr2* grid,                             \
                          symphas::io::write_info winfo,                       \
                          symphas::grid_info ginfo);                           \
  void save_grid_plotting(const vector_t<3>* grid,                             \
                          symphas::io::write_info winfo,                       \
                          symphas::grid_info ginfo);                           \
  void save_grid_plotting(const vector_t<2>* grid,                             \
                          symphas::io::write_info winfo,                       \
                          symphas::grid_info ginfo);                           \
  void save_grid_plotting(const vector_t<1>* grid,                             \
                          symphas::io::write_info winfo,                       \
                          symphas::grid_info ginfo);                           \
  void save_grid_plotting(const scalar_ptr_t(&grid)[3],                        \
                          symphas::io::write_info winfo,                       \
                          symphas::grid_info ginfo);                           \
  void save_grid_plotting(const scalar_ptr_t(&grid)[2],                        \
                          symphas::io::write_info winfo,                       \
                          symphas::grid_info ginfo);                           \
  void save_grid_plotting(const scalar_ptr_t(&grid)[1],                        \
                          symphas::io::write_info winfo,                       \
                          symphas::grid_info ginfo);

#define DECLARE_SAVE_GRID_ALL_FUNCTIONS \
  DECLARE_SAVE_GRID_FUNCTIONS DECLARE_SAVE_GRID_PLOTTING_FUNCTIONS
...
struct write_info {
  ...
  write_info(iter_type index, size_t id)
      : write_info{".", index, id, DataFileType::GENERIC_DATA} {}
  ...
  auto domain_left(Axis ax) const { return intervals.at(ax)[0]; }
 ...
 * is written.
 */
void copy_data_file_name(const char* dir, const char* data_name, int index,
                         size_t id, DataFileType type, char* out);

//! Specialization based on an empty (unspecified) data name.
void copy_data_file_name(const char* dir, int index, size_t id,
                         DataFileType type, char* out);
 ...
 * \param type The type of the data that will be written to the file.
 */
FILE* open_data_file(const char* dir, const char* data_name, int index,
                     size_t id, DataFileType type);
 ...
 */
FILE* open_postproc_plot_file(const char* dir, const char* name, size_t id,
                              const char* mode);
 ...
 */
void write_postproc_plot_file(const char*(*sets), len_type n, const char* dir,
                              const char* name, int index, size_t id,
                              const char* prepend = "");

//! Based on write_postproc_plot_file(), used to create a new plot file.
void overwrite_postproc_plot_file(const char*(*sets), len_type n,
                                  const char* dir, const char* name, int index,
                                  size_t id, int entry_index,
                                  const char* prepend = "");

//! Based on write_postproc_plot_file(), used to add to an existing file.
void append_postproc_plot_file(const char* set, const char* dir,
                               const char* name, int index, size_t id,
                               int entry_index, const char* prepend = "");
 ...
 */
template <typename T,
          typename = std::enable_if_t<!std::is_pointer<T>::value, int>>
void save_grid_plotting(const T* values, symphas::io::write_info winfo,
                        symphas::grid_info ginfo);
 ...
 */
template <typename T, size_t N>
void save_grid_plotting(const T (*values)[N], symphas::io::write_info winfo,
                        symphas::grid_info ginfo);
 ...
 */
template <typename T, size_t N>
void save_grid_plotting(T* const (&values)[N], symphas::io::write_info winfo,
                        symphas::grid_info ginfo);
...
template <typename T,
          typename = std::enable_if_t<!std::is_pointer<T>::value, int>>
void save_grid(const T* values, symphas::io::write_info winfo,
               symphas::grid_info ginfo);
 ...
 */
template <typename T, size_t N>
void save_grid(const T (*values)[N], symphas::io::write_info winfo,
               symphas::grid_info ginfo);
 ...
 */
template <typename T, size_t N>
void save_grid(T* const (&values)[N], symphas::io::write_info winfo,
               symphas::grid_info ginfo);
 ...
 * \param names List of labels given to each of the phase fields that are
 * plotted. This list must have at least as many elements as there are systems,
 * otherwise if there are too few names, the function will access invalid
 * memory. \param directory Parent directory containing the plotting
 * configuration file and its enclosing folder. \param save A save object which
 * determines at which indices in the simulation the model system data will be
 * persisted so that the chosen plotting method will include it in the generated
 * plot. This information is used in the configuration to refer to the data at
 ...
 */
// template<typename M>
// void write_plot_config(M const& model, const char* directory, SaveParams
// const& save);
...
template <typename G>
void print_grid(const char* file_name, G const& grid);
...
template <size_t D>
void print_region(FILE* out, grid::region_interval<D> const& region,
                  char inside = 'X', char outside = '.');
template <size_t D>
void print_region(FILE* out, grid::region_interval_multiple<D> const& region,
                  char inside = 'X', char outside = '.');
template <size_t D>
void print_region(grid::region_interval<D> const& region, char inside = 'X',
                  char outside = '.');
template <size_t D>
void print_region(grid::region_interval_multiple<D> const& region,
                  char inside = 'X', char outside = '.');

template <typename T, size_t D>
void print_array(T const* values, const len_type (&dims)[D]);
template <typename T, size_t D>
void print_array(FILE* out, T const* values, const len_type (&dims)[D]);
template <typename T, size_t D>
void print_array(const char* file_name, T const* values,

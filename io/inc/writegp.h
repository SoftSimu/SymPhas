...
#define MULTI_ALIGN_WIDTH (1 - 2 * MULTI_MARGIN_RATIO ) //!< Percentage of space occupied by the plots.
...
#define ADD_ROW_NUM(N) ((MULTI_COLUMN_NUM(N) - COLUMNS_LAST_ROW(N)) % MULTI_COLUMN_NUM(N))

//! Number of rows.
#define MULTI_ROW_NUM(N) (((N) + ADD_ROW_NUM(N)) / MULTI_COLUMN_NUM(N))

//! Computed width of the multiplot.
#define MULTI_WIDTH_LATEX(N) (MULTI_OUTPUT_WIDTH * std::pow(0.85, std::max(0.0, 0.0 + MULTI_ROW_NUM(N) - MULTI_COLUMN_NUM(N))))


#define MULTI_WIDTH_RATIO MULTI_ALIGN_WIDTH
#define MULTI_WIDTH_PLOT(N) (MULTI_WIDTH_RATIO / MULTI_COLUMN_NUM(N) - 2 * MULTI_ALIGN_SEPARATION)
#define MULTI_HEIGHT_RATIO(N) \
	((MULTI_WIDTH_PLOT(N) + 2 * MULTI_ALIGN_SEPARATION) * MULTI_ROW_NUM(N) + COLORBOX_HEIGHT_RATIO * MULTI_WIDTH_RATIO)
...
1 - (((MULTI_HEIGHT_PLOT(N) + 2 * MULTI_ALIGN_SEPARATION) / MULTI_HEIGHT_RATIO(N)) * (y + 1) - MULTI_ALIGN_SEPARATION / MULTI_HEIGHT_RATIO(N)), \
MULTI_MARGIN_RATIO + ((MULTI_WIDTH_PLOT(N) + 2 * MULTI_ALIGN_SEPARATION)) * (x) + MULTI_ALIGN_SEPARATION, \
MULTI_MARGIN_RATIO + ((MULTI_WIDTH_PLOT(N) + 2 * MULTI_ALIGN_SEPARATION)) * (x + 1) - MULTI_ALIGN_SEPARATION

#define MULTI_COLORBOX_FMT "set colorbox horizontal user origin %.3lf,%.3lf size %.3lf,%.3lf\n"
...
(COLORBOX_HEIGHT_RATIO - COLORBOX_BOTTOM_PADDING) / MULTI_HEIGHT_RATIO(N)
...
#define SINGLE_ALIGN_V_MARGIN (((MULTI_HEIGHT_PLOT(1) / MULTI_HEIGHT_RATIO(1)) - SINGLE_ALIGN_HEIGHT_PR) / 2)

#define SINGLE_ALIGN_MARGIN_VALUES \
1 - (MULTI_ALIGN_SEPARATION / MULTI_HEIGHT_RATIO(1)) - SINGLE_ALIGN_V_MARGIN, \
1 - (SINGLE_ALIGN_HEIGHT_PR + MULTI_ALIGN_SEPARATION / MULTI_HEIGHT_RATIO(1)) - SINGLE_ALIGN_V_MARGIN, \
(1 - SINGLE_ALIGN_HEIGHT_PR) / 2, \
(1 + SINGLE_ALIGN_HEIGHT_PR) / 2

#define SINGLE_COLORBOX_VALUES \
(1 - SINGLE_ALIGN_HEIGHT_PR) / 2, \
SINGLE_ALIGN_V_MARGIN / 2 + (COLORBOX_BOTTOM_PADDING / MULTI_HEIGHT_RATIO(1)), \
(SINGLE_ALIGN_HEIGHT_PR), \
(COLORBOX_HEIGHT_RATIO - COLORBOX_BOTTOM_PADDING) / MULTI_HEIGHT_RATIO(1) / 2
...
namespace symphas::io::gp
{
	namespace alignment
	{
		...
		DLLIO extern double display_width;	//!< Base width value, in inches, of the output.
		DLLIO extern double mmr;			//!< The percentage of space which is the buffer on each side, relative to width.
		DLLIO extern double mas;			//!< Percentage of space between each subplot, relative to the width.
		DLLIO extern double chr;			//!< Height of the colorbox as a percentage of the width.
		DLLIO extern double cbp;			//!< Amount of space given to the colorbox legend, as a percentage of the width.
		...
		inline void load_parameters()
		{
			...
			{
				fprintf(SYMPHAS_INFO, "using alignments file for latex plot "
					"configuration, %s\n", ALIGNMENT_FILE_NAME);
				...
				while (fgets(buffer, LINE_READ_BUFFER, f) != 0)
				{
					...
					if (!key_matches)
					{
						fprintf(SYMPHAS_INFO, "the key '%s' is not a valid "
							"alignment option\n", key);
					}
				}
				fclose(f);
...
R"~(
	...
	void print_plot_ranges(FILE* gnu, symphas::interval_data_type const& intervals)
	{
		fprintf(gnu, gnuset,
			...
			INTERVAL_Y0, INTERVAL_Yn - INTERVAL_Y0, INTERVAL_Yn - INTERVAL_Yh
		);
	}
	...
	void write_plot_config(Model<D, Sp, S...> const& model, const char* directory, const char* const* names, SaveParams const& save)
	{
		for (size_t id = 0; id < sizeof...(S); ++id)
		{
			...
			char plot_name[BUFFER_LENGTH]{},	// name of the plot file
				data_name[BUFFER_LENGTH_R2]{},	// the data name of the plot file, composed of GRID and id
				title_name[BUFFER_LENGTH_R2]{},	// the title of the simulation formatted for filenames
			...
			snprintf(latex_name, sizeof(latex_name) / sizeof(char), OUTPUT_LATEX_FILE_FMT, title_name, PHASEFIELD_DATA_NAME, id, save.get_stop());
			snprintf(fid, sizeof(fid) / sizeof(char), POSTFIX_ID_FMT, id);
			snprintf(data_name, sizeof(data_name) / sizeof(char), "%s%zd", PHASEFIELD_DATA_NAME, id);
			snprintf(plot_name, sizeof(plot_name) / sizeof(char), PHASEFIELD_PLOT_LOC_FMT, directory, data_name);
			...
			if (saves > 1)
			{
				fprintf(gnu, gnu_multi,
					MULTI_PLOT_KEY_VPOS,
					MULTI_WIDTH_LATEX(saves),
					MULTI_HEIGHT_LATEX(saves),
					latex_name,
					MULTI_ROW_NUM(saves),
				...
				for (iter_type index = save.get_start(), i = 0; i < saves; index = save.next_save(index), ++i)
				{
					...
					/* if this is the final save, print additional plot configuration to load the colorbar
						* as well as correctly align the colorbar
						*/
					if (i == saves - 1)
					{
			...
			{
				char display_title[BUFFER_LENGTH];
				snprintf(display_title, sizeof(display_title) / sizeof(char), "%s---" LATEX_KEY_FMT, params::title, names[id], save.get_start());
				
				fprintf(gnu, gnu_single,
					display_title,
					SINGLE_WIDTH_LATEX, SINGLE_WIDTH_LATEX,
					latex_name);

...
constexpr auto choose() {
  if constexpr (K == N || K == 0) {
    ...
    using num = symphas::lib::seq_add_t<
        symphas::lib::seq_repeating_value_t<K, size_t, N - K + 1>,
        std::make_index_sequence<K>>;

    using den = symphas::lib::seq_add_t<
        symphas::lib::seq_repeating_value_t<K, size_t, 1>,
        std::make_index_sequence<K>>;

    return expr::make_fraction<multiply_fold_seq(num{}),
                               multiply_fold_seq(den{})>();
  }
}
...
template <size_t N, typename E,
          typename = std::enable_if_t<(N < MAX_EXPONENT), int>>
...
template <size_t N, typename E0, typename... Es,
          typename = std::enable_if_t<(N > 1 && N < MAX_EXPONENT), int>>
 ...
 * \param e The expression to multiply with itself.
 */
template <size_t N, size_t NN, size_t DD,
          typename = std::enable_if_t<(N > 1 && N < MAX_EXPONENT), int>>
...
auto pow_add(E0 const& a, OpAddList<E1, E2, Es...> const& b,
             std::index_sequence<Ns...>) {
  constexpr size_t N0 = sizeof...(Ns) - 1;
  return ((expr::choose<N0, Ns>() * expr::pow<N0 - Ns>(a) *
           expr::make_pow<Ns>(OpAdd(b))) +
          ...);
}
...
auto pow_add(E0 const& a, E1 const& b, std::index_sequence<Ns...>) {
  constexpr size_t N0 = sizeof...(Ns) - 1;
  return expr::make_add((expr::choose<N0, Ns>() * expr::make_pow<N0 - Ns>(a) *
                         expr::make_pow<Ns>(b))...);
}
...
auto pow(OpAdd<E0, Es...> const& e) {
  return pow_add(expr::get<0>(e), expr::terms_after_first(e),
...
auto pow(OpTerms<V, Term<Gs, Xs>...> const& e) {
  return expr::pow<N>(e.term) *
         pow_terms<N>(expr::terms_after_first(e),
...
auto pow_x(OpExpression<E> const& e) {
  return pow<expr::_Xk_t<X>::N, expr::_Xk_t<X>::D, expr::_Xk_t<X>::sign>(
...
struct make_function {
  ...
  template <typename V, typename coeff_t,
            typename = std::enable_if_t<(expr::is_coeff<coeff_t> ||
                                         std::is_same<OpVoid, coeff_t>::value),
                                        int>>
...
auto make_function(V const& v, OpExpression<E> const& e, std::string name, F f,
                   std::tuple<Arg0, Args...> const& args) {
  return OpFunction<V, E, F, Arg0, Args...>(v, *static_cast<E const*>(&e), f,
                                            args);
}
...
auto make_function(OpExpression<E> const& e, std::string name, F f,
                   std::tuple<Arg0, Args...> const& args) {
  return OpFunction<OpIdentity, E, F, Arg0, Args...>(
      OpIdentity{}, *static_cast<E const*>(&e), f, args);
}
...
auto make_function(V const& v, OpExpression<E> const& e, std::string name,
                   F f) {
...
template <typename V, typename E, typename F>
auto make_function(OpExpression<E> const& e, std::string name, F f) {
  return OpFunction<OpIdentity, E, F, void>(OpIdentity{},
                                            *static_cast<E const*>(&e), f);
}
...
template <typename E>
auto constexpr func_conjugate =
    &symphas::math::conj<typename expr::eval_type<E>::type>;

//! Alias to construct a modulus function to compute the modulus.
template <typename E>
auto constexpr func_modulus =
    &symphas::math::modulus<typename expr::eval_type<E>::type>;

//! Alias to construct the function to return real part.
template <typename E>
auto constexpr func_realpart =
    &symphas::math::real<typename expr::eval_type<E>::type>;

//! Alias to construct the function to return imaginary part.
template <typename E>
auto constexpr func_imagpart =
    &symphas::math::imag<typename expr::eval_type<E>::type>;

//! Alias to construct the function to compute the natural logarithm function.
template <typename E>
auto constexpr func_log =
    &symphas::math::log<typename expr::eval_type<E>::type>;

//! Alias to construct the function to compute the cosine function.
template <typename E>
auto constexpr func_cos =
    &symphas::math::cos<typename expr::eval_type<E>::type>;

//! Alias to construct the function to compute the sine function.
template <typename E>
auto constexpr func_sin =
    &symphas::math::sin<typename expr::eval_type<E>::type>;

//! Alias to construct the function to compute the sine function.
template <typename E>
auto constexpr func_tan =
    &symphas::math::tan<typename expr::eval_type<E>::type>;

//! Alias to construct the function to compute the sine function.
template <typename E>
auto constexpr func_csc =
    &symphas::math::csc<typename expr::eval_type<E>::type>;

//! Alias to construct the function to compute the sine function.
template <typename E>
auto constexpr func_sec =
    &symphas::math::sec<typename expr::eval_type<E>::type>;

//! Alias to construct the function to compute the sine function.
template <typename E>
auto constexpr func_cot =
    &symphas::math::cot<typename expr::eval_type<E>::type>;

//! Alias to construct the function to compute the cosine function.
template <typename E>
auto constexpr func_cosh =
    &symphas::math::cosh<typename expr::eval_type<E>::type>;

//! Alias to construct the function to compute the sine function.
template <typename E>
auto constexpr func_sinh =
    &symphas::math::sinh<typename expr::eval_type<E>::type>;

//! Alias to construct the function to compute the sine function.
template <typename E>
auto constexpr func_tanh =
    &symphas::math::tanh<typename expr::eval_type<E>::type>;

//! Alias to construct the function to compute the cosine function.
template <typename E>
auto constexpr func_acos =
    &symphas::math::acos<typename expr::eval_type<E>::type>;

//! Alias to construct the function to compute the sine function.
template <typename E>
auto constexpr func_asin =
    &symphas::math::asin<typename expr::eval_type<E>::type>;

//! Alias to construct the function to compute the sine function.
template <typename E>
auto constexpr func_atan =
    &symphas::math::atan<typename expr::eval_type<E>::type>;

//! Alias to construct the function to compute the cosine function.
template <typename E>
auto constexpr func_acosh =
    &symphas::math::acosh<typename expr::eval_type<E>::type>;

//! Alias to construct the function to compute the sine function.
template <typename E>
auto constexpr func_asinh =
    &symphas::math::asinh<typename expr::eval_type<E>::type>;

//! Alias to construct the function to compute the sine function.
template <typename E>
auto constexpr func_atanh =
    &symphas::math::atanh<typename expr::eval_type<E>::type>;
...
auto conj(OpExpression<E> const& e) {
  return symphas::internal::make_function<func_conjugate<E>>::get(
      *static_cast<E const*>(&e));
}

template <typename E>
auto modulus(OpExpression<E> const& e) {
  return symphas::internal::make_function<func_modulus<E>>::get(
      *static_cast<E const*>(&e));
}

template <typename E>
auto real(OpExpression<E> const& e) {
  return symphas::internal::make_function<func_realpart<E>>::get(
      *static_cast<E const*>(&e));
}

template <typename E>
auto imag(OpExpression<E> const& e) {
  return symphas::internal::make_function<func_imagpart<E>>::get(
      *static_cast<E const*>(&e));
}

template <typename E>
auto log(OpExpression<E> const& e) {
  return symphas::internal::make_function<func_log<E>>::get(
      *static_cast<E const*>(&e));
}

template <typename E>
auto cos(OpExpression<E> const& e) {
  return symphas::internal::make_function<func_cos<E>>::get(
      *static_cast<E const*>(&e));
}

template <typename E>
auto sin(OpExpression<E> const& e) {
  return symphas::internal::make_function<func_sin<E>>::get(
      *static_cast<E const*>(&e));
}

template <typename E>
auto tan(OpExpression<E> const& e) {
  return symphas::internal::make_function<func_tan<E>>::get(
      *static_cast<E const*>(&e));
}

template <typename E>
auto cosh(OpExpression<E> const& e) {
  return symphas::internal::make_function<func_cosh<E>>::get(
      *static_cast<E const*>(&e));
}

template <typename E>
auto sinh(OpExpression<E> const& e) {
  return symphas::internal::make_function<func_sinh<E>>::get(
      *static_cast<E const*>(&e));
}

template <typename E>
auto tanh(OpExpression<E> const& e) {
  return symphas::internal::make_function<func_tanh<E>>::get(
      *static_cast<E const*>(&e));
}

template <typename E>
auto acos(OpExpression<E> const& e) {
  return symphas::internal::make_function<func_acos<E>>::get(
      *static_cast<E const*>(&e));
}

template <typename E>
auto asin(OpExpression<E> const& e) {
  return symphas::internal::make_function<func_asin<E>>::get(
      *static_cast<E const*>(&e));
}

template <typename E>
auto atan(OpExpression<E> const& e) {
  return symphas::internal::make_function<func_atan<E>>::get(
      *static_cast<E const*>(&e));
}

template <typename E>
auto acosh(OpExpression<E> const& e) {
  return symphas::internal::make_function<func_acosh<E>>::get(
      *static_cast<E const*>(&e));
}

template <typename E>
auto asinh(OpExpression<E> const& e) {
  return symphas::internal::make_function<func_asinh<E>>::get(
      *static_cast<E const*>(&e));
}

template <typename E>
auto atanh(OpExpression<E> const& e) {
  return symphas::internal::make_function<func_atanh<E>>::get(
      *static_cast<E const*>(&e));
}
...
struct OpFunction : OpExpression<OpFunction<V, E, F, Arg0, Args...>> {
   ...
   */
  OpFunction(std::string name, V value, E const& e, F f, Arg0&& arg0,
             Args&&... args)
      : name{name},
        value{value},
        e{e},
        f{f},
        tt{std::forward<Arg0>(arg0), std::forward<Args>(args)...} {}
  ...
  inline auto eval(iter_type n) const {
    return expr::eval(value) *
           function_value(n, std::make_index_sequence<sizeof...(Args) + 1>{});
  }
  ...
  size_t print_length() const {
    return expr::coeff_print_length(value) + e.print_length() + name.length() +
           SYEX_LAMBDA_FUNC_FMT_LEN;
  }
...
struct OpFunction<V, E, F, void> : OpExpression<OpFunction<V, E, F, void>> {
   ...
   * \param f The function pointer or functor object instance.
   */
  OpFunction(V value, E const& e, F f) : value{value}, e{e}, f{f}, name{"?"} {}
   ...
   * \param f The function pointer or functor object instance.
   */
  OpFunction(E const& e, F f) : value{V{}}, e{e}, f{f}, name{"?"} {}

  inline auto eval(iter_type n) const { return expr::eval(value) * f(e, n); }
...
template <typename coeff_t, typename V2, typename E2, typename F2, typename Arg,
          ...
          typename std::enable_if_t<
              (expr::is_coeff<coeff_t> && !expr::is_tensor<V2>), int> = 0>
auto operator*(coeff_t const& value,
               OpFunction<V2, E2, F2, Arg, Args...> const& b) {
...
template <typename coeff_t, typename tensor_t, typename E2, typename F2,
          ...
          typename std::enable_if_t<
              (expr::is_coeff<coeff_t> && expr::is_tensor<tensor_t>), int> = 0>
auto operator*(coeff_t const& value,
               OpFunction<tensor_t, E2, F2, Arg, Args...> const& b) {
...
template <typename coeff_t, typename tensor_t, typename E2, typename F2,
          typename std::enable_if_t<
              (expr::is_coeff<coeff_t> && expr::is_tensor<tensor_t>), int> = 0>
auto operator*(coeff_t const& value,
               OpFunction<tensor_t, E2, F2, void> const& b) {
  return (value * b.value) * OpFunction(b.name, OpIdentity{}, b.e, b.f);
}

template <typename V1, typename E1, typename F1, typename V2>
auto operator*(OpFunction<V2, E1, F1, void> const& a,
               OpFunction<V2, E1, F1, void> const& b) {
  return (a.value + b.value) * OpFunction(b.name, OpIdentity{}, a.e, a.f);
}

template <typename V1, typename E1, typename F1, typename Arg, typename... Args,
          typename V2>
auto operator*(OpFunction<V1, E1, F1, Arg, Args...> const& a,
               OpFunction<V2, E1, F1, Arg, Args...> const& b) {

...
template <typename G>
struct make_data {
  template <typename GG = G,
            std::enable_if_t<std::is_default_constructible_v<GG>, int> = 0>
  G operator()() {
    return {};
  }
...
struct NamedData : G {
  ...
  NamedData(G const& data, const char* name)
      : G(data),
        name{(name != nullptr && std::strlen(name) > 0)
                 ? new char[std::strlen(name) + 1]{}
                 : nullptr} {
  ...
  NamedData(G const& data) : NamedData(data, nullptr) {}
  NamedData() : NamedData(symphas::internal::make_data<G>{}(), nullptr) {}
   ...
   */
  template <typename E>
  NamedData(G const& data, OpExpression<E> const& e)
      : NamedData(data, nullptr) {
  ...
  NamedData(G data = symphas::internal::make_data<G>{}())
      : NamedData(data, nullptr) {}
...
struct NamedData<G*> {
  ...
  NamedData(NamedData<G*> const& other) : NamedData(other.data, other.name) {}

  NamedData(NamedData<G*>&& other) : NamedData() { swap(*this, other); }
  ...
  operator G*() { return data; }

  operator G const*() const { return data; }
 ...
 * so the data object must mimic the ::Scalar_t instead instead, and store the
 * pointer. To this end, deallocating the original pointer array would also make
 * this object invalid.
 */
template <>
struct NamedData<scalar_t> {
  ...
  operator scalar_t() const { return data; }
};
...
struct Variable : G {
  ...
  Variable(G data) noexcept : G(data) {}
};
...
struct Variable<Z, G*> {
  ...
  operator const G*() const { return data; }
...
struct Variable<Z, symphas::ref<G>> : symphas::ref<G> {
  ...
  Variable(symphas::ref<G> data) : parent_type(data) {}
  Variable(G const& data) noexcept : parent_type(data) {}
  Variable(G&& data) noexcept : parent_type(data) {}
  constexpr Variable() : Variable(*((G*)0)) {}
};

template <typename G>
struct DynamicVariable : DynamicIndex {
  constexpr DynamicVariable() : DynamicIndex(), data{nullptr} {}
  DynamicVariable(DynamicIndex const& index, G* data)
      : DynamicIndex(index), data{data} {}

  const auto& get() const { return data[*DynamicIndex::data]; }

  auto& get() { return data[*DynamicIndex::data]; }
...
struct DynamicVariable<NamedData<G*>> : DynamicIndex {
  constexpr DynamicVariable() : DynamicIndex(), data{nullptr} {}
  DynamicVariable(DynamicIndex const& index, NamedData<G*> const& data)
      : DynamicIndex(index), data{data} {}

  const auto& get() const { return data.data[*DynamicIndex::data]; }

  auto& get() { return data.data[*DynamicIndex::data]; }
...
struct DynamicVariable<Variable<Z, G*>> : DynamicIndex {
  constexpr DynamicVariable() : DynamicIndex(), data{nullptr} {}
  DynamicVariable(DynamicIndex const& index, Variable<Z, G*> const& data)
      : DynamicIndex(index), data{data} {}
...
template <size_t Z, typename G>
DynamicVariable(DynamicIndex, Variable<Z, G*>)
    -> DynamicVariable<Variable<Z, G*>>;
...
struct Term : G {
  ...
  inline auto eval(iter_type n) const {
    if constexpr (expr::is_symbol<G>) {
      ...
      if constexpr (expr::_Xk_t<X>::D == 1) {
        auto result =
            pow<size_t(expr::_Xk_t<X>::N)>(expr::BaseData<G>::get(data(), n));
  ...
  size_t print(FILE* out) const {
    if constexpr (expr::_Xk<X> == 1) {
      ...
      return fprintf(out, "%s" SYEX_POW_SEP_A "%u" SYEX_POW_SEP_B,
                     expr::get_op_name(data()), expr::_Xk_t<X>::N);
    } else {
      return fprintf(out, "%s" SYEX_POW_SEP_A "%u/%u" SYEX_POW_SEP_B,
                     expr::get_op_name(data()), expr::_Xk_t<X>::N,
                     expr::_Xk_t<X>::D);
    }
  }
  ...
  size_t print(char* out) const {
    if constexpr (expr::_Xk<X> == 1) {
      return sprintf(out, "%s",
                     expr::get_op_name(*static_cast<G const*>(this)));
    } else if constexpr (expr::_Xk_t<X>::D == 1) {
      return sprintf(out, "%s" SYEX_POW_SEP_A "%u" SYEX_POW_SEP_B,
                     expr::get_op_name(data()), expr::_Xk_t<X>::N);
    } else {
      return sprintf(
          out, "%s" SYEX_POW_SEP_A "%u" SYEX_POW_DIV_SEP "%u" SYEX_POW_SEP_B,
          expr::get_op_name(data()), expr::_Xk_t<X>::N, expr::_Xk_t<X>::D);
    }
  }
  ...
  Term<G, X> const& operator*(OpIdentity) const { return *this; }

  Term<G, X>& operator*(OpIdentity) { return *this; }

  auto& data() { return *static_cast<G*>(this); }

  const auto& data() const { return *static_cast<G const*>(this); }
};
...
struct Term<T*, X> : Term<symphas::pointer_type<T>, X> {
  ...
  Term<T*, X> const& operator*(OpIdentity) const { return *this; }

  Term<T*, X>& operator*(OpIdentity) { return *this; }
};
...
template <>
struct OpTermsList<> {
  inline auto _eval(iter_type) const { return OpIdentity{}; }

#ifdef PRINTABLE_EQUATIONS

  size_t print(FILE* out) const { return 0; }

  size_t print(char* out) const { return 0; }
...
struct cast_term<symphas::lib::types_list<Term<Gs, Xs>...>> {
  ...
  static OpTerms<Term<Gs, Xs>...> cast(
      OpTerms<V, Term<G0s, X0s>...> const& terms) {
  ...
  template <typename V, typename... G0s, expr::exp_key_t... X0s>
  static OpTermsList<Term<Gs, Xs>...> const& cast(
      OpTermsList<V, Term<G0s, X0s>...> const& terms) {
    return *static_cast<OpTermsList<Term<Gs, Xs>...> const*>(&terms);
  }

  template <typename V, typename... G0s, expr::exp_key_t... X0s>
  static OpTermsList<Term<Gs, Xs>...>& cast(
      OpTermsList<V, Term<G0s, X0s>...>& terms) {
    return *static_cast<OpTermsList<Term<Gs, Xs>...>*>(&terms);
  }
};
...
inline auto to_term(OpTermsList<>) { return OpIdentity{}; }
...
const auto& get(OpTerms<V, Term<Gs, Xs>...> const& e) {
  using Nth_type = typename symphas::internal::Nth_type_of_terms<
      N, OpTerms<V, Term<Gs, Xs>...>>::type;
...
// template<typename G0, exp_key_t X0, typename G1, exp_key_t X1, typename...
// Gs, exp_key_t... Xs> const auto& terms_after_first(OpTerms<Term<G0, X0>,
// Term<G1, X1>, Term<Gs, Xs>...> const& e)
//{
//     using cast_to_t = OpTerms<Term<G0, X0>, Term<G1, X1>, Term<Gs, Xs>...>;
//	//return symphas::internal::cast_term<Nth_type>::cast(e).term;
//     return e;
//     //*static_cast<OpTerms<Term<G0, X0>, Term<G1, X1>, Term<Gs, Xs>...>
//     const*>(&e);
// }

// template<typename G0, exp_key_t X0, typename G1, exp_key_t X1, typename...
// Gs, exp_key_t... Xs> auto& terms_after_first(OpTerms<Term<G0, X0>, Term<G1,
// X1>, Term<Gs, Xs>...>& e)
//{
//     return e;
//	//return *static_cast<OpTerms<Term<G0, X0>, Term<G1, X1>, Term<Gs,
// Xs>...>*>(&e);
// }
...
decltype(auto) terms_after_n(OpTerms<V, Term<Gs, Xs>...> const& e) {
  return symphas::internal::to_term(terms_after_n<N>(
...
template <typename G, expr::exp_key_t X1, expr::exp_key_t X2,
          typename std::enable_if_t<!expr::has_identity<G, G>, int> = 0>
...
struct OpTermsList<V, Term<Gs, Xs>...> : OpTermsList<Term<Gs, Xs>...> {
  ...
  OpTermsList(V const& e, Term<Gs, Xs> const&... terms)
      : parent_type(terms...), term{e} {}
  OpTermsList(V const& e, OpTerms<Term<Gs, Xs>...> const& rest)
      : parent_type(*static_cast<OpTermsList<Term<Gs, Xs>...> const*>(&rest)),
        term{e} {}
  OpTermsList(OpTerms<V, Term<Gs, Xs>...> const& list)
      : parent_type(*static_cast<OpTermsList<Term<Gs, Xs>...> const*>(&list)),
        term{list.term} {}
  ...
  inline auto _eval(iter_type n) const { return _eval(term, n); }
...
struct OpTerms<V, Term<Gs, Xs>...> : OpExpression<OpTerms<V, Term<Gs, Xs>...>>,
  ...
  //	: parent_type(terms) {}
  OpTerms(OpTermsList<V, Term<Gs, Xs>...> const& terms) : parent_type(terms) {}

  template <typename E, size_t N = sizeof...(Gs),
            typename std::enable_if_t<(N == 1), int> = 0>
  void operator=(OpExpression<E> const& e) {
    expr::result(*static_cast<E const*>(&e),
...
template <typename V, typename... Gs, expr::exp_key_t... Xs, typename G0,
          expr::exp_key_t X0>
OpTerms(OpTerms<V, Term<Gs, Xs>...>, Term<G0, X0>)
    -> OpTerms<V, Term<G0, X0>, Term<Gs, Xs>...>;
...
auto to_vector_component(Variable<Z, G> const& data) {
  return expr::as_variable<Z>(
      to_vector_component<ax>(*static_cast<G const*>(&data)));
  ;
}

template <Axis ax, typename G>
auto to_vector_component(DynamicVariable<G> const& data) {
  return expr::as_variable(
      to_vector_component<ax>(*static_cast<G const*>(&data)));
  ;
}
...
auto to_vector_component(Term<G, X> const& term) {
  return to_term_element<X>(
      to_vector_component<ax>(*static_cast<G const*>(&term)));
}

template <Axis ax, typename... Gs, expr::exp_key_t... Xs, size_t... Ns,
          size_t M0, size_t... Ms>
auto terms_make_component(OpTerms<Term<Gs, Xs>...> const& b,
                          std::index_sequence<Ns...>,
                          std::index_sequence<M0, Ms...>) {
  return OpTerms(OpIdentity{}, expr::get<Ns>(b)...,
                 to_vector_component<ax>(expr::get<M0>(b)),
                 expr::get<Ms>(b)...);
}
...
auto terms_make_component(OpTerms<Term<Gs, Xs>...> const& b,
  ...
  using mask_t = symphas::lib::seq_join_t<
      std::index_sequence<>,
      std::conditional_t<(expr::eval_type<Term<Gs, Xs>>::rank == 0),
                         std::index_sequence<>, std::index_sequence<Is>>...>;
  constexpr size_t M0 =
      symphas::lib::seq_index_value<mask_t::size() - 1, mask_t>::value;

  return terms_make_component<ax>(
      b, std::make_index_sequence<M0>{},
...
template <typename coeff_t, typename tensor_t, typename... Gs,
          expr::exp_key_t... Xs,
          typename std::enable_if_t<
              (expr::is_coeff<coeff_t> && expr::is_tensor<tensor_t>), int> = 0>
auto operator*(coeff_t const& value,
               OpTerms<tensor_t, Term<Gs, Xs>...> const& b) {
...
template <typename T, typename V, typename... Gs, expr::exp_key_t... Xs,
          ...
          typename std::enable_if_t<
              (!expr::is_tensor<V> &&
               expr::eval_type<OpTerms<Term<Gs, Xs>...>>::rank > 0),
              int> = 0>
auto operator*(OpTensor<T, N0, N1, N, D> const& tensor,
               OpTerms<V, Term<Gs, Xs>...> const& b) {
  constexpr Axis ax = (N1 == 0)   ? Axis::X
                      : (N1 == 1) ? Axis::Y
                      : (N1 == 2) ? Axis::Z
                                  : Axis::NONE;
  if constexpr (N == 1) {
    ...
    auto coeff =
        expr::make_tensor<N0, N>(symphas::internal::tensor_cast::cast(tensor)) *
        expr::coeff(b);
    return coeff * symphas::internal::terms_make_component<ax>(
                       expr::terms_after_first(b),
                       std::make_index_sequence<sizeof...(Gs)>{});
...
auto operator*(OpTensor<T, N0, N1, N, M> const& tensor,
               OpTerms<V, Term<Gs, Xs>...> const& b) = delete;

template <typename E, typename G0, expr::exp_key_t X0, typename... Gs,
          expr::exp_key_t... Xs>
auto operator*(OpExpression<E> const& a,
               OpTerms<Term<G0, X0>, Term<Gs, Xs>...> const& b) {
  return (*static_cast<E const*>(&a)) * OpTerms(OpIdentity{}, b);
}

template <typename E, typename G0, expr::exp_key_t X0, typename... Gs,
          expr::exp_key_t... Xs>
auto operator*(OpTerms<Term<G0, X0>, Term<Gs, Xs>...> const& a,
...
auto invert_terms(OpTerms<V, Term<Gs, Xs>...> const& b,
                  std::index_sequence<Is...>) {
  return OpTerms(~expr::get<Is>(b)...);
}

inline auto combine_terms(std::index_sequence<>, OpTerms<> const& terms_num,
                          OpTerms<> const& terms_den) {
  return OpIdentity{};
}

template <typename... Gs, expr::exp_key_t... Xs, typename G, expr::exp_key_t X,
          size_t... P0s, size_t... P1s>
auto place_at(OpTerms<Term<Gs, Xs>...> const& terms, Term<G, X> const& term0,
              std::index_sequence<P0s...>, std::index_sequence<P1s...>) {
  return OpTerms(expr::get<P0s>(terms)..., term0,
                 expr::get<P1s + sizeof...(P0s)>(terms)...);
}

template <size_t P, typename... Gs, typename G, expr::exp_key_t X,
          expr::exp_key_t... Xs>
auto place_at(OpTerms<Term<Gs, Xs>...> const& terms, Term<G, X> const& term0) {
  return place_at(terms, term0, std::make_index_sequence<P>{},
                  std::make_index_sequence<sizeof...(Gs) - P>{});
}
...
inline auto combine_terms() { return OpTerms<>(); }

template <typename G02, expr::exp_key_t X02, typename... G2s,
          expr::exp_key_t... X2s>
auto combine_terms(
    std::index_sequence<>, OpTerms<> const& terms_num,
    OpTerms<Term<G02, X02>, Term<G2s, X2s>...> const& terms_den) {
  return expr::make_div(OpIdentity{}, OpTerms(OpIdentity{}, terms_den));
}

template <typename G01, expr::exp_key_t X01, typename... G1s,
          expr::exp_key_t... X1s>
auto combine_terms(std::index_sequence<>,
                   OpTerms<Term<G01, X01>, Term<G1s, X1s>...> const& terms_num,
                   OpTerms<> const& terms_den) {
  return OpTerms(OpIdentity{}, terms_num);
}

template <typename G01, expr::exp_key_t X01, typename... G1s,
          expr::exp_key_t... X1s, typename G02, expr::exp_key_t X02,
          typename... G2s, expr::exp_key_t... X2s>
auto combine_terms(
    std::index_sequence<>,
    OpTerms<Term<G01, X01>, Term<G1s, X1s>...> const& terms_num,
    OpTerms<Term<G02, X02>, Term<G2s, X2s>...> const& terms_den) {
  return OpBinaryDiv(OpTerms(OpIdentity{}, terms_num),
                     OpTerms(OpIdentity{}, terms_den));
}

template <size_t P0, size_t... Ps, typename... G1s, expr::exp_key_t... X1s,
          typename... G2s, expr::exp_key_t... X2s, typename G2,
          expr::exp_key_t X2, typename... Ts>
auto combine_terms(std::index_sequence<P0, Ps...>,
                   OpTerms<Term<G1s, X1s>...> const& terms_num,
                   OpTerms<Term<G2s, X2s>...> const& terms_den,
                   Term<G2, X2> const& term0, Ts&&... rest) {
  if constexpr (X2 == 0) {
    return combine_terms(std::index_sequence<Ps...>{}, terms_num, terms_den,
                         std::forward<Ts>(rest)...);
  } else {
    if constexpr (expr::_Xk_t<X2>::sign) {
      return combine_terms(std::index_sequence<Ps...>{}, terms_num,
                           OpTerms(~term0, terms_den),
                           std::forward<Ts>(rest)...);
    } else {
      return combine_terms(std::index_sequence<Ps...>{},
                           place_at<P0>(terms_num, term0), terms_den,
                           std::forward<Ts>(rest)...);
    }
  }
}

template <typename... G1s, expr::exp_key_t... X1s, typename... G2s,
          expr::exp_key_t... X2s, size_t... Is, size_t... Js, size_t... Ms,
          size_t... Ns>
auto combine_terms(OpTerms<Term<G1s, X1s>...> const& a,
                   OpTerms<Term<G2s, X2s>...> const& b,
                   std::index_sequence<Is...>, std::index_sequence<Js...>,
                   std::index_sequence<Ms...>, std::index_sequence<Ns...>) {
  if constexpr ((expr::_Xk_t<X2s>::sign && ...)) {
    return combine_terms(std::index_sequence<Ms...>{},
                         combine_terms(expr::get<Is>(a)...),
                         combine_terms(~expr::get<Js>(b)...),
                         (expr::get<Ms>(a) * expr::get<Ns>(b))...);
  } else {
    return combine_terms(
        std::index_sequence<Ms...>{},
        combine_terms(expr::get<Is>(a)..., expr::get<Js>(b)...), OpTerms<>{},
        (expr::get<Ms>(a) * expr::get<Ns>(b))...);
  }
}
...
constexpr bool commutes_through<G, symphas::lib::types_list<Gs...>> =
    (true && ... && expr::is_commutable<G, Gs>);
...
constexpr int commutes_to<G0, G1, types_list<Gs...>> =
    (commutes_through<
        G0, types_before_index<fixed_max<0, index_of_type<G1, Gs...>>, Gs...>>)
        ? index_of_type<G1, Gs...>
        : -1;
...
constexpr int satisfies_identity<G1, types_list<Gs...>,
                                 types_list<G2, G2s...>> =
    (expr::has_identity<G1, G2>)
        ? commutes_to<G1, G2, types_list<Gs..., G2s...>> - int(sizeof...(Gs))
        : satisfies_identity<G1, types_list<Gs...>,
                             types_list<G2s...>>;  // will combine with a G2 if
                                                   // there is an identity

template <typename G1, typename... Gs>
constexpr int satisfies_identity<G1, types_list<Gs...>, types_list<>> =
    -1;  // will combine with a G2 if there is an identity
...
constexpr int satisfies_combination<G1, types_list<Gs...>, types_list<G2s...>> =
    (expr::is_combinable<G1>)  // checks if G1 combines with another G1 in G2s
                               // list
        ? commutes_to<G1, G1, types_list<Gs..., G2s...>> - int(sizeof...(Gs))
        : satisfies_identity<G1, types_list<Gs...>, types_list<G2s...>>;

template <typename... G1s, expr::exp_key_t... X1s, typename... G2s,
          expr::exp_key_t... X2s, size_t... Is, size_t... Js>
auto combine_terms(OpTerms<Term<G1s, X1s>...> const& a,
                   OpTerms<Term<G2s, X2s>...> const& b,
                   std::index_sequence<Is...>, std::index_sequence<Js...>) {
  using seq_a_match_b = std::integer_sequence<
      int, ((satisfies_combination<G1s, types_after_at_index<Is + 1, G1s...>,
                                   types_list<G2s...>>))...>;

  using seq_a_pick = symphas::lib::seq_join_t<
      std::index_sequence<>,
      std::conditional_t<
          (symphas::lib::seq_index_value<Is, seq_a_match_b>::value >= 0),
          std::index_sequence<Is>, std::index_sequence<>>...>;
  using seq_b_pick = symphas::lib::seq_join_t<
      std::index_sequence<>,
      std::conditional_t<
          (symphas::lib::seq_index_value<Is, seq_a_match_b>::value >= 0),
          std::index_sequence<(
              size_t)symphas::lib::seq_index_value<Is, seq_a_match_b>::value>,
          std::index_sequence<>>...>;

  using seq_a_pick_ =
      symphas::lib::filter_seq_t<std::index_sequence<Is...>, seq_a_pick>;
  using seq_b_pick_ =
      symphas::lib::filter_seq_t<std::index_sequence<Js...>, seq_b_pick>;

  return combine_terms(a, b, seq_a_pick_{}, seq_b_pick_{}, seq_a_pick{},
                       seq_b_pick{});
}
...
template <typename V1, typename... G1s, expr::exp_key_t... X1s, typename V2,
          typename... G2s, expr::exp_key_t... X2s>
auto operator*(OpTerms<V1, Term<G1s, X1s>...> const& a,
  ...
  return expr::coeff(a) * expr::coeff(b) *
         symphas::internal::combine_terms(
             expr::terms_after_first(a), expr::terms_after_first(b),
...
struct construct_term {
  ...
  static auto get(S value, A data) {
    if constexpr (expr::is_coeff<A>) {
      ...
      Term<A, Xk<1>> t(data);
      return OpTerms<S, Term<A, Xk<1>>>(value, t);
    }
  }
  ...
  static auto get(A data) {
    if constexpr (expr::is_coeff<A>) {
      ...
      Term<A, Xk<1>> t(data);
      return OpTerms<OpIdentity, Term<A, Xk<1>>>(OpIdentity{}, t);
    }
  }
  ...
  static auto get(S value, A data) {
    if constexpr (expr::is_coeff<A>) {
      ...
      Term<Variable<Z, A>, Xk<1>> t(data);
      return OpTerms<S, Term<Variable<Z, A>, Xk<1>>>(value, t);
    }
  }
  ...
  static auto get(A data) {
    if constexpr (expr::is_coeff<A>) {
      ...
      Variable<Z, A> var(data);
      Term<Variable<Z, A>, Xk<1>> t(data);
      return OpTerms<OpIdentity, Term<Variable<Z, A>, Xk<1>>>(OpIdentity{}, t);
    }
  }
};
...
struct construct_term<scalar_t> {
  ...
  template <size_t Z>
  static auto get(scalar_t data) {
    return OpTerms(OpIdentity{},
                   Term<Variable<Z, OpLiteral<scalar_t>>, Xk<1>>(data));
  }
};
...
struct construct_term<complex_t> {
  ...
  static auto get(complex_t data) {
    return OpTerms(OpIdentity{},
                   Term<Variable<Z, OpLiteral<complex_t>>, Xk<1>>(data));
  }
};
...
struct construct_term<any_vector_t<T, D>> {
  ...
  static auto get(any_vector_t<T, D> data) {
    return OpTerms(OpIdentity{},
                   Term<OpLiteral<any_vector_t<T, D>>, Xk<1>>(data));
  }

  template <size_t Z, typename S>
  static auto get(S value, any_vector_t<T, D> data) {
    return OpTerms(
        value, Term<Variable<Z, OpLiteral<any_vector_t<T, D>>>, Xk<1>>(data));
  }
  ...
  static auto get(any_vector_t<T, D> data) {
    return OpTerms(
        OpIdentity{},
        Term<Variable<Z, OpLiteral<any_vector_t<T, D>>>, Xk<1>>(data));
  }
};
...
struct construct_term<symphas::ref<A>> {
  ...
  static auto get(symphas::ref<A> data) {
    return OpTerms(OpIdentity{},
                   Term<Variable<Z, symphas::ref<A>>, Xk<1>>(data));
  }
};
...
struct construct_term<Term<G, X>> {
  ...
  static auto get(Term<G, X> data) { return construct(OpIdentity{}, data); }
...
struct construct_term<OpTerms<Vs...>> {
  ...
  template <typename G0, expr::exp_key_t X0, typename... Gs,
            expr::exp_key_t... Xs>
  static auto construct(OpIdentity, Term<G0, X0> term,
                        OpTerms<Term<Gs, Xs>...> data) {
    auto terms = OpTermsList<Term<G0, X0>, Term<Gs, Xs>...>(term, data);
    return OpTerms<OpIdentity, Term<G0, X0>, Term<Gs, Xs>...>(OpIdentity{},
                                                              terms);
  }
  ...
  static auto get(S value, OpTerms<Vs...> data) {
    return value * construct(expr::coeff(data), expr::get<0>(data),
                             expr::terms_after_n<0>(data));
  }

  static auto get(OpTerms<Vs...> data) {
    return construct(expr::coeff(data), expr::get<0>(data),
                     expr::terms_after_n<0>(data));
  }
...
template <typename A>
struct construct_term<DynamicVariable<A>&>
    : construct_term<DynamicVariable<A>> {};
template <typename A>
struct construct_term<NamedData<A>&> : construct_term<NamedData<A>> {};
template <typename... Ts>
struct construct_term<OpTerms<Ts...>&> : construct_term<OpTerms<Ts...>> {};
template <>
struct construct_term<expr::symbols::Symbol&>
    : construct_term<expr::symbols::Symbol> {};

template <>
struct construct_term<NamedData<OpIdentity>> : construct_term<OpIdentity> {};
template <>
struct construct_term<NamedData<OpNegIdentity>>
    : construct_term<OpNegIdentity> {};
template <size_t N, size_t D>
struct construct_term<NamedData<OpFractionLiteral<N, D>>>
    : construct_term<OpFractionLiteral<N, D>> {};
template <size_t N, size_t D>
struct construct_term<NamedData<OpNegFractionLiteral<N, D>>>
    : construct_term<OpNegFractionLiteral<N, D>> {};
template <>
struct construct_term<NamedData<OpVoid>> : construct_term<OpVoid> {};
template <typename... Ts>
struct construct_term<SymbolicCase<Ts...>&>
    : construct_term<SymbolicCase<Ts...>> {};
...
template <typename... Ts>
struct construct_term<OpTermsList<Ts...>&>
    : construct_term<OpTermsList<Ts...>> {};
}  // namespace symphas::internal
 ...
 * more information on specific implementation, see
 * symphas::internal::make_termlvariable.
 *
 ...
 * Delegate to create an OpTerm with the correct template. For
 * more information on specific implementation, see
 * symphas::internal::make_termlvariable. This overload will use the given index
 * to create an OpTerm of a Variable.
 *
 * \param data The data from which to create an OpTerm.
 *
...
template <size_t Z, typename A>
auto make_term(A&& data) {
  return symphas::internal::construct_term<A>::template get<Z>(
      std::forward<A>(data));
}
...
/*!
 * For symbolic algebra manipulation, create a variable with no data, simply a
 * symbolic type, in order to represent a symbol.
 *
 * \tparam Z The index of the Variable to create.
 */
template <size_t Z>
...
auto make_term_dynamic(DynamicIndex const& index, V&& value, A&& data) {
  return expr::make_term(std::forward<V>(value),
                         DynamicVariable(index, std::forward<A>(data)));
}
...
const char* get_op_name(DynamicVariable<NamedData<G*>> const& a) {
  ...
  if (s != NULL) {
    ...
    static char** str =
        symphas::internal::format_dynamic_op_name(a.data.data, a.data.name, a);
    ...
    if (a.index() > end || a.index() < start) {
      ...
      str = symphas::internal::format_dynamic_op_name(
          a.data.data, a.data.name, DynamicIndex(0, start, end));
    }

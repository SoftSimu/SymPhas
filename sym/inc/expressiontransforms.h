...
auto remove_factors(
  ...
  // BUG (May 2026): `factor<N, C>` can fail asymmetrically — extract
  // a factor from one operand but bail on the other (the OpAdd
  // heterogeneous-children case where Fix B in `_factor_adds` returns
  // OpIdentity). If we proceeded with one side's residual but used the
  // ORIGINAL on the other, the resulting division would silently lose
  // the factor. Detect a "real" extraction success by checking that
  // `.first` is not OpIdentity, and skip this factor when only one
  // side succeeded.

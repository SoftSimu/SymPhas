...
inline auto operator*(OpIdentity, scalar_t const& b) { return b; }

//! Multiplication between the identity expression and a complex value.
inline auto operator*(OpIdentity, complex_t const& b) { return b; }

//! Multiplication between the identity expression and an integer value.
inline auto operator*(OpIdentity, int b) { return b; }
...
//! Multiplication between the identity expression and a scalar.
inline auto operator*(scalar_t const& a, OpIdentity) { return a; }

//! Multiplication between the identity expression and a complex value.
inline auto operator*(complex_t const& a, OpIdentity) { return a; }

//! Multiplication between the identity expression and an integer value.
inline auto operator*(int a, OpIdentity) { return a; }
...
//! Multiplication between the identity expression and a scalar.
inline auto operator*(OpNegIdentity, scalar_t const& b) { return -b; }

//! Multiplication between the identity expression and a complex value.
inline auto operator*(OpNegIdentity, complex_t const& b) { return -b; }

//! Multiplication between the identity expression and an integer value.
inline auto operator*(OpNegIdentity, int b) { return -b; }
...
//! Multiplication between the identity expression and a scalar.
inline auto operator*(scalar_t const& a, OpNegIdentity) { return -a; }

//! Multiplication between the identity expression and a complex value.
inline auto operator*(complex_t const& a, OpNegIdentity) { return -a; }

//! Multiplication between the identity expression and an integer value.
inline auto operator*(int a, OpNegIdentity) { return -a; }
...
template <typename T, typename I, typename coeff_t,
          std::enable_if_t<expr::is_coeff<coeff_t>, int> = 0>
auto operator+(OpCoeff<T, I> const& a, coeff_t const& b) {
  auto result = expr::init_coeff_from<add_result_t<T, coeff_t>, I>(a);
  for (iter_type i = 0; i < a.data.len; ++i) {
    result.data[i] = a.data[i] + b;
  }
  return result;
...
template <typename T, typename I, typename coeff_t,
          std::enable_if_t<expr::is_coeff<coeff_t>, int> = 0>
auto operator-(OpCoeff<T, I> const& a, coeff_t const& b) {
  auto result = expr::init_coeff_from<sub_result_t<T, coeff_t>, I>(a);
  for (iter_type i = 0; i < a.data.len; ++i) {
    result.data[i] = a.data[i] - b;
  }
  return result;
...
template <typename T, typename I, typename coeff_t,
          std::enable_if_t<expr::is_coeff<coeff_t>, int> = 0>
auto operator*(OpCoeff<T, I> const& a, coeff_t const& b) {
  auto result = expr::init_coeff_from<mul_result_t<T, coeff_t>, I>(a);
  for (iter_type i = 0; i < a.data.len; ++i) {
    result.data[i] = a.data[i] * b;
  }
  return result;
...
template <typename T, typename I, typename coeff_t,
          std::enable_if_t<expr::is_coeff<coeff_t>, int> = 0>
auto operator/(OpCoeff<T, I> const& a, coeff_t const& b) {
  auto result = expr::init_coeff_from<div_result_t<T, coeff_t>, I>(a);
  for (iter_type i = 0; i < a.data.len; ++i) {
    result.data[i] = a.data[i] / b;
  }
  return result;
...
template <typename T1, typename I, typename T2, size_t... Ns>
auto operator*(OpCoeff<T1, I> const& a, OpTensor<T2, Ns...> const& b) {
  auto result = expr::init_coeff_from<mul_result_t<T1, T2>, I>(a);
  for (iter_type i = 0; i < a.data.len; ++i) {
    result.data[i] = a.data[i] * T2(b);
  }
...
auto operator+(coeff_t const& a, OpCoeff<T, I> const& b) {
  auto result = expr::init_coeff_from<add_result_t<coeff_t, T>, I>(b);
...
template <typename T, typename I, typename coeff_t,
          std::enable_if_t<expr::is_coeff<coeff_t>, int> = 0>
auto operator-(coeff_t const& a, OpCoeff<T, I> const& b) {
  auto result = expr::init_coeff_from<sub_result_t<coeff_t, T>, I>(b);
  for (iter_type i = 0; i < b.data.len; ++i) {
    result.data[i] = a - b.data[i];
  }
  return result;
...
template <typename T, typename I, typename coeff_t,
          std::enable_if_t<expr::is_coeff<coeff_t>, int> = 0>
auto operator*(coeff_t const& a, OpCoeff<T, I> const& b) {
  auto result = expr::init_coeff_from<mul_result_t<coeff_t, T>, I>(b);
  for (iter_type i = 0; i < b.data.len; ++i) {
    result.data[i] = a * b.data[i];
  }
  return result;
...
template <typename T, typename I, typename coeff_t,
          std::enable_if_t<expr::is_coeff<coeff_t>, int> = 0>
auto operator/(coeff_t const& a, OpCoeff<T, I> const& b) {
  auto result = expr::init_coeff_from<div_result_t<coeff_t, T>, I>(b);
  for (iter_type i = 0; i < b.data.len; ++i) {
    result.data[i] = a / b.data[i];
  }
  return result;
...
//! Multiplication between the identity expression and a scalar.
template <typename T, typename I, typename coeff_t,
          std::enable_if_t<!expr::is_expression<coeff_t>, int> = 0>
auto operator+(OpCoeff<T, I> const& a, coeff_t const& b) {
...
inline auto operator*(OpIdentity, OpIdentity) { return OpIdentity{}; }

//! Multiplication between two negative identity expressions.
inline auto operator*(OpNegIdentity, OpNegIdentity) { return OpIdentity{}; }

//! Multiplication between the negative identity and identity expressions.
inline auto operator*(OpNegIdentity, OpIdentity) { return OpNegIdentity{}; }

//! Multiplication between the identity and negative identity expressions.
inline auto operator*(OpIdentity, OpNegIdentity) { return OpNegIdentity{}; }
...
inline auto operator*(OpIdentity, OpVoid) { return OpVoid{}; }

//! Multiplication between the 0 identity and identity expression.
inline auto operator*(OpVoid, OpIdentity) { return OpVoid{}; }

//! Multiplication between the negative identity and 0 identity.
inline auto operator*(OpNegIdentity, OpVoid) { return OpVoid{}; }

//! Multiplication between the 0 identity and negative identity.
inline auto operator*(OpVoid, OpNegIdentity) { return OpVoid{}; }

//! Multiplication between two 0 identities.
inline auto operator*(OpVoid, OpVoid) { return OpVoid{}; }

inline int& operator*=(int& lhs, OpIdentity) { return lhs; }
...
inline scalar_t& operator*=(scalar_t& lhs, OpIdentity) { return lhs; }
...
inline complex_t& operator*=(complex_t& lhs, OpIdentity) { return lhs; }
...
any_vector_t<T1, D>& operator*=(any_vector_t<T1, D>& lhs,
                                OpLiteral<T2> const& v) {
  lhs = lhs * v;
  return lhs;
}
...
//! Apply an inverse to an expression literal.
constexpr inline auto inverse(OpIdentity) { return OpIdentity{}; }

//! Apply an inverse to an expression literal.
constexpr inline auto inverse(OpNegIdentity) { return OpNegIdentity{}; }
...
auto inverse(OpTerms<V, Term<Gs, Xs>...> const& e) {
  return expr::make_div(expr::inverse(expr::coeff(e)),
                        OpTerms(OpIdentity{}, expr::terms_after_first(e)));
}
...
inline OpVoid operator+(OpIdentity, OpNegIdentity) { return OpVoid{}; }

//! Addition between the negative identity expression and identity.
inline OpVoid operator+(OpNegIdentity, OpIdentity) { return OpVoid{}; }

inline auto operator+(OpVoid, OpNegIdentity) { return OpNegIdentity{}; }

inline auto operator+(OpVoid, OpIdentity) { return OpIdentity{}; }
...
inline auto operator*(OpVoid, expr::symbols::Symbol) { return OpVoid{}; }

inline auto operator*(expr::symbols::Symbol, OpVoid) { return OpVoid{}; }
...
inline auto operator-(OpVoid const, OpVoid const) { return OpVoid{}; }

//! Subtraction between two identities.
inline auto operator-(OpIdentity, OpIdentity) { return OpVoid{}; }

//! Subtraction between two negative identities.
inline auto operator-(OpNegIdentity, OpNegIdentity) { return OpVoid{}; }
...
// inline auto operator+(OpTensor<OpNegIdentity, N0, N>, OpTensor<OpNegIdentity,
// N1, N>)
//{
//	return expr::make_add(OpTensor<OpNegIdentity, N0, N>{},
// OpTensor<OpNegIdentity, N1, N>{});
// }
//
// template<size_t N0, size_t N1, size_t N>
// inline auto operator-(OpTensor<OpNegIdentity, N0, N>, OpTensor<OpNegIdentity,
...
template <size_t N, size_t D, typename T,
          typename = std::enable_if_t<symphas::is_simple_data<T>, int>>
...
template <size_t N, size_t D, typename T,
          typename = std::enable_if_t<symphas::is_simple_data<T>, int>>
auto operator/(OpFractionLiteral<N, D> const&, OpLiteral<T> const& value) {
  return static_cast<scalar_t>(OpFractionLiteral<N, D>{}) *
         expr::inverse(value);
}
...
auto operator/(OpLiteral<T> const& value, OpFractionLiteral<N, D> const&) {
  return value *
         static_cast<scalar_t>(expr::inverse(OpFractionLiteral<N, D>{}));
}

template <size_t N, size_t D, typename T,
          typename = std::enable_if_t<symphas::is_simple_data<T>, int>>
auto operator/(OpNegFractionLiteral<N, D> const&, OpLiteral<T> const& value) {
  return static_cast<scalar_t>(OpNegFractionLiteral<N, D>{}) *
         expr::inverse(value);
}
...
auto operator/(OpLiteral<T> const& value, OpNegFractionLiteral<N, D> const&) {
  return value *
         static_cast<scalar_t>(expr::inverse(OpNegFractionLiteral<N, D>{}));
}
...
auto operator/(OpNegFractionLiteral<N, D>, OpExpression<E> const& e) {
  return OpNegFractionLiteral<N, D>{} *
         expr::inverse(*static_cast<E const*>(&e));
}
...
constexpr bool is_same_base =
    std::is_same<typename expr::base_data_type<G1>::type,
                 typename expr::base_data_type<G2>::type>::value;
}  // namespace expr

//! Addition of two variables with data that can be combined.
template <typename A, typename B, typename Dd, typename G, typename Sp,
          typename std::enable_if<expr::is_combinable<G>, int>::type = 0>
auto operator+(OpDerivative<Dd, A, OpTerm<OpIdentity, G>, Sp> const& a,
               OpDerivative<Dd, B, OpTerm<OpIdentity, G>, Sp> const& b) {
  return (expr::coeff(a) + expr::coeff(b)) *
         OpDerivative<Dd, OpIdentity, OpTerm<OpIdentity, G>, Sp>(
             OpIdentity{}, expr::get_enclosed_expression(a), a.solver);
}
...
//! Addition of two variables with data that can be combined.
template <typename A, typename B, typename E, typename T>
auto operator+(OpIntegral<A, E, T> const& a, OpIntegral<B, E, T> const& b) {
  return (expr::coeff(a) + expr::coeff(b)) *
         OpIntegral<OpIdentity, E, T>(
             OpIdentity{}, expr::get_enclosed_expression(a), a.domain);
}
...
//! Subtraction of two variables with data that can be combined.
// template<typename V1, typename V2, typename E, typename T>
// auto operator*(OpIntegral<V1, E, T> const& a, OpIntegral<V2, E, T> const& b)
//{
//	return (expr::coeff(a) * expr::coeff(b)) *
// expr::make_pow<2>(OpIntegral<OpIdentity, E, T>(OpIdentity{},
// expr::get_enclosed_expression(a), a.domain));
// }

//! Subtraction of two variables with data that can be combined.
// template<typename V1, typename V2, typename E1, typename E2, typename E3,
// typename T, typename std::enable_if<expr::is_combinable<E1>, int>::type = 0>
// auto operator+(OpBinaryMul<OpIntegral<V1, E1, T>, E2> const& a,
// OpBinaryMul<OpIntegral<V1, E1, T>, E3> const& b)
//{
//	return expr::make_mul(a.a, (a.b + b.b));
// }
...
auto operator/(OpDerivative<Dd, V1, E, Sp> const& a,
               OpDerivative<Dd, V2, E, Sp> const& b) {
...
auto pick_like_terms(symphas::lib::types_list<Es...>,
                     std::index_sequence<Is...>) {
  // True on the index if there is a like term.
  using mask_t = std::integer_sequence<
      bool, !std::is_same<OpAdd<E0, Es>, add_result_t<E0, Es>>::value...>;

  // return std::make_pair(
  //	symphas::lib::seq_join_t<std::index_sequence<>,
  // std::conditional_t<symphas::lib::seq_index_value<Is, mask_t>::value,
  // std::index_sequence<Is>, std::index_sequence<>>...>{},
  //	symphas::lib::seq_join_t<std::index_sequence<>,
  // std::conditional_t<!symphas::lib::seq_index_value<Is, mask_t>::value,
  // std::index_sequence<Is>, std::index_sequence<>>...>{}
  //);
  return symphas::lib::seq_join_t<
      std::index_sequence<>,
      std::conditional_t<symphas::lib::seq_index_value<Is, mask_t>::value,
                         std::index_sequence<Is>, std::index_sequence<>>...>{};
}
...
auto pick_like_terms(symphas::lib::types_list<> const&,
                     symphas::lib::types_list<Es...>,
                     std::index_sequence<Is...>) {
  return symphas::lib::types_list<symphas::lib::types_list<>,
                                  symphas::lib::types_list<>>{};
}

// Returns a types list of sequences, such that for each sequence, its first
// entry represents the index from E0, E0s which matches with the entries from
// Es using the other entries of the sequence. Is... indexes Es... .
template <size_t N0, typename E0, typename... E0s, typename... Es, size_t... Is>
auto pick_like_terms(symphas::lib::types_list<E0, E0s...> const&,
                     symphas::lib::types_list<Es...>,
                     std::index_sequence<Is...>) {
  ...
  if constexpr (sizeof...(Es) == 0) {
    ...
    using match_seq = decltype(pick_like_terms<E0>(
        types_list<Es...>{}, std::make_index_sequence<sizeof...(Es)>{}));

    using seq_choose =
        seq_join_t<select_types<match_seq, std::index_sequence<Is>...>>;
    using seq_next = filter_seq_t<std::index_sequence<Is...>, seq_choose>;

    using rest_seq = decltype(pick_like_terms<N0 + 1>(
        types_list<E0s...>{}, filter_types_on_index<match_seq, Es...>{},
        seq_next{}));

    return types_list<
        expand_types_list<std::index_sequence<N0>,
                          type_at_index<0, unroll_types_list<rest_seq>>>,
        expand_types_list<seq_choose,
                          type_at_index<1, unroll_types_list<rest_seq>>>>{};
  }
}

template <typename E0, typename... Es, size_t... Is>
auto apply_add(E0 const& e0, OpAdd<Es...> const& e,
               std::index_sequence<Is...>) {
...
auto compile_add(OpAdd<Es...> const& pick, std::index_sequence<Is...>,
                 E0s&&... es) {
...
auto compile_add_remove(OpAdd<Es...> const& pick,
                        std::index_sequence<N0, N1, Ns...>, E0s&&... es) {
  return compile_add(
      pick,
      symphas::lib::filter_seq_t<std::make_index_sequence<sizeof...(Es)>,
                                 std::index_sequence<N0, N1, Ns...>>{},
      std::forward<E0s>(es)...);
}

template <typename... Es, size_t N0, typename... E0s>
auto compile_add_remove(OpAdd<Es...> const& pick, std::index_sequence<N0>,
                        E0s&&... es) {
  return make_add(expr::terms_after_n<N0>(pick), expr::terms_before_n<N0>(pick),
                  std::forward<E0s>(es)...);
}

template <typename... Es, typename... E0s>
auto compile_add_remove(OpAdd<Es...> const& pick, std::index_sequence<>,
                        E0s&&... es) {
  return make_add(pick, std::forward<E0s>(es)...);
}
...
auto add_one(A const& add, OpAdd<Es...> const& e) {
  auto like_seq = pick_like_terms<A>(symphas::lib::types_list<Es...>{},
...
auto add_one(OpAdd<Es...> const& e, B const& add) {
  auto like_seq = pick_like_terms<B>(symphas::lib::types_list<Es...>{},
                                     std::make_index_sequence<sizeof...(Es)>{});
  return compile_add_remove(e, like_seq, apply_add(add, e, like_seq));
}
...
auto compile_add_group(OpAdd<As...> const& add, std::index_sequence<Is...>,
...
template <typename... As, typename... E0s, size_t... Is, size_t... Js>
auto add_group_apply(OpAdd<As...> const& add, std::index_sequence<Is...>,
                     OpAdd<E0s...> const& add0, std::index_sequence<Js...>,
...
template <typename... As, typename... E0s, size_t... Is, size_t... Js,
          size_t P0, size_t... Ps, typename Seq0, typename... Seqs>
auto add_group_apply(OpAdd<As...> const& add, std::index_sequence<Is...>,
                     OpAdd<E0s...> const& add0, std::index_sequence<Js...>,
                     std::index_sequence<P0, Ps...>,
                     symphas::lib::types_list<Seq0, Seqs...>) {
  using namespace symphas::lib;

  using unmatched_seq_a =
      seq_skip_indices<sizeof...(Is), std::index_sequence<P0, Ps...>>;
  using unmatched_seq_b =
      seq_skip_indices<sizeof...(Js), sorted_seq<seq_join_t<Seq0, Seqs...>>>;

  return expr::make_add(
      compile_add_group(add, unmatched_seq_a{}, add0, unmatched_seq_b{}),
      (expr::get<P0>(add) + compile_add(add0, Seq0{})),
      (expr::get<Ps>(add) + compile_add(add0, Seqs{}))...);
}

template <typename... As, typename... E0s, size_t... Is>
auto add_group(OpAdd<As...> const& add, std::index_sequence<Is...>,
               OpAdd<E0s...> const& add0) {
  using namespace symphas::lib;

  using match_terms_t =
      decltype(pick_like_terms<0>(types_list<As...>{}, types_list<E0s...>{},
  ...
  return add_group_apply(add, std::index_sequence<Is...>{}, add0,
                         std::make_index_sequence<sizeof...(E0s)>{}, pick_a{},
                         match_b{});
}

template <typename... As, typename... E0s, typename T0, typename... Ts,
          size_t... Is>
auto add_group(OpAdd<As...> const& add, std::index_sequence<Is...>,
               OpAdd<E0s...> const& e0, T0&& e1, Ts&&... rest);
...
inline auto add_group() { return OpVoid{}; }
...
auto add_group(OpExpression<E1> const& e1, OpExpression<E2> const& e2,
               Ts&&... rest);

template <typename E, typename... E0s, typename... Ts>
auto add_group(OpExpression<E> const& add, OpAdd<E0s...> const& e,
               Ts&&... rest);

template <typename E, typename... E0s, typename... Ts>
auto add_group(OpAdd<E0s...> const& e, OpExpression<E> const& add,
               Ts&&... rest);
...
auto add_group(OpExpression<E> const& add, OpAdd<E0s...> const& e,
               Ts&&... rest) {
  return add_group(add_one(*static_cast<E const*>(&add), e),
                   std::forward<Ts>(rest)...);
}
...
auto add_group(OpAdd<E0s...> const& e, OpExpression<E> const& add,
               Ts&&... rest) {
  return add_group(add_one(e, *static_cast<E const*>(&add)),
                   std::forward<Ts>(rest)...);
}

template <typename... As, typename... E0s, typename... Ts>
auto add_group(OpAdd<As...> const& add, OpAdd<E0s...> const& e, Ts&&... rest) {
  return add_group(add, std::make_index_sequence<sizeof...(As)>{}, e,
                   std::forward<Ts>(rest)...);
}
...
auto add_group(OpExpression<E1> const& e1, OpExpression<E2> const& e2,
               Ts&&... rest) {
  return add_group(*static_cast<E1 const*>(&e1) + *static_cast<E2 const*>(&e2),
                   std::forward<Ts>(rest)...);
}
...
template <typename... As, typename... E0s, typename T0, typename... Ts,
          size_t... Is>
auto add_group(OpAdd<As...> const& add, std::index_sequence<Is...>,
               OpAdd<E0s...> const& add0, T0&& e1, Ts&&... rest) {
  return add_group(add_group(add, std::index_sequence<Is...>{}, add0),
                   std::forward<T0>(e1), std::forward<Ts>(rest)...);
}
...
constexpr bool distribute_adds_predicate<A, OpAdd<Bs...>> =
    (std::is_same_v<mul_result_t<A, Bs>, OpBinaryMul<A, Bs>> && ...);

template <typename... As, typename B>
constexpr bool distribute_adds_predicate<OpAdd<As...>, B> =
    (std::is_same_v<mul_result_t<As, B>, OpBinaryMul<As, B>> && ...);

//! Distributing addition expression into a addition expression.
template <
    typename A0, typename... Bs, size_t... Js,
    std::enable_if_t<distribute_adds_predicate<A0, OpAdd<Bs...>>, int> = 0>
auto distribute_adds(A0 const& a, OpAdd<Bs...> const& b,
...
//! Distributing addition expression into a addition expression.
template <
    typename... As, typename B0, size_t... Is,
    std::enable_if_t<distribute_adds_predicate<OpAdd<As...>, B0>, int> = 0>
auto distribute_adds(OpAdd<As...> const& a, B0 const& b,
                     std::index_sequence<Is...>) {
...
auto distribute_adds(OpAdd<As...> const& a, OpAdd<Bs...> const& b,
                     std::index_sequence<Is...>, std::index_sequence<Js...>) {
  return add_group(
      distribute_adds(expr::get<Is>(a), b, std::index_sequence<Js...>{})...);
}
}  // namespace expr
...
auto operator*(OpAdd<As...> const& a, OpAdd<Bs...> const& b) {
  return expr::distribute_adds(a, b, std::make_index_sequence<sizeof...(As)>{},
...
template <typename... As, typename B2,
          typename std::enable_if_t<(!expr::is_identity<B2> &&
                                     !expr::is_fraction<B2> &&
                                     !expr::is_coeff<OpAdd<As...>>),
                                    int> = 0>
...
template <
    typename A1, typename... Bs,
    typename std::enable_if_t<
        (!expr::is_identity<A1> && !expr::is_coeff<OpAdd<Bs...>>), int> = 0>
...
//! Distributing an LHS expression between operands in addition expression.
template <
    typename A1, typename A2, typename... Bs,
    typename std::enable_if_t<
        (!expr::is_identity<A1> && !expr::is_coeff<OpAdd<Bs...>>), int> = 0>
auto operator*(OpBinaryMul<A1, A2> const& a, OpAdd<Bs...> const& b) {
  return expr::distribute_adds(a, b, std::make_index_sequence<sizeof...(Bs)>{});
}
...
//! Distributing a literal into the first operand in multiplication expression.
template <typename coeff_t, typename B1, typename B2,
          typename std::enable_if_t<(expr::is_coeff<coeff_t> &&
                                     expr::eval_type<coeff_t>::rank == 0),
                                    int> = 0>
auto operator*(coeff_t const& coeff, OpBinaryMul<B1, B2> const& b) {
  return expr::make_mul(coeff * b.a, b.b);
}
...
template <typename coeff_t, typename B1, typename B2,
          typename std::enable_if_t<(expr::is_coeff<coeff_t> &&
                                     expr::eval_type<coeff_t>::rank == 0),
                                    int> = 0>
auto operator*(coeff_t const& coeff, OpBinaryDiv<B1, B2> const& b) {
  return expr::make_div(coeff * b.a, b.b);
}

//! Distributing a literal into the first operand in multiplication expression.
template <typename B1, typename B2>
auto operator*(OpIdentity, OpBinaryMul<B1, B2> const& b) {
  return b;
}

//! Distributing a literal into the first operand in division expression.
template <typename B1, typename B2>
auto operator*(OpIdentity, OpBinaryDiv<B1, B2> const& b) {
  return b;
}

//! Distributing a literal into the first operand in multiplication expression.
template <typename B1, typename B2>
auto operator*(OpVoid, OpBinaryMul<B1, B2> const& b) {
  return OpVoid{};
}

//! Distributing a literal into the first operand in division expression.
template <typename B1, typename B2>
auto operator*(OpVoid, OpBinaryDiv<B1, B2> const& b) {
  return OpVoid{};
}
...
expr::eval_type<E2>::rank == 0), int> = 0> auto operator*(OpExpression<E1>
const& a, OpExpression<E2> const& b)
{
...
typename std::enable_if_t<(expr::eval_type<E1>::rank == 0 ||
expr::eval_type<E2>::rank == 0), int> = 0> auto operator*(OpExpression<E1>
const& a, OpOperator<E2> const& b)
{
return OpOperatorChain(*static_cast<const E1*>(&a), *static_cast<const
E2*>(&b));
}

template<typename E1, typename E2,
typename std::enable_if_t<(expr::eval_type<E1>::rank > 0 &&
expr::eval_type<E2>::rank > 0), int> = 0> auto operator*(OpExpression<E1> const&
a, OpOperator<E2> const& b)
{
...
auto operator-(OpExpression<E> const& a, OpAdd<Bs...> const& b) {
  return expr::add_one(
      *static_cast<E const*>(&a),
      expr::neg_terms(b, std::make_index_sequence<sizeof...(Bs)>{}));
}
...
template <typename... As, typename... Bs>
auto operator-(OpAdd<As...> const& a, OpAdd<Bs...> const& b) {
  return expr::add_group(
      a, std::make_index_sequence<sizeof...(As)>{},
      expr::neg_terms(b, std::make_index_sequence<sizeof...(Bs)>{}));
}
...
//
// template<typename A1, typename A2, typename V, typename... Gs,
// expr::exp_key_t... Xs> auto operator*(OpBinaryMul<A1, A2> const& a,
// OpTerms<V, Term<Gs, Xs>...> const& b)
//{
//	return symphas::internal::terminate_mul(a.a, (a.b * expr::coeff(b)) *
// OpTerms(OpIdentity{}, expr::terms_after_first(b)));
//}
//
// template<typename V, typename... Gs, expr::exp_key_t... Xs, typename B1,
// typename B2> auto operator*(OpTerms<V, Term<Gs, Xs>...> const& a,
// OpBinaryMul<B1, B2> const& b)
//{
...
template <
    ...
    typename E /*, typename std::enable_if_t<!expr::is_coeff<E>, int> = 0*/>
...
//
// template<typename A1, typename A2, typename E, typename
// std::enable_if_t<!expr::is_coeff<E>, int> = 0> auto operator*(OpExpression<E>
// const& a, OpBinaryMul<A1, A2> const& b)
//{
...
/*!
 * Endpoint of the recursion for applying the division rules, which
 * **multiplies** two reduced terms, with a key emphasis on reduced terms. There
 * are several rules for handling various reduced forms. There is also the
 * capability to automatically reduce terms. Makes the assumption that a
 * division is always the first term given to the function.
 */
template <typename A1, typename A2, typename E,
          typename std::enable_if_t<(expr::factor_list_all<E, A2>::value == 0),
                                    int> = 0>
...
template <typename B1, typename B2, typename E,
          typename std::enable_if_t<(expr::factor_list_all<E, B2>::value == 0),
                                    int> = 0>
...
template <
    ...
    typename std::enable_if_t<(expr::factor_list_all<A1, B2>::value == 0 &&
                               expr::factor_list_all<A2, B1>::value == 0),
                              int> = 0>
auto terminate_div(OpBinaryDiv<A1, A2> const& a, OpBinaryDiv<B1, B2> const& b) {
  return expr::make_div(a.a * b.a, a.b * b.b);
}
...
template <typename A1, typename A2, typename B1, typename B2,
          typename std::enable_if_t<(expr::factor_list_all<A2, B2>::value > 0),
                                    int> = 0>
auto terminate_div(OpBinaryMul<A1, A2> const& a, OpBinaryDiv<B1, B2> const& b) {
...
template <
    typename A1, typename A2, typename B1, typename B2,
    typename std::enable_if_t<
        (expr::factor_list_all<OpBinaryMul<A1, A2>, B2>::value == 0), int> = 0>
auto terminate_div(OpBinaryMul<A1, A2> const& a, OpBinaryDiv<B1, B2> const& b) {
  return expr::make_div(a * b.a, b.b);
}
...
template <
    typename coeff_t, typename A, typename B,
    typename std::enable_if_t<
        (expr::is_identity<coeff_t> || expr::is_fraction<coeff_t>), int> = 0>
...
template <typename E, typename B1, typename T, typename... Gs,
          expr::exp_key_t... Xs,
          typename std::enable_if_t<
              ((expr::factor_count<Gs, E>::value == 0) && ...), int> = 0>
auto terminate_div(OpExpression<E> const& a,
                   OpBinaryDiv<B1, OpTerms<T, Term<Gs, Xs>...>> const& b) {
  return expr::make_div(
      expr::inverse(expr::coeff(b.b)) * (*static_cast<E const*>(&a)) * b.a,
      OpTerms(OpIdentity{}, expr::terms_after_first(b.b)));
}

template <typename E, typename A1, typename T, typename... Gs,
          expr::exp_key_t... Xs,
          typename std::enable_if_t<
              ((expr::factor_count<Gs, E>::value == 0) && ...), int> = 0>
auto terminate_div(OpBinaryDiv<A1, OpTerms<T, Term<Gs, Xs>...>> const& a,
...
template <typename A1, typename A2, typename E,
          typename std::enable_if<
              (expr::factor_list_all<OpBinaryMul<A1, A2>, E>::value == 0 &&
               !expr::is_fraction<E> && !expr::is_identity<E>),
              int>::type = 0>
auto operator/(OpBinaryMul<A1, A2> const& a, OpExpression<E> const& b) {
...
template <typename B1, typename B2, typename E,
          typename std::enable_if<
              (expr::factor_list_all<E, OpBinaryMul<B1, B2>>::value == 0 &&
               !expr::is_fraction<E> && !expr::is_identity<E>),
              int>::type = 0>
...
template <
    ...
    typename std::enable_if<(expr::factor_list_all<A1, E>::value == 0 &&
                             !expr::is_fraction<E> && !expr::is_identity<E>),
                            int>::type = 0>
...
template <typename E, typename B1, typename V, typename... Gs,
          expr::exp_key_t... Xs,
          typename std::enable_if_t<(((expr::factor_count<Gs, E>::value == 0) &&
                                      ...) &&
                                     !expr::is_identity<E>),
                                    int> = 0>
...
template <typename E, typename B1, typename V, typename... Gs,
          ...
          typename std::enable_if_t<(((expr::factor_count<Gs, E>::value == 0) &&
                                     ...
                                     !expr::is_identity<E>),
                                    int> = 0>
auto operator*(OpExpression<E> const& a,
               OpBinaryDiv<B1, OpTerms<V, Term<Gs, Xs>...>> const& b) {
  return expr::make_div(
      expr::inverse(expr::coeff(b.b)) * ((*static_cast<E const*>(&a)) * b.a),
      OpTerms(OpIdentity{}, expr::terms_after_first(b.b)));
}
...
template <typename E, typename B1, typename V, typename... Gs,
          ...
          typename std::enable_if_t<(((expr::factor_count<Gs, E>::value == 0) &&
                                     ...
                                     !expr::is_identity<E>),
                                    int> = 0>
auto operator*(OpOperator<E> const& a,
               OpBinaryDiv<B1, OpTerms<V, Term<Gs, Xs>...>> const& b) {
...
template <typename E1, typename E2>
auto operator+(OpOperatorChain<OpIdentity, E1> const& a,
...
auto operator*(OpOperator<E1> const& a, OpChain<V, OpIdentity, E2> const& b) {
  return b.combination(*static_cast<E1 const*>(&a) *
                       expr::get_enclosed_expression(b));
}
...
template <typename E1, typename E2>
auto operator*(OpOperator<E1> const& a,
               OpOperatorChain<OpIdentity, E2> const& b) {
  return expr::make_operator_chain(*static_cast<E1 const*>(&a) * b.g);
}

// template<typename... Es, typename E2>
// auto operator*(OpAdd<Es...> const& a, OpOperatorChain<OpIdentity, E2> const&
// b)
//{
...
// template<typename... Es, typename E2>
// auto operator*(OpOperatorChain<OpIdentity, E2> const& a, OpAdd<Es...> const&
// b)
//{
//	return expr::distribute_adds(a, b,
// std::make_index_sequence<sizeof...(Es)>{});
...
template <typename A1, typename A2, typename E,
          typename std::enable_if_t<
              !(expr::is_identity<E> || expr::is_fraction<E>), int> = 0>
auto operator*(OpBinaryDiv<A1, A2> const& a, OpExpression<E> const& b) {
  return (a.a * (*static_cast<const E*>(&b))) / a.b;
}

template <typename E, typename B1, typename B2,
          typename std::enable_if_t<!expr::is_identity<E>, int> = 0>
auto operator*(OpExpression<E> const& a, OpBinaryDiv<B1, B2> const& b) {
  return ((*static_cast<const E*>(&a)) * b.a) / b.b;
}
...
template <typename A1, typename A2, typename... Bs>
auto operator*(OpBinaryDiv<A1, A2> const& a, OpAdd<Bs...> const& b) {
  return expr::distribute_adds(a.a, b,
                               std::make_index_sequence<sizeof...(Bs)>{}) /
         a.b;
}
...
template <typename... As, typename B1, typename B2>
auto operator*(OpAdd<As...> const& a, OpBinaryDiv<B1, B2> const& b) {
  return expr::distribute_adds(a, b.a,
                               std::make_index_sequence<sizeof...(As)>{}) /
         b.b;
}
...
//	typename std::enable_if_t<(expr::factor_count<G, B2>::value > 0), int> =
// 0> auto operator+(OpBinaryDiv<A1, OpTerm<T, G>> const& a, OpBinaryDiv<B1, B2>
// const& b)
//{
//	auto div = b.b / a.b;
...
//	typename std::enable_if_t<((expr::factor_count<Gs, B2>::value > 0) &&
//...), int> = 0> auto operator+(OpBinaryDiv<A1, OpNLVariable<T, Gs...>> const&
// a, OpBinaryDiv<B1, B2> const& b)
//{
//	auto div = b.b / a.b;
//	return (a.a * div + b.a) / b.b;
//}
...
// auto operator-(OpBinaryDiv<A1, OpTerm<T, G>> const& a, OpBinaryDiv<B1, B2>
// const& b)
//{
//	auto div = b.b / a.b;
//	return (a.a * div - b.a) / b.b;
...
// template<typename A1, typename T, typename... Gs, typename B1, typename B2,
// typename std::enable_if_t<((expr::factor_count<Gs, B2>::value > 0) && ...),
// int> = 0> auto operator-(OpBinaryDiv<A1, OpNLVariable<T, Gs...>> const& a,
// OpBinaryDiv<B1, B2> const& b)
//{
//	auto div = b.b / a.b;
//	return (a.a * div - b.a) / b.b;
//}
...
// auto operator+(OpBinaryDiv<A1, A2> const& a, OpBinaryDiv<B1, OpTerm<T, G>>
// const& b)
//{
//	auto div = a.b / b.b;
...
// typename std::enable_if_t<((expr::factor_count<Gs, A2>::value > 0) && ...),
// int> = 0> auto operator+(OpBinaryDiv<A1, A2> const& a, OpBinaryDiv<B1,
// OpNLVariable<T, Gs...>> const& b)
//{
//	auto div = a.b / b.b;
//	return (a.a + b.a * div) / a.b;
//}
...
//
//
// template<typename A1, typename T1, typename G1, typename B1, typename T2,
// typename G2, 	typename std::enable_if_t<(expr::factor_count<G1,
// G2>::value >
// 0), int> = 0> auto operator+(OpBinaryDiv<A1, OpTerm<T1, G1>> const& a,
// OpBinaryDiv<B1, OpTerm<T2, G2>> const& b)
//{
//	auto div = b.b / a.b;
//	return symphas::internal::terminate_div((a.a * div + b.a),
// expr::inverse(b.b));
//}
//
// template<typename A1, typename T1, typename... G1s, typename B1, typename T2,
// typename G2, 	typename std::enable_if_t<(expr::factor_count_list<G2,
// G1s...>::value > 0), int> = 0> auto operator+(OpBinaryDiv<A1,
// OpNLVariable<T1,
// G1s...>> const& a, OpBinaryDiv<B1, OpTerm<T2, G2>> const& b)
//{
//	auto div = a.b / b.b;
//	return symphas::internal::terminate_div((a.a + b.a * div),
// expr::inverse(a.b));
//}
//
// template<typename A1, typename T1, typename G1, typename B1, typename T2,
// typename... G2s, 	typename std::enable_if_t<(expr::factor_count_list<G1,
// G2s...>::value > 0), int> = 0> auto operator+(OpBinaryDiv<A1, OpTerm<T1, G1>>
// const& a, OpBinaryDiv<B1, OpNLVariable<T2, G2s...>> const& b)
//{
//	auto div = b.b / a.b;
//	return symphas::internal::terminate_div((a.a * div + b.a),
// expr::inverse(b.b));
...
//
// template<typename A1, typename T1, typename... G1s, typename B1, typename T2,
// typename... G2s, 	typename std::enable_if_t<(
//		((expr::factor_count_list<G1s, G2s...>::value >=
// expr::factor_count_list<G1s, G1s...>::value) && ...) &&
//		!((expr::factor_count_list<G1s, G2s...>::value ==
// expr::factor_count_list<G1s, G1s...>::value) && ...)
//		), int> = 0>
// auto operator+(OpBinaryDiv<A1, OpNLVariable<T1, G1s...>> const& a,
// OpBinaryDiv<B1, OpNLVariable<T2, G2s...>> const& b)
//{
//	auto div = b.b / a.b;
//	return symphas::internal::terminate_div((a.a * div + b.a),
// expr::inverse(b.b));
...
// template<typename A1, typename T1, typename... G1s, typename B1, typename T2,
// typename... G2s, 	typename std::enable_if_t<(
//		((expr::factor_count_list<G2s, G1s...>::value >=
// expr::factor_count_list<G2s, G2s...>::value) && ...) &&
//		!((expr::factor_count_list<G2s, G1s...>::value ==
// expr::factor_count_list<G2s, G2s...>::value) && ...)
//		), int> = 0>
// auto operator+(OpBinaryDiv<A1, OpNLVariable<T1, G1s...>> const& a,
...
//
// template<typename A1, typename T1, typename... G1s, typename B1, typename T2,
// typename... G2s, 	typename std::enable_if_t<((expr::factor_count_list<G1s,
// G2s...>::value == expr::factor_count_list<G1s, G1s...>::value) && ...), int>
// = 0> auto operator+(OpBinaryDiv<A1, OpNLVariable<T1, G1s...>> const& a,
// OpBinaryDiv<B1, OpNLVariable<T2, G2s...>> const& b)
//{
//	auto div = a.b / b.b;
//	return symphas::internal::terminate_div((a.a + b.a * div),
  ...
  struct with_check<Term<G2s, X2s>...> {
    static const bool value =
        ((expr::is_combinable<G2s> &&
          symphas::lib::index_of_type<Term<G2s, X2s>, Term<G1s, X1s>...> >=
              0) &&
         ...) &&
...
template <
    typename V1, typename... G1s, expr::exp_key_t... X1s, typename V2,
    typename... G2s, expr::exp_key_t... X2s,
    typename std::enable_if<
        expr::can_combine<Term<G1s, X1s>...>::template with<Term<G2s, X2s>...>,
        int>::type = 0>
auto operator+(OpTerms<V1, Term<G1s, X1s>...> const& a,
               OpTerms<V2, Term<G2s, X2s>...> const& b) {
  return (a.term + b.term) *
         OpTerms(
             OpIdentity{},
             symphas::internal::cast_term<OpTerms<Term<G1s, X1s>...>>::cast(a));
}
...
template <typename A1, typename A2, typename E,
          typename std::enable_if_t<(expr::eval_type<A1>::rank == 0 &&
...
template <typename E, typename B1, typename B2,
          typename std::enable_if_t<(expr::eval_type<E>::rank == 0 &&
                                     expr::eval_type<B1>::rank == 0),
                                    int> = 0>
auto operator+(OpExpression<E> const& a, OpBinaryDiv<B1, B2> const& b) {
...
// auto operator-(OpBinaryDiv<OpBinaryDiv<A1, A2>, A3> const& a, OpAdd<Bs...>
// const& b)
//{
...
template <typename A1, typename A2, typename... Bs,
          typename std::enable_if_t<(expr::eval_type<A1>::rank == 0 &&
                                     expr::eval_type<OpAdd<Bs...>>::rank == 0),
                                    int> = 0>
auto operator+(OpBinaryDiv<A1, A2> const& a, OpAdd<Bs...> const& b) {
...
template <typename... As, typename B1, typename B2,
          typename std::enable_if_t<(expr::eval_type<OpAdd<As...>>::rank == 0 &&
                                     expr::eval_type<B1>::rank == 0),
                                    int> = 0>
auto operator+(OpAdd<As...> const& a, OpBinaryDiv<B1, B2> const& b) {
...
// two more rules are in expressiontransforms.h, when there are unbalanced
// factors between each denominator.
...
template <typename E1,
          typename = std::enable_if_t<
              !(expr::is_identity<E1> || expr::is_fraction<E1>), int>>
auto operator*(OpExpression<E1> const& a, scalar_t b) {
...
template <typename coeff_t, typename T, size_t... Ns,
          typename std::enable_if_t<
              (expr::is_coeff<coeff_t> && !expr::is_tensor<coeff_t>), int> = 0>
auto operator*(coeff_t const& a, OpTensor<T, Ns...> const& tensor) {
  return expr::make_tensor<Ns...>(a *
                                  symphas::internal::tensor_cast::cast(tensor));
}
...
//! Ensure a value constant is always multiplied on the left by anything else.
template <typename E, typename T, size_t... Ns,
          typename std::enable_if_t<expr::has_coeff<E>, int> = 0>
auto operator*(OpExpression<E> const& e, OpTensor<T, Ns...> const& tensor) {
  if constexpr (expr::eval_type<decltype(expr::coeff(
                    std::declval<E>()))>::rank > 0) {
    return (expr::coeff(*static_cast<E const*>(&e)) * tensor) *
           (symphas::internal::tensor_cancel{} * (*static_cast<E const*>(&e)));
  } else {
...
template <typename T, size_t N0, size_t N1, size_t... Ns, typename S, size_t M0,
          size_t M1, size_t... Ms>
auto operator+(OpTensor<T, N0, N1, Ns...> const& a,
               OpTensor<S, M0, M1, Ms...> const& b) {
  return expr::make_add(a, b);
}
...
template <typename T, typename coeff_t,
          typename = std::enable_if_t<
              (expr::is_coeff<coeff_t> && !expr::is_tensor<coeff_t>), int>>
...
template <typename T, size_t D>
struct expr::eval_type<any_vector_t<T, D>>
    : expr::eval_type<OpTensor<T, 0, D>> {};

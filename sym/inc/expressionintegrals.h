...
auto make_integral(V &&v, A &&a, T &&domain) {
  return symphas::internal::make_integral::get(
      std::forward<V>(v), std::forward<A>(a), std::forward<T>(domain));
}
...
auto make_domain_integral(V &&v, A &&a, symphas::grid_info const &info) {
  return symphas::internal::make_integral::get(std::forward<V>(v),
                                               std::forward<A>(a), info);
}
...
auto make_domain_integral(A &&a) {
  return make_domain_integral(
      OpIdentity{}, std::forward<A>(a),
      symphas::grid_info(expr::data_dimensions(std::forward<A>(a))));
}
}  // namespace expr
...
struct OpIntegral : OpExpression<OpIntegral<V, E, T>> {
  ...
  void update(eval_handler_type const &eval_handler,
    ...
    eval_handler.result_sum(e, data);
  }
...
struct OpIntegral<V, E, expr::variational_t<T>>
  ...
  OpIntegral(V value, E const &e, symphas::grid_info const &domain)
      : value{value},
        ...
        data{result_t{}},
        e{e},
        normalization{1. / domain.element_area()} {}
  ...
  void update(eval_handler_type const &eval_handler,
              symphas::lib::types_list<condition_ts...>) {
    /*auto area =
        expr::result_sum_by_term<expr::matching_in_mul<expr::matches_series>,
                                 expr::matches_series>(e);*/
    result_t area{};
    eval_handler.result_sum(e, area);
  ...
  size_t print(FILE *out) const {
    return expr::integral_print<expr::variational_t<T>>{}(out, domain,
                                                          value * e);
  }
...
auto operator*(coeff_t const &value, OpIntegral<V2, E2, T2> const &b) {
  return expr::make_integral(
      (value * b.value) * expr::get_enclosed_expression(b), b.domain);
}
...
auto operator*(coeff_t const &value,
               OpIntegral<V2, E2, expr::variational_t<T2>> const &b) {
  return expr::make_domain_integral(
      (value * b.value) * expr::get_enclosed_expression(b), b.domain);
}
...
template <typename coeff_t, typename tensor_t, typename E2, typename T2,
          typename std::enable_if_t<
              (expr::is_coeff<coeff_t> && expr::is_tensor<tensor_t>), int> = 0>
auto operator*(coeff_t const &value,
               OpIntegral<tensor_t, E2, expr::variational_t<T2>> const &b) {
  return expr::make_domain_integral(
      (value * b.value) * expr::get_enclosed_expression(b), b.domain);
}
...
template <typename V, typename E, typename T>
inline auto make_integral::get(V const &v, OpOperator<E> const &e,
                               T const &domain) {
  return OpIntegral<V, E, T>(v, *static_cast<E const *>(&e), domain);
}

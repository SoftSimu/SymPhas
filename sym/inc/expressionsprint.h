...
struct expr_name_arr {
  expr_name_arr(const char* arr) : value{} { std::strcpy(value, arr); }
...
struct expr_name_arr<0> {
  ...
  expr_name_arr() : expr_name_arr(size_t(0)) {}
  expr_name_arr(int len) : expr_name_arr(size_t(len)) {}
  expr_name_arr(size_t len) : value{(len > 0) ? new char[len]{} : nullptr} {}
  expr_name_arr(const char* value)
      : value{(std::strlen(value) > 0) ? new char[std::strlen(value) + 1]{}
                                       : nullptr} {
  ...
  expr_name_arr(expr_name_arr<0> const& other)
      : expr_name_arr((other.value != nullptr)
                          ? len_type(std::strlen(other.value) + 1)
                          : 0) {
    if (value != nullptr) {
...
struct limit_data {
  limit_data(E const& e = E{}, iter_type index = 0, iter_type offset = 0,
             bool dynamic_index = false)
      : e{e}, index{index}, offset{offset}, dynamic_index{dynamic_index} {}
  limit_data(iter_type index = 0, iter_type offset = 0,
             bool dynamic_index = false)
  ...
  auto operator!() const { return !std::is_same<E, OpVoid>::value; }
...
inline auto get_limit_data(int index) { return limit_data<OpVoid>(index); }

template <typename V, int N, int P>
auto get_limit_data(OpTerm<V, expr::symbols::i_<N, P>> const& index) {
  return limit_data<OpTerm<V, expr::symbols::i_<N, 0>>>(
      expr::coeff(index) * expr::symbols::i_<N, 0>{}, 0, P);
}
...
#define SYEX_DIRECTIONAL_DERIV_VAR_STR(NAME, VALUE, AXIS) \
  "\\frac{\\partial^{" VALUE "}" NAME "}{\\partial " AXIS "^{" VALUE "}}"
#define SYEX_DIRECTIONAL_DERIV_STR(VALUE, AXIS) \
  SYEX_DIRECTIONAL_DERIV_VAR_STR("", VALUE, AXIS)
#define SYEX_DIRECTIONAL_DERIV_1_VAR_STR(NAME, AXIS) \
  "\\frac{\\partial " NAME "}{\\partial " AXIS "}"
#define SYEX_DIRECTIONAL_DERIV_1_STR(AXIS) \
  SYEX_DIRECTIONAL_DERIV_1_VAR_STR("", AXIS)
#define SYEX_DERIV_STR(VALUE) "\\nabla^{" VALUE "}"
#define SYEX_DERIV_STR_1 "\\vec{\\nabla}"
#define SYEX_FUNCTIONAL_DERIV_VAR_STR(NAME, VAR) \
  "\\frac{\\delta " NAME "}{\\delta " VAR "}"
#define SYEX_FUNCTIONAL_DERIV_STR(VAR) SYEX_FUNCTIONAL_DERIV_VAR_STR("", VAR)
#else
//! Display string of the derivative.
#define SYEX_DIRECTIONAL_DERIV_VAR_STR(NAME, VALUE, AXIS) \
  "d" VALUE NAME "/d" AXIS VALUE
#define SYEX_DIRECTIONAL_DERIV_STR(VALUE, AXIS) \
  SYEX_DIRECTIONAL_DERIV_VAR_STR("", VALUE, AXIS)
#define SYEX_DIRECTIONAL_DERIV_1_VAR_STR(NAME, AXIS) \
  SYEX_DIRECTIONAL_DERIV_VAR_STR(NAME, "", AXIS)
#define SYEX_DIRECTIONAL_DERIV_1_STR(AXIS) \
  SYEX_DIRECTIONAL_DERIV_1_VAR_STR("", AXIS)
...
#define SYEX_DIRECTIONAL_DERIV_VAR_WITH_OTHER_FMT(NAME, ORDER, AXIS, OTHER) \
  SYEX_DIRECTIONAL_DERIV_VAR_STR("%s", "%zd", "%s"), ORDER, NAME,           \
      ((AXIS == Axis::X)   ? "x"                                            \
       : (AXIS == Axis::Y) ? "y"                                            \
       : (AXIS == Axis::Z) ? "z"                                            \
                           : OTHER),                                        \
      ORDER
#define SYEX_DIRECTIONAL_DERIV_VAR_FMT(NAME, ORDER, AXIS) \
  SYEX_DIRECTIONAL_DERIV_VAR_WITH_OTHER_FMT(NAME, ORDER, AXIS, "?")
#define SYEX_DIRECTIONAL_DERIV_VAR_LEN(NAME, ORDER)          \
  (STR_ARR_LEN(SYEX_DIRECTIONAL_DERIV_VAR_STR("", "", "")) + \
   2 * symphas::lib::num_digits<ORDER>() + std::strlen(NAME) + 1)

#define SYEX_DIRECTIONAL_DERIV_WITH_OTHER_FMT(ORDER, AXIS, OTHER) \
  SYEX_DIRECTIONAL_DERIV_STR("%zd", "%s"), ORDER,                 \
      ((AXIS == Axis::X)   ? "x"                                  \
       : (AXIS == Axis::Y) ? "y"                                  \
       : (AXIS == Axis::Z) ? "z"                                  \
                           : OTHER),                              \
      ORDER
#define SYEX_DIRECTIONAL_DERIV_FMT(ORDER, AXIS) \
  SYEX_DIRECTIONAL_DERIV_WITH_OTHER_FMT(ORDER, AXIS, "?")
#define SYEX_DIRECTIONAL_DERIV_LEN(ORDER)            \
  (STR_ARR_LEN(SYEX_DIRECTIONAL_DERIV_STR("", "")) + \
   2 * symphas::lib::num_digits<ORDER>() + 1)

#define SYEX_DIRECTIONAL_DERIV_1_VAR_WITH_OTHER_FMT(NAME, AXIS, OTHER) \
  SYEX_DIRECTIONAL_DERIV_1_VAR_STR("%s", "%s"), NAME,                  \
      ((AXIS == Axis::X)   ? "x"                                       \
       : (AXIS == Axis::Y) ? "y"                                       \
       : (AXIS == Axis::Z) ? "z"                                       \
                           : OTHER)
#define SYEX_DIRECTIONAL_DERIV_1_VAR_FMT(NAME, AXIS) \
  SYEX_DIRECTIONAL_DERIV_1_VAR_WITH_OTHER_FMT(NAME, AXIS, "?")
#define SYEX_DIRECTIONAL_DERIV_1_VAR_LEN(NAME)                                 \
  (STR_ARR_LEN(SYEX_DIRECTIONAL_DERIV_1_VAR_STR("", "")) + std::strlen(NAME) + \
   1)

#define SYEX_DIRECTIONAL_DERIV_1_WITH_OTHER_FMT(AXIS, OTHER)     \
  SYEX_DIRECTIONAL_DERIV_1_STR("%s"), ((AXIS == Axis::X)   ? "x" \
                                       : (AXIS == Axis::Y) ? "y" \
                                       : (AXIS == Axis::Z) ? "z" \
                                                           : OTHER)
#define SYEX_DIRECTIONAL_DERIV_1_FMT(AXIS) \
  SYEX_DIRECTIONAL_DERIV_1_WITH_OTHER_FMT(AXIS, "?")
#define SYEX_DIRECTIONAL_DERIV_1_LEN \
  (STR_ARR_LEN(SYEX_DIRECTIONAL_DERIV_1_STR("")) + 1)

#define SYEX_FUNCTIONAL_DERIV_VAR_FMT(NAME, VAR) \
  SYEX_FUNCTIONAL_DERIV_VAR_STR("%s", "%s"), NAME, VAR
#define SYEX_FUNCTIONAL_DERIV_VAR_LEN(NAME, VAR)                            \
  (STR_ARR_LEN(SYEX_FUNCTIONAL_DERIV_VAR_STR("", "")) + std::strlen(NAME) + \
   std::strlen(VAR))

#define SYEX_FUNCTIONAL_DERIV_FMT(VAR) SYEX_FUNCTIONAL_DERIV_STR("%s"), VAR
#define SYEX_FUNCTIONAL_DERIV_LEN(VAR) \
  (STR_ARR_LEN(SYEX_FUNCTIONAL_DERIV_STR("")) + std::strlen(VAR))

#define SYEX_DERIV_STR_FMT(ORDER) SYEX_DERIV_STR("%zd"), ORDER
#define SYEX_DERIV_STR_LEN(ORDER) \
  (STR_ARR_LEN(SYEX_DERIV_STR("")) + symphas::lib::num_digits<ORDER>())
...
//\
 //	{
//\
 //		static const char c = (ax == Axis::X) ? 'x' : (ax == Axis::Y) ?
//'y' : (ax == Axis::Y) ? 'z' : '?' \
 //		\
...
//\
 //		static constexpr size_t print_length() { return STR_ARR_LEN(FMT)
//- 1; }		\
 //	};
...
#define SYEX_PRINT_FUNCTION_STRUCT_IMPL(F, FMT_STR)                            \
  ...
  static size_t print(                                                         \
      wrap_f<&symphas::math::F<typename expr::eval_type<E>::type>>, FILE* out, \
      const char* str) {                                                       \
    return fprintf(out, FMT_STR, str);                                         \
  }                                                                            \
  static size_t print(                                                         \
      wrap_f<&symphas::math::F<typename expr::eval_type<E>::type>>, char* out, \
      const char* str) {                                                       \
    return sprintf(out, FMT_STR, str);                                         \
  }                                                                            \
  static constexpr size_t print_length(                                        \
      wrap_f<&symphas::math::F<typename expr::eval_type<E>::type>>) {          \
    return sizeof(FMT_STR) / sizeof(char) - 3;                                 \
  }

#ifdef LATEX_PLOT
#define SYEX_DEFINE_FUNCTION_STRUCT_DELEGATE(F, FMT_STR, _) \
  SYEX_PRINT_FUNCTION_STRUCT_IMPL(F, FMT_STR)
#else
#define SYEX_DEFINE_FUNCTION_STRUCT_DELEGATE(F, _, FMT_STR) \
  SYEX_PRINT_FUNCTION_STRUCT_IMPL(F, FMT_STR)
#endif

//! Generates code which will associate a function with a display name.
#define SYEX_PRINT_FUNCTION_STRUCT(F, FMT_STR_LATEX, FMT_STR) \
  SYEX_DEFINE_FUNCTION_STRUCT_DELEGATE(F, FMT_STR_LATEX, FMT_STR)
...
#define SYEX_VECTOR_FMT_LEN \
  STR_ARR_LEN(SYEX_VECTOR_FMT_A SYEX_VECTOR_FMT_SEP SYEX_VECTOR_FMT_B)
...
#define SYEX_FT_OF_EXPR_FMT_LEN \
  (STR_ARR_LEN(SYEX_FT_OF_EXPR_FMT_A SYEX_FT_OF_EXPR_FMT_B) - 1)
#define SYEX_FT_OF_OP_FMT SYEX_FT_OF_OP_FMT_A "%s" SYEX_FT_OF_OP_FMT_B
#define SYEX_FT_OF_OP_FMT_LEN \
  (STR_ARR_LEN(SYEX_FT_OF_OP_FMT_A SYEX_FT_OF_OP_FMT_B) - 1)

#define SYEX_IFT_OF_EXPR_FMT SYEX_IFT_OF_EXPR_FMT_A "%s" SYEX_IFT_OF_EXPR_FMT_B
#define SYEX_IFT_OF_EXPR_FMT_LEN \
  (STR_ARR_LEN(SYEX_IFT_OF_EXPR_FMT_A SYEX_IFT_OF_EXPR_FMT_B) - 1)
#define SYEX_IFT_OF_OP_FMT SYEX_IFT_OF_OP_FMT_A "%s" SYEX_IFT_OF_OP_FMT_B
#define SYEX_IFT_OF_OP_FMT_LEN \
  (STR_ARR_LEN(SYEX_IFT_OF_OP_FMT_A SYEX_IFT_OF_OP_FMT_B) - 1)
...
DLLEXPR extern int NAME_PTR_POS;  //!< Current position in selecting name for
                                  //!< arbitrary data pointers.
DLLEXPR extern std::vector<const void*>
    NAME_PTRS;  //!< List of all data pointers with names associated with them.
DLLEXPR extern std::vector<char*>
    MORE_NAMES;  //!< List of overflow names for data pointers.
...
const char* get_op_name(VectorComponent<ax, G> const& a) {
  try {
    ...
    const char* name =
        get_op_name(SymbolID<G>::get(*static_cast<G const*>(&a)));
    ...
    if (with_component == map.end()) {
      ...
      map[name][std::strlen(name) + 1] = (ax == Axis::X)   ? 'x'
                                         : (ax == Axis::Y) ? 'y'
                                         : (ax == Axis::Z) ? 'z'
                                                           : '?';
...
const char* get_op_name(Variable<N> const& a) {
  ...
  if (N >= NN) {
    ...
    for (size_t i = NN; i <= N; ++i) {
      new_names[i] =
          new char[STR_ARR_LEN(prefix) + symphas::lib::num_digits(N)];
...
inline char*& get_op_name_reference(const void* ptr) {
  if (!ptr) {
    ...
    constexpr size_t MAX_NAME_COUNT =
        sizeof(VARIABLE_NAMES) / sizeof(*VARIABLE_NAMES);
...
struct print_f_op<E> {
 protected:
  static inline const char default_fmt[] =
      SYEX_UNKNOWN_FUNCTION_TOKEN SYEX_DERIV_APPLIED_EXPR_FMT;

  static size_t print(wrap_base, FILE* out) {
    return fprintf(out, default_fmt);
  }
  static size_t print(wrap_base, char* out) {
    return sprintf(out, default_fmt);
  }
  static constexpr size_t print_length(wrap_base, char* out, const char* str) {
    return sizeof(default_fmt) / sizeof(char) - 3;
  }

  SYEX_PRINT_FUNCTION_STRUCT(conj, "\\overline{%s}",
                             "conj" SYEX_DERIV_APPLIED_EXPR_FMT);
  SYEX_PRINT_FUNCTION_STRUCT(modulus, "\\left|{%s}\\right|", "|%s|");
  SYEX_PRINT_FUNCTION_STRUCT(real, "\\Re" SYEX_DERIV_APPLIED_EXPR_FMT,
                             "Re" SYEX_DERIV_APPLIED_EXPR_FMT);
  SYEX_PRINT_FUNCTION_STRUCT(imag, "\\Im" SYEX_DERIV_APPLIED_EXPR_FMT,
                             "Im" SYEX_DERIV_APPLIED_EXPR_FMT);
  SYEX_PRINT_FUNCTION_STRUCT(cos, "\\cos" SYEX_DERIV_APPLIED_EXPR_FMT,
                             "cos" SYEX_DERIV_APPLIED_EXPR_FMT);
  SYEX_PRINT_FUNCTION_STRUCT(sin, "\\sin" SYEX_DERIV_APPLIED_EXPR_FMT,
                             "sin" SYEX_DERIV_APPLIED_EXPR_FMT);
  SYEX_PRINT_FUNCTION_STRUCT(tan, "\\tan" SYEX_DERIV_APPLIED_EXPR_FMT,
                             "tan" SYEX_DERIV_APPLIED_EXPR_FMT);
  SYEX_PRINT_FUNCTION_STRUCT(cosh, "\\cosh" SYEX_DERIV_APPLIED_EXPR_FMT,
                             "cosh" SYEX_DERIV_APPLIED_EXPR_FMT);
  SYEX_PRINT_FUNCTION_STRUCT(sinh, "\\sinh" SYEX_DERIV_APPLIED_EXPR_FMT,
                             "sinh" SYEX_DERIV_APPLIED_EXPR_FMT);
  SYEX_PRINT_FUNCTION_STRUCT(tanh, "\\tanh" SYEX_DERIV_APPLIED_EXPR_FMT,
                             "tanh" SYEX_DERIV_APPLIED_EXPR_FMT);
  SYEX_PRINT_FUNCTION_STRUCT(acos, "\\acos" SYEX_DERIV_APPLIED_EXPR_FMT,
                             "acos" SYEX_DERIV_APPLIED_EXPR_FMT);
  SYEX_PRINT_FUNCTION_STRUCT(asin, "\\asin" SYEX_DERIV_APPLIED_EXPR_FMT,
                             "asin" SYEX_DERIV_APPLIED_EXPR_FMT);
  SYEX_PRINT_FUNCTION_STRUCT(atan, "\\atan" SYEX_DERIV_APPLIED_EXPR_FMT,
                             "atan" SYEX_DERIV_APPLIED_EXPR_FMT);
  SYEX_PRINT_FUNCTION_STRUCT(acosh, "\\acosh" SYEX_DERIV_APPLIED_EXPR_FMT,
                             "acosh" SYEX_DERIV_APPLIED_EXPR_FMT);
  SYEX_PRINT_FUNCTION_STRUCT(asinh, "\\asinh" SYEX_DERIV_APPLIED_EXPR_FMT,
                             "asinh" SYEX_DERIV_APPLIED_EXPR_FMT);
  SYEX_PRINT_FUNCTION_STRUCT(atanh, "\\atanh" SYEX_DERIV_APPLIED_EXPR_FMT,
...
struct print_deriv<O, Axis::NONE, false> {
  ...
  static size_t print_length(Sp const& = 0) {
    if constexpr (O == 1) {
      return STR_ARR_LEN(SYEX_DERIV_STR_1);
    } else if constexpr (O % 2 == 1) {
      return STR_ARR_LEN(SYEX_DERIV_STR_1 + SYEX_DERIV_STR_LEN(O - 1));
    } else {
      return SYEX_DERIV_STR_LEN(O);
    }
  }
  ...
  static size_t print(FILE* out, SymbolicDerivative<G>) {
    return fprintf(out, SYEX_DIRECTIONAL_DERIV_STR("%zd", "%s"), O,
                   expr::get_op_name(G{}), O);
  }
  ...
  static size_t print(FILE* out, const char* name, SymbolicDerivative<G>) {
    return fprintf(out, SYEX_DIRECTIONAL_DERIV_VAR_STR("%s", "%zd", "%s"), O,
                   name, expr::get_op_name(G{}), O);
  }
  ...
  static size_t print(char* out, SymbolicDerivative<G>) {
    return sprintf(out, SYEX_DIRECTIONAL_DERIV_STR("%zd", "%s"), O,
                   expr::get_op_name(G{}), O);
  }
  ...
  static size_t print(char* out, const char* name, SymbolicDerivative<G>) {
    return sprintf(out, SYEX_DIRECTIONAL_DERIV_VAR_STR("%s", "%zd", "%s"), O,
                   name, expr::get_op_name(G{}), O);
  }
...
struct print_deriv<O, Axis::NONE, true> {
  ...
  static size_t print(FILE* out, G const& var) {
    ...
    if constexpr (O == 1) {
      n += fprintf(out,
                   SYEX_DIRECTIONAL_DERIV_1_WITH_OTHER_FMT(Axis::NONE, buffer));
    } else {
      n += fprintf(
          out, SYEX_DIRECTIONAL_DERIV_WITH_OTHER_FMT(O, Axis::NONE, buffer));
    }
    delete[] buffer;
    return n;
  }
  ...
  static size_t print(FILE* out, const char* name, G const& var) {
    ...
    auto buffer = set_var_string(var);
    if constexpr (O == 1) {
      n += fprintf(out, SYEX_DIRECTIONAL_DERIV_1_VAR_WITH_OTHER_FMT(
                            name, Axis::NONE, buffer));
    } else {
      n += fprintf(out, SYEX_DIRECTIONAL_DERIV_VAR_WITH_OTHER_FMT(
                            name, O, Axis::NONE, buffer));
    }
    delete[] buffer;
    return n;
  }
  ...
  static size_t print(char* out, const char* name, G const& var) {
    ...
    auto buffer = set_var_string(var);
    if constexpr (O == 1) {
      n += sprintf(out, SYEX_DIRECTIONAL_DERIV_1_VAR_WITH_OTHER_FMT(
                            name, Axis::NONE, buffer));
    } else {
      n += sprintf(out, SYEX_DIRECTIONAL_DERIV_VAR_WITH_OTHER_FMT(
                            name, O, Axis::NONE, buffer));
    }
    delete[] buffer;
    return n;
...
template <Axis ax, size_t... Os>
struct next_deriv_type;
template <Axis ax, size_t O1, size_t O2, size_t... Os>
struct next_deriv_type<ax, O1, O2, Os...> {
  using type = print_mixed_deriv<ax, O1, O2, Os...>;
};
template <Axis ax, size_t O1>
struct next_deriv_type<ax, O1> {
  using type = print_deriv<O1, ax, true>;
};

template <Axis ax, size_t O, size_t... Os>
struct print_mixed_deriv<ax, O, Os...> {
  static const Axis next_ax = (ax == Axis::X)   ? Axis::Y
                              : (ax == Axis::Y) ? Axis::Z
                                                : Axis::NONE;

  using next_type = typename next_deriv_type<next_ax, Os...>::type;
...
template <typename G>
auto var_str(
    SymbolicFunctionalDerivative<DynamicVariable<NamedData<G>>> const& solver) {
  return solver.get_name();
}

template <size_t Z, typename G>
auto var_str(
    SymbolicFunctionalDerivative<Variable<Z, NamedData<G>>> const& solver) {
  return solver.get_name();
}

template <typename G0>
struct print_functional_deriv {
  /*	print_functional_deriv(const char* name) : var{ (std::strlen(name) > 0)
     ? new char[std::strlen(name) + 1] : nullptr }
          {
                  std::strcpy(var, name);
          }
     ...
     {}
          ...
          print_functional_deriv(print_functional_deriv<G0> const& other) :
     print_functional_deriv(other.var) {}
          print_functional_deriv(print_functional_deriv<G0>&& other) :
     print_functional_deriv()
          {
                  swap(*this, other);
          }
          print_functional_deriv<G0>& operator=(print_functional_deriv<G0>
     other)
          {
                  swap(*this, other);
                  return *this;
          ...
          friend void swap(print_functional_deriv<G0>& first,
     print_functional_deriv<G0>& second)
          {
                  using std::swap;
...
auto select_print_deriv(
    typename Solver<Sp>::template mixed_derivative<O1, O2>) {
  return print_mixed_deriv<Axis::X, O1, O2>{};
}

template <typename Sp, size_t O1, size_t O2, size_t O3>
auto select_print_deriv(
    typename Solver<Sp>::template mixed_derivative<O1, O2, O3>) {
...
auto get_fourier_name(T const& t) {
  ...
  return std::string(SYEX_FT_OF_OP_FMT_A) + std::string(name) +
 ...
 * coefficient which is prepended to an expression.
 */
inline size_t coeff_print_length(OpIdentity) { return 0; }
 ...
 * coefficient which is prepended to an expression.
 */
inline size_t coeff_print_length(OpNegIdentity) { return 1; }
...
template <typename T>
size_t print_tensor_entries_1(char* out, T const& value, size_t P0, size_t P1,
                              size_t N, size_t M) {
  size_t n = 0;
...
template <typename T>
size_t print_tensor_entries_1(FILE* out, T const& value, size_t P0, size_t P1,
                              size_t N, size_t M) {
  size_t n = 0;
...
template <typename ostream_t, typename T, size_t P0, size_t P1, size_t N,
          size_t M>
size_t print_tensor(ostream_t* out, T const& value, std::index_sequence<P0, P1>,
                    std::index_sequence<N, M>) {
  return print_tensor_entries_1(out, value, P0, P1, N, M);
}

template <typename ostream_t, typename T, size_t P0, size_t N>
size_t print_tensor(ostream_t* out, T const& value, std::index_sequence<P0>,
                    std::index_sequence<N>) {
  return print_tensor_entries_1(out, value, P0, 0, N, 1);
}

template <typename T>
size_t print_tensor_length_1(T const& value, size_t P0, size_t P1, size_t N,
                             size_t M) {
  size_t n = 0;
...
size_t print_tensor_length(T const& value, std::index_sequence<P0, P1>,
                           std::index_sequence<N, M>) {
  return print_tensor_length_1(value, P0, P1, N, M);
}

template <typename T, size_t P0, size_t N>
size_t print_tensor_length(T const& value, std::index_sequence<P0>,
                           std::index_sequence<N>) {
...
size_t coeff_print_length(OpTensor<T, Ns...> const& value) {
  using p_seq = symphas::lib::seq_join_t<symphas::lib::types_before_index<
      sizeof...(Ns) / 2, std::index_sequence<Ns>...>>;
  using n_seq = symphas::lib::seq_join_t<symphas::lib::types_after_at_index<
      sizeof...(Ns) / 2, std::index_sequence<Ns>...>>;
...
template <typename T, size_t D>
size_t print_with_coeff(char* out, const char* expr,
                        any_vector_t<T, D> const& value);
template <typename T, typename I>
size_t print_with_coeff(char* out, const char* expr,
                        OpCoeff<T, I> const& value);
...
template <typename T, size_t D>
size_t print_with_coeff(FILE* out, const char* expr,
                        any_vector_t<T, D> const& value);
template <typename T, typename I>
size_t print_with_coeff(FILE* out, const char* expr,
                        OpCoeff<T, I> const& value);
 ...
 *
 * \param out The file to put the printed expression.  * \param expr The
...
template <size_t N, size_t LN, size_t I, size_t L0,
          size_t L = symphas::lib::num_digits<N>()>
void print_number(expr_name_arr<L0>& ptr) {
  constexpr size_t N0 = N / symphas::math::fixed_pow<10, L - I - 1>;
  ptr.value[LN + I] = (N0 == 0)   ? '0'
                      : (N0 == 1) ? '1'
                      : (N0 == 2) ? '2'
                      : (N0 == 3) ? '3'
                      : (N0 == 4) ? '4'
  ...
  if constexpr (L > 1) {
    print_number<N0 * symphas::math::fixed_pow<10, L - I - 1> - N, LN, I + 1>(
        ptr);
  }
}
}  // namespace
...
template <
    int N, size_t L, size_t L0 = L - 1,
    size_t NN = (N < 0) ? static_cast<size_t>(-N) : static_cast<size_t>(N),
#ifdef LATEX_PLOT
    size_t L1 = 1
#else
    size_t L1 = 0
...
expr_name_arr<L0 + L1 + L2 + L3 + 3> print_with_subscript(
    const char (&term)[L]) {
...
#endif
          >
expr_name_arr<0> print_with_subscript(const char (&term)[L],
...
auto print_list(E0&& e0, Es&&... es) {
  const char* names[]{expr::get_op_name(std::forward<E0>(e0)),
                      expr::get_op_name(std::forward<Es>(es))...};
  len_type len = 0;
...
auto get_fourier_name(OpExpression<E> const& e) {
  ...
  e.print(name);
  return std::string(SYEX_FT_OF_EXPR_FMT_A) + std::string(name) +
         std::string(SYEX_FT_OF_EXPR_FMT_B);
}
...
struct noise_name_print {
  ...
  auto operator()() { return STR_ARR_LEN(SYEX_NOISE_TOKEN_NONE); }
};
...
struct noise_name_print<expr::NoiseType::POISSON> {
  ...
  auto operator()() { return STR_ARR_LEN(SYEX_NOISE_TOKEN_POISSON); }
};
...
struct symbolic_eval_print<SymbolicListIndex<E0>> {
  ...
  size_t operator()(SymbolicListIndex<E0> const& data, E const& e) {
    return data.e.print_length() + e.print_length() +
           STR_ARR_LEN(SYEX_ARRAY_A SYEX_ARRAY_B SYEX_ARRAY_SUBSCRIPT_A
                           SYEX_ARRAY_SUBSCRIPT_B);
  }
};
...
inline size_t print_limit(FILE* out, limit_data<OpVoid> const& e,
                          bool upper = false);
template <typename E>
size_t print_limit(char* out, limit_data<E> const& e, bool upper = false);
inline size_t print_limit(char* out, limit_data<OpVoid> const& e,
                          bool upper = false);
template <typename E>
size_t print_limit_length(limit_data<E> const& e, bool upper = false);
inline size_t print_limit_length(limit_data<OpVoid> const& e,
                                 bool upper = false);
...
size_t print_limit(FILE* out, OpTerm<V, expr::symbols::i_<N, P>>,
                   bool upper = false) {
  if constexpr (P > 0) {
    return fprintf(out, "%s+%d", expr::get_op_name(expr::symbols::i_<N, 0>{}),
                   P);
...
size_t print_limit(FILE* out, OpAdd<Es...> const& e,
                   std::index_sequence<I0, Is...>, bool upper = false) {
  size_t n = 0;
...
size_t print_limit(char* out, OpTerm<V, expr::symbols::i_<N, P>>,
                   bool upper = false) {
  if constexpr (P > 0) {
    return sprintf(out, "%s+%d", expr::get_op_name(expr::symbols::i_<N, 0>{}),
                   P);
  } else if constexpr (P < 0) {
    return sprintf(out, "%s-%d", expr::get_op_name(expr::symbols::i_<N, 0>{}),
                   -P);
...
size_t print_limit(expr::symbols::i_<N, P>, FILE* out,
                   expr::series_limits<T1, T2> const& limit,
                   bool upper = false) {
  size_t n = 0;
  n += fprintf(out, "%s%s=", SYEX_SUM_LIM_A,
               expr::get_op_name(expr::symbols::i_<N, P>{}));
...
template <int N, int P, typename T1, typename T2>
size_t print_limit(expr::symbols::i_<N, P>, char* out,
                   expr::series_limits<T1, T2> const& limit,
                   bool upper = false) {
  size_t n = 0;
  n += sprintf(out, "%s%s=", SYEX_SUM_LIM_A,
               expr::get_op_name(expr::symbols::i_<N, P>{}));
...
template <typename V, int N, int P>
size_t print_limit_length(OpTerm<V, expr::symbols::i_<N, P>>,
                          bool upper = false) {
  if constexpr (P > 0) {
    return 1 + std::strlen(expr::get_op_name(expr::symbols::i_<N, 0>{})) +
           symphas::lib::num_digits(P);
  } else if constexpr (P < 0) {
    return 2 + std::strlen(expr::get_op_name(expr::symbols::i_<N, 0>{})) +
           symphas::lib::num_digits(P);
  } else {
...
template <typename... Es, size_t I0, size_t... Is>
size_t print_limit_length(OpAdd<Es...> const& e, std::index_sequence<I0, Is...>,
                          bool upper = false) {
  size_t n = 0;
...
size_t print_limit_length(std::pair<L, R> const& e, bool upper = false) {
  size_t n = 0;
  n += STR_ARR_LEN(
      SYEX_SUM_LIM_COMPARE_A SYEX_SUM_LIM_COMPARE_SEP SYEX_SUM_LIM_COMPARE_B);
...
template <typename sub_t, typename E, int... I0s, int... P0s, typename... T1s,
          typename... T2s, typename B>
struct symbolic_eval_print<SymbolicSeries<
    expr::sum_op, sub_t,
    symphas::lib::types_list<
        E, symphas::lib::types_list<expr::symbols::i_<I0s, P0s>...>,
        symphas::lib::types_list<expr::series_limits<T1s, T2s>...>, B>>> {
  using sum_t = SymbolicSeries<
      expr::sum_op, sub_t,
      symphas::lib::types_list<
          E, symphas::lib::types_list<expr::symbols::i_<I0s, P0s>...>,
          symphas::lib::types_list<expr::series_limits<T1s, T2s>...>, B>>;
  ...
  template <size_t... Ns>
  size_t print_limits(
      FILE* out, std::tuple<expr::series_limits<T1s, T2s>...> const& limits,
      std::index_sequence<Ns...>) {
    return (
        print_limit(
            symphas::lib::type_at_index<Ns, expr::symbols::i_<I0s, P0s>...>{},
            out, std::get<Ns>(limits)) +
        ...);
  }

  template <size_t... Ns>
  size_t print_limits(
      char* out, std::tuple<expr::series_limits<T1s, T2s>...> const& limits,
      std::index_sequence<Ns...>) {
    size_t n = 0;
    ((n += print_limit(
          symphas::lib::type_at_index<Ns, expr::symbols::i_<I0s, P0s>...>{},
          out + n, std::get<Ns>(limits))),
     ...);
    return n;
  }

  template <size_t... Ns>
  size_t print_limits_length(
      std::tuple<expr::series_limits<T1s, T2s>...> const& limits,
      std::index_sequence<Ns...>) {
    size_t n = 0;
    ((n += print_limit_length(
          symphas::lib::type_at_index<Ns, expr::symbols::i_<I0s, P0s>...>{},
          std::get<Ns>(limits))),
     ...);
    return n;
  ...
  size_t operator()(char* out, sum_t const& sum, E0 const& e) {
    ...
    n += print_limits(out + n, sum.limits,
                      std::make_index_sequence<sizeof...(I0s)>{});
  ...
  size_t operator()(sum_t const& sum, E0 const& e) {
    ...
    n += print_limits_length(sum.limits,
                             std::make_index_sequence<sizeof...(I0s)>{});
    n += e.print_length();
    return n;
  }
...
struct integral_print {
  ...
  size_t operator()(FILE* out, T const& domain, E const& e) {
    ...
    n += fprintf(out,
                 SYEX_INTEGRAL_LIM_A "%s" SYEX_INTEGRAL_LIM_SEP
                                     "%s" SYEX_INTEGRAL_LIM_B,
                 "", "");
  ...
  size_t operator()(symphas::grid_info const&, E const& e) {
    size_t n = 0;
    n += STR_ARR_LEN(
        SYEX_INTEGRAL_SYMBOL SYEX_INTEGRAL_LIM_A SYEX_INTEGRAL_LIM_SEP
            SYEX_INTEGRAL_LIM_B SYEX_INTEGRAL_DOMAIN_SYM
                SYEX_INTEGRAL_INTEGRATION_SYM SYEX_INTEGRAL_A SYEX_INTEGRAL_B);
    n += e.print_length();
    return n;
  }
};
 ...
 * not used. Therefore, the print function can still be invoked but there is no
 * effect.
 */
template <typename... T>
auto printe(T&&...) {}
...
/*!
 * Empty implementation for the obtaining the name of a variable when
 * #PRINTABLE_EQUATIONS is not used. Therefore, this function can still be
 * invoked but there is no effect.
 */
template <typename... T>
auto get_op_name(T&&...) {
  return "";

...
#define SYEX_MUL_FMT_LEN \
  (STR_ARR_LEN(SYEX_MUL_FMT_A SYEX_MUL_SEP SYEX_MUL_FMT_B) - 1)

#define SYEX_DIV_FMT SYEX_DIV_FMT_A "%s" SYEX_DIV_SEP "%s" SYEX_DIV_FMT_B
#define SYEX_DIV_FMT_LEN \
  (STR_ARR_LEN(SYEX_DIV_FMT_A SYEX_DIV_SEP SYEX_DIV_FMT_B) - 1)
...
#define SYEX_FRA_FMT_LEN \
  (STR_ARR_LEN(SYEX_DIV_FMT_A SYEX_DIV_SEP_OP SYEX_DIV_FMT_B) - 1)

#define SYEX_LAMBDA_FUNC_FMT_A "%s("
#define SYEX_LAMBDA_FUNC_FMT_B ")"
#define SYEX_LAMBDA_FUNC_FMT \
  SYEX_DIV_FMT_A SYEX_LAMBDA_FUNC_FMT_A "%s" SYEX_LAMBDA_FUNC_FMT_B
#define SYEX_LAMBDA_FUNC_FMT_LEN \
  (STR_ARR_LEN(SYEX_LAMBDA_FUNC_FMT_A SYEX_LAMBDA_FUNC_FMT_B) - 3)
...
auto tensor_multiply(OpTensor<T, N0, N1, NA, NB, Ns...>,
                     OpTensor<S, M0, M1, MA, MB, Ms...>) = delete;

// Matrix multiplication
template <typename T, typename S, size_t N0, size_t N1, size_t N2, size_t N3,
          size_t NA, size_t NB, size_t NC>
auto tensor_multiply(OpTensor<T, N0, N1, NA, NB> const& a,
                     OpTensor<S, N2, N3, NB, NC> const& b) {
  if constexpr (N1 == N2) {
...
auto tensor_multiply(OpTensor<T, N0, NA> const& a,
                     OpTensor<S, 0, N1, 1, NB> const& b) {
  T at = tensor_cast::cast(a);
  S bt = tensor_cast::cast(b);
...
// Matrix multiplication
template <typename T, typename S, size_t N0, size_t N1, size_t N2, size_t NA,
          size_t NB>
auto tensor_multiply(OpTensor<T, N0, N1, NA, NB> const& a,
                     OpTensor<S, N2, NB> const& b) {
  if constexpr (N1 == N2) {
    T at = tensor_cast::cast(a);
    S bt = tensor_cast::cast(b);
...
auto tensor_multiply(OpTensor<OpIdentity, 0, 0> const& a,
                     OpTensor<T, Ms...> const& b) {
  return OpIdentity{};
}

template <typename T, size_t... Ms>
auto tensor_multiply(OpTensor<T, Ms...> const& a,
                     OpTensor<OpIdentity, 0, 0> const& b) {
  return OpIdentity{};
}

template <typename T, typename S, size_t... Ms>
auto tensor_multiply(OpTensor<T, Ms...> const& a, OpTensor<S, 0, 1> const& b) {
  auto v = symphas::internal::tensor_cast::cast(a) *
           symphas::internal::tensor_cast::cast(b);
  return expr::make_tensor<Ms...>(v);
}

template <typename T, typename S, size_t... Ms>
auto tensor_multiply(OpTensor<T, 0, 0, 1, 1> const& a,
                     OpTensor<S, Ms...> const& b) {
  auto v = symphas::internal::tensor_cast::cast(a) *
           symphas::internal::tensor_cast::cast(b);
  return expr::make_tensor<Ms...>(v);
}
...
template <typename S>
auto tensor_multiply(OpTensor<OpIdentity, 0, 0> const& a,
                     OpTensor<S, 0, 1> const& b) {
  return OpIdentity{};
}

template <typename S>
auto tensor_multiply(OpTensor<S, 0, 0, 1, 1> const& a,
                     OpTensor<OpIdentity, 0, 0> const& b) {
  return OpIdentity{};
}
...
auto tensor_multiply(OpTensor<T, 0, 1> const& a, OpTensor<S, 0, 1> const& b) {
  return symphas::internal::tensor_cast::cast(a) *
         symphas::internal::tensor_cast::cast(b);
}
...
template <typename T, typename S>
auto tensor_multiply(OpTensor<S, 0, 1> const& a,
                     OpTensor<T, 0, 0, 1, 1> const& b) {
  auto v = symphas::internal::tensor_cast::cast(a) *
           symphas::internal::tensor_cast::cast(b);
...
struct tensor_to_vector<OpTensor<T, N1, NA>> {
  using type =
      any_vector_t<std::invoke_result_t<decltype(&T::eval), T, iter_type>, NA>;
};

template <typename T, size_t N1, size_t N2, size_t NA, size_t NB>
struct tensor_to_vector<OpTensor<T, N1, N2, NA, NB>> {
  using type = any_vector_t<
      any_vector_t<std::invoke_result_t<decltype(&T::eval), T, iter_type>, NB>,
      NA>;
};
...
/*!
 * A tensor turns the expression for which it is a coefficient into an entry
 * of a tensor.
 *
 * A tensor of rank 1 is a column vector, and of rank 2 is a matrix. A row
 * vector is a tensor of rank 2 and has only 1 row. Multiplication only for
 * rank 1 and 2 vectors is supported, and there are no tensor products.
 */
template <typename T, size_t... Ns>
struct OpTensor : OpExpression<OpTensor<T, Ns...>> {
  ...
  auto operator-(OpTensor<S, Ns...> const& a) const {
    return expr::make_tensor<Ns...>(cast() -
                                    symphas::internal::tensor_cast::cast(a));
  }
  ...
  auto operator-() const { return expr::make_tensor<Ns...>(-cast()); }

  operator symphas::internal::tensor_to_vector_t<OpTensor<T, Ns...>>() const {
    symphas::internal::tensor_to_vector_t<OpTensor<T, Ns...>> vector;
    symphas::internal::set_vector<Ns...>(
        vector, symphas::internal::tensor_cast::cast(*this));
    return vector;
  }

  explicit operator T const&() const { return cast(); }

  explicit operator T&() { return cast(); }
  ...
  size_t print_length() const { return expr::coeff_print_length(*this); }
 ...
 protected:
  const T& cast() const { return symphas::internal::tensor_cast::cast(*this); }
...
auto operator*(OpTensor<T1, Ns...> const& tensor, OpLiteral<T2> const& value) {
  return expr::make_tensor<Ns...>(symphas::internal::tensor_cast::cast(tensor) *
                                  value);
}

template <typename T1, typename T2, size_t N0, size_t N1, size_t NB>
auto operator*(any_vector_t<T1, 1> const& value,
               OpTensor<T2, 0, N1, 1, NB> const& tensor) {
  return expr::make_tensor<0, N1, 1, NB>(
      value[0] * symphas::internal::tensor_cast::cast(tensor));
}

template <typename T1, typename T2, size_t N0, size_t N1, size_t NB>
auto operator*(any_vector_t<T1, 2> const& value,
               OpTensor<T2, 0, N1, 1, NB> const& tensor) {
...
template <typename T1, typename T2, size_t N0, size_t N1, size_t NB>
auto operator*(any_vector_t<T1, 3> const& value,
               OpTensor<T2, 0, N1, 1, NB> const& tensor) {
  auto v = (*static_cast<T2 const*>(&tensor));
...
auto operator*(OpTensor<T1, N0, NA> const& tensor,
               any_vector_t<T2, 1> const& value) {
  return expr::make_tensor<N0, NA>(value[0] *
                                   (*static_cast<T1 const*>(&tensor)));
}

template <typename T1, typename T2, size_t N0, size_t N1, size_t NA>
auto operator*(OpTensor<T1, N0, N1, NA, 2> const& tensor,
               any_vector_t<T2, 2> const& value) {
  return expr::make_tensor<N0, NA>(value[N1] *
                                   (*static_cast<T1 const*>(&tensor)));
}

template <typename T1, typename T2, size_t N0, size_t N1, size_t NA>
auto operator*(OpTensor<T1, N0, N1, NA, 3> const& tensor,
               any_vector_t<T2, 3> const& value) {
  return expr::make_tensor<N0, NA>(value[N1] *
                                   (*static_cast<T1 const*>(&tensor)));
}

template <typename T, size_t D>
auto operator*(OpTensor<OpIdentity, 0, 0> const&,
               OpLiteral<any_vector_t<T, D>> const&) {
  return OpIdentity{};
}
...
struct OpLiteral : OpExpression<OpLiteral<T>> {
  ...
  inline T eval(iter_type = 0) const { return value; }
  ...
  auto operator-() const { return expr::make_literal(-value); }
...
struct OpLiteral<any_vector_t<T, D>>
  ...
  operator const any_vector_t<T, D>&() const { return value; }

  operator any_vector_t<T, D>&() { return value; }
  ...
  size_t print(FILE* out) const { return expr::print_with_coeff(out, value); }

  size_t print(char* out) const { return expr::print_with_coeff(out, value); }

  size_t print_length() const { return expr::coeff_print_length(value); }
  ...
  if (int(value) == value) {
    ...
    if (std::abs(p) >= EXPR_VALUE_DISPLAY_PRECISION) {
      ...
      return fprintf(
          out,
          SYEX_SCIENTIFIC_A
          "%." STR(EXPR_VALUE_DISPLAY_PRECISION) "lf" SYEX_SCIENTIFIC_VALUE_A
                                                 "%d" SYEX_SCIENTIFIC_VALUE_B
                                                     SYEX_SCIENTIFIC_B,
          b, p);
    } else {
...
struct OpFractionLiteral : OpExpression<OpFractionLiteral<N, D>> {
  ...
  size_t print(FILE* out) const { return fprintf(out, SYEX_FRA_FMT, N, D); }

  size_t print(char* out) const { return sprintf(out, SYEX_FRA_FMT, N, D); }
...
struct OpFractionLiteral<N, 1> : OpExpression<OpFractionLiteral<N, 1>> {
  ...
  size_t print(FILE* out) const { return fprintf(out, "%zd", N); }

  size_t print(char* out) const { return sprintf(out, "%zd", N); }

  size_t print_length() const { return symphas::lib::num_digits<N>(); }
...
template <typename T>
constexpr OpLiteral<T>::OpLiteral(OpIdentity)
    : value{symphas::lib::get_identity<T>()} {}
template <typename T>
constexpr OpLiteral<T>::OpLiteral(OpNegIdentity)
    : value{-symphas::lib::get_identity<T>()} {}
template <typename T>
...
struct construct_tensor {
  ...
  using pos_t =
      seq_join_t<types_before_index<N / 2, std::index_sequence<Ns>...>>;
  using dim_t =
      seq_join_t<types_after_at_index<N / 2, std::index_sequence<Ns>...>>;
...
constexpr auto expr::make_tensor(T const& v) {
  return symphas::internal::construct_tensor<N0, N1, Ns...>{}(
      expr::make_literal(v));
}
...
struct DynamicIndex : OpExpression<DynamicIndex> {
  ...
  DynamicIndex(iter_type&& data)
      : DynamicIndex(std::move(data), len_type(data)) {}
  ...
  DynamicIndex(iter_type* data, len_type start_index, len_type end_index)
      : DynamicIndex(*data, start_index, end_index) {}
  ...
  DynamicIndex(DynamicIndex&& other) : DynamicIndex() { swap(*this, other); }
  ...
  auto eval(iter_type = 0) const { return index(); }
  ...
  iter_type index() const { return *data; }

  len_type start() const { return start_index; }

  len_type end() const { return end_index; }

  len_type range() const { return end() - start() + 1; }

  explicit operator iter_type() const { return index(); }
  ...
  bool is_local() const { return (data == &data0); }
  ...
  void fix(DynamicIndex fixed) { swap(*this, fixed); }

  void fix(DynamicIndexSet const& fixed);

  bool same(DynamicIndex const& other) { return data == other.data; }

  bool same(iter_type* other) { return data == other; }
...
struct DynamicIndexSet {
  ...
  DynamicIndexSet(DynamicIndex const& index, iter_type value)
      : index{index.data}, value{std::move(value)} {}
  DynamicIndexSet(DynamicIndex const& index, iter_type* value)
      : index{index.data}, value{*value} {}

  operator bool() { return *index == value.index(); }
...
struct coeff_data {
  ...
  coeff_data(len_type len)
      : data{(len > 0) ? new T[len]{} : nullptr}, len{len} {}

  constexpr coeff_data() : coeff_data(nullptr, 0) {}

  coeff_data(coeff_data<T> const& other) : coeff_data(other.data, other.len) {}
  coeff_data(coeff_data<T>&& other) : coeff_data() { swap(*this, other); }

  auto operator=(coeff_data<T> other) { swap(*this, other); }
...
#ifdef PRINTABLE_EQUATIONS
  size_t print(FILE* out) const { return data.print(out); }

  size_t print(char* out) const { return data.print(out); }

  size_t print_length() const { return data.print_length(); }
#endif
...
struct SymbolID<DynamicIndex> {
  static const char* get(DynamicIndex const& data) {
    return SymbolID<expr::symbols::placeholder_N_symbol>::get(
...
//	typename std::enable_if_t<(expr::is_identity<E1> &&
// expr::is_identity<E2>), int> = 0> constexpr bool operator==(E1 const& a, E2
// const& b)
//{
...
//	typename std::enable_if_t<(expr::is_identity<A> &&
// expr::is_identity<B>), int> = 0> constexpr bool operator<(A const& a, B
// const&
// b)
//{
//	return a.eval() < b.eval();
...
//
// template<typename E1, typename E2,
//	typename std::enable_if_t<expr::identity_comparator_enabled<E1, E2>,
// int> = 0> inline bool operator==(E1 const& a, E2 const& b)
//{
//	return OpLiteral<identity_eval_t>(a).eval() ==
// OpLiteral<identity_eval_t>(b).eval();
//}
//
// template<typename E1, typename E2,
//	typename std::enable_if_t<expr::identity_comparator_enabled<E1, E2>,
...
// template<typename E1, typename E2,
//	typename std::enable_if_t<(expr::identity_comparator_enabled<E1, E2> ||
//(expr::is_identity<E1> && expr::is_identity<E2>)), int> = 0> inline bool
// operator!=(E1 const& a, E2 const& b)
//{
//	return !(a == b);
//}
//
...
size_t binary_print(OpAddList<Es...> const& e, os_type* out,
                    std::index_sequence<Is...>) {
  size_t n = 0;
...
struct cast_add<void> {
  ...
  static decltype(auto) cast(OpAdd<Es...> const& adds,
                             std::index_sequence<Is...>) {
...
inline auto to_add(OpAddList<>) { return OpVoid{}; }
...
inline auto to_add(OpVoid) { return OpVoid{}; }
...
const auto& get(OpAdd<Es...> const& e) {
  using Nth_type =
      typename symphas::internal::Nth_type_of_add<N, OpAdd<Es...>>::type;
...
decltype(auto) terms_before_n(OpAdd<Es...> const& e) {
  if constexpr (N == 0) {
    ...
    return symphas::internal::cast_add<void>::cast(
        e, std::make_index_sequence<N>{});
  }
}
...
decltype(auto) terms_after_n(OpAdd<Es...> const& e) {
  return symphas::internal::to_add(
      terms_after_n<N>(*static_cast<OpAddList<Es...> const*>(&e)));
}

template <size_t N, typename... Es>
decltype(auto) terms_after_n(OpAdd<Es...>& e) {
  return symphas::internal::to_add(
      terms_after_n<N>(*static_cast<OpAddList<Es...>*>(&e)));
}
...
struct OpAddList<E0, Es...> : OpAddList<Es...> {
  ...
  OpAddList(E0 const& e, OpAdd<Es...> const& rest)
      : parent_type(rest), term{e} {}
  // OpAddList(OpAdd<E0, Es...> const& list) :
  //     parent_type(*static_cast<OpAddList<Es...> const*>(&list)), term{
  //     list.term } {}
  ...
  auto _eval(std::index_sequence<Is...>, iter_type n = 0) const {
    return (
        static_cast<
            symphas::internal::Nth_type_of_add_t<Is, OpAdd<E0, Es...>> const&>(
            *this)
            .term.eval(n) +
        ...);
  }
  ...
  bool coeff_sign() const { return (expr::coeff(term).eval() < 0); }
  ...
  size_t print(FILE* out) const {
    ...
    return n + symphas::internal::binary_print(
                   *this, out, std::make_index_sequence<sizeof...(Es)>{});
  }

  size_t print(char* out) const {
    size_t n = term.print(out);
    return n + symphas::internal::binary_print(
                   *this, out + n, std::make_index_sequence<sizeof...(Es)>{});
  }
...
struct OpAdd<E0, Es...> : OpExpression<OpAdd<E0, Es...>>, OpAddList<E0, Es...> {
 ...
 public:
  ...
  inline auto eval(iter_type n = 0) const { return parent_type::_eval(n); }
...
inline OpVoid expr::make_add() { return OpVoid{}; }

inline OpVoid expr::make_add(OpVoid, OpVoid) { return OpVoid{}; }
...
auto expr::make_add(E1&& a, E2&& b, E3&& c, Es&&... es) {
  return make_add(make_add(std::forward<E1>(a), std::forward<E2>(b)),
                  std::forward<E3>(c), std::forward<Es>(es)...);
}
...
template <typename E1, typename E2>
size_t mul_print(FILE* out, OpExpression<E1> const& a,
...
template <typename E1, typename E2>
size_t mul_print(char* out, OpExpression<E1> const& a,
                 OpExpression<E2> const& b) {
  size_t n = 0;
  bool neg1 = expr::eval(expr::coeff(*static_cast<E1 const*>(&a))) < 0 ||
              expr::is_add<E1>;
...
template <typename E1, typename E2>
size_t mul_print(char* out, OpOperator<E1> const& a,
                 OpExpression<E2> const& b) {
  size_t n = 0;
  bool neg1 = false;
  bool neg2 = expr::eval(expr::coeff(*static_cast<E2 const*>(&b))) < 0 ||
...
template <typename V, typename E1, typename T, typename E2>
size_t mul_print(char* out, OpIntegral<V, E1, T> const& a,
                 OpExpression<E2> const& b) {
  size_t n = 0;
  bool neg1 = expr::eval(expr::coeff(a)) < 0;
  bool neg2 = true;
...
template <typename V1, typename E1, typename T1, typename V2, typename E2,
          typename T2>
size_t mul_print(FILE* out, OpIntegral<V1, E1, T1> const& a,
...
template <typename V1, typename E1, typename T1, typename V2, typename E2,
          typename T2>
size_t mul_print(char* out, OpIntegral<V1, E1, T1> const& a,
                 OpIntegral<V2, E2, T2> const& b) {
  size_t n = 0;
  bool neg1 = false;
  bool neg2 = false;
...
struct OpBinaryMul : OpExpression<OpBinaryMul<E1, E2>> {
  ...
  inline auto eval(iter_type n = 0) const { return a.eval(n) * b.eval(n); }
...
template <typename E1, typename E2,
          typename std::enable_if_t<(expr::eval_type<E1>::rank == 0 ||
                                    ...
                                    int> = 0>
auto operator*(OpExpression<E1> const& a, OpExpression<E2> const& b) {
  return expr::make_mul(*static_cast<const E1*>(&a),
...
auto operator*(OpExpression<E1> const& a, OpOperator<E2> const& b) {
  return OpOperatorChain(*static_cast<const E1*>(&a),
                         *static_cast<const E2*>(&b));
}

template <
    typename A, typename B, typename E2,
    typename std::enable_if_t<(expr::eval_type<OpBinaryDiv<A, B>>::rank == 0 ||
                               expr::eval_type<E2>::rank == 0),
                              int> = 0>
...
template <
    typename A, typename B, typename E2,
    typename std::enable_if_t<(expr::eval_type<OpBinaryMul<A, B>>::rank == 0 ||
                               expr::eval_type<E2>::rank == 0),
                              int> = 0>
...
struct OpBinaryDiv : OpExpression<OpBinaryDiv<E1, E2>> {
  ...
  //	typename = std::enable_if_t<(std::is_default_constructible<AA>::value&&
  // std::is_default_constructible<BB>::value), int>>
  OpBinaryDiv() : a{E1{}}, b{E2{}} {}

  inline auto eval(iter_type n) const { return a.eval(n) / b.eval(n); }
...
struct coeff_factor_impl<OpAdd<As...>> {
  using type =
      decltype(expr::make_fraction<
               GCD_of<coeff_factor_numerator<
                   typename coeff_factor_impl<As>::type>::value...>,
               GCD_of<coeff_factor_denominator<
                   typename coeff_factor_impl<As>::type>::value...>>());
};
...
auto make_div(OpEvaluable<E1> const& a, OpEvaluable<E2> const& b) {
  ...
  using fr_t = decltype(fr1 / fr2);
  constexpr int N = symphas::internal::coeff_factor_numerator<fr_t>::value;
  constexpr int D = symphas::internal::coeff_factor_denominator<fr_t>::value;

  auto _fr1 = expr::make_integer<N>() / fr1;
  auto _fr2 = expr::make_integer<D>() / fr2;

  return OpBinaryDiv(_fr1 * (*static_cast<const E1*>(&a)),
                     _fr2 * (*static_cast<const E2*>(&b)));
}
...
template <typename E1, typename E2, size_t R1 = expr::eval_type<E1>::rank,
          ...
          typename std::enable_if_t<(R1 > 0 || R2 > 0), int> = 0>
auto make_div(OpEvaluable<E1> const& a, OpEvaluable<E2> const& b) {
  return OpBinaryDiv((*static_cast<const E1*>(&a)),
                     (*static_cast<const E2*>(&b)));
}
...
struct char_to_val {
  ...
  constexpr auto left(symphas::lib::char_list<ln...>,
                      std::index_sequence<Is...>) {
    using namespace symphas::lib;
    return (expr::make_integer<get_value<ln>() *
                               fixed_pow<10, sizeof...(Is) - 1 - Is>>() +
            ... + OpVoid{});
  }

  template <char... ln>
  constexpr auto left(symphas::lib::char_list<ln...>) {
    return left(symphas::lib::char_list<ln...>{},
                std::make_index_sequence<sizeof...(ln)>{});
  }
  ...
  constexpr auto right(symphas::lib::char_list<'.', rn...>,
                       std::index_sequence<Is...>) {
    using namespace symphas::lib;
    return (expr::make_fraction<get_value<rn>(), fixed_pow<10, Is + 1>>() +
            ... + OpVoid{});
  }

  template <char... rn>
  constexpr auto right(symphas::lib::char_list<'.', rn...>) {
    return right(symphas::lib::char_list<'.', rn...>{},
                 std::make_index_sequence<sizeof...(rn)>{});
  }

  constexpr auto right(symphas::lib::char_list<>) { return OpVoid{}; }

  constexpr auto operator()() {
    using namespace symphas::lib;
    return left(typename filter_decimal_left<
                symphas::lib::char_list<>,
                symphas::lib::char_list<n...>>::type{}) +
           right(typename filter_decimal_right<
                 symphas::lib::char_list<>,
                 symphas::lib::char_list<n...>>::type{});
  }
};

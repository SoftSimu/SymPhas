...
struct i_ : Symbol {
  ...
  auto operator-() const { return -i_op_type<N, P>{}; }
  ...
  auto operator=(OpVoid) const { return index_eq_N<i_<N, P>, 0>{}; }

  auto operator=(OpIdentity) const { return index_eq_N<i_<N, P>, 1>{}; }

  auto operator=(OpNegIdentity) const { return index_eq_N<i_<N, P>, -1>{}; }
  ...
  auto operator!=(OpVoid) const { return index_neq_N<i_<N, P>, 0>{}; }

  auto operator!=(OpIdentity) const { return index_neq_N<i_<N, P>, 1>{}; }

  auto operator!=(OpNegIdentity) const { return index_neq_N<i_<N, P>, -1>{}; }
  ...
  auto operator=(OpAdd<OpTerm<OpIdentity, expr::symbols::i_<N0, P0>>,
                       OpFractionLiteral<M, 1>>) const {
    return index_eq<i_<N, P>, expr::symbols::i_<N0, P0 + M>>{};
  }

  template <int N0, int P0, int M>
  auto operator=(OpAdd<OpTerm<OpIdentity, expr::symbols::i_<N0, P0>>,
                       OpNegFractionLiteral<M, 1>>) const {
    return index_eq<i_<N, P>, expr::symbols::i_<N0, P0 - M>>{};
  }

  template <int N0, int P0>
  auto operator=(
      OpAdd<OpTerm<OpIdentity, expr::symbols::i_<N0, P0>>, OpIdentity>) const {
...
auto operator&&(expr::symbols::index_eq<I0, T0>,
                expr::symbols::index_eq<I0, T1>) {
...
DEFINE_SYMBOL_ID(
    (size_t N), (expr::symbols::arg_t<N>),
    static char* name = expr::print_with_subscript<N>("arg").new_str();
    return name;)
ALLOW_COMBINATION((size_t N), (expr::symbols::arg_t<N>))

DEFINE_SYMBOL_ID(
    (int N, int P), (expr::symbols::i_<N, P>),
    static char* name = expr::print_with_subscript<N>("i").new_str();
    return name;)
ALLOW_COMBINATION((int N, int P), (expr::symbols::i_<N, P>))
DEFINE_BASE_DATA((int N, int P), (expr::symbols::i_<N, P>),
                 expr::symbols::Symbol{}, expr::symbols::Symbol{})

// DEFINE_SYMBOL_ID((int... Ns, int... Ps),
// (expr::symbols::v_id_type<expr::symbols::i_<Ns, Ps>...>), static char* name =
// expr::print_with_subscripts<Ns...>("v").new_str(); return name;)
DEFINE_SYMBOL_ID(
    (int... Ns, int... Ps),
    (expr::symbols::v_id_type<expr::symbols::i_<Ns, Ps>...>),
    static char* name = expr::print_with_subscript(
                            "v", print_list(expr::symbols::i_<Ns, Ps>{}...))
                            .new_str();
    return name;)
ALLOW_COMBINATION((int... Ns, int... Ps),
                  (expr::symbols::v_id_type<expr::symbols::i_<Ns, Ps>...>))
DEFINE_BASE_DATA((int... Ns, int... Ps),
                 (expr::symbols::v_id_type<expr::symbols::i_<Ns, Ps>...>),
                 expr::symbols::Symbol{}, expr::symbols::Symbol{})
...
auto operator+(expr::symbols::i_<N0, P0>, expr::symbols::i_<N1, P1>) {
  return expr::symbols::i_op_type<N0, P0>{} +
         expr::symbols::i_op_type<N1, P1>{};
}
...
/*!
 * Data can be stored symbolically in an expression so that it can be
 * conveniently updated outside of the expression. In other cases, a set value
 * be passed, which will be copied into the SymbolicData object and managed
 * locally.
 *
 * In order to hold a copy, a reference to a data must be passed using
 * std::ref().
 */
template <typename T>
struct SymbolicData {
  ...
  SymbolicData(SymbolicData<T> const& other)
      : data{(other.is_local) ? new T(*other.data) : other.data},
        is_local{other.is_local} {}

  SymbolicData(SymbolicData<T>&& other) : SymbolicData() { swap(*this, other); }
  ...
  bool is_local;  //!< Indicates whether the data has been allocated within the
                  //!< object
};

//! Stores a list data that is used symbolically.
/*!
 * A list of data can be stored symbolically in an expression so that the
 * expression can operate over multiple instances of a particular data, which
 * can also be updated outside the expression. In other cases, set values can be
 * passed, which will be copied into the SymbolicDataArray object and managed
 * locally.
 *
 * In order to hold a copy, a reference to a data must be passed using
 * std::ref().
...
struct SymbolicDataArray : SymbolicData<T> {
  ...
  //! Takes a copy of the data and manages it locally within the SymbolicData.
  SymbolicDataArray(len_type len)
      : parent_type(get_initialized_array(len), false), len{len} {}

  //! Takes a copy of the data and manages it locally within the SymbolicData.
  SymbolicDataArray(T* data, len_type len, bool is_local = true)
      : parent_type((is_local) ? get_initialized_array(len) : data, is_local),
        len{len} {
    if (is_local) {
  ...
  SymbolicDataArray(SymbolicDataArray<T> const& other)
      : parent_type(
            (other.is_local) ? get_initialized_array(other.len) : other.data,
            other.is_local),
        len{other.len} {
    if (is_local) {
  ...
  friend void swap(SymbolicDataArray<T>& first, SymbolicDataArray<T>& second) {
    using std::swap;
    swap(static_cast<SymbolicData<T>&>(first),
         static_cast<SymbolicData<T>&>(second));
 ...
 public:
  ...
  auto result(SymbolicDataArray<T0> const& other, SymbolicData<T> lhs,
...
auto operator+(SymbolicDataArray<T> const& lhs,
               SymbolicDataArray<T0> const& rhs) {
  SymbolicData<T> lhs0;
...
template <typename T>
auto operator+(SymbolicDataArray<T> const&,
               SymbolicDataArray<expr::symbols::Symbol> const&) {
  return SymbolicDataArray<expr::symbols::Symbol>{};
}

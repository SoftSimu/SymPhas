...
struct OpExponential : OpExpression<OpExponential<V, E>> {
  ...
  size_t print_length() const {
    return expr::coeff_print_length(value) + SYEX_EXP_FMT_LEN +
           e.print_length();
  }
...
template <typename V, size_t N, size_t D>
inline auto make_exponential::get(V const& v,
                                  OpNegFractionLiteral<N, D> const e) {
  return get(v, expr::make_literal(e.eval()));
}
}  // namespace symphas::internal
...
auto operator*(OpExponential<V1, E1> const& a, OpExponential<V2, E2> const& b) {
  return symphas::internal::make_exponential::get(a.value * b.value, a.e + b.e);
}
...
auto operator*(coeff_t const& value, OpExponential<V, E> const& b) {
  return symphas::internal::make_exponential::get(
      value * b.value, expr::get_enclosed_expression(b));
}
...
auto operator*(coeff_t const& value, OpExponential<tensor_t, E> const& b) {
  return (value * b.value) *
         symphas::internal::make_exponential::get(
             OpIdentity{}, expr::get_enclosed_expression(b));
}
...
auto exp(OpExpression<E> const& a) {
  return symphas::internal::make_exponential::get(*static_cast<E const*>(&a));
}

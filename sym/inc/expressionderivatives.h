...
template <typename G>
struct SymbolicDerivative<expr::variational_t<DynamicVariable<G>>>
    : SymbolicDerivative<G> {
  SymbolicDerivative() : index{} {}
  SymbolicDerivative(DynamicIndex const& index) : index{index} {}
...
template <typename G>
struct SymbolicDerivative<expr::variational_t<DynamicVariable<NamedData<G>>>>
    : SymbolicDerivative<G> {
#ifdef PRINTABLE_EQUATIONS

  SymbolicDerivative() : index{}, name{0, ""} {}
  SymbolicDerivative(DynamicIndex const& index) : index{index}, name{0, ""} {}
  SymbolicDerivative(DynamicVariable<NamedData<G>> const& var)
      : index{var}, name{0, construct_name(var.data.name)} {}

  const char* get_name() const { return name.name; }

  char* get_name() { return name.name; }

  inline expr::expr_name_arr<0> construct_name(const char* old_name) {
    const char* s = std::strstr(old_name, "%d");
    if (s != NULL) {
      char* format = new char[std::strlen(old_name) + 1];
      std::strcpy(format, old_name);
      format[iter_type(s - old_name) + 1] = 's';

      const char* placeholder_str =
          expr::SymbolID<expr::symbols::placeholder_N_symbol>::get(
              expr::symbols::placeholder_N_symbol{});
      expr::expr_name_arr str(std::strlen(old_name) - 1 +
                              std::strlen(placeholder_str));
      sprintf(str, format, placeholder_str);
...
struct SymbolicDerivative<expr::variational_t<Variable<Z, NamedData<G>>>>
    : SymbolicDerivative<G> {
#ifdef PRINTABLE_EQUATIONS
...
template <size_t Z, typename G>
struct SymbolicDerivative<expr::variational_t<Variable<Z, G>>>
    : SymbolicDerivative<G> {
  SymbolicDerivative() {}
...
struct SymbolicDerivative<expr::variational_t<GridSymbol<G, D>>>
    : SymbolicDerivative<G> {
  SymbolicDerivative() : index{} {}
  SymbolicDerivative(DynamicIndex const& index) : index{index} {}
...
template <typename G>
SymbolicDerivative(DynamicVariable<G>)
    -> SymbolicDerivative<expr::variational_t<DynamicVariable<G>>>;
template <size_t Z, typename G>
SymbolicDerivative(Variable<Z, G>)
    -> SymbolicDerivative<expr::variational_t<Variable<Z, G>>>;
template <typename E>
SymbolicDerivative(E) -> SymbolicDerivative<E>;

template <typename V, typename E, typename G>
using OpFunctionalDerivative =
    OpDerivative<std::index_sequence<1>, V, E, SymbolicFunctionalDerivative<G>>;
...
template <typename G>
constexpr bool is_functional_derivative<SymbolicFunctionalDerivative<G>> = true;
}  // namespace expr
 ...
 */
template <Axis ax, size_t O>
struct make_operator_mixed_directional_derivative
    : make_operator_directional_derivative<ax, O> {};
...
using nth_directional_derivative_apply = make_derivative<
    typename Solver<Sp>::template directional_derivative<ax, O>>;

//! Alias for constructing nth derivative.
template <typename Sp, size_t... Os>
using nth_mixed_derivative_apply =
    make_derivative<typename Solver<Sp>::template mixed_derivative<Os...>>;

//! Alias for constructing nth derivative.
template <Axis ax, size_t O, typename Sp>
using nth_derivative_apply =
    make_derivative<typename Solver<Sp>::template derivative<ax, O>>;
...
struct setup_result_data<expr::symbols::Symbol> {
  expr::symbols::Symbol operator()(grid::dim_list const& dims) { return {}; }
};
...
auto make_derivative(A&& a, B&& solver) {
  return symphas::internal::make_derivative<Dd>::get(std::forward<A>(a),
                                                     std::forward<B>(solver));
}
...
auto make_derivative(V&& v, A&& a, B&& solver) {
  return symphas::internal::make_derivative<Dd>::get(
      std::forward<V>(v), std::forward<A>(a), std::forward<B>(solver));
}

//! Create a non-directional derivative expression with the given term.
/*!
 * Create a derivative of the given expression using the given solver
 * to numerically approximate the derivative. The directive will be correctly
 * constructed for the given axis. If the order is even, then the axis is
 * irrelevant.
 *
 * \param v The coefficient to the derivative.
 * \param a The expression being differentiated.
...
auto make_derivative(V&& v, A&& a, solver_op_type<Sp> solver) {
  return symphas::internal::nth_derivative_apply<ax, O, Sp>::get(
      std::forward<V>(v), std::forward<A>(a), solver);
}

//! Create a non-directional derivative expression with the given term.
/*!
 * Create a non-directional derivative of the given expression using the given
 * solver to numerically approximate the derivative. The directive will be
 * correctly constructed for the given axis. If the order is even, then the axis
 * is irrelevant.
 *
 * \param v The coefficient to the derivative.
 * \param a The expression being differentiated.
...
auto make_derivative(A&& a, solver_op_type<Sp> solver) {
  return symphas::internal::nth_derivative_apply<ax, O, Sp>::get(
      std::forward<A>(a), solver);
}

//! Create a symbolic derivative expression with the given term, of the given
//! order.
/*!
 * Create a derivative of the given expression using the given derivative order,
 * and with respect to the given term.
 *
 * \param a The expression being differentiated.
 *
...
auto make_derivative(A&& a) {
  return make_derivative<std::index_sequence<O>>(std::forward<A>(a),
                                                 SymbolicDerivative<G>{});
}
...
auto make_derivative(A&& a, B&& b) {
  return make_derivative<std::index_sequence<O>>(
      OpIdentity{}, std::forward<A>(a),
      SymbolicDerivative<G>(std::forward<B>(b)));
}
...
auto make_derivative(A&& a) {
  return make_derivative<std::index_sequence<O>>(
      std::forward<A>(a), SymbolicDerivative<Variable<Z>>{});
}
...
auto make_derivative(V&& v, A&& a) {
  return make_derivative<std::index_sequence<O>>(
      std::forward<V>(v), std::forward<A>(a),
      SymbolicDerivative<Variable<Z>>{});
}
...
auto make_symbolic_derivative(
    OpExpression<E> const& e,
    OpTerm<OpIdentity, Variable<Z, G>> const& symbol) {
  return expr::make_derivative<O, Z>(*static_cast<E const*>(&e));
}

template <size_t O, typename E, size_t Z, typename G>
auto make_symbolic_derivative(
    OpOperator<E> const& e, OpTerm<OpIdentity, Variable<Z, G>> const& symbol) {
  return expr::make_derivative<O, Z>(*static_cast<E const*>(&e));
}

template <size_t Z, typename V, typename E>
auto make_functional_derivative(V const& value, OpExpression<E> const& e) {
  return OpFunctionalDerivative<V, E, Variable<Z>>(value,
...
auto make_functional_derivative(V const& value, OpExpression<E> const& e,
                                Variable<Z, G>) {
  return OpFunctionalDerivative<V, E, Variable<Z, G>>(
...
auto make_functional_derivative(OpExpression<E> const& e, Variable<Z, G>) {
  return make_functional_derivative(OpIdentity{}, *static_cast<E const*>(&e),
                                    Variable<Z, G>{});
}
...
template <typename V, typename E, typename G>
auto make_functional_derivative(V const& value, OpExpression<E> const& e,
                                DynamicVariable<G> const& symbol) {
  return OpFunctionalDerivative<V, E, DynamicVariable<G>>(
      value, *static_cast<E const*>(&e), symbol);
}
...
auto make_functional_derivative(OpExpression<E> const& e,
                                DynamicVariable<G> const& symbol) {
  return make_functional_derivative(OpIdentity{}, *static_cast<E const*>(&e),
                                    symbol);
}
...
template <typename E, typename G>
auto make_functional_derivative(OpOperator<E> const& e,
                                DynamicVariable<G> const& symbol) {
  return make_functional_derivative(OpIdentity{}, *static_cast<E const*>(&e),
                                    symbol);
}
...
template <typename V, typename E, size_t Z, typename G>
auto make_functional_derivative(
    V const& value, OpExpression<E> const& e,
    OpTerm<OpIdentity, Variable<Z, G>> const& symbol) {
  return OpFunctionalDerivative<V, E, Variable<Z, G>>(
      value, *static_cast<E const*>(&e),
      SymbolicDerivative(expr::get<1>(symbol).data()));
}

template <typename E, size_t Z, typename G>
auto make_functional_derivative(
    E&& e, OpTerm<OpIdentity, Variable<Z, G>> const& symbol) {
...
auto make_functional_derivative(
    V const& value, OpExpression<E> const& e,
    OpTerm<OpIdentity, DynamicVariable<G>> const& symbol) {
  return OpFunctionalDerivative<V, E, DynamicVariable<G>>(
      value, *static_cast<E const*>(&e),
      SymbolicDerivative(expr::get<1>(symbol).data()));
}
...
template <typename V, typename E, typename G>
auto make_functional_derivative(
    V const& value, OpOperator<E> const& e,
    OpTerm<OpIdentity, DynamicVariable<G>> const& symbol) {
  return OpFunctionalDerivative<V, E, DynamicVariable<G>>(
      value, *static_cast<E const*>(&e),
      SymbolicDerivative(expr::get<1>(symbol).data()));
...
template <typename E, typename G>
auto make_functional_derivative(
    E&& e, OpTerm<OpIdentity, DynamicVariable<G>> const& symbol) {
  return make_functional_derivative(OpIdentity{}, std::forward<E>(e), symbol);
}

template <typename V, typename E, typename G>
auto make_functional_derivative(V const& value, OpExpression<E> const& e,
                                OpTerm<OpIdentity, G> const& symbol) {
  return OpFunctionalDerivative<V, E, G>(value, *static_cast<E const*>(&e));
}
...
auto make_operator_derivative(A&& solver) {
  return symphas::internal::make_operator_derivative<O>::get(
      std::forward<A>(solver));
}
...
auto make_operator_derivative(V&& v, A&& solver) {
  return symphas::internal::make_operator_derivative<O>::get(
      std::forward<V>(v), std::forward<A>(solver));
}
...
auto make_operator_derivative(V&& v) {
  return symphas::internal::make_operator_derivative<O>::get(
      std::forward<V>(v), SymbolicDerivative<G>{});
}
...
auto make_operator_derivative(V&& v) {
  return symphas::internal::make_operator_derivative<O>::get(
      std::forward<V>(v), SymbolicDerivative<Variable<Z>>{});
}
...
auto make_operator_directional_derivative(A&& solver) {
  return symphas::internal::make_operator_directional_derivative<ax, O>::get(
      std::forward<A>(solver));
}
 ...
 * \tparam O The order of the derivative to generate.
 */
template <Axis ax, size_t O, size_t Z>
auto make_operator_directional_derivative() {
  return symphas::internal::make_operator_directional_derivative<ax, O>::get(
      SymbolicDerivative<Variable<Z>>{});
}
 ...
 * \tparam O The order of the derivative to generate.
 */
template <Axis ax, size_t O, size_t Z, typename V>
auto make_operator_directional_derivative(V&& v) {
  return symphas::internal::make_operator_directional_derivative<ax, O>::get(
      std::forward<V>(v), SymbolicDerivative<Variable<Z>>{});
}
 ...
 * \tparam O The order of the derivative to generate.
 */
template <Axis ax, size_t O, typename G, typename V>
auto make_operator_directional_derivative(V&& v) {
  return symphas::internal::make_operator_directional_derivative<ax, O>::get(
      std::forward<V>(v), SymbolicDerivative<G>{});
}

//! Create a derivative operator, as a directional derivative.
/*!
 * Create a derivative of the given order, using the given solver
 * to apply the numerical approximation.
 *
 * \tparam ax The axis the derivative is applied.
 * \param solver The solver which approximates the derivative.
 *
 * \tparam O The order of the derivative to generate.
 */
template <Axis ax, size_t O, typename A>
auto make_operator_derivative(A&& solver) {
  return make_operator_directional_derivative<ax, O>(std::forward<A>(solver));
}
...
template <Axis ax, size_t O, typename V, typename A>
auto make_operator_derivative(V&& v, A&& solver) {
  return make_operator_directional_derivative<ax, O>(std::forward<V>(v),
                                                     std::forward<A>(solver));
}

//! Create a symbolic derivative operator, as a directional derivative.
/*!
 * Create a derivative of the given order for symbolic differentiation.
 *
 * \tparam ax The axis the derivative is applied.
...
template <Axis ax, size_t O, typename G>
auto make_operator_derivative() {
  return make_operator_directional_derivative<ax, O, G>();
}
...
template <Axis ax, size_t O, typename G, typename V>
auto make_operator_derivative(V&& v) {
  return make_operator_directional_derivative<ax, O, G>(std::forward<V>(v));
}
...
template <Axis ax, size_t O, size_t Z>
auto make_operator_derivative() {
  return make_operator_directional_derivative<ax, O, Z>();
}
...
template <Axis ax, size_t O, size_t Z, typename V>
auto make_operator_derivative(V&& v) {
  return make_operator_directional_derivative<ax, O, Z>(std::forward<V>(v));
}
 ...
 */
template <
    Axis ax1, size_t O1, Axis ax2, size_t O2, Axis ax3, size_t O3, typename A,
    typename std::enable_if_t<(ax1 != ax2 && ax2 != ax3 && ax1 != ax3), int> =
        0>
auto make_operator_mixed_derivative(A&& solver) {
  return symphas::internal::make_operator_mixed_derivative < (ax1 == Axis::X)
             ? O1
         : (ax2 == Axis::X) ? O2
         : (ax3 == Axis::X) ? O3
                            : 0,
 ...
 * \tparam O The order of the derivative to generate.
 */
template <Axis ax1, size_t O1, Axis ax2, size_t O2, Axis ax3, size_t O3,
          typename V, typename A,
          typename std::enable_if_t<(ax1 != ax2 && ax2 != ax3 && ax1 != ax3),
                                    int> = 0>
auto make_operator_mixed_derivative(V&& v, A&& solver) {
  return symphas::internal::make_operator_mixed_derivative < (ax1 == Axis::X)
         ...
         : (ax3 == Axis::Z)
             ? O3
             : 0 > ::get(std::forward<V>(v), std::forward<A>(solver));
}
...
auto make_operator_mixed_derivative(V&& v, A&& solver) {
  return make_operator_directional_derivative<ax1, O1>(std::forward<V>(v),
                                                       std::forward<A>(solver));
}
 ...
 * \tparam O The order of the derivative to generate.
 */
template <size_t O, typename V, size_t Z, typename G>
auto make_operator_functional_derivative(V&& v, Variable<Z, G> const& symbol) {
  return OpOperatorDerivative<O, V,
                              SymbolicFunctionalDerivative<Variable<Z, G>>>(
      std::forward<V>(v), SymbolicDerivative(symbol));
}
 ...
 * \tparam O The order of the derivative to generate.
 */
template <size_t O, typename V, typename G>
auto make_operator_functional_derivative(V&& v,
                                         DynamicVariable<G> const& symbol) {
  return OpOperatorDerivative<O, V,
                              SymbolicFunctionalDerivative<DynamicVariable<G>>>(
      std::forward<V>(v), SymbolicDerivative(symbol));
}
 ...
 */
template <size_t O, typename V, size_t Z, typename G>
auto make_operator_functional_derivative(
    V&& v, OpTerm<OpIdentity, Variable<Z, G>> const& symbol) {
  return OpOperatorDerivative<O, V,
                              SymbolicFunctionalDerivative<Variable<Z, G>>>(
      std::forward<V>(v), SymbolicDerivative(expr::get<1>(symbol).data()));
}
...
template <size_t O, typename V, typename G>
auto make_operator_functional_derivative(
    V&& v, OpTerm<OpIdentity, DynamicVariable<G>> const& symbol) {
  return OpOperatorDerivative<O, V,
                              SymbolicFunctionalDerivative<DynamicVariable<G>>>(
      std::forward<V>(v), SymbolicDerivative(expr::get<1>(symbol).data()));
}
 ...
 */
template <size_t O, typename V, typename G>
auto make_operator_functional_derivative(V&& v, G const& symbol) {
  return OpOperatorDerivative<O, V, SymbolicFunctionalDerivative<G>>(
      std::forward<V>(v));
}
...
//! Create a functional derivative operator.
/*!
 * Create a functional derivative corresponding to the given field variable.
 *
 * \param solver The solver which approximates the derivative.
 *
 * \tparam ax The axis along which the directional derivative is applied.
 ...
 */
template <size_t O, typename G>
auto make_operator_functional_derivative(G&& solver) {
  return make_operator_functional_derivative<O>(OpIdentity{},
                                                std::forward<G>(solver));
}
 ...
 * \tparam O The order of the derivative to generate.
 */
template <typename V, size_t Z, typename G>
auto make_operator_functional_derivative(V&& v, Variable<Z, G> const& symbol) {
  return OpOperatorDerivative<1, V,
                              SymbolicFunctionalDerivative<Variable<Z, G>>>(
      std::forward<V>(v), SymbolicDerivative(symbol));
}
...
template <typename V, typename G>
auto make_operator_functional_derivative(V&& v, G const& symbol) {
  return OpOperatorDerivative<1, V, SymbolicFunctionalDerivative<G>>(
      std::forward<V>(v));
}
...
template <typename G>
auto make_operator_functional_derivative(G&& solver) {
  return make_operator_functional_derivative(OpIdentity{},
                                             std::forward<G>(solver));
}
...
auto break_up_derivative(solver_op_type<Sp> solver,
  ...
  if constexpr (O == 0) {
    ...
    return (
        (expr::make_column_vector<static_cast<size_t>(axs), sizeof...(axs)>() *
    ...
    return break_up_derivative<2>(solver, symphas::lib::axis_list<axs...>{}) *
           break_up_derivative<O - 2>(solver,
                                      symphas::lib::axis_list<axs...>{});
  }
}
...
struct OpDerivative : OpExpression<OpDerivative<Dd, V, E, Sp>> {
  ...
  size_t print_length() const {
    return expr::coeff_print_length(value) +
           ...
           1 + print_type::print_length() + e.print_length();
  }
  ...
  friend auto const& expr::get_enclosed_expression(
      OpDerivative<Dd0, V0, E0, Sp0> const&);
  template <typename Dd0, typename V0, typename E0, typename Sp0>
...
struct OpDerivative<Dd, V, OpTerm<OpIdentity, G>, Sp>
    : OpExpression<OpDerivative<Dd, V, OpTerm<OpIdentity, G>, Sp>>, Dd {
  ...
  OpDerivative(V0 value, OpTerm<V1, G> const& e, solver_op_type<Sp> solver)
      : value{expr::coeff(e) * value}, solver{solver}, data{expr::data(e)} {}
  OpDerivative(V value, G data, solver_op_type<Sp> solver)
      : value{value}, solver{solver}, data{data} {}
  ...
  friend decltype(auto) expr::get_enclosed_expression(
      OpDerivative<Dd0, V0, OpTerm<OpIdentity, G0>, Sp0> const&);
  template <typename Dd0, typename V0, typename G0, typename Sp0>
  friend decltype(auto) expr::get_enclosed_expression(
      OpDerivative<Dd0, V0, OpTerm<OpIdentity, G0>, Sp0>&);
...
/*!
 * Represents the symbolic differentiation with respect to a variable defined by
 * the template parameter `G`. This is a specialization of the base derivative
 * object which applies a concrete solver to an expression. This object cannot
 * evaluate any derivatives, and is only used to represent differentiation in
 * order to perform symbolic differentiation of terms.
 *
 * \tparam O The order of the derivative.
 * \tparam G Defines the variable against which the expression is
 * differentiated. For example, this can be an std::index_sequence type. \tparam
 * V Type of the coefficient. \tparam E Expression type that the derivative
 * applies to.
 */
...
struct OpDerivative<std::index_sequence<O>, V, E, SymbolicDerivative<G>>
  ...
  auto operator-() const {
    return symphas::internal::make_derivative<std::index_sequence<O>>::get(
        -value, e, solver);
  }

#ifdef PRINTABLE_EQUATIONS
  using print_type =
      decltype(symphas::internal::select_print_deriv(std::index_sequence<O>{}));
  ...
  friend auto const& expr::get_enclosed_expression(
      OpDerivative<std::index_sequence<O0>, V0, E0,
                   SymbolicDerivative<G0>> const&);
  template <size_t O0, typename V0, typename E0, typename G0>
  friend auto& expr::get_enclosed_expression(
      OpDerivative<std::index_sequence<O0>, V0, E0, SymbolicDerivative<G0>>&);

 public:
  V value;  //!< Value multiplying the result of this derivative.
  SymbolicDerivative<G>
      solver;  //!< Ensures this object is valid in the symbolic algebra.
...
struct OpDerivative<std::index_sequence<O>, V, OpTerm<OpIdentity, G1>,
                    SymbolicDerivative<G2>>
    : OpExpression<
          OpDerivative<std::index_sequence<O>, V, OpTerm<OpIdentity, G1>,
                       SymbolicDerivative<G2>>> {
  using E = OpTerm<OpIdentity, G1>;

  static const size_t order = O;  //!< The order of this derivative.

  OpDerivative(V value, OpTerm<OpIdentity, G1> e,
               SymbolicDerivative<G2> const& solver)
      : value{value}, solver{solver}, e{e} {}
  ...
  template <typename V0, typename V1,
            typename std::enable_if_t<
                std::is_convertible<mul_result_t<V0, V1>, V>::value, int> = 0>
  OpDerivative(V0 value, OpTerm<V1, G1> const& e,
               SymbolicDerivative<G2> const& solver)
      : value{e.term * value}, solver{solver}, e{e} {}

  OpDerivative(V value, G1 data, SymbolicDerivative<G2> const& solver)
      : value{value},
        solver{solver},
        e{OpTerm<OpIdentity, G1>(OpIdentity{}, Term(data))} {}

  template <typename V0, typename V1,
            typename std::enable_if_t<
                std::is_convertible<mul_result_t<V0, V1>, V>::value, int> = 0>
  OpDerivative(V0 value, OpTerm<V1, G1> const& e)
      : value{e.value * value}, solver{}, e{e} {}

  OpDerivative(V value, G1 data)
      : value{value},
        solver{},
        e{OpTerm<OpIdentity, G1>(OpIdentity{}, Term(data))} {}
  ...
  size_t print_length() const {
    return expr::coeff_print_length(value) +
           STR_ARR_LEN(
               SYEX_DERIV_APPLIED_EXPR_FMT_A SYEX_DERIV_APPLIED_EXPR_FMT_B) -
           1 + print_type::print_length(G2{}) +
           std::strlen(expr::get_op_name(e.data));
  }
  ...
  friend auto const& expr::get_enclosed_expression(
      OpDerivative<std::index_sequence<O0>, V0, OpTerm<OpIdentity, G10>,
                   SymbolicDerivative<G20>> const&);
  template <size_t O0, typename V0, typename G10, typename G20>
  friend auto& expr::get_enclosed_expression(
      OpDerivative<std::index_sequence<O0>, V0, OpTerm<OpIdentity, G10>,
                   SymbolicDerivative<G20>>&);

 public:
  V value;  //!< Value multiplying the result of this derivative.
  SymbolicDerivative<G2>
      solver;  //!< Solver that applies the derivative function.
...
struct OpDerivative<std::index_sequence<1>, V, E,
                    SymbolicFunctionalDerivative<G>>
    : OpExpression<OpDerivative<std::index_sequence<1>, V, E,
                                SymbolicFunctionalDerivative<G>>> {
   ...
   * object.
   */
  OpDerivative(V value, E const& e,
               SymbolicFunctionalDerivative<G> const& solver =
                   SymbolicFunctionalDerivative<G>{})
      : value{value},
        solver{solver},
        implicit_solver{expr::get_solver(e)},
        e{e} {}

  OpDerivative(E const& e, SymbolicFunctionalDerivative<G> const& solver =
                               SymbolicFunctionalDerivative<G>{})
      : value{OpIdentity{}},
        solver{solver},
        implicit_solver{expr::get_solver(e)},
  ...
  auto operator-() const {
    return symphas::internal::make_derivative<std::index_sequence<1>>::get(
        -value, e, solver);
  }

#ifdef PRINTABLE_EQUATIONS

  using print_type = decltype(symphas::internal::select_print_deriv(
      SymbolicFunctionalDerivative<G>{}));
  ...
  size_t print_length() const {
    return expr::coeff_print_length(value) +
           STR_ARR_LEN(
               SYEX_DERIV_APPLIED_EXPR_FMT_A SYEX_DERIV_APPLIED_EXPR_FMT_B) -
           1 + print_type::print_length(solver) + e.print_length();
  }
 ...
 public:
  ...
  SymbolicFunctionalDerivative<G>
      solver;  //!< Ensures this object is valid in the symbolic algebra.
  solver_t implicit_solver;  //!< The implicit solver, derived from the given
                             //!< expression.
...
struct OpDerivative<std::index_sequence<1>, V, OpTerm<OpIdentity, G1>,
                    SymbolicFunctionalDerivative<G2>>
    : OpExpression<
          OpDerivative<std::index_sequence<1>, V, OpTerm<OpIdentity, G1>,
                       SymbolicFunctionalDerivative<G2>>> {
  using E = OpTerm<OpIdentity, G1>;
  static const size_t order = 1;  //!< The order of this derivative.
   ...
   */
  OpDerivative(V value, E const& e,
               SymbolicFunctionalDerivative<G2> const& solver =
                   SymbolicFunctionalDerivative<G2>{})
      : value{value},
        solver{solver},
        implicit_solver{expr::get_solver(e)},
  ...
  OpDerivative(E const& e, SymbolicFunctionalDerivative<G2> const& solver =
                               SymbolicFunctionalDerivative<G2>{})
      : value{OpIdentity{}},
        solver{solver},
        implicit_solver{expr::get_solver(e)},
...
auto operator*(coeff_t const& value, OpDerivative<Dd2, V2, E2, Sp> const& b) {
  return symphas::internal::make_derivative<Dd2>::get(
      value * b.value, expr::get_enclosed_expression(b), b.solver);
}

template <typename coeff_t, typename Dd2, typename tensor_t, typename E2,
          typename Sp,
          typename std::enable_if_t<
              (expr::is_coeff<coeff_t> && expr::is_tensor<tensor_t>), int> = 0>
auto operator*(coeff_t const& value,
               OpDerivative<Dd2, tensor_t, E2, Sp> const& b) {
  return (value * b.value) * symphas::internal::make_derivative<Dd2>::get(
                                 expr::get_enclosed_expression(b), b.solver);
}
...
// auto operator*(OpLiteral<S1> const& a, OpDerivative<Dd2, V2,
// OpTerm<OpIdentity, G2>, T2> const& b)
//{
//	return symphas::internal::make_derivative<Dd2>::template get_g(a.value *
// b.value, b.data, b.solver);
//}
//
// template<typename fraction_t, typename coeff_t, typename Dd2, typename G2,
// typename Sp2, 	typename =
// std::enable_if_t<(expr::is_fraction<fraction_t> &&
//(expr::is_fraction<coeff_t> || expr::is_identity<coeff_t>)), int>> auto
// operator*(fraction_t, OpDerivative<Dd2, coeff_t, OpTerm<OpIdentity, G2>, Sp2>
// const& b)
//{
//	return symphas::internal::make_derivative<Dd2>::template
// get_g(fraction_t{} * coeff_t{}, b.data, b.solver);
//}
//
// template<typename fraction_t, typename coeff_t, typename Dd2, typename G2,
// typename G0, 	typename =
// std::enable_if_t<(expr::is_fraction<fraction_t> &&
//(expr::is_fraction<coeff_t> || expr::is_identity<coeff_t>)), int>> auto
// operator*(fraction_t, OpDerivative<Dd2, coeff_t, OpTerm<OpIdentity, G2>,
// SymbolicDerivative<G0>> const& b)
//{
//	auto&& e = expr::get_enclosed_expression(b);
//	return symphas::internal::make_derivative<Dd2>::template
// get(fraction_t{} * coeff_t{}, e, b.solver);
//}
...
struct OpOperatorDerivative : OpOperator<OpOperatorDerivative<O, V, Sp>> {
  ...
  OpOperatorDerivative(V value, solver_op_type<Sp> solver = {})
      : value{value}, solver{solver} {}
  ...
  auto operator-() const {
    return symphas::internal::make_operator_derivative<O>::get(-value, solver);
  }
  ...
  //{
  //	return expr::coeff(other) * value * expr::make_operator_derivative<O +
  // O1>(solver);
  // }
  ...
  //{
  //	return OpOperator<OpOperatorDerivative<O, V,
  // Sp>>::operator*(*static_cast<E const*>(&e));
  // }
  ...
  size_t print_length() const {
    return expr::coeff_print_length(value) +
           symphas::internal::print_deriv<O>::print_length();
  }
...
auto operator+(OpOperatorDerivative<O, V1, Sp> const& a,
               OpOperatorDerivative<O, V2, Sp> const& b) {
  return symphas::internal::make_operator_derivative<O>::get(a.value + b.value,
                                                             a.solver);
}
...
auto operator*(OpOperatorDerivative<O1, V1, Sp> const& a,
               OpOperatorDerivative<O2, V2, Sp> const& b) {
  return symphas::internal::make_operator_derivative<O1 + O2>::get(
      a.value * b.value, a.solver);
}
...
template <typename coeff_t, size_t O2, typename V2, typename Sp2,
          typename std::enable_if_t<
              (expr::is_coeff<coeff_t> && !expr::is_tensor<V2>), int> = 0>
auto operator*(coeff_t const& value,
               OpOperatorDerivative<O2, V2, Sp2> const& b) {
  return symphas::internal::make_operator_derivative<O2>::get(value * b.value,
                                                              b.solver);
}

template <typename coeff_t, typename tensor_t, size_t O2, typename Sp2,
          typename std::enable_if_t<
              (expr::is_tensor<coeff_t> && expr::is_tensor<tensor_t>), int> = 0>
auto operator*(coeff_t const& value,
               OpOperatorDerivative<O2, tensor_t, Sp2> const& b) {
  return (value * b.value) *
         symphas::internal::make_operator_derivative<O2>::get(b.solver);
}
...
struct OpOperatorDirectionalDerivative
    : OpOperator<OpOperatorDirectionalDerivative<ax, O, V, Sp>> {
  ...
  static_assert(O <= DERIV_MAX_ORDER);
  OpOperatorDirectionalDerivative(V value, solver_op_type<Sp> solver)
      : value{value}, solver{solver} {}
  ...
  size_t print_length() const {
    return expr::coeff_print_length(value) +
           symphas::internal::print_deriv<O, ax, true>::print_length();
  }
   ...
   */
  template <typename E,
            typename std::enable_if_t<!(expr::is_symbol<expr::eval_type_t<E>> &&
  ...
  //! Apply to a Symbol.
  /*!
   * Apply the generalized derivative to an expression to produce a Chain
   * operator.
   */
  template <typename E,
            typename std::enable_if_t<(expr::is_symbol<expr::eval_type_t<E>> &&
                                      ...
                                      int> = 0>
  auto _apply_impl(OpExpression<E> const& e) const {
    return symphas::internal::nth_directional_derivative_apply<ax, O, Sp>::get(
        value, *static_cast<E const*>(&e), solver);
  }
...
auto operator*(OpOperatorDirectionalDerivative<ax, O1, V1, Sp> const& a,
  ...
  return symphas::internal::make_operator_directional_derivative<
      ax, O1 + O2>::get(a.value * b.value, a.solver);
}
...
auto operator*(coeff_t const& value,
               OpOperatorDirectionalDerivative<ax2, O2, V, Sp2> const& b) {
  return symphas::internal::make_operator_directional_derivative<ax2, O2>::get(
      value * expr::coeff(b), b.solver);
}

template <typename coeff_t, typename tensor_t, Axis ax2, size_t O2,
          typename Sp2,
          typename std::enable_if_t<
              (expr::is_coeff<coeff_t> && expr::is_tensor<tensor_t>), int> = 0>
auto operator*(
    coeff_t const& value,
    OpOperatorDirectionalDerivative<ax2, O2, tensor_t, Sp2> const& b) {
  return (value * expr::coeff(b)) *
         symphas::internal::make_operator_directional_derivative<ax2, O2>::get(
             OpIdentity{}, b.solver);
}
...
struct OpOperatorMixedDerivative
    : OpOperator<OpOperatorMixedDerivative<V, Sp, Os...>> {
  ...
  OpOperatorMixedDerivative(V value, solver_op_type<Sp> solver)
      : value{value}, solver{solver} {}
  ...
  auto operator-() const {
    return symphas::internal::make_operator_mixed_derivative<Os...>::get(
        -value, solver);
  }
...
#ifdef PRINTABLE_EQUATIONS
  using print_type = decltype(symphas::internal::select_print_deriv<Sp>(
 ...
 protected:
  ...
  template <typename E,
            typename std::enable_if_t<(expr::is_symbol<expr::eval_type_t<E>> &&
                                      ...
                                      int> = 0>
  auto _apply_impl(OpExpression<E> const& e) const {
    return symphas::internal::nth_mixed_derivative_apply<Sp, Os...>::get(
        value, *static_cast<E const*>(&e), solver);
  }
...
auto operator*(OpOperatorMixedDerivative<V1, Sp, O1s...> const& a,
  ...
  return symphas::internal::make_operator_mixed_derivative<(O1s + O2s)...>::get(
      a.value * b.value, a.solver);
}
...
template <typename coeff_t, typename V, typename Sp2, size_t... Os,
          typename std::enable_if_t<
              (expr::is_coeff<coeff_t> && !expr::is_tensor<V>), int> = 0>
auto operator*(coeff_t const& value,
               OpOperatorMixedDerivative<V, Sp2, Os...> const& b) {
  return symphas::internal::make_operator_mixed_derivative<Os...>::get(
      value * expr::coeff(b), b.solver);
}
...
template <typename coeff_t, typename tensor_t, typename Sp2, size_t... Os,
          typename std::enable_if_t<
              (expr::is_coeff<coeff_t> && expr::is_tensor<tensor_t>), int> = 0>
auto operator*(coeff_t const& value,
               OpOperatorMixedDerivative<tensor_t, Sp2, Os...> const& b) {
  return (value * expr::coeff(b)) *
         symphas::internal::make_operator_mixed_derivative<Os...>::get(
             OpIdentity{}, b.solver);
}

template <Axis ax, size_t... Os, size_t O2, typename V1, typename V2,
          typename Sp, Axis... axs>
auto mul_mixed_directional(
    OpOperatorMixedDerivative<V1, Sp, Os...> const& a,
    OpOperatorDirectionalDerivative<ax, O2, V2, Sp> const& b,
    symphas::internal::axis_list<axs...>) {
  return symphas::internal::make_operator_mixed_derivative<(
      (ax == axs) ? Os + O2 : Os)...>::get(a.value * b.value, a.solver);
}

template <Axis ax, size_t... Os, size_t O2, typename V1, typename V2,
          typename Sp, Axis... axs>
auto mul_mixed_directional(
    OpOperatorDirectionalDerivative<ax, O2, V2, Sp> const& a,
    OpOperatorMixedDerivative<V1, Sp, Os...> const& b,
    symphas::internal::axis_list<axs...>) {
  return symphas::internal::make_operator_mixed_derivative<(
      (ax == axs) ? Os + O2 : Os)...>::get(a.value * b.value, a.solver);
...
auto operator*(OpOperatorMixedDerivative<V1, Sp, Os...> const& a,
               OpOperatorDirectionalDerivative<ax, O2, V2, Sp> const& b) {
  return mul_mixed_directional(a, b,
...
// Applying the sum of directional derivatives of the same (even) order is the
// same as applying the equivalent derivative. I.e. if `O` is two, then this
// will be the laplacian.
...
auto operator*(
    OpOperatorCombination<
        OpOperatorCombination<OpOperatorDirectionalDerivative<ax2, 2, V, Sp>,
                              OpOperatorDirectionalDerivative<ax3, 2, V, Sp>>,
        OpOperatorDirectionalDerivative<ax1, 2, V, Sp>> const& op,
    OpExpression<E> const& e) {
  return symphas::internal::make_operator_derivative<2>::get(V{}, op.g.solver) *
         (*static_cast<E const*>(&e));
}

template <Axis ax1, Axis ax2, typename V, typename Sp, typename E,
          typename std::enable_if_t<
...
auto operator*(OpOperatorCombination<
                   ...
                   OpOperatorDirectionalDerivative<ax2, 2, V, Sp>> const& op,
               OpExpression<E> const& e) {
  return symphas::internal::make_operator_derivative<2>::get(V{}, op.f.solver) *
         (*static_cast<E const*>(&e));
}

template <size_t O, typename V, typename Sp, typename E,
          typename std::enable_if_t<(expr::grid_dim<E>::value == 1), int> = 0>
auto operator*(OpOperatorDirectionalDerivative<Axis::X, O, V, Sp> const& op,
               OpExpression<E> const& e) {
  return symphas::internal::make_operator_derivative<O>::get(op.value,
                                                             op.solver) *
         (*static_cast<E const*>(&e));
}
...
auto operator+(OpCombination<OpOperatorDirectionalDerivative<ax1, 2, V, Sp>,
  ...
  return symphas::internal::make_operator_derivative<2>::get(V{}, dop.solver) *
...
template <size_t R, Axis ax, size_t O, typename E, typename Sp,
          size_t R0 = expr::eval_type<E>::rank,
          size_t N = expr::grid_dim<E>::value - R,
          typename std::enable_if_t<(R0 == 0), int> = 0>
auto apply_directional_derivative(OpExpression<E> const& e,
                                  solver_op_type<Sp> solver) {
  return symphas::internal::nth_directional_derivative_apply<ax, O, Sp>::get(
...
template <size_t R, Axis ax, size_t O, typename E, typename Sp,
          size_t R0 = expr::eval_type<E>::rank,
          size_t R1 = expr::eval_type<E>::template rank_<1>,
...
auto apply_directional_derivative(OpExpression<E> const& e,
  ...
  auto d = ctensor * apply_directional_derivative<0, ax, O>(
                         rtensor * (*static_cast<E const*>(&e)), solver);
  if constexpr (R > 1) {
    return d + apply_directional_derivative<R - 1, ax, O>(
                   (*static_cast<E const*>(&e)), solver);
  } else {
    return d;
  }
...
auto apply_directional_derivative(OpExpression<E> const& e,
  ...
  auto d = apply_directional_derivative<R0, ax, O>(
               (*static_cast<E const*>(&e) * ctensor), solver) *
           rtensor;
  if constexpr (R > 1) {
    return d + apply_directional_derivative<R - 1, ax, O>(
                   (*static_cast<E const*>(&e)), solver);
...
template <size_t R, size_t... Os, typename E, typename Sp,
          size_t R0 = expr::eval_type<E>::rank,
          size_t N = expr::grid_dim<E>::value - R,
          typename std::enable_if_t<(R0 == 0), int> = 0>
auto apply_directional_mixed(OpExpression<E> const& e,
                             solver_op_type<Sp> solver) {
  return symphas::internal::nth_mixed_derivative_apply<Sp, Os...>::get(
      *static_cast<E const*>(&e), solver);
}
...
auto apply_directional_mixed(OpExpression<E> const& e,
  ...
  auto d = ctensor * apply_directional_mixed<0, Os...>(
                         rtensor * (*static_cast<E const*>(&e)), solver);
  if constexpr (R > 1) {
    return d + apply_directional_mixed<R - 1, Os...>(
                   (*static_cast<E const*>(&e)), solver);
  } else {
    return d;
  }
...
struct apply_derivative<R, O, 0, 0> {
  ...
  auto operator()(OpExpression<E> const& e, solver_op_type<Sp> solver) {
    ...
    return deriv_type::get(*static_cast<E const*>(&e), solver);
  }
};
...
struct apply_derivative<R, O, R0, 0> {
  ...
  auto operator()(OpExpression<E> const& e, solver_op_type<Sp> solver) {
    ...
    auto d = ctensor * symphas::internal::nth_derivative_apply<ax, O, Sp>::get(
                           rtensor * (*static_cast<E const*>(&e)), solver);
    if constexpr (R > 1) {
      return d + apply_derivative<R - 1, O, R0, 0>{}(
                     (*static_cast<E const*>(&e)), solver);
    } else {
      return d;
    }
...
struct apply_derivative<R, O, 0, 1> {
  ...
  template <typename E, typename Sp>
  auto get_component(OpExpression<E> const& e, solver_op_type<Sp> solver) {
    ...
    return ctensor * symphas::internal::nth_derivative_apply<ax, O, Sp>::get(
                         *static_cast<E const*>(&e), solver);
  }

  template <typename E, typename Sp, size_t... Is>
  auto operator()(OpExpression<E> const& e, solver_op_type<Sp> solver,
                  std::index_sequence<Is...>) {
    return (apply_derivative<R + Is, O, 0, 1>{}.get_component(
                (*static_cast<E const*>(&e)), solver) +
            ...);
  }
  ...
  auto operator()(OpExpression<E> const& e, solver_op_type<Sp> solver) {
    return OpOperatorChain(
        OpIdentity{},
        this->operator()(*static_cast<E const*>(&e), solver,
                         std::make_index_sequence<expr::grid_dim<E>::value>{}));
    // constexpr size_t N = expr::grid_dim<E>::value - R;
    //
    // constexpr size_t D = expr::grid_dim<E>::value;
    // constexpr Axis ax = (N == 1) ? Axis::X : (N == 2) ? Axis::Y : Axis::Z;
    // auto ctensor = expr::make_column_vector<N - 1, D>();
    //
    // auto d = ctensor * symphas::internal::nth_derivative_apply<ax, O,
    // Sp>::template get(*static_cast<E const*>(&e), solver); if constexpr (N >
    // 1)
    //{
    //	return d + apply_derivative<R + 1, O, 0, 1>{}((*static_cast<E
    // const*>(&e)), solver);
    // }
    // else
    //{
...
struct apply_derivative<R, O, R0, 1> {
  ...
  auto operator()(OpExpression<E> const& e, solver_op_type<Sp> solver) {
    ...
    auto d = apply_derivative<0, O, 0, 1>{}(
                 rtensor * (*static_cast<E const*>(&e)), solver) *
             rtensor;
    if constexpr (R > 1) {
      return d + apply_derivative<R - 1, O, R0, 1>{}(
                     (*static_cast<E const*>(&e)), solver);
    } else {
      return d;
    }
...
// template<size_t R, size_t O, typename E, typename Sp, size_t R0 =
// expr::eval_type<E>::rank, 	typename std::enable_if_t<(R0 == 0 && O % 2 ==
// 0),
// int> = 0> auto apply_derivative(OpExpression<E> const& e, solver_op_type<Sp>
// solver)
//{
//	return symphas::internal::nth_derivative_apply<Axis::X, O, Sp>::template
// get(*static_cast<E const*>(&e), solver);
// }
...
//	auto d = ctensor * symphas::internal::nth_derivative_apply<ax, O,
// Sp>::template get(rtensor * (*static_cast<E const*>(&e)), solver); 	if
// constexpr (R > 1)
//	{
//		return d + apply_derivative<R - 1, O>((*static_cast<E
// const*>(&e)), solver);
//	}
//	else
//	{
...
// template<size_t R, size_t O, typename E, typename Sp, size_t R0 =
// expr::eval_type<E>::rank, size_t N = expr::grid_dim<E>::value - R, 	typename
// std::enable_if_t<(R0 == 0 && O % 2 == 1), int> = 0> auto
// apply_derivative(OpExpression<E> const& e, solver_op_type<Sp> solver)
//{
//	constexpr size_t D = expr::grid_dim<E>::value;
//	constexpr Axis ax = (N == 1) ? Axis::X : (N == 2) ? Axis::Y : Axis::Z;
//	auto ctensor = expr::make_column_vector<N - 1, D>();
...
//	auto d = apply_derivative<0, O>(rtensor * (*static_cast<E const*>(&e)),
// solver) * rtensor; 	if constexpr (R > 1)
//	{
//		return d + apply_derivative<R - 1, O>((*static_cast<E
// const*>(&e)), solver);
//	}
...
auto apply_derivative_dot(OpExpression<E> const& e, solver_op_type<Sp> solver) {
  ...
  auto d = symphas::internal::nth_derivative_apply<ax, O, Sp>::get(
      rtensor * (*static_cast<E const*>(&e)), solver);
  if constexpr (R > 1) {
    return d +
           apply_derivative_dot<R - 1, O>((*static_cast<E const*>(&e)), solver);
  } else {
    return d;
  }
}
...
auto apply_derivative_dot(OpExpression<E> const& e, solver_op_type<Sp> solver) {
  if constexpr (expr::is_symbol<expr::eval_type_t<E>> &&
                expr::grid_dim<E>::value == 0) {
    return expr::make_mul(expr::make_operator_derivative<O>(solver),
                          *static_cast<E const*>(&e));
  } else {
...
auto apply_derivative_dot(OpExpression<E> const& e,
  ...
  return expr::make_derivative<O, G>(*static_cast<E const*>(&e), solver);
}
...
struct initialize_derivative_order {
  ...
  auto operator()(V const& v, OpExpression<E> const& e,
    ...
    if constexpr (expr::is_symbol<expr::eval_type_t<E>> &&
      ...
      return v *
             apply_derivative<R, O, R>{}(*static_cast<E const*>(&e), solver);
    }
  }
...
template <size_t O>
template <typename V, typename Sp>
inline auto make_operator_derivative<O>::get(V const& v,
                                             solver_op_type<Sp> solver) {
...
template <Axis ax, size_t O>
template <typename V, typename Sp>
inline auto make_operator_directional_derivative<ax, O>::get(
    V const& v, solver_op_type<Sp> solver) {
...
inline auto make_operator_directional_derivative<ax, O>::get(
    solver_op_type<Sp> solver) {
  return OpOperatorDirectionalDerivative<ax, O, OpIdentity, Sp>(OpIdentity{},
                                                                solver);
}

template <size_t... Os>
template <typename V, typename Sp>
inline auto make_operator_mixed_derivative<Os...>::get(
    V const& v, solver_op_type<Sp> solver) {
...
inline auto make_derivative<Dd>::get(V const& v, OpTerm<S, G> const& e,
  ...
  return OpDerivative<Dd, decltype(coeff), OpTerm<OpIdentity, G>,
                      SymbolicDerivative<G0>>(
...
inline auto make_derivative<Dd>::get(V const& v, OpExpression<E> const& e,
                                     SymbolicDerivative<G0> const& solver) {
  return OpDerivative<Dd, V, E, SymbolicDerivative<G0>>(
      v, *static_cast<E const*>(&e), solver);
}
...
template <typename Dd>
template <typename V, typename G, typename Sp>
inline auto make_derivative<Dd>::get_g(V const& v, G g,
                                       solver_op_type<Sp> solver) {
...
template <typename Dd>
template <typename V, typename coeff_t, typename Sp,
          typename std::enable_if_t<
              (expr::is_coeff<coeff_t> || expr::is_identity<coeff_t>), int>>
...
//
// template<size_t O, typename V, typename Sp, typename E, size_t R =
// expr::eval_type<E>::rank, 	typename std::enable_if_t<(R > 0), int> = 0>
...
//	{
//		return expr::coeff(d) *
// symphas::internal::apply_derivative_dot<O>(e, d.solver);
//	}
// }
...
template <Axis ax, size_t O, typename V, typename Sp>
template <typename E>
auto OpOperatorDirectionalDerivative<ax, O, V, Sp>::operator*(
...
template <typename V, typename Sp, size_t... Os>
template <typename E>
auto OpOperatorMixedDerivative<V, Sp, Os...>::operator*(
    OpExpression<E> const& e) const {
  if constexpr (expr::is_coeff<E>) {
    return *static_cast<E const*>(&e) * (*this);
  } else {
...
// auto operator*(OpOperatorDerivative<O, V, Sp> const& d, OpLiteral<T> const&
// e)
//{
//	return e * d;
//}
...
// auto operator*(OpOperatorDirectionalDerivative<ax, O, V, Sp> const& d, E
// const& e)
//{
//	if constexpr (expr::is_coeff<E>)
//	{
...
// auto operator*(OpOperatorDirectionalDerivative<ax, O, V, Sp> const& d,
// OpLiteral<T> const& e)
//{
//	return e * d;
//}
...
// auto operator*(OpOperatorMixedDerivative<V, Sp, Os...> const& d, OpLiteral<T>
// const& e)
//{
//	return e * d;
//}
...
template <size_t O, typename V, typename Sp>
template <typename E>
auto OpOperatorDerivative<O, V, Sp>::_apply_impl(
    OpExpression<E> const& e) const {
  return symphas::internal::initialize_derivative_order<O>{}(
      value, *static_cast<E const*>(&e), solver);
}
...
template <typename E,
          typename std::enable_if_t<!(expr::is_symbol<expr::eval_type_t<E>> &&
                                      expr::grid_dim<E>::value == 0),
                                    int>>
auto OpOperatorDirectionalDerivative<ax, O, V, Sp>::_apply_impl(
    OpExpression<E> const& e) const {
  constexpr size_t R = expr::eval_type<E>::rank;
  return value * symphas::internal::apply_directional_derivative<R, ax, O>(
                     *static_cast<E const*>(&e), solver);
}
...
template <typename E,
          typename std::enable_if_t<!(expr::is_symbol<expr::eval_type_t<E>> &&
                                      expr::grid_dim<E>::value == 0),
                                    int>>
auto OpOperatorMixedDerivative<V, Sp, Os...>::_apply_impl(
    OpExpression<E> const& e) const {
  constexpr size_t R = expr::eval_type<E>::rank;
  return value * symphas::internal::apply_directional_mixed<R, Os...>(
                     *static_cast<E const*>(&e), solver);
}
...
auto make_laplacian(E&& e, solver_op_type<Sp> solver) {
  return symphas::internal::initialize_derivative_order<2>{}(
      OpIdentity{}, std::forward<E>(e), solver);
...
auto make_bilaplacian(E&& e, solver_op_type<Sp> solver) {
  return symphas::internal::initialize_derivative_order<4>{}(
      OpIdentity{}, std::forward<E>(e), solver);
  // return symphas::internal::bilaplacian_apply<Axis::X, Sp>::template
...
auto make_gradlaplacian(E&& e, solver_op_type<Sp> solver) {
  return symphas::internal::initialize_derivative_order<3>{}(
      OpIdentity{}, std::forward<E>(e), solver);
  // return symphas::internal::gradlaplacian_apply<ax, Sp>::template
  // get(OpIdentity{}, std::forward<E>(e), solver);
}
...
auto make_gradient(E&& e, solver_op_type<Sp> solver) {
  return symphas::internal::initialize_derivative_order<1>{}(
      OpIdentity{}, std::forward<E>(e), solver);
...
template <typename E>
auto _get_solver(OpOptimized<E> const& e);
template <size_t O, typename V, typename Sp>
...
auto _get_solver(OpPow<X, V, E> const& e);

template <typename Sp, typename E>
decltype(auto) _get_solver(Sp const& solver, OpExpression<E> const& e) {
  return solver;
}

template <typename E>
decltype(auto) _get_solver(int const&, OpExpression<E> const& e) {
  return _get_solver(*static_cast<E const*>(&e));
}

template <typename Sp, typename E>
decltype(auto) _get_solver(Sp const& solver, OpOperator<E> const& e) {
  return solver;
}

template <typename E>
decltype(auto) _get_solver(int const&, OpOperator<E> const& e) {
  return _get_solver(*static_cast<E const*>(&e));
}

template <typename Sp, typename... Es>
decltype(auto) _get_solver(Sp const& solver, OpAddList<Es...> const& e) {
  return solver;
}

template <typename... Es>
decltype(auto) _get_solver(int const&, OpAddList<Es...> const& e) {
  return _get_solver(e);
}

template <typename Sp, typename... Es>
decltype(auto) _get_solver(Sp const& solver, OpAdd<Es...> const& e) {
  return solver;
}

template <typename... Es>
decltype(auto) _get_solver(int const&, OpAdd<Es...> const& e) {
  return _get_solver(e);
}
...
inline int _get_solver(OpAddList<> const& e) { return 0; }
...
template <typename E>
auto _get_solver(OpOptimized<E> const& e) {
  return _get_solver(_get_solver(e.e), e.term);
}
...
template <Axis ax>
struct derivative_alias_axis {};

template <typename A>
auto d_(A&& solver) {
  return expr::make_operator_derivative<1>(std::forward<A>(solver));
}

template <typename G>
auto dF_(G&& symbol) {
  return expr::make_operator_functional_derivative(std::forward<G>(symbol));
}

constexpr derivative_alias_axis<Axis::X> dx;
constexpr derivative_alias_axis<Axis::Y> dy;
constexpr derivative_alias_axis<Axis::Z> dz;

/*
template<typename A>
auto d_dx(A&& solver)
{
        return expr::make_operator_directional_derivative<Axis::X,
1>(std::forward<A>(solver));
}

template<typename A>
auto d2_dx2(A&& solver)
{
        return expr::make_operator_directional_derivative<Axis::X,
2>(std::forward<A>(solver));
}

template<typename A>
auto d_dy(A&& solver)
{
        return expr::make_operator_directional_derivative<Axis::Y,
1>(std::forward<A>(solver));
}

template<typename A>
auto d2_dy2(A&& solver)
{
        return expr::make_operator_directional_derivative<Axis::Y,
2>(std::forward<A>(solver));
}
...
auto d_dz(A&& solver)
{
        return expr::make_operator_directional_derivative<Axis::Z,
1>(std::forward<A>(solver));
}

template<typename A>
auto d2_dz2(A&& solver)
{
        return expr::make_operator_directional_derivative<Axis::Z,
2>(std::forward<A>(solver));
}
...
template <typename V, typename Sp, size_t O, Axis ax>
auto operator/(OpOperatorDerivative<O, V, Sp> const& op,
               derivative_alias_axis<ax> const&) {
  return expr::make_operator_directional_derivative<ax, O>(expr::coeff(op),
                                                           op.solver);
}

template <size_t O, typename Sp, size_t N>
auto operator^(OpOperatorDerivative<O, OpIdentity, Sp> const& op,
               OpFractionLiteral<N, 1>) {
  return expr::make_operator_derivative<O * N>(op.solver);
}

template <size_t O, typename Sp>
auto operator^(OpOperatorDerivative<O, OpIdentity, Sp> const& op, OpIdentity) {
  return op;
}

template <Axis ax, size_t O, typename Sp, size_t N>
auto operator^(OpOperatorDirectionalDerivative<ax, O, OpIdentity, Sp> const& op,
               OpFractionLiteral<N, 1>) {
  return expr::make_operator_directional_derivative<ax, O * N>(op.solver);
}
...
auto operator^(OpExpression<E> const& e, OpFractionLiteral<N, 1>) {
  return expr::make_pow<N>(*static_cast<E const*>(&e));
}

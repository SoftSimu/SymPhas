...
#define SYEX_COMBINATION_FMT                           \
  SYEX_COMBINATION_FMT_A "%s" SYEX_COMBINATION_FMT_SEP \
                         "%s" SYEX_COMBINATION_FMT_B
#define SYEX_COMBINATION_FMT_LEN                               \
  (STR_ARR_LEN(SYEX_COMBINATION_FMT_A SYEX_COMBINATION_FMT_SEP \
                   SYEX_COMBINATION_FMT_B) -                   \
   1)
#define SYEX_COMBINATION_APPLY_FMT_LEN \
  (STR_ARR_LEN(SYEX_COMBINATION_FMT_A SYEX_COMBINATION_FMT_B) - 1)

#define SYEX_CHAIN_FMT \
  SYEX_CHAIN_FMT_A "%s" SYEX_CHAIN_FMT_SEP "%s" SYEX_CHAIN_FMT_B
#define SYEX_CHAIN_FMT_LEN \
  (STR_ARR_LEN(SYEX_CHAIN_FMT_A SYEX_CHAIN_FMT_SEP SYEX_CHAIN_FMT_B) - 1)
#define SYEX_CHAIN_APPLY_FMT_LEN \
  (STR_ARR_LEN(SYEX_COMBINATION_FMT_A SYEX_COMBINATION_FMT_B) - 1)

#define SYEX_OP_MAP_FMT SYEX_OP_MAP_FMT_A "%s" SYEX_OP_MAP_FMT_B
#define SYEX_OP_MAP_FMT_LEN \
  (STR_ARR_LEN(SYEX_OP_MAP_FMT_A SYEX_OP_MAP_FMT_B) - 1)
 ...
 *
 * The details associated with the actual evaluation of an operator (applying
 * the operator to an actual expression) is the more interesting part:
 *
 * 1. It supports operations to "rechain" operators that are applied to it.
 * 2. It typically needs to be pruned in order to evaluate the system, before it
 * is executed, so that the base data is updated.
 * 3. **Most importantly** The operators that it applies need to have a
 * specialized implementation for only ::OpTerm, not just to avoid wasting time
 * memory, but because the combination implementation will apply operators to
 * variables and will NOT update each applied operator before evaluating the
 * combination result. Therefore, each operator needs to have an implementation
 * where it computes the results directly of a variable without any intermediate
 * data that would need to be updated.
 *
 * \tparam E The specialized operator, used for the CRTP strategy.
 */
template <typename E>
struct OpOperator : OpEvaluable<E> {
  inline auto eval(iter_type) const { return expr::symbols::Symbol{}; }
  ...
  template <typename E0,
            typename std::enable_if_t<(expr::is_symbol<expr::eval_type_t<E0>> &&
                                       expr::grid_dim<E0>::value == 0),
                                      int> = 0>
  auto apply(OpExpression<E0> const& e) const {
    return OpOperatorChain(cast(), *static_cast<E0 const*>(&e));
  }

  template <typename E0, typename std::enable_if_t<
                             !(expr::is_symbol<expr::eval_type_t<E0>> &&
                               expr::grid_dim<E0>::value == 0),
                             int> = 0>
  auto apply(OpExpression<E0> const& e) const {
    return cast().apply_impl(*static_cast<E0 const*>(&e));
  ...
  template <
      ...
      std::enable_if_t<!std::is_same<OpOperatorCombination<B1, B2>, E>::value,
                       int> = 0>
  ...
  template <typename B1, typename B2,
            std::enable_if_t<!std::is_same<OpOperatorChain<B1, B2>, E>::value,
                             int> = 0>
...
template <typename A1, typename A2, typename B1, typename B2>
auto operator+(OpOperatorChain<A1, A2> const& a,
               OpOperatorChain<B1, B2> const& b) {
  return OpOperatorCombination(a, b);
}

//
// template<typename E, typename coeff_t, typename
// std::enable_if_t<expr::is_coeff<coeff_t>, int> = 0> auto
// operator*(OpOperator<E> const& e, coeff_t const& v)
//{
...
auto operator+(OpOperator<E> const& a, OpOperator<E> const& b) {
  if constexpr (std::is_same<expr::symbols::Symbol,
                             expr::eval_type_t<E>>::value) {
    return OpOperatorChain(OpIdentity{} + OpIdentity{},
                           *static_cast<E const*>(&b));
  } else {
    return OpOperatorCombination(*static_cast<E const*>(&a),
                                 *static_cast<E const*>(&b));
  }
}
...
//{
//	return (*static_cast<E1 const*>(&a)).operator+(*static_cast<E2
// const*>(&b));
// }
//
// template<typename E1, typename E2>
...
template <typename A, typename coeff_t,
          typename = std::enable_if_t<
              (expr::is_fraction<coeff_t> || expr::is_identity<coeff_t>), int>>
...
auto operator*(OpBinaryMul<A1, OpOperator<E>> const& a,
               OpExpression<A2> const& b) {
  return a.a * (a.b * (*static_cast<const A2*>(&b)));
}

template <typename E1, typename E2>
auto operator+(OpExpression<E1> const& a, OpOperator<E2> const& b) {
  return expr::make_add(*static_cast<E1 const*>(&a),
...
/*!
 * Distributing an operator will split apart the expression by add/subtraction
 * to apply the operator to each term of the expression. The operator itself is
 * not expanded.
 */
template <typename A1, typename A2, typename... Es>
auto distribute_operator(OpOperatorCombination<A1, A2> const& a,
                         OpAdd<Es...> const& b);
 ...
 */
template <typename A1, typename A2, typename... Es>
auto distribute_operator(OpOperatorChain<A1, A2> const& a,
                         OpAdd<Es...> const& b);
...
auto distribute_operator(OpOperatorCombination<A1, A2> const& a,
                         OpAdd<Es...> const& b, std::index_sequence<Is...>) {
...
auto distribute_operator(OpOperatorCombination<A1, A2> const& a,
                         OpAdd<Es...> const& b) {
  return expr::distribute_operator(a, b,
...
auto distribute_operator(OpOperator<A1> const& a,
                         OpCombination<B1, B2, E> const& b) {
  return (*static_cast<A1 const*>(&a))(
             b.combination.f(expr::get_enclosed_expression(b))) +
         (*static_cast<A1 const*>(&a))(
             b.combination.g(expr::get_enclosed_expression(b)));
}

template <typename A1, typename B1, typename B2>
auto distribute_operator(OpOperator<A1> const& a,
                         OpOperatorCombination<B1, B2> const& b) {
  return (*static_cast<A1 const*>(&a))(b.f) +
         (*static_cast<A1 const*>(&a))(b.g);
}
 ...
 * to an expression. The expression is not changed (i.e. it is not split by
 * add/subtraction). This is opposed to distributing an operator, which will
 * attempt to apply each individual operator term of a chain or combination to
 * an expression without breaking apart the expression.
 *
 ...
 */
template <typename A1, typename A2, typename E>
auto expand_operator(OpOperatorCombination<A1, A2> const& a,
                     OpEvaluable<E> const& b) {
  return expand_operator(a.f, *static_cast<E const*>(&b)) +
         expand_operator(a.g, *static_cast<E const*>(&b));
}
 ...
 */
template <typename A1, typename A2, typename E>
auto expand_operator(OpOperatorChain<A1, A2> const& a,
                     OpEvaluable<E> const& b) {
...
struct OpOperatorCombination : OpOperator<OpOperatorCombination<A1, A2>> {
  ...
  inline auto eval(iter_type n = 0) const { return f.eval(n) + g.eval(n); }

  auto operator-() const { return -f + (-g); }
  ...
  template <
      typename E, typename AA1 = A1, typename AA2 = A2,
      typename std::enable_if_t<
          !(std::is_same<
                mul_result_t<A1, E>,
                decltype(std::declval<AA1>().apply(std::declval<E>()))>::
                value /*std::invoke_result_t<decltype(&A1::template
                         apply<E>), A1,
                         E>>::value*/
            &&
            std::
                is_same<mul_result_t<A2, E>, decltype(std::declval<AA2>().apply(
                                                 std::declval<E>()))>::value /*std::invoke_result_t<decltype(&A2::template apply<E>), A2, E>>::value*/),
          int> = 0>
  auto operator*(OpExpression<E> const& e) {
    return expr::make_mul(*this, *static_cast<E const*>(&e));
    // return a.f * (*static_cast<E const*>(&e)) + a.g * (*static_cast<E
    // const*>(&e));
  }

  //! Apply the chain operation to an expression.
  template <typename E, typename AA1 = A1, typename AA2 = A2,
            typename std::enable_if_t<
                (std::is_same<mul_result_t<AA1, E>,
                              decltype(std::declval<AA1>().apply(
                                  std::declval<E>()))>::value &&
                 std::is_same<mul_result_t<AA2, E>,
                              decltype(std::declval<AA2>().apply(
                                  std::declval<E>()))>::value),
                int> = 0>
  auto operator*(OpExpression<E> const& e) {
  ...
  //! Apply the chain operation to an expression.
  inline auto apply_impl(OpVoid) const { return OpVoid{}; }
...
//	!(std::is_same<mul_result_t<A1, E>,
// decltype(std::declval<A1>().apply(std::declval<E>()))>::value/*std::invoke_result_t<decltype(&A1::template
// apply<E>), A1, E>>::value*/
//		&& std::is_same<mul_result_t<A2, E>,
// decltype(std::declval<A2>().apply(std::declval<E>()))>::value/*std::invoke_result_t<decltype(&A2::template
// apply<E>), A2, E>>::value*/), int> = 0> auto
// operator*(OpOperatorCombination<A1, A2> const& a, OpExpression<E> const& e)
//{
...
// template<typename E, typename A1, typename A2, typename std::enable_if_t<
//	(std::is_same<mul_result_t<A1, E>,
// decltype(std::declval<A1>().apply(std::declval<E>()))>::value
//		&& std::is_same<mul_result_t<A2, E>,
// decltype(std::declval<A2>().apply(std::declval<E>()))>::value), int> = 0>
// auto
// operator*(OpOperatorCombination<A1, A2> const& a, OpExpression<E> const& e)
//{
...
//
// template<typename coeff_t, typename A1, typename A2, typename =
// std::enable_if_t<expr::is_coeff<coeff_t>, int>> auto operator*(coeff_t const&
// a, OpOperatorCombination<A1, A2> const& b)
//{
//	return a * b.f + a * b.g;
//}
...
struct OpCombination : OpExpression<OpCombination<A1, A2, E>> {
 ...
 protected:
  template <typename A>
  static auto _make_eval_expr(OpOperator<A> const& a,
                              OpExpression<E> const& e) {
    return (*static_cast<A const*>(&a))(*static_cast<E const*>(&e));
  }

  template <typename A>
  static auto _make_eval_expr(OpExpression<A> const& a,
                              OpExpression<E> const& e) {
    return (*static_cast<A const*>(&a)) * (*static_cast<E const*>(&e));
  }
  ...
  using expr_type_f = std::invoke_result_t<
      decltype(&OpCombination<A1, A2, E>::make_eval_expr<A1>), A1, E>;
  using expr_type_g = std::invoke_result_t<
      decltype(&OpCombination<A1, A2, E>::make_eval_expr<A2>), A2, E>;
 ...
 public:
  ...
  OpCombination(OpOperatorCombination<A1, A2> const& combination, E const& e)
      : combination{combination},
        eval_expr_f{make_eval_expr(combination.f, e)},
        eval_expr_g{make_eval_expr(combination.g, e)},
        e{e} {}

  template <typename eval_handler_type, typename... condition_ts>
  inline auto update(eval_handler_type const& eval_handler,
  ...
  auto operator-() const { return (-combination)(e); }
  ...
  friend auto const& expr::get_enclosed_expression(
      OpCombination<A10, A20, E0> const&);
  template <typename A10, typename A20, typename E0>
...
template <typename coeff_t, typename A1, typename A2, typename E,
          typename std::enable_if_t<
              (expr::is_coeff<coeff_t> && expr::is_tensor<coeff_t> &&
               expr::eval_type<OpOperatorCombination<A1, A2>>::rank == 0),
              int> = 0>
auto operator*(coeff_t const& value, OpCombination<A1, A2, E> const& b) {
...
struct OpOperatorChain : OpOperator<OpOperatorChain<A1, A2>> {
  ...
  auto operator-() const { return ::OpOperatorChain(-f, g); }
  ...
  A1 f;  //!< Operator which is applied to the result of operator `g` on an
         //!< expression.
...
// std::enable_if_t<!expr::is_coeff<E>, int> = 0> auto
// operator*(OpOperatorChain<A1, A2> const& chain, E const& e)
//{
...
auto make_operator_chain(OpEvaluable<E0> const& e0, OpEvaluable<E1> const& e1) {
  return OpOperatorChain(*static_cast<E0 const*>(&e0),
                         *static_cast<E1 const*>(&e1));
}
...
auto _get_eval_expr(OpOperatorChain<A1, OpIdentity> const& combination,
                    E const& e) {
...
struct OpChain : OpExpression<OpChain<A1, A2, E>> {
 ...
 public:
  ...
  inline auto eval(iter_type n) const { return eval_expr.eval(n); }
...
template <typename coeff_t, typename V,
          typename std::enable_if_t<(expr::is_coeff<coeff_t>), int> = 0>
auto operator*(coeff_t const& value,
               OpOperatorChain<V, OpIdentity> const& combination) {
...
template <typename coeff_t,
          typename std::enable_if_t<(expr::is_coeff<coeff_t>), int> = 0>
auto operator*(coeff_t const& value,
               OpOperatorChain<OpIdentity, OpIdentity> const& combination) {
...
template <typename coeff_t, typename A1, typename A2, typename E,
          typename std::enable_if_t<
              (expr::is_coeff<coeff_t> && expr::is_tensor<coeff_t> &&
               expr::eval_type<OpOperatorChain<A1, A2>>::rank == 0),
              int> = 0>
auto operator*(coeff_t const& value, OpChain<A1, A2, E> const& b) {
...
size_t pow_print(FILE* out, OpExpression<E> const& e) {
  ...
  if constexpr (expr::_Xk_t<X>::D == 1) {
    ...
    n += fprintf(out, "%u" SYEX_POW_DIV_SEP "%u", expr::_Xk_t<X>::N,
                 expr::_Xk_t<X>::D);
  }
...
size_t pow_print(char* out, OpExpression<E> const& e) {
  ...
  if constexpr (expr::_Xk_t<X>::D == 1) {
    ...
    n += sprintf(out + n, "%u" SYEX_POW_DIV_SEP "%u", expr::_Xk_t<X>::N,
                 expr::_Xk_t<X>::D);
  }
...
struct OpPow : OpExpression<OpPow<X, V, E>> {
  ...
  size_t print_length() const {
    size_t n = STR_ARR_LEN(SYEX_POW_SEP_A SYEX_POW_SEP_B) +
               symphas::lib::num_digits<expr::_Xk_t<X>::N>() + e.print_length();
...
//
//	constexpr expr::exp_key_t _X = (sign) ? N0 + D0 : (N0 < D0) ? D0 - N0 :
// N0 - D0; 	constexpr expr::exp_key_t _sign = (sign) ? sign : (N0 < D0) ?
// true : false; 	auto result = expr::coeff(e) * p *
// reevaluate(expr::make_pow<_X>(p));
//
//	if constexpr (_sign)
//	{
//		return -result;
...
auto operator*(coeff_t const& value, OpPow<X, tensor_t, E> const& e) {
  return (value * e.value) *
         expr::make_pow<X>(expr::get_enclosed_expression(e));
}
...
size_t print_map_length(OpMap<G, V, E> const& map) {
  return expr::coeff_print_length(map.value) + map.e.print_length() +
         SYEX_OP_MAP_FMT_LEN;
}

template <typename S, typename T, size_t D, typename V, typename E>
size_t print_map(OpMap<MapGridInverseFourier<S, T, D>, V, E> const& map,
                 FILE* out) {
  size_t n = expr::print_with_coeff(out, map.value);
...
template <typename S, typename T, size_t D, typename V0, typename V1,
          typename G>
size_t print_map(
    OpMap<MapGridInverseFourier<S, T, D>, V0, OpTerm<V1, G>> const& map,
    FILE* out) {
  size_t n = expr::print_with_coeff(out, map.value);
...
template <typename S, typename T, size_t D, typename V0, typename V1,
          typename G>
size_t print_map(OpMap<MapGridFourier<S, T, D>, V0, OpTerm<V1, G>> const& map,
                 FILE* out) {
  size_t n = expr::print_with_coeff(out, map.value);
...
template <typename S, typename T, size_t D, typename V, typename E>
size_t print_map_length(
    OpMap<MapGridInverseFourier<S, T, D>, V, E> const& map) {
  return expr::coeff_print_length(map.value) + map.e.print_length() +
         SYEX_IFT_OF_EXPR_FMT_LEN;
}
...
size_t print_map_length(
    OpMap<MapGridInverseFourier<S, T, D>, V0, OpTerm<V1, G>> const& map) {
  return expr::coeff_print_length(map.value) + map.e.print_length() +
         SYEX_IFT_OF_OP_FMT_LEN;
}

template <typename S, typename T, size_t D, typename V, typename E>
size_t print_map_length(OpMap<MapGridFourier<S, T, D>, V, E> const& map) {
  return expr::coeff_print_length(map.value) + map.e.print_length() +
         SYEX_FT_OF_EXPR_FMT_LEN;
}

template <typename S, typename T, size_t D, typename V0, typename V1,
          typename G>
size_t print_map_length(
    OpMap<MapGridFourier<S, T, D>, V0, OpTerm<V1, G>> const& map) {
  return expr::coeff_print_length(map.value) + map.e.print_length() +
         SYEX_FT_OF_OP_FMT_LEN;
}
 ...
 *
 * The primary difference between ::OpMap and ::OpFunctionApply is that OpMap is
 * more general and can deal with transformations over the whole grid, rather
...
struct OpMap : OpExpression<OpMap<G, V, E>> {
  ...
  void update(eval_handler_type const& eval_handler,
              symphas::lib::types_list<condition_ts...>) {
    eval_handler.result(e, data.src, data.len);
    data.update();
  }
  ...
  auto operator-() const { return expr::make_map<G>(-value, e); }
...
struct OpMap<void, V, E> : OpExpression<OpMap<void, V, E>> {
   ...
   * \param e The expression which is evaluated and mapped.
   */
  OpMap(V value, E const& e) : value{value}, e{e} { update(); }

  inline auto eval(iter_type n) const { return expr::eval(value); }
  ...
  auto operator-() const { return expr::make_map<void>(-value, e); }
...
//! Returns an index from the sequential grid using a half complex (FFTW format)
//! grid.
/*!
...
struct eval_fftw_hcts<2> {
  ...
  auto operator()(iter_type n, OpExpression<E> const& e, const len_type* dims)
    ...
    // Hermitian half
      ...
      return symphas::internal::cast_to_result<expr::eval_type_t<E>>(
          conj(static_cast<E const*>(&e)->eval(n00)));
    }
  }
};
...
struct eval_fftw_hcts<3> {
  ...
  auto operator()(iter_type n, OpExpression<E> const& e, const len_type* dims)
    ...
    if (i0 >= dn) {
      ...
      iter_type k00 = k0;
      return symphas::internal::cast_to_result<expr::eval_type_t<E>>(
          conj(static_cast<E const*>(&e)->eval(i00 + j00 * dn +
                                               k00 * dn * dims[1])));
    } else {
...
//! Returns an index from the half complex (FFTW format) grid using sequential
//! grid.
/*!
 * \param n The index corresponding to the sequential grid.
 * \param src The half complex grid the value is taken from.
...
//	{
//		return
// symphas::internal::expression_iterator<parent_type>(cast());
//	}
//
//	symphas::internal::expression_iterator<parent_type> end(len_type len)
// const
//	{
//		return
// symphas::internal::expression_iterator<parent_type>(cast(), len);
//	}
//
//	auto& cast() const
//	{
 ...
 */
template <typename E>
struct OpMap<symphas::internal::HCTS, OpIdentity, E>
    : OpExpression<OpMap<symphas::internal::HCTS, OpIdentity, E>> {
  // using parent_type = OpExpression<OpMap<symphas::internal::HCTS, OpIdentity,
  // E>>; using parent_type::e;
  ...
  auto operator-() const { return expr::make_map<symphas::internal::HCTS>(-e); }
  ...
  template <typename E0>
  friend auto const& expr::get_enclosed_expression(
      OpMap<symphas::internal::HCTS, OpIdentity, E0> const&);
  template <typename E0>
  friend auto& expr::get_enclosed_expression(
      OpMap<symphas::internal::HCTS, OpIdentity, E0>&);
 ...
 */
template <typename E>
struct OpMap<symphas::internal::STHC, OpIdentity, E>
    : OpExpression<OpMap<symphas::internal::STHC, OpIdentity, E>> {
  // using parent_type = OpExpression<OpMap<symphas::internal::STHC, OpIdentity,
  // E>>; using parent_type::e;
  ...
  auto operator-() const { return expr::make_map<symphas::internal::STHC>(-e); }
...
auto operator*(coeff_t const& value, OpMap<G2, tensor_t, E2> const& b) {
  return (value * b.value) *
         expr::make_map<G2>(expr::get_enclosed_expression(b));
}

template <typename coeff_t, typename E,
          typename std::enable_if_t<expr::is_coeff<coeff_t>, int> = 0>
auto operator*(coeff_t const& value,
               OpMap<symphas::internal::HCTS, OpIdentity, E> const& b) {
  return expr::make_map<symphas::internal::HCTS>(
      value * expr::get_enclosed_expression(b));
}

template <typename coeff_t, typename E,
          typename std::enable_if_t<expr::is_coeff<coeff_t>, int> = 0>
auto operator*(coeff_t const& value,
               OpMap<symphas::internal::STHC, OpIdentity, E> const& b) {
  return expr::make_map<symphas::internal::STHC>(
      value * expr::get_enclosed_expression(b));
}
...
struct OpMap<VectorComponent<ax>, V, E>
  ...
  auto operator-() const { return expr::to_axis<ax>(-e); }
...
auto poisson_solver(OpExpression<E> const& e) {
  if constexpr (expr::is_coeff<E>) {
    ...
    return expr::make_map<symphas::internal::PoissonSolver>(
        *static_cast<E const*>(&e));
  }
}
...
auto make_fourier_map(V const& value, OpExpression<E> const& e) {
  ...
  if constexpr (std::is_same<rt, scalar_t>::value) {
    return hcts(
        expr::make_map<grid_pair_type>(value, *static_cast<E const*>(&e)));
  } else {
...
template <typename V, typename E, size_t D = expr::grid_dim<E>::value,
          size_t... Rs, size_t R = sizeof...(Rs)>
auto make_fourier_map(V const& value, OpExpression<E> const& e,
                      std::index_sequence<Rs...>) {
  return (make_fourier_map(
...
template <typename V, typename E, size_t D = expr::grid_dim<E>::value,
          ...
          typename std::enable_if_t<(R > 0), int> = 0>
auto make_fourier_map(V const& value, OpExpression<E> const& e) {
  return make_fourier_map(value, *static_cast<E const*>(&e),
...
auto make_inv_fourier_map(V const& value, OpExpression<E> const& e) {
  ...
  if constexpr (std::is_same<rt, scalar_t>::value) {
    return expr::make_map<grid_pair_type>(value,
                                          sthc(*static_cast<E const*>(&e)));
  } else {
    return expr::make_map<grid_pair_type>(value, *static_cast<E const*>(&e));
  }
}
...
template <typename target_type, typename V, typename E,
          size_t D = expr::grid_dim<E>::value, size_t... Rs,
          size_t R = sizeof...(Rs)>
auto make_inv_fourier_map(V const& value, OpExpression<E> const& e,
                          std::index_sequence<Rs...>) {
  return (make_inv_fourier_map<target_type>(
              expr::make_column_vector<Rs, R>() * value,
              expr::make_row_vector<Rs, R>() * (*static_cast<E const*>(&e))) +
          ...);
}
...
template <typename target_type, typename V, typename E,
          ...
          typename std::enable_if_t<(R > 0), int> = 0>
auto make_inv_fourier_map(V const& value, OpExpression<E> const& e) {
  return make_inv_fourier_map<target_type>(value, *static_cast<E const*>(&e),
                                           std::make_index_sequence<R>{});
}
...
auto make_inv_fourier_map(OpExpression<E> const& e) {
  return expr::make_inv_fourier_map<target_type>(OpIdentity{},
                                                 *static_cast<E const*>(&e));
}
}  // namespace expr
...
template <typename V, typename E>
auto symphas::internal::make_map<symphas::internal::HCTS>::get(
    V value, OpExpression<E> const& e) {
  return get(value * *static_cast<E const*>(&e));
}
...
auto symphas::internal::make_map<symphas::internal::HCTS>::get(
    OpExpression<E> const& e) {
  return OpMap<symphas::internal::HCTS, OpIdentity, E>(
      *static_cast<E const*>(&e));
}
...
template <typename V, typename E>
auto symphas::internal::make_map<symphas::internal::STHC>::get(
    V value, OpExpression<E> const& e) {
  return get(value * *static_cast<E const*>(&e));
}
...
auto symphas::internal::make_map<symphas::internal::STHC>::get(
    OpExpression<E> const& e) {
  return OpMap<symphas::internal::STHC, OpIdentity, E>(
      *static_cast<E const*>(&e));
}
...
using OpFourierTransform =
    OpMap<MapGridFourier<source_type, complex_t, expr::grid_dim<E>::value>, V,
          E>;

template <typename target_type, typename V, typename E>
using OpInverseFourierTransform = OpMap<
    MapGridInverseFourier<complex_t, target_type, expr::grid_dim<E>::value>, V,
    E>;
...
template <typename coeff_t, typename E,
          typename std::enable_if_t<expr::is_coeff<coeff_t>, int> = 0>
auto operator+(OpMap<symphas::internal::STHC, OpIdentity, E> const& a,
               coeff_t const& b) {
...
template <typename coeff_t, typename E,
          typename std::enable_if_t<expr::is_coeff<coeff_t>, int> = 0>
auto operator+(coeff_t const& a,
               OpMap<symphas::internal::STHC, OpIdentity, E> const& b) {
  return expr::sthc(a + expr::get_enclosed_expression(b));
}

template <typename E1, typename E2>
auto operator+(OpMap<symphas::internal::STHC, OpIdentity, E1> const& a,
               OpMap<symphas::internal::STHC, OpIdentity, E2> const& b) {
  return expr::sthc(expr::get_enclosed_expression(a) +
...
template <typename coeff_t, typename E,
          typename std::enable_if_t<expr::is_coeff<coeff_t>, int> = 0>
auto operator+(OpMap<symphas::internal::HCTS, OpIdentity, E> const& a,
               coeff_t const& b) {
  return expr::hcts(expr::get_enclosed_expression(a) + b);
}
...
template <typename E1, typename E2>
auto operator+(OpMap<symphas::internal::HCTS, OpIdentity, E1> const& a,
               OpMap<symphas::internal::HCTS, OpIdentity, E2> const& b) {
  return expr::hcts(expr::get_enclosed_expression(a) +
                    expr::get_enclosed_expression(b));
}

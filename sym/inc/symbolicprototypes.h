...
/*!
 * Defines elements for symbolic manipulation that work in conjunction with
 * regular symbolic expression types.
 */
...
enum class SeriesOp { ADD, SUB, MUL, DIV };
...
// auto make_symbolic_eval(V const& value, SymbolicSeries<Op, E, Inds> const&
// data, Ts const&... ts);

// template<typename V, typename E0, typename... T0s, typename... Ts>
// auto make_symbolic_eval(V const& value, SymbolicFunction<E0, T0s...> const&
// data, Ts const&... ts);

// template<typename V, typename E, typename... Ts>
// auto make_symbolic_eval(V const& value, OpExpression<E> const& e, Ts
// const&... ts);

// template<typename V, expr::NoiseType nt, typename T, size_t D, typename E>
// auto make_symbolic_eval(V const& value, NoiseData<nt, T, D> const& noise,
// OpExpression<E> const& e);

// template<typename V, expr::NoiseType nt, typename T, size_t D, typename E,
// typename... Ts> auto make_symbolic_eval(V const& value, NoiseData<nt, T, D>
// const& noise, SymbolicFunction<E, Ts...> const& f);
...
template <typename I0, int N0>
using index_neq_N = symphas::lib::types_list<std::false_type, I0,
                                             std::integer_sequence<int, N0>>;
...
using index_itN =
    symphas::lib::types_list<index_eq<I0, T0>, index_eq_N<I0, N1>>;
template <typename I0, int N0, typename T1>
using index_Nti =
    symphas::lib::types_list<index_eq_N<I0, N0>, index_eq<I0, T1>>;
template <typename I0, int N0, int N1>
using index_NtN =
    symphas::lib::types_list<index_eq_N<I0, N0>, index_eq_N<I0, N1>>;
}  // namespace symbols

template <typename E>
constexpr bool has_index =
    symphas::internal::has_index_impl<symphas::lib::types_list<E>>;
...
using SymbolicTernaryCase =
    SymbolicCase<expr::case_entry<expr::symbols::index_eq<I, G>, A>,
                 expr::case_entry<expr::symbols::index_neq<I, G>, B>>;

template <typename I, int N, typename A, typename B>
using SymbolicTernaryNCase =
    SymbolicCase<expr::case_entry<expr::symbols::index_eq_N<I, N>, A>,
...
using SymbolicSum =
    SymbolicSeries<expr::sum_op, T, symphas::lib::types_list<E, Seq, B, C>>;
...
template <typename Op, typename... Is, typename E,
          typename = std::enable_if_t<
              ((std::is_convertible<Is, expr::symbols::Symbol>::value && ... &&
                true)),
              int>>

 ...
 * \ingroup Op
 *
 * Definitions used in adding a new object to structs used in accessing an
 * objects data. Adds a new object to the expression algorithm; allows the
 * object to be given a name and its data accessed.
 *
 * - The `TEMPLATES` argument is the argument to the template parameters; the
 * first argument may be empty if the object is not parameterized. It is always
 * surrounded in parentheses.
 * - The `TYPES` argument second argument is the object type, it is also
 * surrounded in parentheses.
 * - The `DATA_NAME` parameter is the name of the data member within the object.
 * - The `RETURN` parameter depends on the context of the macro, but is
 * typically the data element corresponding to the new data object being added.
 *
...
#define DEFINE_SYMBOL_ID_CONDITION(TEMPLATES, TYPE, CONDITION, ...)           \
  template <SINGLE_ARG TEMPLATES>                                             \
  struct expr::SymbolID<SINGLE_ARG TYPE,                                      \
                        typename std::enable_if_t<SINGLE_ARG CONDITION>> {    \
    static decltype(auto) get([[maybe_unused]] SINGLE_ARG TYPE const& data) { \
      __VA_ARGS__;                                                            \
    }                                                                         \
  };
...
/* definition helpers for creating a new data type for use in expression
 * templates
 */
#define SYEX_IMPL_DEFINE_BASE_DATA(TEMPLATES, TYPE, CONST_RETURN_TYPE,    \
                                   RETURN_TYPE, RETURN_EVAL, RETURN_DATA) \
  template <SINGLE_ARG TEMPLATES>                                         \
  struct expr::BaseData<SINGLE_ARG TYPE> {                                \
    static CONST_RETURN_TYPE get(SINGLE_ARG TYPE const& data,             \
                                 [[maybe_unused]] iter_type n) {          \
...
#define DEFINE_BASE_DATA_REDIRECT(TEMPLATES, REDIRECT_TEMPLATE, TYPE)          \
  SYEX_IMPL_DEFINE_BASE_DATA(                                                  \
      (SINGLE_ARG TEMPLATES), (SINGLE_ARG TYPE), decltype(auto),               \
      decltype(auto),                                                          \
      (expr::BaseData<SINGLE_ARG REDIRECT_TEMPLATE>::get(data, n)),            \
      (expr::BaseData<SINGLE_ARG REDIRECT_TEMPLATE>::get(data)))               \
  template <SINGLE_ARG TEMPLATES>                                              \
  struct expr::original_data_type<SINGLE_ARG TYPE> {                           \
    using type =                                                               \
        typename expr::original_data_type<SINGLE_ARG REDIRECT_TEMPLATE>::type; \
  };

//! Define a new base data access method for an array.
/*!
 * Use this for a new object which contains data and which should be used
 * by the symbolic algebra functionality. By defining a expr::BaseData, a
variable
 * defined with this type can be evaluated and return a value.
 *
 * When writing a parameter for the \p RETURN_ELEMENT or \p RETURN_DATA
arguments,
...
```
 * which would define a new ::BaseData specialization for the Grid class (one
already
...
#define DEFINE_BASE_DATA(TEMPLATES, TYPE, RETURN_ELEMENT, RETURN_DATA)         \
  SYEX_IMPL_DEFINE_BASE_DATA((SINGLE_ARG TEMPLATES), (SINGLE_ARG TYPE),        \
                             decltype(auto), decltype(auto), (RETURN_ELEMENT), \
                             (RETURN_DATA))
...
#define DEFINE_BASE_DATA_INHERITED(TEMPLATES, TYPE, PARENT_TYPE)              \
  DEFINE_BASE_DATA_REDIRECT((SINGLE_ARG TEMPLATES), (SINGLE_ARG PARENT_TYPE), \
                            (SINGLE_ARG TYPE))                                \
  DEFINE_BASE_TYPE((SINGLE_ARG TEMPLATES), (SINGLE_ARG TYPE),                 \
                   (SINGLE_ARG PARENT_TYPE))
...
#define DEFINE_BASE_DATA_POINT(TEMPLATES, TYPE, DATA_NAME)    \
  DEFINE_BASE_DATA((SINGLE_ARG TEMPLATES), (SINGLE_ARG TYPE), \
                   (data.DATA_NAME), (data))

/* definition helpers that combine data type creation with symbol definition to
 * entirely define a new struct for use in expressions
 */
...
#define ADD_EXPR_TYPE(TEMPLATES, TYPE, DATA_NAME)                      \
  DEFINE_BASE_DATA((SINGLE_ARG TEMPLATES), (SINGLE_ARG TYPE), data[n], \
                   data.DATA_NAME)                                     \
  DEFINE_SYMBOL_ID((SINGLE_ARG TEMPLATES), (SINGLE_ARG TYPE),          \
 ...
 * The last parameter is the name of the objects underlying data member.
 *
 * Additionally, the type must have its elements able to be accessed using
 * `operator[]`.
 */
...
#define ALLOW_COMBINATION_CONDITION(TEMPLATES, TYPES, CONDITION)    \
  ALLOW_COMBINATION((SINGLE_ARG TEMPLATES), (SINGLE_ARG TYPES)COMMA \
                    typename std::enable_if_t<(CONDITION)COMMA int>)
 ...
 * not be allowed to be combined with other objects in the same nonlinear
 * variable, instead it will be accumulated into a nonlinear variable of only
 * its own types. Specializes the behaviour of expr::grid_can_multiply.
 *
 * **NOTE**: this is for the base types; this means that all children will be
 * combined if the parent is added to this rule. Adding a child type that has
 * its parent registered as a base will not add the child to this expression
 * logic.
 *
 * \param TEMPLATES The templates used in specializing the types.
 * \param TYPES The types which specialize the behaviour of
 * expr::grid_can_multiply. \param CONDITION The condition under which to apply
 * the multiple restriction.
 */
#define RESTRICT_COMMUTATIVITY_CONDITION(TEMPLATES, TYPES, CONDITION) \
  RESTRICT_COMMUTATIVITY((SINGLE_ARG TEMPLATES),                      \
                         (SINGLE_ARG TYPES COMMA                      \
                          typename std::enable_if_t<(CONDITION)COMMA int>))

//! Define the base type that the new data object corresponds to.
/*!
 * In the case of more complicated object inheritance heirarchies, a base type
 * needs to be defined in order for the expressions to detect the correct type
 * in their evaluations, as the type heirarchy will be descended into to
 * determine relationships.
 *
 * (for instance, it may be useful to define
 * a child of an object if it has certain identites, that way it is
 * parameterized as a special type to be accepted by the appropriate function)
 *
 * \param TEMPLATE Template arguments to the to the defining objects.
 * \param TYPE The base type itself.
 * \param PARENT Parent of the base type that the given TYPE is translated to
 ...
 * \param TYPE The base type itself.
 * \param PARENT Parent of the base type that the given TYPE is translated to
 * when using this object. \param CONDITION The compile-time condition under
 * which the base type will be valid.
 */
#define DEFINE_BASE_TYPE_CONDITION(TEMPLATE, TYPE, PARENT, CONDITION)          \
  DEFINE_BASE_TYPE(TEMPLATE,                                                   \
                   TYPE COMMA typename std::enable_if_t<(CONDITION)COMMA int>, \
                   (SINGLE_ARG PARENT))

/* in some specializations of grids, variables should still be able to divide
 * each other in particular, if the specialization has a power associated with
 * it, then the expression logic should be able to manually divide the grids
 * even if their types don't match exactly
 */

//! Allows division between objects of the specified type.
/*!
 * Defines a rule to allow division between variables which define this type.
 * That is, the given type \p TYPES
 */
...
#define DEFINE_FACTOR_COUNT_CONDITION(TEMPLATES, TYPE1, TYPE2, COUNT, \
                                      CONDITION)                      \
  DEFINE_FACTOR_COUNT(                                                \
      TEMPLATES, TYPE1,                                               \
      TYPE2 COMMA typename std::enable_if_t<(CONDITION)COMMA int>, COUNT)

//! Add a 'dataless' symbol to the symbolic algebra.
/*!
 * Adds a dataless symbol to the symbolic algebra library. Dataless means that
 * the symbol doesn't store or define any data, it is simply a symbol for
 * algebra manipulations, and being distinguished from other symbols.
 *
 * The name is defined by the provided parameter, \p TYPE, and a new class of
 * that name is added to the namespace expr::symbols.
 *
...
#define ADD_EXPR_TYPE_SYMBOL(TYPE)                                            \
  ...
  // DEFINE_BASE_DATA((), (expr::symbols::TYPE ## _symbol),
  // expr::symbols::Symbol{}, expr::symbols::Symbol{})
...
#define ADD_EXPR_TYPE_SYMBOL_TEMPLATE_WITH_ID(TYPE, TEMPLATES, TYPES, ID)     \
  ...
  struct TYPE##_symbol : Symbol {};                                           \
  template <SINGLE_ARG TEMPLATES>                                             \
  using TYPE = OpTerm<OpIdentity, TYPE##_symbol<SINGLE_ARG TYPES>>;           \
  }                                                                           \
  DEFINE_SYMBOL_ID(                                                           \
      TEMPLATES, (expr::symbols::TYPE##_symbol<SINGLE_ARG TYPES>), return ID) \
  DEFINE_BASE_TYPE(TEMPLATES, (expr::symbols::TYPE<SINGLE_ARG TYPES>),        \
                   (expr::symbols::TYPE##_symbol<SINGLE_ARG TYPES>))          \
...
#define ADD_EXPR_TYPE_SYMBOL_TEMPLATE(TYPE, TEMPLATES, TYPES)         \
  ADD_EXPR_TYPE_SYMBOL_TEMPLATE_WITH_ID(TYPE, (SINGLE_ARG TEMPLATES), \
                                        (SINGLE_ARG TYPES), #TYPE)
 ...
 * needs some way to be uniquely represented and printed to output.
 * expr::SymbolID class needs to be specialized and a get function implemented
 * for each new object that could be used in an expression, in order to return
 * the address of some part of the unique object. i.e. the address of a data, or
 * simply the pointer of an array, is the unique element.
 *
 * \ingroup Op
 */
template <typename A, typename Enable = void>
struct SymbolID {
  static A* get(A& a) { return &a; }
  static const A* get(A const& a) { return &a; }
};
...
struct BaseData {
  ...
  static const auto& get(A const& data, iter_type n) { return data; }

  static const auto& get(A const& data) { return data; }

  static auto& get(A& data, iter_type n) { return data; }

  static auto& get(A& data) { return data; }

  static const auto& get(A* data, iter_type n) { return data[n]; }

  static const auto& get(A* data) { return data; }
};
...
template <typename A>
struct BaseData<A*> {
  static A& get(A* data, iter_type n) { return data[n]; }

  static A* get(A* data) { return data; }

  static A const& get(const A* data, iter_type n) { return data[n]; }

  static const A* get(const A* data) { return data; }
};

//! Specialization based on expr::BaseData.
template <typename T>
struct BaseData<Block<T>> {
  static auto const& get(Block<T> const& data, iter_type n) { return data[n]; }
  static auto const& get(Block<T> const& data) { return data; }
  static auto& get(Block<T>& data, iter_type n) { return data[n]; }
  static auto& get(Block<T>& data) { return data; }
};
...
struct BaseData<MultiBlock<N, T>> {
  ...
  static auto const& get(MultiBlock<N, T> const& data) { return data; }
...
struct BaseData<VectorComponentData<ax, T*, D>> {
  ...
  static auto get(VectorComponentData<ax, T*, D> const& data) { return data; }
  static auto& get(VectorComponentData<ax, T*, D>& data, iter_type n) {
    return data.values[n];
  }
  static auto get(VectorComponentData<ax, T*, D>& data) { return data; }
};
...
struct BaseData<OpLiteral<T>> {
  static T get(T data, iter_type n) { return data; }
...
struct BaseData<OpCoeff<T, SymbolicData<int>>> {
  ...
  static T get(OpCoeff<T, SymbolicData<int>> data) { return data.eval(); }
};

template <typename T, typename I>
struct BaseData<OpCoeff<T, I>> {
  static T get(OpCoeff<T, I> data, iter_type n) { return T{}; }
  static T get(OpCoeff<T, I> data) { return T{}; }
};
...
// template<typename T, size_t D>
// struct construct_result_data<BoundaryGrid<T, D>> :
// construct_result_data<Grid<T, D>>
//{
...
struct construct_result_data {
 protected:
  ...
  template <size_t N, typename T>
  construct_result_data<MultiBlock<N, T>> get_constructor(
      MultiBlock<N, T>) const {
    return {};
  }
  template <typename T, size_t D>
  construct_result_data<Grid<T, D>> get_constructor(Grid<T, D>) const {
    return {};
  }
  template <typename T, size_t D>
  construct_result_data<MultiBlock<D, T>> get_constructor(
      Grid<any_vector_t<T, D>, D>) const {
    return {};
  }
  template <typename T>
 ...
 public:
  template <typename E>
  G operator()(E&& data) const {
    return decltype(get_constructor(std::declval<G>())){}(
        std::forward<E>(data));
  }
};
...
struct grid_can_commute {
  static const bool value =
      grid_can_commute_all<A>::value && grid_can_commute_all<B>::value;
};

template <template <typename, size_t> typename G, typename T, typename S,
          size_t D>
struct grid_can_commute<G<any_vector_t<T, D>, D>, G<any_vector_t<S, D>, D>,
                        int> {
  static const bool value = false;
};
...
struct grid_divides_count {
  static const size_t value =
      (grid_can_combine<C>::value && std::is_same<C, G>::value) ? 1 : 0;
};
...
/*!
 * Returns the number of factors of `Variable<Y, G>` (where `G` is any type): it
 * just acts as a wrapper so that the enumerated type `Y` can be processed by
 * expr::factor_count.
 *
...
struct factor_count {
  static const size_t value =
      expr::grid_divides_count<typename expr::base_data_type<C>::type,
...
struct check_is_expression {
  ...
  static constexpr std::false_type _get_value(...) { return {}; }
 ...
 */
template <typename E, typename Enable = int>
constexpr bool is_combinable =
    grid_can_combine<typename base_data_type<E>::type>::value;
...
constexpr bool is_commutable =
    expr::grid_can_commute<typename expr::base_data_type<G1>::type,
                           typename expr::base_data_type<G2>::type, int>::value;

//! Indicates whether or not there is a multiplication rule between two grids.
template <typename G1, typename G2>
constexpr bool has_identity =
    grid_has_identity<typename base_data_type<G1>::type,
...
// D), (F<T, D>), data.as_grid()[n], data.as_grid())
...
DEFINE_SYMBOL_ID((typename T, size_t D), (Grid<any_vector_t<T, D>, D>),
...
struct expr::factor_count<C, Term<G, X>> {
  static const size_t value =
      ((expr::_Xk_t<X>::N - expr::_Xk_t<X>::N % expr::_Xk_t<X>::D) /
       expr::_Xk_t<X>::D) *
...
//! Specialization based on expr::factor_count;
template <typename C, typename G, expr::exp_key_t X>
struct expr::factor_count<OpTerm<OpIdentity, C>, Term<G, X>> {
  static const size_t value =
      ((expr::_Xk_t<X>::N - expr::_Xk_t<X>::N % expr::_Xk_t<X>::D) /
       expr::_Xk_t<X>::D) *
      expr::factor_count<C, G>::value;
...
struct expr::factor_count<C, OpTerms<V, Term<Gs, Xs>...>> {
  static const size_t value =
      ((expr::factor_count<C, Term<Gs, Xs>>::value + ...));
};
...
struct expr::factor_count<C, OpBinaryMul<E1, E2>> {
  static const size_t value =
      expr::factor_count<C, E1>::value + expr::factor_count<C, E2>::value;
};
...
struct expr::factor_count<OpCoeffSwap<expr::symbols::i_<N, P0>>,
                          OpCoeff<T, expr::symbols::i_<N, P1>>> {
  static const size_t value = 1;
};

template <typename T, int N, int P>
struct expr::factor_count<OpCoeffSwap<expr::symbols::i_<N, P>>,
                          OpCoeff<T, expr::symbols::i_<N, P>>> {
  static const size_t value = 1;
};

template <typename T, size_t N>
struct expr::factor_count<expr::symbols::placeholder_N_symbol_<N>,
                          OpCoeff<T, expr::symbols::placeholder_N_symbol_<N>>> {
  static const size_t value = 1;
};
...
struct expr::factor_count<GridSymbol<C, D0>, GridSymbol<T, D1>> {
  static const size_t value = (D0 == D1) && (expr::factor_count<C, T>::value ||
...
//! Specialization based on expr::factor_count;
template <typename C, size_t D, typename V, typename... Gs,
          expr::exp_key_t... Xs>
struct expr::factor_count<GridSymbol<C, D>, OpTerms<V, Term<Gs, Xs>...>> {
  static const size_t value =
      expr::factor_count<C, OpTerms<V, Term<Gs, Xs>...>>::value;
};
...
struct expr::factor_count<Variable<Y>, Variable<Z, G>> {
  static const size_t value =
      expr::factor_count<Variable<Y>, Variable<Z>>::value;
};

//! Specialization based on expr::factor_count;
template <size_t Y, size_t Z, Axis ax0, Axis ax1, typename G, typename F>
struct expr::factor_count<Variable<Y, VectorComponent<ax0, G>>,
                          Variable<Z, VectorComponent<ax1, F>>> {
  static const size_t value =
      (ax0 == ax1) ? expr::factor_count<Variable<Y, G>, Variable<Z, F>>::value
                   : 0;
};

//! Specialization based on expr::factor_count;
template <size_t Y, size_t Z, Axis ax0, Axis ax1, typename F>
struct expr::factor_count<Variable<Y, VectorComponent<ax0>>,
                          Variable<Z, VectorComponent<ax1, F>>> {
  static const size_t value =
      (ax0 == ax1) ? expr::factor_count<Variable<Y>, Variable<Z, F>>::value : 0;
};

//! Specialization based on expr::factor_count;
template <size_t Y, size_t Z, Axis ax1, typename G, typename F>
struct expr::factor_count<Variable<Y, G>,
                          Variable<Z, VectorComponent<ax1, F>>> {
...
//! Specialization based on expr::factor_count;
template <size_t Y, size_t Z>
struct expr::factor_count<Variable<Y, symphas::internal::exclusive_swap>,
                          Variable<Z>> {
  static const size_t value =
      expr::factor_count<Variable<Y>, Variable<Z>>::value;
};
...
//! Specialization based on expr::factor_count;
template <size_t Y, size_t Z, typename G>
struct expr::factor_count<DynamicVariable<Variable<Y, G*>>,
                          DynamicVariable<Variable<Z, G*>>> {
  static const size_t value =
      expr::factor_count<Variable<Y>, Variable<Z, G>>::value;
};
...
struct count_symbolic_case_factors<void, void,
                                   symphas::lib::types_list<As...>> {
  static const size_t value = 1;
};
}  // namespace symphas::internal
...
struct expr::factor_count<SymbolicCaseSwap<C, T>,
                          SymbolicCase<expr::case_entry<As, Bs>...>> {
  // static const size_t value = ((std::is_same<C, As>::value || ...)) ? 1 : 0;
  static const size_t value = symphas::internal::count_symbolic_case_factors<
      C, T, symphas::lib::types_list<As...>>::value;
  //  static const size_t value = ((std::is_same<C, As>::value || ...)) ? 1 : 0;
};

////! Specialization based on expr::factor_count;
// template <typename... Ts>
// struct expr::factor_count<SymbolicCaseSwap<void, void>, SymbolicCase<Ts...>>
// {
//   static const size_t value = 1;
// };
//
...
// struct expr::factor_count<SymbolicCaseSwap<void, void>,
//                           SymbolicCase<expr::case_entry<As, Bs>...>> {
//   static const size_t value = 1;
// };
...
//! Specialization based on expr::factor_count;
// template<typename... As, typename... Bs>
// struct expr::factor_count<SymbolicCaseSwap<>,
// SymbolicCase<expr::case_entry<As, Bs>...>>
//{
//	static const size_t value = 1;
// };
...
struct expr::factor_count<SymbolicCaseSwap<void, void>,
                          SymbolicCaseSwap<C, T>> {
  static const size_t value = 1;
};

template <typename C, typename G0, typename... Gs>
struct expr::factor_count_list {
  static const size_t value = (expr::factor_count<C, G0>::value + ... +
                               expr::factor_count<C, Gs>::value);
};
...
struct expr::sum_factors<OpTerms<V, Term<Gs, Xs>...>,
                         symphas::lib::types_list<Fs...>> {
  static const int value =
      (expr::factor_count_list<Fs, Term<Gs, Xs>...>::value + ... + 0);
};

/*
 * type traits for evaluating an expression and collecting all the datas which
 * can be factored out
 */
 ...
 public:
  using type = typename std::conditional_t<
      ...
      symphas::lib::types_list<
          std::pair<std::index_sequence<N>, std::index_sequence<I0>>>,
      symphas::lib::types_list<>>;
};
...
struct expr::factor_list_variables<std::index_sequence<I0, I1, Is...>, E1, E2> {
 protected:
  using types_1 = typename expr::factor_list_variables<std::index_sequence<I0>,
                                                       E1, E2>::type;
  using types_2 =
      typename expr::factor_list_variables<std::index_sequence<I1, Is...>, E1,
                                           E2>::type;
...
struct expr::factor_list_data<symphas::lib::types_list<T0>, E1, E2> {
 ...
 public:
  using type = typename std::conditional_t<
      (N > 0), symphas::lib::types_list<std::pair<std::index_sequence<N>, T0>>,
      symphas::lib::types_list<>>;
};
...
struct expr::factor_list_data<symphas::lib::types_list<T0, T1, Ts...>, E1, E2> {
 protected:
  using types_1 = typename expr::factor_list_data<symphas::lib::types_list<T0>,
                                                  E1, E2>::type;
  using types_2 =
      typename expr::factor_list_data<symphas::lib::types_list<T1, Ts...>, E1,
                                      E2>::type;

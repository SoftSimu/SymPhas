...
struct iterator_policy_expression<symphas::sequential_iterator_policy> {
  ...
  template <typename E>
  static auto begin(OpEvaluable<E> const &e,
                    grid::region_size const &interval) {
...
struct iterator_policy_expression<symphas::selection_iterator_policy> {
  ...
  template <typename E, size_t D>
  static auto begin(OpEvaluable<E> const &e,
                    grid::region_index_list<D> const &iters) {
    return expression_iterator_selection(*static_cast<E const *>(&e),
                                         iters.iters);
  }

  template <typename E, size_t D>
  static auto end(OpEvaluable<E> const &e,
                  grid::region_index_list<D> const &iters) {
    return expression_iterator_selection(*static_cast<E const *>(&e),
                                         iters.iters, iters.len);
  }
};
...
struct OpEvaluable {
   ...
   */
  template <size_t D>
  expr::expression_iterator_region<E, D> begin(
      grid::region_interval<D> const &interval) const {
   ...
   */
  template <size_t D>
  expr::expression_iterator_region<E, D> end(
      grid::region_interval<D> const &interval) const {
    return expr::expression_iterator_region<E, D>(cast(), interval,
                                                  grid::length<D>(interval));
  }
   ...
   * point to.
   */
  expr::expression_iterator_selection<E> end(iter_type *iters,
                                             len_type len) const {
  ...
  auto begin(picked_iterator_t, arg_t &&data = arg_t{}) const {
    return expr::iterator_policy_expression<picked_iterator_t>::begin(
        cast(), std::forward<arg_t>(data));
  }
  ...
  auto end(picked_iterator_t, arg_t &&data = arg_t{}) const {
    return expr::iterator_policy_expression<picked_iterator_t>::end(
        cast(), std::forward<arg_t>(data));
  }

  auto &cast() const { return *static_cast<E const *>(this); }
};
...
struct OpExpression : OpEvaluable<E> {
   ...
   */
  inline auto eval(iter_type n) const { return cast().eval(n); }
   ...
   * \param n The index at which to evaluate the data.
   */
  auto operator[](iter_type n) const { return cast().eval(n); }
   ...
   */
  size_t print(FILE *out) const { return cast().print(out); }
   ...
   */
  size_t print(char *out) const { return cast().print(out); }
   ...
   */
  size_t print_length() const { return cast().print_length(); }

#else

  size_t print(...) const { return 0; }

  size_t print_length() const { return 0; }
...
struct VectorComponent<ax, symphas::ref<G>> : symphas::ref<G> {
  ...
  VectorComponent(symphas::ref<G> &&data) noexcept
...
template <Axis ax, typename T, size_t D>
struct VectorComponentRegionData;

template <Axis ax, typename T, size_t D>
struct VectorComponentRegionData<ax, T *, D> {
  T *values;
  ...
  template <size_t... Is>
  VectorComponentRegionData(T *data, grid::select_region<D> const &region,
...
struct GridData {
  ...
  __host__ __device__ const T &operator*() const { return *data; }
...
struct GridData<MultiBlock<N, T>, N> {
  GridData(MultiBlock<N, T> const &data = MultiBlock<N, T>(0),
           len_type const *dims = nullptr)
  ...
  const auto &operator[](iter_type i) const { return data.get()[i]; }

  auto &operator[](iter_type i) { return data.get()[i]; }

  operator const MultiBlock<N, T> &() const { return data.get(); }

  operator MultiBlock<N, T> &() { return data.get(); }
...
template <>
inline constexpr bool symphas::is_simple_data<expr::symbols::Symbol> = true;
...
VectorComponentRegionData<ax, T *, N> resolve_axis_component(
    RegionalGrid<any_vector_t<T, N>, N> const &data) {
...
template <Axis ax, size_t N, typename T>
VectorComponentRegionData<ax, T *, N> resolve_axis_component(
    RegionalGrid<any_vector_t<T, N>, N> &data) {
  return {data.values[symphas::axis_to_index(ax)], data.region, data.dims,
          data.empty[symphas::axis_to_index(ax)]};
}
...
template <Axis ax, typename T, size_t D>
VectorComponentData<ax, T, D> resolve_axis_component(
    any_vector_t<T, D> const &data) {
...
/*!
 * Constructs a compile time constant integer as a fraction type. This is a
 * wrapper for the expr::make_fraction() function, particularly useful when the
...
/*!
 * For coefficients which should be selected from a list, construct a
 * coefficient which can select them based on their index.
 */
template <typename T>
 ...
 * \param e The expression.
 */
template <expr::exp_key_t X, typename V, typename V0, typename... Gs,
          expr::exp_key_t... Xs>
...
//! Returns the given data in a wrapper imitating a Grid with specified
//! dimensions.
/*!
 * The given data is wrapped in an object which imitates a Grid in the symbolic
 * algebra functionality, so that dimensions can be interpreted. This is used
 * when the given data is a non-grid type, such as a raw pointer.
 *
 ...
 */
template <size_t D, typename T0, typename T1>
auto make_unit_row_vector(T0 const &direction0, T1 const &direction1);

//! The unit row vector, which can be defined with one or two (for 3D) angles.
/*!
 * The correct unit vector will be chosen according to the dimension. In 3D, the
 * second direction is assumed to be 0.
 */
 ...
 */
template <size_t D, typename T0>
auto make_unit_row_vector(T0 const &direction0);
...
struct search_index_in_v<
    N0, expr::symbols::v_id_type<expr::symbols::i_<N1, P>, Is...>> {
  static const bool value =
      (N0 == N1)
          ? P
          : search_index_in_v<N0, expr::symbols::v_id_type<Is...>>::value;
};
...
constexpr bool
    has_matching_i<expr::symbols::i_<N0, P0>,
...
constexpr bool has_matching_i<
    expr::symbols::i_<N0, P0>,
    GridSymbol<expr::symbols::v_id_type<expr::symbols::i_<Ns, Ps>...>, D>> =
    has_matching_i<expr::symbols::i_<N0, P0>,
                   expr::symbols::v_id_type<expr::symbols::i_<Ns, Ps>...>>;
...
struct select_v_i_impl<expr::symbols::i_<N0, P0>,
...
struct select_v_i_impl<expr::symbols::i_<N0, P0>,
  ...
  using type = typename select_v_i_impl<
      ...
      symphas::lib::expand_types_list<
          symphas::lib::types_list<Vs...>,
          typename test_select_v_i_impl<
              has_matching_i<expr::symbols::i_<N0, P0>, T0>, T0>::type>,
      symphas::lib::types_list<Ts...>>::type;
};

// template<int N0, int P0, typename... Vs, int N1, int P1, int... Ns, int...
// Ps, typename... Rest> struct select_v_i_impl<expr::symbols::i_<N0, P0>,
// symphas::lib::types_list<Vs...>,
// symphas::lib::types_list<expr::symbols::v_id_type<expr::symbols::i_<N1, P1>,
// expr::symbols::i_<Ns, Ps>...>, Rest...>>
//{
//	//static const bool flag = symphas::lib::index_of_value<int, N0, Ns...>
//>= 0;
//	//
//	//using type = typename select_v_i_impl<
//	//	expr::symbols::i_<N0, P0>,
...
// symphas::lib::types_list<Vs...,
// expr::symbols::v_id_type<expr::symbols::i_<N1, P1>, expr::symbols::i_<Ns,
// Ps>...>>, symphas::lib::types_list<Rest...>>::type;
// };

// template<int N0, int P0, typename... Vs, int... Ns, int... Ps, size_t D,
// typename... Rest> struct select_v_i_impl<expr::symbols::i_<N0, P0>,
// symphas::lib::types_list<Vs...>,
// symphas::lib::types_list<GridSymbol<expr::symbols::v_id_type<expr::symbols::i_<Ns,
// Ps>...>, D>, Rest...>>
//{
//	using type = typename select_v_i_impl<expr::symbols::i_<N0, P0>,
// symphas::lib::types_list<Vs...>,
// symphas::lib::types_list<expr::symbols::v_id_type<expr::symbols::i_<Ns,
// Ps>...>, Rest...>>::type;
// };

// template<int N0, int P0, typename... Vs, typename T, typename... Rest>
// struct select_v_i_impl<expr::symbols::i_<N0, P0>,
// symphas::lib::types_list<Vs...>, symphas::lib::types_list<T, Rest...>>
//{
//	using type = typename select_v_i_impl<
//		expr::symbols::i_<N0, P0>,
//		symphas::lib::types_list<Vs...>,
...
// template<int N0, int P0, typename... Vs>
// struct select_v_i_<expr::symbols::i_<N0, P0>,
// symphas::lib::types_list<Vs...>>
//{
...
using select_v_i_ =
    typename select_v_i_impl<I, symphas::lib::types_list<>, List>::type;
...
constexpr bool
    is_v_type<expr::symbols::v_id_type<expr::symbols::i_<Ns, Ps>...>> = true;
template <int... Ns, int... Ps, size_t D>
constexpr bool is_v_type<
    GridSymbol<expr::symbols::v_id_type<expr::symbols::i_<Ns, Ps>...>, D>> =
    true;
...
struct select_v_nested_impl<
    2, symphas::lib::types_list<expr::symbols::i_<N0s, P0s>...>,
    symphas::lib::types_list<Vs...>, symphas::lib::types_list<>> {
  using type = symphas::lib::types_list<Vs...>;
};

template <int... N0s, int... P0s, typename... Vs, int... Ns, int... Ps,
          typename... Rest>
struct select_v_nested_impl<
    1, symphas::lib::types_list<expr::symbols::i_<N0s, P0s>...>,
    symphas::lib::types_list<Vs...>,
    symphas::lib::types_list<
        expr::symbols::v_id_type<expr::symbols::i_<Ns, Ps>...>, Rest...>> {
  static const bool flag =
      symphas::lib::filter_seq_t<std::integer_sequence<int, Ns...>,
                                 std::integer_sequence<int, N0s...>>::size() ==
      0;
  ...
  using type = typename select_v_nested_impl<
      ...
      std::conditional_t<
          false,
          symphas::lib::types_list<
              Vs..., expr::symbols::v_id_type<expr::symbols::i_<Ns, Ps>...>>,
          symphas::lib::types_list<Vs...>>,
      symphas::lib::types_list<Rest...>>::type;
};

template <int... N0s, int... P0s, typename... Vs, int... Ns, int... Ps,
          size_t D, typename... Rest>
struct select_v_nested_impl<
    1, symphas::lib::types_list<expr::symbols::i_<N0s, P0s>...>,
    symphas::lib::types_list<Vs...>,
    symphas::lib::types_list<
...
template <int... N0s, int... P0s, typename... Vs, typename T, typename... Rest>
struct select_v_nested_impl<
    0, symphas::lib::types_list<expr::symbols::i_<N0s, P0s>...>,
    symphas::lib::types_list<Vs...>, symphas::lib::types_list<T, Rest...>> {
  using type = typename select_v_nested_impl<
      2, symphas::lib::types_list<expr::symbols::i_<N0s, P0s>...>,
      symphas::lib::types_list<Vs...>, symphas::lib::types_list<Rest...>>::type;
};

template <typename Is, typename List>
using select_v_nested_ =
    typename select_v_nested_impl<2, Is, symphas::lib::types_list<>,
                                  List>::type;
...
struct select_v_impl<
    1, symphas::lib::types_list<Vs...>,
    symphas::lib::types_list<expr::symbols::v_id_type<I0, Is...>, Rest...>> {
  using type = typename select_v_impl<
      2, symphas::lib::types_list<Vs..., expr::symbols::v_id_type<I0, Is...>>,
      symphas::lib::types_list<Rest...>>::type;
};

template <typename... Vs, typename I0, typename... Is, size_t D,
          typename... Rest>
struct select_v_impl<
    1, symphas::lib::types_list<Vs...>,
    symphas::lib::types_list<GridSymbol<expr::symbols::v_id_type<I0, Is...>, D>,
                             Rest...>> {
  using type = typename select_v_impl<
      1, symphas::lib::types_list<Vs...>,
      symphas::lib::types_list<expr::symbols::v_id_type<I0, Is...>,
                               Rest...>>::type;
};

template <typename... Vs, typename T, typename... Rest>
struct select_v_impl<2, symphas::lib::types_list<Vs...>,
                     symphas::lib::types_list<T, Rest...>> {
  using type =
      typename select_v_impl<size_t(is_v_type<T>),
                             symphas::lib::types_list<Vs...>,
                             symphas::lib::types_list<T, Rest...>>::type;
};

template <typename... Vs, typename T, typename... Rest>
struct select_v_impl<0, symphas::lib::types_list<Vs...>,
                     symphas::lib::types_list<T, Rest...>> {
  using type = typename select_v_impl<2, symphas::lib::types_list<Vs...>,
                                      symphas::lib::types_list<Rest...>>::type;
};
...
using select_v_ =
    typename select_v_impl<2, symphas::lib::types_list<>, List>::type;

template <typename... Ts>
struct select_all_i_impl_;

template <int N, typename... Is>
struct select_all_i_impl_<expr::symbols::i_<N, 0>,
...
struct select_all_i_impl_<
    expr::symbols::i_<N, 0>, symphas::lib::types_list<Is...>,
    symphas::lib::types_list<expr::symbols::v_id_type<expr::symbols::i_<N, P0>>,
                             Rest...>> {
  using type = typename select_all_i_impl_<
      expr::symbols::i_<N, 0>,
      symphas::lib::types_list<Is..., expr::symbols::i_<N, P0>>,
      symphas::lib::types_list<Rest...>>::type;
};
...
struct select_all_i_impl_<
    expr::symbols::i_<N, 0>, symphas::lib::types_list<Is...>,
    symphas::lib::types_list<expr::symbols::i_<N0, P0>, Rest...>> {
  using type =
      typename select_all_i_impl_<expr::symbols::i_<N, 0>,
                                  symphas::lib::types_list<Is...>,
                                  symphas::lib::types_list<Rest...>>::type;
};
...
struct select_all_i_impl_<
    expr::symbols::i_<N, 0>, symphas::lib::types_list<Is...>,
    symphas::lib::types_list<expr::symbols::i_<N, P0>, Rest...>> {
  using type = typename select_all_i_impl_<
      expr::symbols::i_<N, 0>,
      symphas::lib::types_list<Is..., expr::symbols::i_<N, P0>>,
      symphas::lib::types_list<Rest...>>::type;
...
struct select_all_i_impl_<expr::symbols::i_<N, 0>,
                          symphas::lib::types_list<Is...>,
                          symphas::lib::types_list<Other, Rest...>> {
  using type =
      typename select_all_i_impl_<expr::symbols::i_<N, 0>,
                                  symphas::lib::types_list<Is...>,
                                  symphas::lib::types_list<Rest...>>::type;
...
struct select_all_i_impl_<expr::symbols::i_<N, P>,
                          symphas::lib::types_list<Vs...>> {
  using type =
      typename select_all_i_impl_<expr::symbols::i_<N, 0>,
                                  symphas::lib::types_list<>,
...
struct select_all_i_impl_<
    symphas::lib::types_list<expr::symbols::i_<N0, P0>,
                             expr::symbols::i_<Ns, Ps>...>,
    symphas::lib::types_list<Vs...>> {
  using type = symphas::lib::expand_types_list<
      typename select_all_i_impl_<expr::symbols::i_<N0, 0>,
                                  symphas::lib::types_list<>,
                                  symphas::lib::types_list<Vs...>>::type,
      typename select_all_i_impl_<expr::symbols::i_<Ns, 0>,
                                  symphas::lib::types_list<>,
                                  symphas::lib::types_list<Vs...>>::type...>;
};
...
template <int N, typename... Is>
struct select_only_i_impl_<expr::symbols::i_<N, 0>,
                           symphas::lib::types_list<Is...>,
                           symphas::lib::types_list<>> {
  using type = symphas::lib::types_list<Is...>;
};
...
struct select_only_i_impl_<
    expr::symbols::i_<N, 0>, symphas::lib::types_list<Is...>,
    symphas::lib::types_list<expr::symbols::i_<N, P0>, Rest...>> {
  using type = typename select_only_i_impl_<
      expr::symbols::i_<N, 0>,
      symphas::lib::types_list<Is..., expr::symbols::i_<N, P0>>,
      symphas::lib::types_list<Rest...>>::type;
};
...
struct select_only_i_impl_<expr::symbols::i_<N, 0>,
                           ...
                           symphas::lib::types_list<Other, Rest...>> {
  using type =
      typename select_only_i_impl_<expr::symbols::i_<N, 0>,
                                   symphas::lib::types_list<Is...>,
                                   symphas::lib::types_list<Rest...>>::type;
};

template <int N, int P, typename... Vs>
struct select_only_i_impl_<expr::symbols::i_<N, P>,
                           symphas::lib::types_list<Vs...>> {
  using type =
      typename select_only_i_impl_<expr::symbols::i_<N, 0>,
                                   symphas::lib::types_list<>,
...
struct select_only_i_impl_<
    ...
    symphas::lib::types_list<Vs...>> {
  using type = symphas::lib::expand_types_list<
      typename select_only_i_impl_<expr::symbols::i_<N0, 0>,
                                   symphas::lib::types_list<>,
                                   symphas::lib::types_list<Vs...>>::type,
      typename select_only_i_impl_<expr::symbols::i_<Ns, 0>,
                                   symphas::lib::types_list<>,
                                   symphas::lib::types_list<Vs...>>::type...>;
};
...
template <int... Ns>
struct select_unique_i_impl_<
    symphas::lib::types_list<expr::symbols::i_<Ns, 0>...>,
    symphas::lib::types_list<>> {
  using type = typename select_unique_i_impl_<
      symphas::lib::sorted_seq<std::integer_sequence<int, Ns...>>>::type;
};
...
struct select_unique_i_impl_<
    symphas::lib::types_list<Is...>,
    symphas::lib::types_list<
        expr::symbols::index_neq_N<expr::symbols::i_<N, P>, M0>, Rest...>> {
  using type = typename select_unique_i_impl_<
      symphas::lib::types_list<Is..., expr::symbols::i_<N, 0>>,
      symphas::lib::types_list<Rest...>>::type;
};
...
struct select_unique_i_impl_<
    symphas::lib::types_list<Is...>,
    symphas::lib::types_list<
        expr::symbols::index_neq<expr::symbols::i_<N, P>, B>, Rest...>> {
  using type = typename select_unique_i_impl_<
      symphas::lib::types_list<Is..., expr::symbols::i_<N, 0>>,
      symphas::lib::types_list<B, Rest...>>::type;
};
...
struct select_unique_i_impl_<
    symphas::lib::types_list<Is...>,
    symphas::lib::types_list<
        expr::symbols::index_eq_N<expr::symbols::i_<N, P>, M0>, Rest...>> {
  using type = typename select_unique_i_impl_<
      symphas::lib::types_list<Is..., expr::symbols::i_<N, 0>>,
      symphas::lib::types_list<Rest...>>::type;
};
...
struct select_unique_i_impl_<
    symphas::lib::types_list<Is...>,
    symphas::lib::types_list<
        expr::symbols::index_eq<expr::symbols::i_<N, P>, B>, Rest...>> {
  using type = typename select_unique_i_impl_<
      symphas::lib::types_list<Is..., expr::symbols::i_<N, 0>>,
      symphas::lib::types_list<B, Rest...>>::type;
};
...
struct select_unique_i_impl_<
    symphas::lib::types_list<Is...>,
    symphas::lib::types_list<symphas::lib::types_list<A, B>, Rest...>> {
  using type = symphas::lib::expand_types_list<
      typename select_unique_i_impl_<symphas::lib::types_list<>,
                                     symphas::lib::types_list<A>>::type,
...
struct select_unique_i_impl_<
    symphas::lib::types_list<Is...>,
    symphas::lib::types_list<expr::symbols::i_<N, P>, Rest...>> {
  using type = typename select_unique_i_impl_<
      symphas::lib::types_list<Is..., expr::symbols::i_<N, 0>>,
      symphas::lib::types_list<Rest...>>::type;
};
...
struct select_unique_i_impl_<
    symphas::lib::types_list<Is...>,
    symphas::lib::types_list<expr::symbols::v_id_type<expr::symbols::i_<N, P>>,
                             Rest...>> {
  using type = typename select_unique_i_impl_<
      symphas::lib::types_list<Is..., expr::symbols::i_<N, 0>>,
      symphas::lib::types_list<Rest...>>::type;
...
struct select_unique_i_impl_<symphas::lib::types_list<Is...>,
                             symphas::lib::types_list<Other, Rest...>> {
  using type =
      typename select_unique_i_impl_<symphas::lib::types_list<Is...>,
                                     symphas::lib::types_list<Rest...>>::type;
};
...
struct select_unique_i_impl_<symphas::lib::types_list<Es...>> {
  using type = typename symphas::lib::combine_types_unique<
      typename select_unique_i_impl_<symphas::lib::types_list<>,
                                     symphas::lib::types_list<Es...>>::type>::
      type;
};
...
struct filter_for_unique_vs<
  ...
  using type = typename symphas::lib::combine_types_unique<
      expr::symbols::i_<I0, 0>, expr::symbols::i_<I0s, 0>...>::type;
};

template <int I0, int P0, int... I0s, int... P0s>
struct filter_for_unique_vs<expr::symbols::v_<expr::symbols::i_<I0, P0>>,
                            expr::symbols::v_<expr::symbols::i_<I0s, P0s>>...> {
 ...
 *
 * This is different from ::base_data_type because this will return the data
 * type that the variable represents, whereas ::base_data_type has fewer
 * specializations, and returns only the variable itself.
 */
template <typename A>
...
template <typename E>
static auto test_coeff_neg(decltype(E::value) *) ->
    typename coeff_neg_trait<decltype(E::value)>::type;
template <typename E>
static long test_coeff_neg(...);
...
constexpr bool is_identity =
    (std::is_same<E, OpIdentity>::value ||
     std::is_same<E, OpNegIdentity>::value || std::is_same<E, OpVoid>::value);
...
//	((std::is_same<E1, OpIdentity>::value && std::is_convertible<E2,
// identity_eval_t>::value) || 		(std::is_same<E2, OpIdentity>::value &&
// std::is_convertible<E1, identity_eval_t>::value) || (std::is_same<E1,
// OpNegIdentity>::value && std::is_convertible<E2, identity_eval_t>::value) ||
//		(std::is_same<E2, OpNegIdentity>::value &&
// std::is_convertible<E1, identity_eval_t>::value));

//! True if the given type is a fraction.
/*!
 * This value is true if the given type is any of OpFractionLiteral, or its
 * negative.
 *
 * \tparam E The given type that is checked.
 */
...
constexpr bool has_coeff<OpTerms<V, Gs...>> =
    expr::is_coeff<V> || symphas::is_simple_data<V>;
...
template <typename V, typename... Gs>
constexpr bool has_nmi_coeff<OpTerms<V, Gs...>> =
    sizeof(typename symphas::internal::coeff_neg_trait<V>::type) ==
    sizeof(char);
 ...
 */
template <typename E>
constexpr bool has_val_coeff =
    has_coeff<E> && !(has_nmi_coeff<E> || has_pmi_coeff<E>);
...
struct grid_dim {
 protected:
  ...
  template <typename T, size_t D>
  static constexpr std::index_sequence<D> _infer_dimension(GridCUDA<T, D>) {
    return {};
  }

  template <typename T, size_t D>
  static constexpr std::index_sequence<D> _infer_dimension(
  ...
  static constexpr std::index_sequence<0> _infer_dimension(...) { return {}; }

  static constexpr size_t infer_dimension() {
    return symphas::lib::seq_index_value<
        -1, symphas::lib::sorted_seq<decltype(_infer_dimension(
                std::declval<E>()))>>::value;
  }
...
using enclosing_parent_storage_t =
    typename parent_storage_type<G>::enclosing_wrapped_type;
...
//! Provides the value of the derivative order of the given operator or
//! expression.
template <typename E>
...
struct derivative_order<OpOperatorChain<E1, E2>> {
  const static size_t value =
      derivative_order<E1>::value + derivative_order<E2>::value;
};

template <typename E1, typename E2>
struct derivative_order<OpOperatorCombination<E1, E2>> {
  const static size_t value =
      fixed_min<derivative_order<E1>::value, derivative_order<E2>::value>;
};
...
struct derivative_order<OpCombination<E1, E2, E>> {
  const static size_t value =
      derivative_order<OpOperatorCombination<E1, E2>>::value;
};

template <typename A, typename B>
struct derivative_order<OpBinaryMul<A, B>> {
  const static size_t value =
      derivative_order<A>::value + derivative_order<B>::value;
};
...
// fixed_max<expr::grid_dim<Ts>::dimension...>; 	static const size_t
// value = dimension;
// };
...
struct expr::grid_dim<GaussianSmoothing<D, grid_type>> {
  static const size_t dimension = D;
  static const size_t value = dimension;
};
...
struct expr::grid_dim<OpSymbolicEval<V, sub_t, SymbolicFunction<E, Ts...>>> {
  static const size_t dimension =
      fixed_max<expr::grid_dim<sub_t>::dimension, expr::grid_dim<E>::dimension>;
  static const size_t value = dimension;
};
...
struct expr::op_types<Substitution<Ts...>> {
  using type = typename symphas::lib::combine_types_unique<
      typename op_types<Ts>::type...>::type;
};
...
struct expr::op_types<OpAdd<Es...>> {
  using type = typename symphas::lib::combine_types_unique<
      typename expr::op_types<Es>::type...>::type;
};
...
//! Specialization based on expr::op_types.
template <size_t D, template <typename, size_t> typename grid_type>
struct expr::op_types<GaussianSmoothing<D, grid_type>> {
  using type = decltype(GaussianSmoothing<D, grid_type>::data);
};
...
struct expr::op_types<SymbolicFunction<E, Ts...>> {
  using type = typename symphas::lib::combine_types_unique<
      typename expr::op_types<E>::type,
      typename expr::op_types<Ts>::type...>::type;
};
...
//! Get the expression that the OpConvolution applies to.
template <typename V, typename E, typename... Ts, typename Seq, typename A,
          typename B, typename E0, typename... T0s>
struct expr::op_types<OpSum<V, E, Substitution<SymbolicDataArray<Ts>...>, Seq,
                            A, B, SymbolicFunction<E0, T0s...>>> {
 protected:
  using At = typename expr::op_types<E>::type;
  using Bt = typename symphas::lib::expand_types_list<
      typename expr::op_types<SymbolicDataArray<Ts>>::type...>;
...
struct expr::term_types<symphas::lib::types_list<Gs...>> {
  using type =
      symphas::lib::types_list<typename expr::original_data_type<Gs>::type...>;
};
...
struct get_first_storage_type<symphas::lib::types_list<G0, Gs...>> {
  using type =
      typename get_first_storage_type<symphas::lib::types_list<Gs...>>::type;
};
}  // namespace symphas::internal
...
struct expr::storage_type {
  using data_t = symphas::lib::type_at_index<
      0, symphas::lib::unroll_types_list<expr::term_types_t<E>>>;
  using storage_type_t = typename symphas::internal::get_first_storage_type<
      expr::term_types_t<E>>::type;
  using check_t =
      std::conditional_t<std::is_same<storage_type_t, void>::value,
                         std::conditional_t<std::is_same<data_t, void>::value,
                                            expr::symbols::Symbol, data_t>,
                         storage_type_t>;

  template <template <typename, size_t> typename enc_type>
  struct grid_class_wrap {
    using type = typename symphas::internal::grid_class_wrap<
        expr::eval_type_t<E>, expr::grid_dim<E>::value, enc_type>::type;
  };
  ...
  static block_class_wrap _pack_grid(
      VectorComponent<ax, MultiBlock<N, T>> const &g) {
    return {};
  }
  ...
  template <Axis ax, size_t N, typename T>
  static block_class_wrap _pack_grid(
      VectorComponent<ax, MultiBlockCUDA<N, T>> const &g) {
    return {};
  }
#endif
  ...
  static type_wrap<expr::eval_type_t<E>> _pack_grid(...) { return {}; }

  static auto pack_grid(check_t g) { return _pack_grid(g); }

 public:
  using type =
      typename std::invoke_result_t<decltype(&expr::storage_type<E>::pack_grid),
                                    check_t>::type;
};
...
struct expr::has_state<
    OpDerivative<std::index_sequence<O>, V, E, SymbolicDerivative<G0>>> {
  static const bool value = false;
};
...
struct expr::has_state<OpBinaryMul<E1, E2>> {
  static const bool value =
      expr::has_state<E1>::value || expr::has_state<E2>::value;
};
...
//! Specialization based on expr::is_operator_like.
template <size_t D, template <typename, size_t> typename grid_type>
struct expr::is_operator_like<GaussianSmoothing<D, grid_type>> {
  static const bool value = true;
};
...
struct expr::is_operator_like<OpOperatorCombination<A1, A2>> {
  static const bool value =
      expr::is_operator_like<A1>::value && expr::is_operator_like<A2>::value;
};
...
struct expr::is_operator_like<OpCombination<A1, A2, E>> {
  static const bool value =
      expr::is_operator_like<OpOperatorCombination<A1, A2>>::value;
};
...
struct expr::is_operator_like<OpChain<A1, A2, E>> {
  static const bool value =
      expr::is_operator_like<OpOperatorChain<A1, A2>>::value;
};
...
//! Get the expression that the OpConvolution applies to.
template <typename V, typename sub_t, typename E, typename... Ts>
struct expr::is_operator_like<
    OpSymbolicEval<V, sub_t, SymbolicFunction<E, Ts...>>> {
  static const bool value = expr::is_operator_like<E>::value;
};
...
//! Specialization based on expr::is_linear.
template <size_t D, template <typename, size_t> typename grid_type>
struct expr::is_linear<GaussianSmoothing<D, grid_type>> {
  static const bool value = true;
};
...
struct expr::is_linear<OpConvolution<V, E1, E2>> {
  static const bool value =
      expr::is_linear<E1>::value && expr::is_linear<E2>::value;
};
...
struct expr::is_linear<OpCombination<A1, A2, E>> {
  static const bool value =
      expr::is_linear<OpOperatorCombination<A1, A2>>::value &&
      expr::is_linear<E>::value;
};
...
struct expr::is_linear<OpChain<A1, A2, E>> {
  static const bool value = expr::is_linear<OpOperatorChain<A1, A2>>::value &&
                            expr::is_linear<E>::value;
};
...
struct expr::is_combination<OpChain<A1, A2, E>> {
  static const bool value = expr::is_combination<A1>::value ||
                            expr::is_combination<A2>::value ||
                            expr::is_combination<E>::value;
};

template <typename V, typename E, typename T, typename Seq, typename A,
...
template <typename E>
struct expr::expression_predicate : expr::is_linear<E>,
                                    expr::is_combination<E> {
...
template <Axis ax, size_t O, typename V, typename Sp>
struct expr::is_directional_derivative<
    OpOperatorDirectionalDerivative<ax, O, V, Sp>> {
  static const bool value = true;
};

template <typename V, typename Sp, size_t... Os>
struct expr::is_directional_derivative<
    OpOperatorMixedDerivative<V, Sp, Os...>> {
  static const bool value = true;
};
...
struct vars {
  ...
  static constexpr auto get_ids() { return std::index_sequence<>{}; }
...
struct expr::vars<Variable<Z, G>> {
  static constexpr auto get_ids() { return std::index_sequence<Z>{}; }
...
struct expr::vars<DynamicVariable<G>> {
  static constexpr auto get_ids() { return expr::vars<G>::get_ids(); }
...
struct expr::vars<SymbolicData<T>> {
  static constexpr auto get_ids() { return expr::vars<T>::get_ids(); }
...
struct expr::vars<SymbolicDataArray<std::tuple<Term<Ts>...>>> {
  static constexpr auto get_ids() {
    return symphas::lib::fold_unique_ids(
        symphas::lib::seq_join(expr::vars<Ts>::get_ids()...));
  }
...
struct expr::vars<OpTerms<V, Term<Gs, Xs>...>> {
  static constexpr auto get_ids() {
    return symphas::lib::fold_unique_ids(
        symphas::lib::seq_join(expr::vars<Gs>::get_ids()...));
  }
...
template <typename V, typename E, typename... Ts, typename Seq, typename A,
          typename B, typename C>
struct expr::vars<
    OpSum<V, E, Substitution<SymbolicDataArray<Ts>...>, Seq, A, B, C>> {
  static constexpr auto get_ids() {
    return symphas::lib::fold_unique_ids(symphas::lib::seq_join(
        expr::vars<E>::get_ids(),
        expr::vars<SymbolicDataArray<Ts>>::get_ids()...));
  }

  template <size_t Y>
  static constexpr bool has_id() {
    return (expr::vars<SymbolicDataArray<Ts>>::template has_id<Y>() || ... ||
            expr::vars<E>::template has_id<Y>());
  }

  template <size_t Y>
  static constexpr auto only_id() {
    return (expr::vars<SymbolicDataArray<Ts>>::template only_id<Y>() && ... &&
            expr::vars<E>::template only_id<Y>());
  }

  template <size_t Y>
  static constexpr auto each_id() {
    return (expr::vars<SymbolicDataArray<Ts>>::template each_id<Y>() && ... &&
            expr::vars<E>::template each_id<Y>());
  }
};
...
struct expr::vars<OpConvolution<V, E1, E2>> {
  static constexpr auto get_ids() {
    return symphas::lib::fold_unique_ids(symphas::lib::seq_join(
        expr::vars<E1>::get_ids(), expr::vars<E2>::get_ids()));
  }

  template <size_t Y>
  static constexpr bool has_id() {
    return expr::vars<E1>::template has_id<Y>() ||
           expr::vars<E1>::template has_id<Y>();
  }

  template <size_t Y>
  static constexpr auto only_id() {
    return expr::vars<E1>::template only_id<Y>() &&
           expr::vars<E2>::template only_id<Y>();
  }

  template <size_t Y>
  static constexpr auto each_id() {
    return expr::vars<E1>::template each_id<Y>() &&
           expr::vars<E2>::template each_id<Y>();
  }
};
...
struct expr::vars<OpAdd<Es...>> {
  static constexpr auto get_ids() {
    return symphas::lib::fold_unique_ids(
        symphas::lib::seq_join(expr::vars<Es>::get_ids()...));
  }
...
struct expr::derivative_index<L, OpOperatorCombination<A1, A2>> {
 protected:
  static const size_t raw_value = (expr::derivative_index<L, A1>::value |
...
struct expr::derivative_index<L, OpCombination<A1, A2, E>> {
 public:
  static const size_t value =
      expr::derivative_index<L, OpOperatorCombination<A1, A2>>::value;
};
...
struct expr::derivative_index<L, OpChain<A1, A2, E>> {
 public:
  static const size_t value =
      expr::derivative_index<L, OpOperatorChain<A1, A2>>::value;
};
...
struct expr::derivative_index<L,
                              OpOperatorDirectionalDerivative<ax, O, V, Sp>> {
  static const size_t value = expr::derivative_index_raw<O>::value;
};
...
struct expr::derivative_index<L, OpDerivative<Dd, V, E, Sp>> {
 protected:
  static const size_t raw_value =
      expr::derivative_index_raw<OpDerivative<Dd, V, E, Sp>::order>::value;
...
struct expr::derivative_index<L, OpAdd<Es...>> {
 protected:
  static const size_t value_1 =
      fixed_min<expr::derivative_index<L, Es>::value...>;
  static const size_t value_2 =
      fixed_max<expr::derivative_index<L, Es>::value...>;
...
struct expr::derivative_index<L, OpBinaryMul<A, B>> {
  static const size_t value =
      expr::derivative_index<L, OpOperatorChain<A, B>>::value;
};
...
/*!
 * Expression representing the negative of the multiplicative identity, a
 * special type of expression used primarily in the simplification of
 * expressions. It is also used as the coefficient to other expressions and can
...
template <typename E,
          typename std::enable_if_t<(!has_coeff<E> && !(expr::is_fraction<E> ||
                                                        expr::is_identity<E>)),
                                    int> = 0>
auto coeff(OpEvaluable<E> const &e) {
  return OpIdentity{};
}

inline OpIdentity coeff(symbols::Symbol) { return {}; }
...
inline auto coeff(OpVoid) { return OpVoid{}; }
...
template <typename E, typename std::enable_if_t<
                          (!has_coeff<E> && expr::is_coeff<E>), int> = 0>
...
template <typename... Es,
          typename std::enable_if_t<(is_coeff<Es> && ...), int> = 0>
...
template <typename E>
struct coeff_type<
    E, std::enable_if_t<(!has_coeff<E> &&
                         !(is_identity<E> || is_fraction<E> || is_tensor<E>)),
                        int>> {
...
struct coeff_type<coeff_t,
                  std::enable_if_t<(is_identity<coeff_t> ||
                                    is_fraction<coeff_t> || is_tensor<coeff_t>),
                                   int>> {
  using type = coeff_t;
};

// template<typename V, typename G0, exp_key_t X0, typename... Gs, exp_key_t...
// Xs> struct coeff_type<OpTerms<V, Term<G0, X0>, Term<Gs, Xs>...>,
//	std::enable_if_t<has_coeff<OpTerms<V, Term<G0, X0>, Term<Gs, Xs>...>>,
// int>>
//{
//	using type = V;
// };
...
struct expr::eval_type {
  template <typename T, size_t D>
  static constexpr std::index_sequence<1> _get_rank_1(
      any_vector_t<T, D> const &) {
    return {};
  }

  template <typename T, size_t D>
  static constexpr std::index_sequence<D> _get_rank_1(
      any_row_vector_t<T, D> const &) {
    return {};
  }

  template <typename T, size_t N, size_t M>
  static constexpr std::index_sequence<M> _get_rank_1(
      any_matrix_t<T, N, M> const &) {
    return {};
  }

  static constexpr std::index_sequence<0> _get_rank_1(...) { return {}; }

  template <typename T, size_t D>
  static constexpr std::index_sequence<D> _get_rank(
      any_vector_t<T, D> const &) {
    return {};
  }

  template <typename T, size_t D>
  static constexpr std::index_sequence<1> _get_rank(
      any_row_vector_t<T, D> const &) {
    return {};
  }

  template <typename T, size_t N, size_t M>
  static constexpr std::index_sequence<N> _get_rank(
      any_matrix_t<T, N, M> const &) {
    return {};
  }

  static constexpr std::index_sequence<0> _get_rank(...) { return {}; }

  template <typename E0>
  static constexpr std::index_sequence<expr::eval_type<E0>::rank> _get_rank(
      OpExpression<E0> const &e) {
    return {};
  }

  template <typename E0>
  static constexpr std::index_sequence<expr::eval_type<E0>::template rank_<1>>
  _get_rank_1(OpExpression<E0> const &e) {
    return {};
  }
  ...
  static auto _get_eval(...) { return expr::symbols::Symbol{}; }
  ...
  // Use `decltype(get_eval(declval<>()))` rather than
  // `invoke_result_t<decltype(&get_eval), E0>` to stay SFINAE-friendly:
  // libstdc++'s `invoke_result_t` gates on a hard-error
  // `static_assert(__is_complete_or_unbounded(...))` that fires when
  // any argument type is mid-instantiation, breaking surrounding
  // partial-specialization selection (e.g. an autogen-internal
  // `coeff_t * OpTerms` overload's enable_if).
  template <typename E0>
  using eval_t =
...
struct trait_arg_type {
  using type = std::conditional_t<symphas::is_simple_data<T>, OpLiteral<T>, T>;
};
}  // namespace
 ...
 * the created template object will count the given expr::symbols::arg types
 * used in the expression to understand how many arguments can be passed. When
 * expressions are substituted in the template in order to expand it, their
 * order in the template substitution list (the paramters to the `operator()`
 * method of the template object) will be matched in order of increasing index
 * of the argument objects used to construct the template expression.
 */
...
constexpr bool all_ne<N0, N1, N2, Ns...> =
    (N0 != N1) ? all_ne<N0, N2, Ns...> && all_ne<N1, N2, Ns...> : false;
...
constexpr bool all_different<T0, T1, T2, Ts...> =
    (!std::is_same_v<T0, T1>)
        ? all_different<T0, T2, Ts...> && all_different<T1, T2, Ts...>
        : false;
...
//! to placeholders.
/*!
 * Expression templates are essentially patterns that other expressions are
 * substituted into, in order to build another expression.
 *
 * These templates can be created without specifying an argument list, and
 * the created function object will count the given expr::symbols::arg_t types
 * used in the expression to understand how many arguments can be passed. When
 * expressions are substituted in the function in order to expand it, their
 * order in the function substitution list (the paramters to the `operator()`
 * method of the function object) will be matched in order of increasing index
 * of the argument objects used to construct the function expression.
 */
template <typename... Ts>
...
/*!
 * Functions are objects that substitute given data into their arguments so that
 * the enclosed expression can be evaluated with any data that is swapped on the
 * fly. See SymbolicFunction.
 *
 * Functions can be created without specifying an argument list, and
 * the created function object will not take any arguments as placeholders. When
 * expressions are substituted in the function in order to expand it, their
 * order in the function substitution list (the paramters to the `operator()`
 * method of the function object) will be matched in order of increasing index
 * of the argument objects used to construct the function expression.
...
auto const &get_enclosed_expression(
    OpMap<symphas::internal::HCTS, OpIdentity, E> const &e) {
  return e.e;
}
//! Get the expression that the OpMap applies to.
template <typename E>
auto const &get_enclosed_expression(
    OpMap<symphas::internal::STHC, OpIdentity, E> const &e) {
  return e.e;
}
...
template <typename Dd, typename V, typename G, typename Sp>
decltype(auto) get_enclosed_expression(
    OpDerivative<Dd, V, OpTerm<OpIdentity, G>, Sp> const &e) {
...
auto const &get_enclosed_expression(
    OpDerivative<std::index_sequence<O>, V, OpTerm<OpIdentity, G>,
...
template <size_t O, typename V, typename E, typename G0>
auto const &get_enclosed_expression(
    OpDerivative<std::index_sequence<O>, V, E, SymbolicDerivative<G0>> const
        &e) {
  return e.e;
}
...
template <typename V, typename E, typename F, typename Arg0, typename... Args>
auto const &get_enclosed_expression(
    OpFunction<V, E, F, Arg0, Args...> const &e) {
  return e.e;
}
...
//! Get the expression that the OpConvolution applies to.
template <typename V, typename sub_t, typename E, typename... Ts>
const auto &get_enclosed_expression(
...
template <typename E>
auto &get_enclosed_expression(
    OpMap<symphas::internal::HCTS, OpIdentity, E> &e) {
  return e.e;
}
...
template <typename E>
auto &get_enclosed_expression(
    OpMap<symphas::internal::STHC, OpIdentity, E> &e) {
  return e.e;
}
...
template <typename Dd, typename V, typename G, typename Sp>
decltype(auto) get_enclosed_expression(
    OpDerivative<Dd, V, OpTerm<OpIdentity, G>, Sp> &e) {
  return OpTerm<OpIdentity, G>(OpIdentity{}, e.data);
}
...
template <size_t O, typename V, typename E, typename G0>
auto &get_enclosed_expression(
    OpDerivative<std::index_sequence<O>, V, E, SymbolicDerivative<G0>> &e) {
  return e.e;
}
...
template <typename V, typename sub_t, typename E, typename... Ts>
auto &get_enclosed_expression(
    OpSymbolicEval<V, sub_t, SymbolicFunction<E, Ts...>> &e) {
  return e.f.e;
}
...
template <typename Dd, typename V, typename G, typename Sp>
auto &get_result_data(OpDerivative<Dd, V, OpTerm<OpIdentity, G>, Sp> &e) =
    delete;
...
auto const &get_result_data(
    OpDerivative<Dd, V, OpTerm<OpIdentity, G>, Sp> const &e) = delete;
...
//! Get the grid storing the underlying data of the OpIntegral.
template <typename V, typename E, typename T>
auto const &get_result_data(OpIntegral<V, E, SymbolicDerivative<T>> const &e) =
    delete;
...
template <typename V, typename G0, exp_key_t X0, typename... Gs,
          exp_key_t... Xs>
decltype(auto) terms_after_first(
    OpTerms<V, Term<G0, X0>, Term<Gs, Xs>...> const &e);
template <typename V, typename G0, exp_key_t X0, typename... Gs,
          exp_key_t... Xs>
